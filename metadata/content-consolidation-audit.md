...
第二轮以第一轮保留的 454 篇正文为输入，再次审阅每篇正文的标题、章节结构、责任链和与同章文章的重叠关系。最终形成 126 个合并组，覆盖 295 篇输入正文；其余 159 篇经复核后独立保留。规范章节正文净减少 169 篇，收敛为 285 篇；加上前言和附录，全书共 298 篇。

| 章节 | 第一轮后 | 第二轮后 | 章节 | 第一轮后 | 第二轮后 |
| --- | ---: | ---: | --- | ---: | ---: |
| ch01 | 50 | 28 | ch14 | 28 | 17 |
| ch02 | 34 | 21 | ch15 | 10 | 7 |
| ch03 | 8 | 6 | ch16 | 11 | 8 |
| ch04 | 17 | 10 | ch17 | 10 | 7 |
| ch05 | 17 | 9 | ch18 | 25 | 16 |
| ch06 | 5 | 4 | ch19 | 22 | 14 |
| ch07 | 14 | 9 | ch20 | 22 | 14 |
| ch08 | 12 | 8 | ch21 | 16 | 11 |
| ch09 | 10 | 7 | ch22 | 31 | 18 |
| ch10 | 7 | 5 | ch23 | 10 | 7 |
| ch11 | 7 | 5 | ch24 | 19 | 12 |
| ch12 | 3 | 2 | ch25 | 19 | 12 |
| ch13 | 22 | 13 | ch26 | 25 | 15 |
| **合计** | **454** | **285** |  |  |  |

本轮不以删除正文换数量：每个合并成员的正文、来源、版本边界、命令、代码和排障步骤都进入目标稿；只统一 H1/frontmatter、标题层级、脚注命名空间、连续编号与交叉引用。可执行方案见 [`2026-08-24-content-consolidation-v2.yaml`](2026-08-24-content-consolidation-v2.yaml)，454 条旧路径的逐条去向见 [`2026-08-24-content-consolidation-v2-map.json`](2026-08-24-content-consolidation-v2-map.json)，落盘与复查逻辑见 [`../scripts/consolidate_articles_v2.py`](../scripts/consolidate_articles_v2.py)。

### 第二轮验收结果

- 26 章共 285 篇规范正文，章内编号连续；`SUMMARY.md` 对 285 篇正文各收录一次，章节 README 索引与正文集合完全一致。
- `scripts/check-metadata.py` 检查 285/285 通过，0 warning、0 failure；285 篇正文的 `tags`、`related_chapters` 和合并来源字段均已规范化。
- `scripts/check-summary-links.py` 检查 324 个目录链接，0 broken、0 duplicate；全库活动正文中的 1,479 个本地 Markdown 链接均可解析。
- 454 条旧正文路径均在映射表中有唯一去向；所有合并稿通过脚注定义/引用配对检查，跨章引用另按映射表做过语义复核。
- `queue.json` 和 `source-index.json` 中所有非空的当前目标路径均存在；已关闭 finding 保留发生时路径，唯一 open finding 指向当前规范正文。
- 合并脚本通过 `py_compile`，工作树通过 `git diff --check`。当前环境未安装 `mdbook`，因此没有执行最终 HTML 构建；目录、元数据与链接层验收不受影响。

## 第一轮总体进度（2026-08-11，历史）
...
第一轮结束时规范正文总数为 454 篇。这里的“已完成”表示该章每篇正文均已阅读并完成当轮结构收敛，不代表所有技术结论都已达到发布状态；当前数量以本页第二轮结果和 `metadata/progress.json` 为准。
