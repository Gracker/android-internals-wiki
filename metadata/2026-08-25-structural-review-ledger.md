...
| 第 23 章：内存实践 | 7 | 7 | 已完成 | 逐篇确认 Java 存活集/分配、泄漏、Native/VMA、图片、架构扩容、端侧模型和线上治理的责任边界。修正 23.3/23.4 合并稿自指与分段收束，把 23.5 的 64 位迁移前移到三种架构选项中，扩充 23.6 两个过窄标题，并补齐 5 篇全文小结。目标顺序随全书统一重编号落地。 |
| 第 24 章：I/O 与网络优化 | 12 | 12 | 已完成 | 逐篇确认本地数据、网络连接/协议、受限网络与设备连接的责任边界。压缩 24.5 的重复离线同步段，重组 24.8/24.9 合并稿层级，统一 OkHttp 5.4.0 示例，修正 CameraX 错链和 Photo Picker 版本错误；24.11 重命名以覆盖 Data Saver。目标顺序随全书统一重编号落地。 |
| 第 25 章：功耗与包体积优化 | 11 | 11 | 已完成 | 逐篇确认诊断、后台执行、具体耗电源、CPU/ADPF/热治理和体积分发的责任边界。压缩 25.2 重复 FGS 教程，修正 25.3/25.9 合并稿自指，区分 25.5 三条产物链和 25.8 源码下钻层级，清理重复资料并补齐全文收束。目标顺序随全书统一重编号落地。 |
| 第 26 章：应用可观测性 | 15 | 15 | 已完成 | 逐篇确认采集架构、稳定性/启动证据、在线排障、实验门禁、领域指标和高级采集技术的责任边界。压缩 26.6 重复的 `ApplicationExitInfo` 教程，修正 26.3/26.10 错链，重命名网络对账与功耗指标标题，为 12 篇补齐全文收束。累计完成 274/274；全书章节号和已确认的章内目标顺序待统一重编号一次落地。 |
...
| 文章 | 标题契约与推进线 | 章节边界 | 处理结果 |
| --- | --- | --- | --- |
| 23.1 内存泄漏检测与治理 | 从对象应否存活进入常见持有路径、引用链证据、本地/线上工具与生产闭环。 | 负责“不再需要但仍可达”的对象；23.4 负责正常对象的存活集和分配，23.6 负责跨内存类型的统一监控。 | 保留；补充相邻专题入口，明确本文只负责泄漏治理，补齐全文小结。 |
| 23.2 Bitmap 与图片内存优化 | 从像素预算和存储位置进入采样/复用、Hardware Bitmap、案例与排查清单。 | 负责 Bitmap 解码与像素内存；22.5 负责图片请求到渲染的端到端实践，10.4 负责图形内存统计。 | 保留；删除连续重复入口，补齐覆盖监控、优化和验收的全文小结。 |
| 23.3 Native 与虚拟内存管理优化 | 前半按分配器、所有权和安全错误治理 Native Heap，后半按 VMA、驻留页、缺页与回收治理地址空间。 | 两部分共享“Java 堆外内存”的证据口径，但分别处理逻辑分配与内核映射；统一线上治理交给 23.6。 | 保留合并；删除重复链接和自指，补充分段小结、监控边界、编号化源码锚点与全文小结。 |
| 23.4 Java Heap、GC 与 Compose 内存分配 | 按存活集与缓存 → 高频分配与 GC 抖动 → Compose 状态/生命周期与分配热点推进。 | 负责托管堆基线和分配速率；泄漏、图片与生产监控分别交给 23.1、23.2、23.6。 | 保留六稿合并；清除五处当前文章自指，把 Compose 分段改为“状态与分配热点”，为三条主线分别收束并补全文小结。 |
...
| 23.6 内存监控与线上治理 | 从统计口径、阈值与告警进入资源降级、退出归因、诊断产物、案例沉淀和发布验收。 | 负责跨 Java、Native、映射、图形与进程的生产闭环；具体根因调查回到 23.1—23.5。 | 保留合并；把“OOM 预警与主动回收”扩为压力/降级/退出归因，把“快照”扩为全部线上诊断产物，补齐关联和全文小结标题。 |
| 23.7 端侧大模型推理的内存管理 | 按内存组成、设备预算、运行时生命周期、压力信号、后端、测量、降级、多进程与验收推进。 | 负责 LLM 权重、KV Cache 和后端工作区这一专项负载；通用虚拟内存、进程策略和线上监控分别回到 23.3、23.5、23.6。 | 保留；修正两处旧标题入口，补充独立的全文小结。 |

章内统一重编号的目标顺序为：Java Heap/GC/Compose 基线 → 泄漏治理 → Native/VMA → Bitmap/图片 → `largeHeap`/多进程/64 位 → 端侧大模型 → 线上监控与治理。该顺序让统一对象模型先于故障专题、专项负载先于生产治理，并随全书统一重编号一次落地。

## 第 24 章逐篇结论

| 文章 | 标题契约与推进线 | 章节边界 | 处理结果 |
| --- | --- | --- | --- |
| 24.1 文件 I/O、SAF 与 ContentResolver 性能 | 先讲直接文件与键值存储的读写/同步边界，再进入 URI、Provider 和 SAF 授权路径。 | 负责文件与文档 I/O；数据库和缓存分别交给 24.2/24.5，媒体集合交给 24.9。 | 保留两段合并；区分文件 I/O 小结并补全文小结。 |
| 24.2 数据库与序列化性能 | 前半围绕查询、索引、事务和迁移，后半围绕数据格式、编码分配和版本兼容。 | 数据库控制选取与持久化，序列化控制对象到字节的转换；两者共享端到端数据路径但不混用指标。 | 保留合并；区分数据库/序列化小结并补全文收束。 |
| 24.3 移动网络架构、连接与容灾 | 从进程内网络架构、默认网络和连接生命周期进入重试/幂等，再以基线、故障实验和发布门禁收束。 | 负责通用移动网络客户端和容灾；协议、DNS、Wi-Fi 与受限网络由后续专题展开。 | 重命名两个重复 H2，合并重复导航，统一 OkHttp 5.4.0 当前锚点并补全文小结。 |
| 24.4 HTTP/2、HTTP/3、gRPC 与 ECH | 先比较连接复用、QUIC 与 RPC 语义，再讲 ECH 的配置、降级、安全和验证。 | 负责应用传输协议与加密握手；24.3 负责请求生命周期，24.7 负责解析执行边界。 | 保留两段合并；区分协议/RPC 与 ECH 小结，补全文收束。 |
| 24.5 数据缓存与离线优先架构 | 从缓存层级、键、失效与一致性进入本地真源、Outbox、同步和冲突处理。 | 缓存负责可重建副本，离线优先负责本地真源和可恢复写入；二者通过失效与同步边界衔接。 | 删除前半篇重复的完整离线同步教程，改为边界过渡；区分检查清单与资料并补全文小结。 |
| 24.6 Wi-Fi 评分、网络选择与连接切换性能 | 从 Wi-Fi 候选与 Connectivity 默认网络选择进入应用观测、OEM 差异和 HTTP/3 迁移。 | 负责系统选网和切网证据；24.3 负责业务容灾，24.4 负责协议，24.7 负责 DNS。 | 统一 EventListener 到 OkHttp 5.4.0 签名，重命名泛化“扩展”，修正相邻入口并补全文小结。 |
| 24.7 HTTPDNS 与 OkHttp Dns 的执行边界 | 从同步 `Dns.lookup()` 路径进入后台刷新/内存快照、Network 隔离、安全、Fast Fallback 和故障验证。 | 负责自定义解析的执行与安全合同；连接、协议和系统 DNS 机制分别回到 24.3/24.4/12.2。 | 保留；修复元数据尾字符和错误的源码链接标签，补相邻入口与全文小结。 |
| 24.8 低带宽、流媒体与本地网络适配 | 依次建立卫星/低带宽数据预算，区分三类网络路径，再展开流媒体上限、本地网络授权和跨路径验证。 | 三个主题共享“按网络能力选择策略”，但预算、媒体质量和局域网权限保持独立决策。 | 保留合并；重组五个顶层层级，删除同篇编号自指，补全文小结。 |
| 24.9 MediaStore、Photo Picker 与媒体转码 | 前半按索引/卷/查询/写入/同步治理媒体集合，后半按选择授权、打开、转码、缓存和多选队列处理单项媒体。 | MediaStore 负责集合索引，Photo Picker 负责逐项授权；转码可能出现在两条读取路径，但缓存所有权不同。 | 保留合并；清理三处同篇编号自指，区分两套检查清单和资料，修正 AndroidX 1.11.0 版本并补全文小结。 |
| 24.10 BluetoothSocket read() 断开语义与长连接治理 | 从 Android 17 EOF 变化进入读写阻塞、协议分帧、状态机、重连预算、后台执行和测试。 | 负责蓝牙字节流长连接；11.4 负责扫描/连接功耗，24.3 的 HTTP 错误分类不能直接套用。 | 保留；重命名问题范围标题，修正扫描功耗编号并统一全文小结。 |
| 24.11 Android 17 移动数据配额与 Data Saver 执行链路 | 从统计与五类配额概念追到 NetworkPolicy、netd、quota2、Telephony 和 UID 规则，再落到公开 API 与诊断。 | 负责平台总量限制、Data Saver 和应用观察边界；不把字节配额写成手机网络限速，Android TV 例外单列。 | 原标题只覆盖两个服务，改名以覆盖 Data Saver 和完整执行链；补全文小结。 |
...
章内统一重编号的目标顺序为：文件 I/O/SAF → 数据库/序列化 → 缓存/离线优先 → MediaStore/Photo Picker → 移动网络架构/容灾 → HTTP/2/3/gRPC/ECH → HTTPDNS → Wi-Fi 选网/切网 → 低带宽/流媒体/本地网络 → 移动数据配额/Data Saver → BluetoothSocket → NFC。该顺序先完成本地数据面，再进入公网网络栈，最后处理设备连接，并随全书统一重编号一次落地。
...
| 25.4 定位与传感器功耗优化 | 从请求参数、批处理和生命周期进入传感器采样、融合、后台限制与验证。 | 负责持续硬件采样这一专项负载；平台 LocationManager 机制回到 1.28。 | 保留；统一全文小结和资料标题。 |
| 25.5 应用体积分析与优化：DEX、Native SO 与资源 | 先统一制品和设备交付口径，再分别沿 DEX/R8、ELF/ABI 和资源表/素材三条链路分析，最后都落到发布差异报告。 | 负责发布产物还能删除或缩小什么；25.6 负责何时向哪台设备交付。 | 保留三段合并；为三组资料和版本边界标明作用域，合并重复 Native 入口并补跨链路全文小结。 |
| 25.6 App Bundle 与按需分发 | 从 AAB/split 语义进入 Dynamic Feature、PAD、非 Play 渠道和体积门禁。 | 负责设备选择、模块和下载时机，不把分包误写成删除 base 中的无用内容。 | 保留；统一全文小结标题。 |
| 25.7 Hybrid/WebView 功耗与原生化取舍 | 从能耗边界和论文证据进入 WebView CPU/内存/网络来源、同机实验、选型、线上监控与低端机。 | 负责按页面证明 Hybrid 成本并决定局部/整体原生化；WebView 渲染和 APM 细节回到 18.10、19.11、22.6。 | 保留；通读确认论文限制没有被外推为 WebView 结论，统一全文小结标题。 |
| 25.8 PerformanceHintManager 与 ADPF 能效验证 | 先讲 Session、线程和能效模式，再用 PowerMonitor/Perfetto/实验验证，最后下钻系统服务与 Power HAL。 | 负责周期性负载如何向系统表达目标并验证能效；一般 CPU 减量先由 25.10 处理，热降级由 25.11 处理。 | 保留合并；把后半篇明确标为源码追踪，补充从应用主线到系统下钻的过渡并统一全文小结。 |
| 25.9 后台音频、AudioTrack 与 Offload 功耗 | 前半处理 Android 17 后台播放资格、FGS/WIU 和资源释放，后半处理 AudioTrack/AAudio/Media3 Offload 选择与验证。 | 后台资格决定能否持续发声，音频路径决定时延、功能和功耗；两层不能互相替代。 | 保留两段合并；修正两处同篇编号自指，区分后台音频/Offload 小结并补全文小结。 |
| 25.10 应用层 CPU 优化实战指南 | 从 CPU 口径和线程池进入采样、预加载、周期任务、过量 CPU 退出、锁、协程、ADPF 与验证流程。 | 负责先减少工作和控制并发；25.8 负责稳定周期负载的 ADPF 能效协作，25.11 负责热约束后的降载。 | 保留合并；确认 excessive CPU 与 fixed-rate 案例都服务于应用 CPU 治理，补全文小结。 |
| 25.11 热节流适配与性能退化治理 | 从公开热信号和系统路径进入 headroom、工作量状态机、因果证据、ADPF/Game Mode、后台限制与测试。 | 负责持续负载进入热平衡后的主动降级；通用 CPU 减量与能效提示分别回到 25.10、25.8。 | 保留；合并两对重复延伸阅读并补全文小结。 |

章内统一重编号的目标顺序为：功耗诊断/OEM → 后台执行/FGS → WakeLock/Alarm/WorkManager → 定位/传感器 → 后台音频/Offload → Hybrid/WebView → 应用 CPU → PerformanceHintManager/ADPF → 热节流 → DEX/Native SO/资源体积 → App Bundle/按需分发。该顺序先建立诊断和执行资格，再进入具体负载与通用计算/热治理，最后把阅读对象从运行时切换到发布制品，并随全书统一重编号一次落地。
...
| 26.7 eBPF、ATrace 与线上系统追踪 | 先讲受信设备上的 syscall/Binder 证据，再进入 Profilo/ATrace 的应用内会话、兼容和替代方案。 | 负责系统与应用低层追踪能力；普通 App 权限和公开替代路径是核心边界。 | 保留两段合并；扩写全文小结以共同覆盖 eBPF、ATrace 和 Profilo；目标位置移到高级采集组。 |
| 26.8 ApplicationStartInfo 与启动归因上报 | 从系统启动记录、字段和时间戳进入业务 envelope、cold-start trigger、退出关联与门禁。 | 负责线上启动样本的系统归因；8.2/21.1 负责启动机制与通用度量。 | 保留；补全文小结，目标位置紧跟退出记录。 |
| 26.9 Android Vitals 与 Play Console 质量指标归因 | 从 core vital 口径和阈值进入内部 APM 对照、Reporting API 入仓和商店风险。 | 负责 Play 外部质量证据；26.4/26.5 负责内部实验与门禁。 | 保留；补全文小结，明确无事件级关联键和分母差异。 |
| 26.10 线上存储、I/O 与 SQLite 可观测性 | 从事件 scope 与采集路径进入不良 I/O、SQLite、容量/损坏、采样和 Matrix/Perfetto 扩展。 | 只定义存储证据与工具边界；24.x 负责优化方法。 | 修正受限诊断从 26.5 到 26.3 的错链，统一全文小结。 |
| 26.11 线上网络质量监控与接入层对账 | 从客户端阶段、系统网络快照和库边界进入接入层对账、告警、证据包与 QUIC。 | 负责客户端与入口是否为同次请求的证据模型；24.x 负责连接与协议优化。 | 标题由泛化“协同”改为正文实际主线“对账”，统一全文小结。 |
| 26.12 Battery Historian 与功耗指标集成 | 从状态、资源、归因和能量四种证据进入平台结构、受控实验、公开 API、WakeLock 和 APM 集成。 | 负责功耗证据如何进入指标系统；25.x 负责具体优化。 | 标题把泛化“性能指标”收紧为“功耗指标”，补全文小结。 |
| 26.13 编译期字节码插桩与监控自动化 | 从 AGP/ASM 边界进入完整控制流、R8、网络/帧/Kotlin 特例、验证和运行时桥接。 | 负责可控构建产物中的自动观测点；26.15 负责调试态或私有 ART 动态能力。 | 保留；补全文小结。 |
| 26.14 heapprofd、procfs CPU 与 Page Fault 分析 | 按 Native 分配采样 → CPU 累计计数 → 缺页来源推进，形成低层资源诊断链。 | 三类信号只用于互相缩小原因范围，不把分配、CPU 和 fault 数互相换算。 | 重写总导语以覆盖三篇合并内容，补三类证据共同收束。 |
...
章内统一重编号的目标顺序为：采集架构 → Crash/ANR → 在线排障 → A/B/回归 → 性能评分/发布门禁 → ApplicationExitInfo → ApplicationStartInfo → Android Vitals → 存储/I/O/SQLite → 网络与接入层对账 → 功耗指标 → 编译期字节码插桩 → heapprofd/procfs/Page Fault → eBPF/ATrace/Profilo → ART 动态追踪/JVMTI。该顺序保留“发现—处置—验证—发布”的运营闭环，再按系统记录、领域信号和由低到高风险的采集技术下钻，并随全书统一重编号一次落地。
