...
## [2026-07-07] 14.11 Battery Historian 与功耗分析工具 — OEM厂商功耗统计实现差异研究缺口

### 盲区描述
不同OEM厂商的PowerStats HAL实现存在显著差异，包括采样频率、测量精度、数据粒度等方面，导致跨设备功耗分析结果存在系统性偏差。Google Pixel、Samsung Exynos、小米/MTK等主流厂商的实现差异未被充分讨论，影响开发者对不同设备的功耗分析和优化。

### 重要程度
高

### 建议研究方向
- 主流厂商PowerStats HAL实现差异分析
- 跨设备功耗数据标准化校准方法研究
- 设备功耗基准测试体系建立
- OEM功耗数据隐私保护机制对精度的影响研究
...
## [2026-07-07] 1.1 Android 分层架构 — 知识盲区

### 盲区描述
LMK从内核态到用户态迁移的完整影响分析缺失，包括内存回收策略变化、响应时间差异、用户态LMK与内核LMK的权衡取舍，这直接影响系统内存管理优化策略的制定。

### 重要程度
高

### 建议研究方向
- 对比Android 10前后的LMK实现差异（kernel LMK vs userspace lmkd）
- 分析PSI（Pressure Stall Information）与用户态LMK的协同机制
- 研究用户态LMK的响应延迟和内存压力判断精度
- 评估LMK迁移对不同内存压力场景下的系统稳定性影响

### 关联章节
1.1（本章）、2.5（内存管理）、4.1（OOM治理）

## [2026-07-07] 1.1 Android 分层架构 — 知识盲区

### 盲区描述
Binder线程池实现机制和调优参数未说明，包括Binder线程创建策略、线程优先级、任务队列管理、超时控制等关键性能参数，这些直接影响高频IPC场景下的性能表现。

### 重要程度
高

### 建议研究方向
- 分析Binder线程池的创建和管理机制（Binder驱动层面）
- 研究不同进程类型的Binder线程配置差异
- 评估Binder线程数量对IPC延迟和吞吐量的影响
- 开发Binder线程池调优最佳实践指南

### 关联章节
1.1（本章）、13.3（IPC通信）、14.1（性能监控）

## [2026-07-07] 1.1 Android 分层架构 — 知识盲区

### 盲区描述
VNDK隔离对native库加载性能的具体量化影响缺失，包括内存占用、库解析开销、运行时性能差异等关键指标，这些影响系统启动和运行时性能的评估。
...
- 量化VNDK隔离带来的内存和性能开销
- 分析不同库加载场景下的性能影响
- 研究VNDK-less架构对性能的改善效果
- 评估VNDK隔离对A/B分发包体积的影响

### 关联章节
1.1（本章）、3.1（Native开发）、18.1（启动优化）


## [2026-07-10] 26 可观测性 — 网络监控架构（客户端+接入层+自动化报警）

### 来源
[结构参考: Clippings/线上疑难问题该如何排查和跟踪？-Android开发高手课-极客时间 20.md（第17讲 网络优化下）]

### 知识点
1. 客户端网络监控维度体系：时延（DNS/建连/首包/总时间）、多维分析（网络类型/国家/运营商/版本/机型/域名）、错误分类（DNS失败/连接失败/超时/错误码）
2. 接入层（服务端）监控的必要性：实时性（秒级 vs 客户端分钟级）、可靠性（客户端上报通道可能受网络故障影响）
3. 监控分层架构：实时监控（PV/错误率，分钟级）vs 离线监控（全维度展开，小时/天级）
4. 自动化报警算法对比：基于规则（失败率暴涨/流量暴跌）vs 基于时间序列/神经网络智能报警

### 重要程度
中

### 建议加工方向
- 结合 Android 17 的网络监控能力（NetworkStatsManager、ConnectivityManager.NetworkCallback）设计客户端监控方案
- 分析大厂网络监控平台架构案例（微信分钟级监控报警系统）
- [需确认: AIW ch26 是否已有网络可观测性专门章节，若无需创建]
