...
## [Task9 Deep Review] 4.1 Android 内存模型全景 — 2026-04-21
- **类型**：数据缺失
- **位置**：L469-L483
- **问题**：`Qualcomm 建议 ZRAM=物理内存 75%` 和 `2x-4x 压缩比` 都没有一手来源或设备条件，当前写法容易被读者当成通用调参结论。
- **建议**：把这组数字降格为 OEM case-by-case 经验值，补设备类型、压缩算法、工作负载条件，或者删除无法追溯的数据。

## [Task9 Deep Review] 8.5 案例集 — 2026-04-21
- **类型**：数据缺失
- **位置**：L254-L319
- **问题**：页面切换案例使用了精确的 500ms→150ms 分阶段数据，但正文又注明它是“综合多个电商 App 的典型经验归纳”，与章节“真实案例”定位冲突。
- **建议**：要么补真实 trace / 设备 / 测试口径，要么明确改成“合成案例/方法演示”，避免把综合经验包装成单一实测结果。

## [Task9 Deep Review] 9.3 ANR 分析方法 — 2026-04-21
- **类型**：版本差异
- **位置**：L72 / L369
...
## [Task9 Deep Review] 15.4 竞品分析方法 — 2026-04-21
- **类型**：数据支撑
- **位置**：L280
- **问题**：“APK 体积每增加 10MB，安装转化率下降约 1.5%”给了精确数字，但正文没有附来源、国家/渠道范围、样本期或实验条件。当前写法像通用结论，读者无法判断是否适用于 Play、厂商商店还是特定市场。
- **建议**：补上原始来源和适用边界；如果拿不到一手出处，建议改成“包体积会显著影响转化率，尤其在弱网和低存储市场更明显”这类有边界的定性表述。

## [Task9 Deep Review] 9.5 案例集 — 2026-04-21
- **类型**：交叉引用一致性
- **位置**：L71-L75 / L204-L237
- **问题**：章节开头把案例 3 标成“SharedPreferences 等待导致 Broadcast ANR”，但正文案例 3 讲的是 Activity 生命周期切换时 `QueuedWork.waitToFinish()` 卡住主线程。案例摘要和正文口径不一致，读者会误解这个案例到底在复盘哪一类 ANR。
- **建议**：统一摘要和正文的类型标签。如果想强调同一机制也会触发 Broadcast ANR，需要在案例标题或“举一反三”里把触发条件补清楚。


## [Task9 Deep Review] 10.5 案例集 — 2026-04-21
- **类型**：数据缺失
- **位置**：L195 / L213
- **问题**：“每增加一个设置了 alpha 的 View，renderD128 内存增加约 25MB” 缺少分辨率、窗口尺寸、buffer 格式、机型和 GPU 驱动版本等实验条件。这个数字更像特定设备 case，当前写法容易被读者误当成通用规律。
- **建议**：补充机型、分辨率、位深、是否全屏离屏缓冲区等条件，或降格为“某华为 Android 10 + PowerVR 设备上的实测个案”。

## [Task9 Deep Review] 10.5 案例集 — 2026-04-21
- **类型**：数据缺失
- **位置**：L225
- **问题**：“renderD128 相关的 OOM 崩溃率下降至基线水平” 没有给出优化前后数值、样本周期或基线定义，读者无法判断方案收益量级。
- **建议**：补充崩溃率前后对比、覆盖版本和观测周期；如果拿不到数值，就改成定性结论并显式标注无公开数据。

## [Task9 Deep Review] 17.1 OEM 性能优化的通用思路 — 2026-04-21
- **类型**：数据缺失
- **位置**：L173-L177
- **问题**：Trinity Engine “启动速度提升 28%，加载时间缩短 21%” 没有标出测试场景、机型、对照组和 workload，当前只能当市场宣传数据，不能直接作为技术结论。
- **建议**：补充官方演示场景、机型和测试口径；如果拿不到条件，改成“OPPO 宣传口径”并降低确定性。

## [Task9 Deep Review] 17.1 OEM 性能优化的通用思路 — 2026-04-21
- **类型**：数据缺失
- **位置**：L175
- **问题**：“一个用户常用的 App 一般不超过 10 个” 没有来源、样本量或地域条件。这个数字会直接影响读者对预测模型稀疏度的理解。
- **建议**：补充来源和样本范围，或改成“常用 App 集合通常远小于全量安装集”这类不带伪精确数字的表达。

## [Task9 Deep Review] 17.2 SoC 平台差异 — 2026-04-21
- **类型**：版本差异
- **位置**：L82 / L88
- **问题**：“Oryon 与苹果 M 系列同源” 和 “Tensor G5 预计将是首款完全自研芯片” 都是强结论或前瞻判断，但没有时间戳和证据等级。前者更适合写成“共享 Nuvia 团队渊源”，后者则需要显式标注为预测性信息。
- **建议**：把表述改成版本化、时间戳化描述，避免把产业猜测写成稳定事实。
...
## [External Review] 15 Methodology README — 2026-04-20
- **类型**：内容补全
- **位置**：15 README 章节导读
