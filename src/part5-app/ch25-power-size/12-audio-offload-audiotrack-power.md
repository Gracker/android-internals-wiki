...
last_verified_against: "Current Android Developers AudioTrack/AAudio/Android 17 features/Media3 docs + Android 17 CDD retrieved 2026-08-15；AOSP android-17.0.0_r1 AAudio.h/AudioTrack.java"
...
last_draft_polish_at: "2026-08-15T15:56:17+08:00"
last_draft_polish_run_id: "20260815-155617-gracker-writing-453"
last_review_finalize_at: "2026-08-15T15:56:17+08:00"
last_review_finalize_run_id: "20260815-155617-gracker-writing-453"
---
...
Audio offload（音频卸载）解决的是这类长时间播放的 CPU 参与度问题。平台把音频处理交给专用硬件或 DSP（Digital Signal Processor，数字信号处理器），应用可以一次写入更多数据；硬件 buffer（缓冲区）保存待播放内容后，框架数据管道可以暂停，CPU 因而有机会休眠。低延迟场景不适合这条路径。游戏音效、乐器、语音通话和实时互动仍应关注 AAudio（原生低延迟音频 API）的低延迟模式、MMAP（应用和硬件通过共享内存交换音频数据的路径）、缓冲区大小、callback（数据回调）的稳定性与线程调度，详见 1.16 节。

应用侧需要完成能力探测、接入、验证和小流量发布。AudioFlinger（原生音频混音与输出服务）、AAudio 与 MMAP 的机制详见 1.16 节；MediaCodec（编解码接口）、Media3（Jetpack 媒体库）与播放管线详见 18.21 节；Android 17 后台音频 hardening（生命周期限制）详见 25.11 节。
...
- 后台线：退后台或锁屏后是否满足 Android 17 后台音频 hardening、FGS（前台服务）与 while-in-use（WIU，应用可见或明确用户操作触发时授予的能力）规则，详见 25.11 节。
