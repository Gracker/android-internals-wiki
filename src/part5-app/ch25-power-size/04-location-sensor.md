...
last_verified_against: Android Developers location, sensors, FGS, GNSS, Wi-Fi RTT and BLE docs retrieved 2026-08-15 + Google Play services LocationRequest docs + AOSP android-17.0.0_r1
confidence: high
consolidated_from:
...
- type: official
  path: https://developer.android.com/reference/android/location/GnssMeasurementRequest.Builder
- type: official
  path: https://developer.android.com/develop/connectivity/wifi/wifi-rtt
- type: official
  path: https://developer.android.com/reference/android/net/wifi/rtt/RangingResult
- type: official
  path: https://developer.android.com/develop/connectivity/bluetooth/ble/background
- type: official
  path: https://developer.android.com/reference/android/bluetooth/le/BluetoothLeScanner
- type: aosp
...
定位与传感器功耗主要由采样频率、精度、批处理、唤醒和后台持续时间决定。优化时应从业务所需的新鲜度和准确度反推请求参数，再用系统记录验证硬件活动是否真正减少。

## 功耗边界

定位请求可能启用 GNSS（全球卫星导航系统）、Wi-Fi 扫描、蜂窝测位和传感器融合；传感器监听又会带来采样、FIFO（先进先出硬件缓冲区）交付、应用处理器唤醒与后续计算。功耗评审不能只看 API 名称，要同时检查数据从哪里产生、多久产生一次、何时交付，以及应用收到数据后做了多少 CPU（中央处理器）、存储和网络工作。

定位与传感器 API 大多表达期望条件或上限，并不承诺准确的到达周期。其他客户端请求、权限精度、设备硬件、传感器 HAL（硬件抽象层）、待机状态和省电策略都会改变结果。业务应定义可接受的精度、新鲜度、交付延迟、持续时间和退出条件，再选择一次性定位、连续 FLP（融合位置提供器）、Geofencing（地理围栏）、被动定位或传感器批处理。诊断工具见 §25.1，后台限制见 §25.2。
...
主线程回调只做轻量状态更新；轨迹压缩、写库和网络操作应移到受控执行器。解除注册返回 `Task<Void>`（Google Play services 的异步结果），需要严格确认停止完成的测试可以等待该任务，不能只依据页面回调已经执行。
...
图中的 JNI 指 Java Native Interface（Java 原生接口），负责 Java 框架与原生 C++ 层之间的调用。
...
返回 `false` 表示传感器不受支持或启用失败，不能把注册动作当成必然成功。one-shot（触发一次后自动停用）传感器要用 `requestTriggerSensor()`。如果 `fifoMaxEventCount` 为零，应用可以继续接收事件，但不要把 `maxReportLatencyUs` 当成可用的硬件批处理能力。

`SensorManager.flush(listener)` 是异步操作：调用后，FIFO 中已有事件按正常回调送达，随后才调用 `SensorEventListener2.onFlushCompleted()`。需要保存尾部事件时，应在 flush 成功后等待完成回调，再注销 listener（监听器）；还要准备超时退出。`flush()` 返回 `false` 表示该监听器没有已注册传感器，或至少一个 flush 请求失败。硬件没有原生 flush 支持时，框架仍可以发送一个简单的完成事件。

### wake-up（唤醒型）、non-wake-up（非唤醒型）与后台状态

wake-up sensor 在 FIFO 填满或最大报告延迟到期时可以唤醒应用处理器。non-wake-up sensor 在处理器挂起时不会主动唤醒；其 FIFO 填满后可循环覆盖旧事件，等处理器因其他原因醒来再交付。业务需要在完整保留事件和尽量不唤醒处理器之间选择。

Android 9（API 28）及以上设备不会向后台应用交付 continuous、on-change 或 one-shot 传感器事件。长时间运动采集需要用户可见的前台服务，并在服务停止时解除注册。Android 17 `SensorEventConnection::hasSensorAccess()` 还会检查 UID（系统分配的应用身份）是否活跃、进程是否被冻结以及传感器隐私开关；保留监听器不能让后台进程持续收到数据。

### Android 12 的 200 Hz 限制

200 Hz（赫兹，即每秒 200 个事件）是目标 SDK 与权限相关的高采样率限制，不是后台速率上限。目标版本为 Android 12（API 31）及以上且没有 `HIGH_SAMPLING_RATE_SENSORS` 的应用，通过 `registerListener()` 读取三类原始传感器时最多为 200 Hz：
...
Android 17 的 `SystemSensorManager` 使用 5000 微秒作为 200 Hz 周期边界；native（原生 C++ 层）`SensorService` 再按目标 SDK、权限和麦克风隐私状态调整采样周期与 direct channel（直接通道）档位。可调试包在 Java 层请求超限速率时会收到 `SecurityException`；非调试包可能由 native 层直接限到允许值，因此业务不能只依靠异常判断是否限速。
...
- [`SystemSensorManager.java`](https://android.googlesource.com/platform/frameworks/base/+/android-17.0.0_r1/core/java/android/hardware/SystemSensorManager.java)：应用请求、200 Hz 周期边界与高采样率权限检查。
...
传感器驱动和 sensor hub 固件通常由设备厂商实现，通用 AOSP（Android 开源项目）不能给出一条适用于所有设备的 Linux 驱动路径。这里不据此推断内核行为；后续若引用通用内核实现，统一使用 `android17-6.18-2026-06_r6`，不能拿旧内核分支解释 Android 17 设备。
...
检查调用 UID、请求是否 active（活动中）、间隔、质量，以及业务拥有者是否已结束。`last location` 仍存在只说明有缓存，不能证明 provider 正在工作；监听器数量多也不自动等于泄漏。设备提供 powerstats rail（硬件电源轨累计数据）时，它仍是设备级累计量，不是当前应用的 GNSS 精确归因。

## GNSS 原始测量、Wi-Fi RTT 与 BLE 测距

这三类 API 用于采集测量值，不能替代普通定位，也没有跨设备固定的精度或功耗。Wi-Fi RTT 指 round-trip time（往返时间）测距；BLE 指 Bluetooth Low Energy（低功耗蓝牙）。

- GNSS 原始测量需要 `ACCESS_FINE_LOCATION`。API 31 新增的 [full tracking](https://developer.android.com/reference/android/location/GnssMeasurementRequest.Builder) 会要求芯片关闭占空比，也就是停止周期性休眠，只在算法确需连续载波相位时开启。看到某个载波频率只证明本次观测包含该信号，不证明定位解算使用了双频。
- Wi-Fi RTT 从扫描得到的真实 `ScanResult` 构建请求。目标版本 33 及以上使用 `NEARBY_WIFI_DEVICES`，回调按 MAC（媒体访问控制）地址关联；Android 17 的 [`STATUS_BUSY_TRY_LATER`](https://developer.android.com/reference/android/net/wifi/rtt/RangingResult) 表示对端忙，应等待 `getRetryAfterDurationMillis()` 给出的时间再重试，不能立刻循环请求。
- BLE RSSI（接收信号强度指示）受人体遮挡、姿态、天线和多径影响，适合区域判定或排序，不应直接承诺米级距离。后台运行不会自动把扫描改成 `SCAN_MODE_LOW_POWER`；使用 `ScanCallback`（扫描回调对象）的无过滤扫描会在熄屏时暂停，进程结束后也无法继续回调。需要在进程未运行时接收匹配结果，可使用带过滤条件的 [`PendingIntent` 扫描](https://developer.android.com/develop/connectivity/bluetooth/ble/background)。所有扫描都要设置结束条件，并用原 `ScanCallback` 显式调用 `stopScan()`。

门店场景可以先用地理围栏缩小候选区域，再在用户进入相关功能或满足后台条件时启动有界 RTT 或 BLE 会话。每个测量会话都要记录能力检查、权限、开始、停止、样本数、失败和重试，避免定位已结束后扫描仍在运行。

## 定位和传感器的回归守门

定位与传感器回归要固定设备、系统版本、权限状态、位置开关、网络条件、屏幕状态、业务持续时间和移动轨迹。只比较一次总耗电值无法定位原因；测试记录还要保存请求参数、回调次数、每批数据量、事件时间、应用处理时长和退出后的残留注册。

这些命令用于查看 Android 17 上的平台位置请求、传感器连接和 UID 级电量归因。执行前先复现目标业务状态，执行后保存完整输出用于同版本对比。
...
`dumpsys location` 用来核对 provider、请求间隔、权限级别和前后台状态；`dumpsys sensorservice` 显示活跃连接、采样周期、批处理延迟、FIFO 与 wake lock（阻止处理器休眠的唤醒锁）相关信息；`batterystats` 用于把位置、传感器、唤醒和进程活动关联到应用 UID。不同厂商的字段可能不同，回归脚本应保存原始文本并只解析稳定字段。
...
- 目标版本 31 及以上在有无 `HIGH_SAMPLING_RATE_SENSORS` 时请求高于 200 Hz，并切换麦克风隐私开关；记录有效采样周期，不能只看请求值。
