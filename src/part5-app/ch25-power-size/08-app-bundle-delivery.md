...
25.6 和 25.7 处理产物中还能删除或缩小什么，这里关注哪些产物应在什么时间交给哪台设备。两个问题相互独立：AAB 不会删除 base module（基础模块）中的无用代码，R8（代码缩减与优化器）也不会决定低频功能是否延后下载。
...
Texture Compression Format Targeting（纹理压缩格式定向）允许 AAB 携带多种 GPU（图形处理器）纹理格式，由 Play 为设备选择受支持的格式，适合游戏或图形密集应用。普通 UI（用户界面）图片仍应按 25.7 的 WebP、AVIF、VectorDrawable 与网络图片策略处理。PAD 的模式、路径和更新语义以 [Play Asset Delivery](https://developer.android.com/guide/playcore/asset-delivery) 为准。
