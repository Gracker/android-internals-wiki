...
设备分级的目的，是为一次具体工作选择可承受的资源预算。图片解码关心内存和屏幕尺寸，视频播放关心编解码能力，复杂动画关心 GPU、刷新率和当前温控状态。把这些差异压成一个“高、中、低”总分，往往会让某项能力很强、另一项能力较弱的设备收到错误策略。

平台源码锚点为 Android 17 / API 37 / `android-17.0.0_r1`。普通应用不应读取隐藏的 `SystemProperties`、私有 `DeviceConfig` 命名空间或内核节点来猜测整机性能；这些接口不属于公开 SDK，访问权限和字段语义都可能变化。涉及内存语义时，以 Android 17 framework 对 Linux 可访问内存的解释为准；内核版本锚定 `android17-6.18-2026-06_r6`，应用策略仍只依赖公开 Android API。
...
`ActivityManager.isLowRamDevice()` 是面向应用的稳定提示，设备厂商通过系统配置声明它，适合收缩高内存功能。`getMemoryClass()` 返回当前应用近似的 Java 堆上限，单位是 MB；它不表示整机 RAM，也没有涵盖 native 内存（非 Java 堆内存）、图形和共享内存。

`ActivityManager.MemoryInfo` 中几个字段的语义不同：

- `totalMem` 从 API 16 提供，表示内核可访问的总内存，不含基带、TEE（Trusted Execution Environment，可信执行环境）等内核不可访问的固定保留区。
...
屏幕分辨率和刷新率同时代表能力与负载。120 Hz 面板提供更高刷新上限，每帧时间预算约为 8.3 ms；更高分辨率会增加每帧需要处理的像素量和内存带宽。渲染策略需要结合当前窗口尺寸、目标刷新率和帧数据，不能把屏幕规格简单加到“高端分”上。
...
这份数据仍是“信号集合”，不会自动成为性能等级。除 `availableProcessors` 只是采集时快照外，其余结果可连同 `Build.FINGERPRINT`（系统构建指纹）、应用版本和策略版本存入本地；系统升级、应用升级或远端规则版本变化时再重新计算策略。不要在每个页面、每一帧重复采集。

若应用最低版本低于示例中的 API 门槛，应保留版本判断。适用范围从 Android 10 开始，`isLowRamDevice`、`memoryClass` 与 `totalMem` 均已可用。

## 从能力向量生成工作负载策略

同一台设备可以同时得到“内存受限”和“媒体能力较强”两个判断。策略解析器应按业务维度读取这组能力向量，不要先算出通用总分再分配功能。
...
- 每次有效查询至少包含一次同步 Binder 跨进程调用，耗时可能超过 1 ms，不应在主线程、渲染线程或其他关键线程等待。
- 调用方应读取 `getCpuHeadroomMinIntervalMillis()` 或 `getGpuHeadroomMinIntervalMillis()`，遵守设备给出的最小轮询间隔。

这些 headroom 值描述当前工作负载还能获得多少容量，适合参与会话期调节。它们不适合作为稳定机型等级，也不应成为首帧前必须同步取得的数据。

动态质量控制还要防止来回振荡。常用办法是加入迟滞，也就是为降级和恢复设置不同阈值，再加上每个质量档位必须保持的最短驻留时间。以下状态机只允许画质在 `NORMAL` 与 `REDUCED` 两种状态间按规则切换，用温控和丢帧率说明这套逻辑：
...
- 只有在 trace（性能时间线记录）证明并发初始化发生 CPU 争用、锁竞争或 I/O 拥塞时，才减少并发任务；线程少不等于启动更快。
...
- `RGB_565` 只能在不需要 alpha 透明通道且可接受色彩精度损失的内容上评估，不能作为低内存设备的全局开关。
...
远端配置适合修正规则和控制灰度发布范围，但不能成为冷启动网络依赖。灰度发布是只向一部分稳定分组逐步开放新策略。可采用以下数据契约，也就是明确规定配置字段、版本、校验和失效方式：

- APK 内置经过验证的安全默认值。
- 本地保存上一份通过校验的配置；拉取失败继续使用它。
- 配置带 `schemaVersion`（格式版本）、`policyRevision`（规则修订号）、适用应用版本、过期时间和各项边界。
- 未识别字段按兼容规则忽略，越界值拒绝生效。
- 高风险开关提供独立的紧急关闭开关（kill switch），服务端可快速回退到上一版本。
- 策略在一个前台会话内保持稳定；运行中收到的新配置先记录，到下一次前台会话再切换。
- 记录分组、配置版本和曝光事件；“曝光”表示该策略已实际应用到这次用户行为，只有发生曝光的样本才进入实验分析。

灰度分组应使用符合隐私规则的稳定标识做确定性散列，使同一用户或安装实例稳定进入同一组；不要读取 IMEI、序列号等硬件标识。随后在能力画像或机型群内随机分配实验组。直接比较“高端组”和“低端组”会把用户、系统版本、地区和业务行为差异混进结论。汇总数据还可能出现辛普森悖论：各设备层内都变好，合计结果却因流量构成改变而变差。
...
每条策略都需要一个主要指标和一组护栏指标，也就是实验不允许明显恶化的副指标。例如，减少列表预取的主要指标可以是内存峰值，护栏包括滚动卡顿、图片等待时间、网络请求数和用户操作成功率。只看平均 FPS（每秒帧数）或平均启动时间，会掩盖长尾和受影响最重的设备。

验证流程可按以下顺序执行：

1. 用 trace 或 heap（堆内存）数据确认瓶颈归属，明确策略要减少哪类成本。
2. 在代表性的设备组合上跑 Macrobenchmark（端到端性能基准）或场景基准，覆盖冷、温、热启动以及关键交互。
