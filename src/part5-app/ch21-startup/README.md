...
第 8 章分析系统侧的响应速度与启动路径；这里聚焦 App 侧的任务执行顺序、Profile、运行时分配、首帧呈现与专项 SDK 治理。跨场景设备分级策略移入 16.4，通用缓存与数据局部性统一由 5.8 承载。
...
- 排查启动期分配、DI 容器或 Compose 首次组合时，依次阅读 21.7、21.8、21.9；业务缓存和设备策略分别见 5.8、16.4。
