...
调用方需要按场景传入经过实测的字节预算和维度比，不能把示例函数变成全业务共用的固定阈值。这段记录还要和页面生命周期合起来看。页面退出后，如果同一 `scene` 的大图对象仍在 HPROF 中可达，按 23.1 的引用链方法处理；如果对象已释放，但原生堆峰值过高，重点查解码尺寸、缓存上限和并发解码数量。
...
2. **生命周期是否按页面释放**：页面退出后查 HPROF，确认 Activity、Adapter、ImageView、图片请求和 Bitmap 是否仍被引用。引用链处理见 23.1。
