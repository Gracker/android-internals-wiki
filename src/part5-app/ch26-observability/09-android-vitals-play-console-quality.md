...
User-perceived LMK 常呈现为界面消失或进度丢失，Java crash handler 不会收到异常。Android 11 及以上的 `ApplicationExitInfo` 能补充历史退出原因，但查询到 `REASON_LOW_MEMORY` 仍只是诊断起点。设备上同时运行的进程、交换空间（swap）、内核回收压力和应用自身内存峰值都可能参与决策，详见 23.6 节。
