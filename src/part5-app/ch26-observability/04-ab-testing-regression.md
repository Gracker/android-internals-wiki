...
性能实验既要识别平均收益，也要防止平均值掩盖长尾问题、设备分层差异或业务指标劣化。发布阶段应预先定义随机化单位、样本比例、护栏和回归阈值，并在同一版本与场景内完成异常归因。

## 发布阶段需要回答的问题

性能治理进入发布阶段后，团队需要回答一组风险问题：新版本能否继续扩大灰度，某个配置是否适合放量，启动优化有没有伤到资源受限设备，页面改版有没有推高慢帧率。A/B 实验（A/B Test）用随机对照估计方案的因果影响；性能回归是新改动让既有指标变差，回归防护用于在代码合入、候选包和生产环境灰度阶段发现这种劣化。二者相关，但证据等级不同。
...
随机化单元和统计单元必须一致。按用户分桶时，同一用户的多次启动、帧或会话是相关观测，不能当作互相独立的样本。常见处理是先在用户级聚合，再比较两组；如果必须保留事件级模型，就使用按随机化单元聚类的标准误，或按该单元做自助重采样（bootstrap）。否则，活跃用户贡献更多事件会让置信区间过窄。

Firebase A/B Testing 的 Remote Config 实验可以配置目标人群、基线与变体、权重和指标。官方文档有两个容易忽略的边界：只有触发激活事件（activation event）的目标用户会进入实验结果，但其他命中目标条件的用户仍可能收到实验值；实验开始后不能修改变体（variant）权重。自建平台应把“被分配”“成功获取”“成功激活”“使用功能”“成功上报”拆成漏斗。主分析优先遵循预先指定的实验分配口径；按激活者补充分析时，要说明该口径可能受网络、启动成功率和变体自身崩溃影响。
...
A/A Test 给两组下发同一方案，用于检查分桶、曝光、上报和统计任务是否校准。它不能证明平台永远无偏，也不要求每次指标差异都恰好为零；团队应观察长期假阳性率、置信区间覆盖率和样本比例失衡（Sample Ratio Mismatch，SRM）。SRM 检查要覆盖全量与预注册关键分群，并沿“实验分配 → 配置拉取 → 配置激活 → 指标产生 → 上传成功”逐层定位丢失。多项主张、多分群或多次查看结果时，应预先选择多重比较或序贯控制方法，不能对每次查看都套用固定终点检验。

### 长尾指标的判定协议

P90/P99 的主效应应直接定义为 `Qτ(variant) - Qτ(baseline)`：`Qτ` 表示分位点函数，`τ` 是 0.90 或 0.99，`variant` 与 `baseline` 分别表示实验组和基线组。应直接为两组分位差构造区间；只看两组分位值区间是否重叠，代替不了差值区间。按用户或安装分桶时，聚类自助法（cluster bootstrap）必须按随机化单元重采样，并让被抽中的单元携带其全部合格事件。直接按帧、启动或请求重采样会破坏单元内相关性，也会让高活跃用户获得额外权重。

尾部报告不能只给一个 P99。还要预先登记体验阈值 `L`，报告 `P(X > L)` 的违约率、超过阈值的时长（excess duration）、独立实验单元数和每单元事件分布。若区间同时包含可接受改善和不可接受退化，结论就是证据不足（inconclusive），不能用单点方向替代不确定性。
...
每天查看一次普通固定终点 p 值，并在第一次越过显著性阈值时停止，会抬高假阳性率。平台应在实验登记时选择固定终点（fixed horizon），或选择带 alpha spending 的有限次分组序贯检验：预先把一类错误预算分配给每次查看。另一种选择是经过校准、允许按规则随时查看的 anytime-valid inference。Crash、ANR、数据损坏和严重性能伤害可以触发安全停止，但安全停止只说明需要止损，不自动证明方案有效。

多个确认性主张要控制家族错误率（family-wise error），也就是多项判断中至少出现一次假阳性的概率；大量设备、场景和时间分群只能标为探索性（exploratory），发现后进入复现实验。数据清洗也要在揭盲前登记：负耗时、时钟混用、重复事件和错误的数据关联（join）可以判无效；低端机、热降频、冷缓存与弱网造成的真实慢样本属于用户体验，不能因为数值极端而删除。缩尾处理（winsorization，将极端值替换为边界值）或截尾均值（trimmed mean，去掉两端样本后求均值）可以作为敏感性分析，不能替代原始分位值与违约率。
...
Android 官方性能测试文档把运行时性能测试分成实验室测试（local testing）和现场测试（field testing）。实验室测试使用基准测试库在可控设备上复现用户流程；现场测试观察真实用户环境。CI 应保存设备、系统镜像、构建配置、编译模式、测试数据与测量产物；缺少这些环境信息时，趋势变化可能只是设备温度、后台负载或构建配置发生了漂移。
...
Macrobenchmark 适合做实验室基线，但基准设备应是物理设备，目标包应接近发布版：不可调试、通过 `profileable` 允许测试工具采集性能剖析信息，并尽量开启与生产环境一致的优化。固定设备不代表环境恒定，仍要记录电量、温控等待、后台干扰、系统构建指纹（build fingerprint）、`CompilationMode` 编译模式和 App 数据准备方式。官方明确不建议用模拟器数值代表用户设备。
...
线上告警要同时处理不确定性和数据新鲜度。样本量不足时只记录风险；服务端故障、活动流量、配置切换和采样策略更新要标成外部事件。连续窗口之间没有天然的独立性，不能用“连续命中次数”冒充统计置信度。更稳的做法是固定观察窗口，报告效应区间与数据延迟，并把重复告警合并到同一事件。
...
性能门禁可以允许人工豁免，但测试环境异常应先标记为无效运行，不应把它伪装成业务豁免。有效豁免需记录批准人、证据、适用构建、到期条件、用户影响和补偿计划；到期后恢复门禁，不能形成永久白名单。
