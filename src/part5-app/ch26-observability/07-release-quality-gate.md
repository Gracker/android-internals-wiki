...
Clippings 的发布章节把移动应用交付拆成开发、编译 CI、测试、灰度和发布几个阶段；把数据验证平台放在灰度决策旁边，而不是等事故出现后再查。这里沿用这个结构，只把判断对象收束到 Android 性能与稳定性。[结构参考: Clippings/线上疑难问题该如何排查和跟踪？-Android开发高手课-极客时间 1.md；Clippings/线上疑难问题该如何排查和跟踪？-Android开发高手课-极客时间 29.md]
...
灰度发布的价值在于把候选包放回真实用户分布里验证。Clippings 的发布章节把灰度效率拆成测试效率和数据验证效率，这个拆法适合性能门禁：线下测试决定能不能开始灰度，线上数据决定能不能扩大灰度。[结构参考: Clippings/线上疑难问题该如何排查和跟踪？-Android开发高手课-极客时间 29.md]
