...
存储故障很少只有一个症状。一次“打开页面卡住”可能包含主线程读文件、SQLite 连接等待和目录扫描；一次“数据丢了”可能来自磁盘空间不足、损坏恢复策略或尚未持久化的写入。本章关注如何留下足以区分这些路径的线上证据。文件 I/O 和 SQLite 的优化方法分别见 24.1、24.2，通用采集预算见 26.3。
...
本章事件字段用于把问题导航到这些章节，不重复给出另一套优化规则。
