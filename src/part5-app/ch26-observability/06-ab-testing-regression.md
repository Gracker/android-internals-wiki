...
本文的平台结论以 Android 17 / API 37 / `android-17.0.0_r1` 为上界。实验统计与 CI 策略属于应用和数据平台设计；Android 版本主要影响 Macrobenchmark 指标是否可用，以及 `ApplicationExitInfo` 能提供哪些退出证据。本节没有依赖内核实现的判断，因此不附加 Android 17 kernel tag。
