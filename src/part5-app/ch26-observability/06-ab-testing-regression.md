...
26.3 节已经讲过性能指标采集与上报，这里只处理实验设计、回归检测、持续集成与持续交付（CI/CD）门禁和劣化归因。重点是怎样把实验室基准测试、生产环境分流实验、灰度监控和发布决策放到同一套判断流程里。
...
`TraceSectionMetric` 和业务 trace 名称要提前统一。实验室基准测试里 `home.bind_data` 变慢，生产环境同名摘要也变慢，归因系统就能把告警指到首页数据绑定阶段；如果实验室叫 `HomeBind`，生产环境叫 `feed_first_render`，后端只能靠人工判断。26.3 节已经建议自定义 Trace 和生产环境摘要使用同名阶段，这里直接复用该规则。
...
5. 如果 Trace 指向业务阶段，派给业务模块；如果指向系统阶段，关联到 13.2、15.6、21.x、22.x、23.x 对应章节做实验室复现。
...
26.7 会继续处理发版质量门禁，把回归检测结果放进发布前检查清单、灰度放量和回滚决策。
