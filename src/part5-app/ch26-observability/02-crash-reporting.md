...
### 持久化:Java 与 Native 分两条路径落盘

崩溃持久化不能走同一条路。Java handler 和 Native signal handler 能做的事完全不同。

**Java handler 路径**:Java `UncaughtExceptionHandler` 仍然运行在 JVM 里,可以安全写文件、序列化 JSON、分配内存。Java Crash 的持久化策略:
...
**Native signal handler 路径**:Native 信号处理器受 async-signal-safe 约束,不能调用 `malloc`、`free`、`printf` 或任何堆操作,也不能使用锁或做 JSON 序列化。Android 上 Native Crash 的现场保存依赖系统 debuggerd 或第三方 Crashpad/Breakpad 客户端:

- **debuggerd 路径**(系统级):`system/core/debuggerd/crash_dump.cpp` 读取 `siginfo_t`、寄存器和进程信息生成 tombstone,写入 `/data/tombstones/tombstone_XX`(Android 10+ 逐步 proto 化)。这套路径由 debuggerd 常驻进程通过 `sigaction()` 注册信号处理,崩溃时 fork 出 `crash_dump` 子进程完成 tombstone 写入--handler 内只通过 `write()` 向 debuggerd 写管道,不在进程内做堆分配。
- **Crashpad/Breakpad 路径**(SDK 级):第三方 SDK 使用 out-of-process handler 模型--崩溃时由独立 handler 进程生成 minidump,写入 App 私有数据目录(如 `/data/data/<package>/databases/crashpad/`),不阻塞应用主线程。Crashpad Android client 通过 `sigaction()` 注册 `SIGSEGV`、`SIGABRT` 等信号,支持 RequestCrashDumpHandler(与已运行 handler 通信)和 LaunchAtCrashHandler(崩溃时启动)双策略。

Native Crash 样本的持久化在两处:系统 tombstone(全局环形缓冲,有覆盖风险)和 App 目录下的 minidump(受 App 存储配额限制)。SDK 端侧不做复杂 JSON envelope 构造;只记录 `crash_id`、时间戳和 dump 文件路径,完整组装放到下次启动完成。

**下次启动补偿链路**:不管 Java 还是 Native 崩溃,下次启动或独立上传进程都要做三件事:扫描本地 crash store、通过 `ActivityManager.getHistoricalProcessExitReasons()` 拉取系统记录的退出原因、合并 SDK envelope 与系统 exit reason 做去重上报。Android 11(API 30)+ 引入的 `ApplicationExitInfo` 补充了系统视角的退出信息--即使 SDK 信号处理器没抓到现场,也能从 `REASON_CRASH_NATIVE`(退出原因码 8)/ `REASON_CRASH`(退出原因码 7)等退出原因中确认进程退出类型。API 31+ 还可通过 `getTraceInputStream()` 读取 Native Crash 的 tombstone protobuf。SDK envelope 与系统 exit reason 的去重键见本节附录 A.1。

[结构参考: Clippings/线上疑难问题该如何排查和跟踪?-Android开发高手课-极客时间 33.md]

### 上报:优先保证摘要送达,再补附件

Crash 上传不要依赖"本次崩溃进程还能把网络请求发完"。稳定方案是下次启动或独立上传进程扫描本地 crash store 和系统退出记录（`ActivityManager.getHistoricalProcessExitReasons()`），先上传摘要，再按服务端返回的需求上传附件。SDK envelope 与 `ApplicationExitInfo` 的去重键见本节附录 A.1；合并规则是：同 pid + 同时间戳窗口内的系统退出原因与 SDK 样本视为同一次崩溃，不重复上报；仅系统记录到而 SDK 未成功落盘的样本，以 `ApplicationExitInfo` 作为补漏数据源上报。
