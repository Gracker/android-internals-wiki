...
本章以 Android 17 / API 37 / `android-17.0.0_r1` 为平台锚点，以 CameraX 1.6.1 为稳定库基线。涉及跨设备缓冲区共享与同步时，kernel 源码锚点是 `android17-6.18-2026-06_r6`。本文集中处理应用侧配置和诊断；HAL3 request-result、BufferQueue、fence 与显示管线的完整推导见 [18.14 Android 17 Camera 渲染管线](../../part2-performance/ch18-rendering-pipelines/14-camera-pipeline.md)。
...
截至 2026-07-29，AndroidX 官方版本页列出的 CameraX 稳定版是 1.6.1，预览版是 1.7.0-alpha02。没有可核验的 3.0 主版本发布物。文件名中的 `camerax-3.0` 是早期选题遗留，正文不据此创造版本能力。

CameraX 与 Android API level 分开发布。Android 17 设备可以运行较旧的 CameraX，而旧库未必认识平台新增的 metadata。CameraX 1.5.1 及更早版本在部分 Android 17 设备遇到新的 dynamic range profile 时可能在绑定阶段崩溃；官方要求升级到 1.5.2、1.6.0 或更高版本。本章直接采用 1.6.1。
...
- CameraX 1.7 仍处于 alpha 阶段，其中的 GPU `ImageAnalysis`、QHD 录像等接口不属于本章基线。
...
因此，旧文章里把所有录像尾部耗时都归到平台 `MediaMuxer` 已不准确。诊断要记录 CameraX 版本与输出格式。
...
仓内 [14.9 Camera 性能分析](../../part3-tools/ch14-other-tools/09-camera-performance-analysis.md) 给出了 Android 17 的 Perfetto 配置、CameraService open、stream 首缓冲区、请求延迟 histogram 与 ZSL 时间边界。本章不重复其采集配置。
...
- [14.9 Camera 性能分析](../../part3-tools/ch14-other-tools/09-camera-performance-analysis.md)
