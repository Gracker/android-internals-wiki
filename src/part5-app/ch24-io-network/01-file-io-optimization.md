...
本文的平台源码锚点是 Android 17 / API 37 / `android-17.0.0_r1`。涉及页缓存、回写和 `fsync()` 时，内核锚点是 `android17-6.18-2026-06_r6`。
...
普通文件 I/O 的难点不是 API，而是并发、可见性和持久性。多个线程同时写一个文件、读取方看到半成品，或写入失败后只留下损坏版本，都会把性能问题变成数据问题。
...
[自动发现] 文件 I/O 优化要保留可观测入口。至少记录单次读写耗时、数据量、调用线程和业务场景。调试构建用 StrictMode 抓主线程读写；灰度构建可对慢操作采样。分析时区分读取、写入、编码/解析、锁等待和 `fsync()`，并关联存储剩余空间与同期系统负载。不要记录用户文件内容、完整路径或敏感键值。
