...
本章以 Android 17 / API 37 / `android-17.0.0_r1` 为平台锚点。Scoped Storage 和 FUSE 的演进见 6.1、6.4 节；图片加载与缓存见 22.6、24.6 节；普通文件 I/O 的线程治理见 24.1 节。
...
对账前确认卷仍然挂载，且媒体读取权限没有收窄。权限变化造成的“不可见”不能记为用户删除。后续 37.1 SDK Extension 增加了 deleted-files 相关 API，但它不属于本章的 `android-17.0.0_r1` 锚点。
