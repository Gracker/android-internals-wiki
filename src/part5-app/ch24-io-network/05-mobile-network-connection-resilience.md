...
本文以 Android 17 / API 37 / `android-17.0.0_r1` 为主要平台，同时标出 37.1 新增的 `HttpEngine` 计时能力。本文前文已给出请求事件与重试上限，24.6 解释 HTTP/2、HTTP/3 和 QUIC，24.7 解释 HTTPDNS 接入；这里讨论样本设计、对照实验和发布阻断规则。发布阻断规则是持续集成或发布系统中的自动判断：指标越过约定阈值时停止合入或发布。
...
NXDOMAIN 表示 DNS 明确回答“该域名不存在”，与查询超时或返回空记录不是同一种失败。超时参数来自用户路径截止时间和各阶段历史分布，不能复制一组全局数字。网络库内部恢复、业务重试、图片库重试和 WorkManager 退避都要计入同一个逻辑操作；退避指连续失败后逐步延长下一次尝试前的等待。有关实现顺序与幂等边界见 24.5。
