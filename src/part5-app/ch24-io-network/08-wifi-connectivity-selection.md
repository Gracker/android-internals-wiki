...
支付、下单和上传仍需幂等与恢复协议。连接迁移只能改变传输连续性，不能提供业务一致性。协议细节见 [24.6 HTTP/2、HTTP/3、gRPC 与 ECH](06-http2-http3-grpc-ech.md)。
