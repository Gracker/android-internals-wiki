...
连接复用仍取决于 OkHttp 的 `Address`（连接地址配置）、`Route`（具体连接路径）、证书和连接健康状态。把资源分散到多个域名的“域名分片”可能增加 DNS、TCP 与 TLS 成本，也可能阻止符合条件的 HTTP/2 连接合并。资格检查见 [24.3 移动网络架构、连接与容灾](03-mobile-network-connection-resilience.md)。
...
平台源码以 Android 开源项目（AOSP）的 `android-17.0.0_r1` 标签为基准。ECH 的 Android 实现位于用户空间的 DNS 与 TLS 组件，没有供应用调用的内核接口。TLS 1.3、证书链、连接池和 HTTP/2、HTTP/3 原理见 12.1、24.3 与 24.4。
...
| 私网地址、`.local` 或设备发现失败 | `ACCESS_LOCAL_NETWORK` 未获授权 | 按 24.8 的本地网络权限路径分析 |
...
12.1 解释 TLS 1.3、证书、CT、SNI、混合公钥加密（HPKE）、会话恢复和安全连接成本。ECH 使用 HPKE 加密 ClientHello 内层。Android 17 的按域名策略、配置获取和接入验证构成本节的范围。握手耗时、证书链和 0-RTT 等问题仍按 12.1 的方法分析；0-RTT 指恢复会话时在完整握手结束前发送早期数据。

### 与 24.8 本地网络权限适配的关系

24.8 处理 Android 17 `ACCESS_LOCAL_NETWORK`、局域网设备发现、Google Cast 投屏、物联网（IoT）设备与本地 HTTP 服务。ECH 与网络安全配置适用于公网 HTTPS 连接，两类问题的失败阶段不同。
