...
24.5 节负责公网请求的弱网处理和重试预算。Bluetooth 套接字可以复用失败分类、安全重试规则和总时间预算，但错误仍需独立建模，不能归入 HTTP 的 DNS、连接或读取超时。

26.10 节负责线上网络质量监控。Bluetooth 套接字应作为独立通道上报，原始事件保留 `socket_type`、`read_exit_reason`、`connection_id` 和 `retry_stop_reason`。
