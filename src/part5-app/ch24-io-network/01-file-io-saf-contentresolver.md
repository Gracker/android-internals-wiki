---
title: 文件 I/O、SAF 与 ContentResolver 性能
chapter: '24.1'
section: '24.1'
status: finalized
...
- src/part5-app/ch24-io-network/01-file-io-optimization.md
- src/part5-app/ch24-io-network/18-saf-documentfile-contentresolver-performance.md
---

# 文件 I/O、SAF 与 ContentResolver 性能

文件 I/O 成本来自系统调用、缓冲、页缓存、同步写和存储设备；SAF 又增加 URI 授权、ContentResolver、Provider 与 Binder。路径选择应同时满足权限模型和性能需求。
...
主线程 I/O 的风险来自不可预估的尾延迟。读路径可能命中页缓存，也可能触发缺页、存储读取和目录遍历；写路径通常先写入页缓存，需要持久化时还会通过 `fsync()` 等待文件系统完成相应回写。线程等待期间不一定消耗很多 CPU，却无法继续处理输入、布局和绘制。对应到 ANR（Application Not Responding，应用无响应），就是主线程没有及时返回消息循环，详见 [9.1 ANR 类型与触发条件](../../part2-performance/ch09-anr/01-anr-mechanism-types-triggers.md)。
