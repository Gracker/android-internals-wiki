...
本节从应用架构处理四个问题：怎样复用连接，怎样选择解析策略，怎样隔离不同类型的流量，以及怎样在网络变化和请求失败时控制重试。一次调用的分段计时与协议细节见 [12.3 网络性能深入](../../part2-performance/ch12-apk-network/03-network-performance-deep.md)，TLS 信任边界见 [12.4 网络安全与 TLS 性能](../../part2-performance/ch12-apk-network/04-network-security-tls-performance.md)。
...
退避可以采用带随机抖动的指数策略，但基数、上限和次数必须来自业务恢复目标、服务端容量与整次截止时间。文章无法给出对所有接口都安全的一组毫秒值。
