...
| Material 3 Adaptive | 1.2.0 稳定版 | 本文代码使用这一版公开 API；Large / Extra-large 需要显式开启 |
...
下面的依赖用于本文的 Compose canonical layout 和直接读取折叠特征。三个 adaptive artifact 保持同一版本，WindowManager 单独定版。
...
桌面窗口还要覆盖鼠标 hover、滚轮、右键、键盘快捷键、Tab 焦点和 caption Insets。WindowManager 1.6 alpha 才引入的精确指针 engagement API 不属于本文 1.5.1 稳定基线，不能用 alpha API 描述稳定版实现。
