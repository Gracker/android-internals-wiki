...
previous_sources:
  # 1.11.4 与 BOM 2026.06.00 的旧锚点，保留用于核对升级差异。
  - https://dl.google.com/dl/android/maven2/androidx/compose/runtime/runtime/1.11.4/runtime-1.11.4-sources.jar
  - https://dl.google.com/dl/android/maven2/androidx/compose/runtime/runtime-android/1.11.4/runtime-android-1.11.4-sources.jar
...
本篇聚焦 Snapshot，同时说明 Recomposer、ComposeView 与业务并发边界。Compose 到 Android 17 显示系统的完整路径见 [§18.23 Jetpack Compose 渲染管线架构](../../part2-performance/ch18-rendering-pipelines/23-compose-rendering-pipeline.md)。
...
编译器结论要与运行时跟踪、重组计数和业务标记配合。详细配置见 [§22.22 Compose Compiler Metrics 与重组诊断](22-compose-compiler-recomposition-diagnostics.md)。
...
Composition tracing 通过 `androidx.compose.runtime:runtime-tracing` 暴露可组合函数的源码位置与重组切片。它适合回答“哪个可组合函数在执行”，无法自动显示“哪个 StateObject 的哪次赋值造成这轮失效”。配置与录制见 [§22.22 Compose Runtime Tracing](22-compose-compiler-recomposition-diagnostics.md)。
