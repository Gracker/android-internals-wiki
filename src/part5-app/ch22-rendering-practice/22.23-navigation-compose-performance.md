...
- 审计结论必须区分 Navigation、Compose Runtime、业务 IO、图片解码和动画绘制。Navigation 只是触发页面切换的入口，不应负责所有首帧慢的归因。
...
Navigation 2.9.8 的 exact source JAR 是本文 AndroidX 行为的主要依据；Android 17 与 kernel 固定 tag 只负责宿主帧和调度边界。升级任一版本后，应重新核对 `NavHost`、entry lifecycle、状态恢复和预测返回实现。
