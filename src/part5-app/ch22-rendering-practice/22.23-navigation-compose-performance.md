...
sources:
  ...
  - "developer.android.com/jetpack/androidx/releases/navigation#2.9.8"
  - "developer.android.com/guide/navigation/design/type-safety"
  - "developer.android.com/guide/navigation/backstack/multi-back-stacks"
  - "developer.android.com/jetpack/androidx/releases/hilt#1.4.0"
  ...
  - "developer.android.com/topic/performance/benchmarking/macrobenchmark-metrics"
  - "developer.android.com/topic/performance/jankstats"
  - "perfetto.dev/docs/data-sources/frametimeline"
  - "dl.google.com — androidx.navigation:navigation-compose:2.9.8 sources"
  - "dl.google.com — androidx.navigation:navigation-runtime:2.9.8 sources"
  - "dl.google.com — androidx.navigation:navigation-common:2.9.8 sources"
  - "dl.google.com — androidx.hilt:hilt-lifecycle-viewmodel-compose:1.4.0 sources"
  - "AOSP android-17.0.0_r1 — frameworks/base, frameworks/native"
  - "AOSP kernel android17-6.18-2026-06_r6 — kernel/sched"
...
- 对共享元素、复杂 enter/exit transition 或大屏多 Pane 导航，结论必须以 Macrobenchmark、Perfetto 或 FrameMetrics 数据为准；本章不写无测试条件的“提升 xx%”结论。[具体收益数值需项目基准测试确认]
...
本章的 AndroidX 基线固定为 `androidx.navigation:navigation-compose:2.9.8`。该版本于 2026 年 4 月发布，并修复了 `NavHost` 在预测返回期间可能触发空指针异常的竞态。类型安全路由从 Navigation 2.8.0 起已经稳定，本章示例统一使用这套 API。[Navigation 2.9.8 发布说明](https://developer.android.com/jetpack/androidx/releases/navigation#2.9.8)
...
所有固定收益数字都需要目标应用的基准测试。文章只给出源码可核对的生命周期、状态与渲染边界。
...
普通 `NavHost` 没有为每个 destination 建立独立 Surface。进入页和离开页的 Compose 内容写进同一个 App Window buffer；SurfaceFlinger 通常只看到宿主窗口 layer。完整显示路径见 [18.25 Compose 渲染管线](../../part2-performance/ch18-rendering-pipelines/25-compose-rendering-pipeline.md)，动画阶段见 [22.21 Compose 动画性能](21-compose-animation-performance.md)。
...
若页面嵌入 `SurfaceView`、`TextureView`、视频或 WebView，出图拓扑会发生分叉。本节关于“同一个 App Window buffer”的描述只覆盖普通 View/Compose/HWUI 主体。
...
- TestNavHostController 只承担正确性测试，性能结论来自真实窗口和真机。
...
Navigation 2.9.8 的 exact source JAR 是本文 AndroidX 行为的主要依据；Android 17 与 kernel 固定 tag 只承担宿主帧和调度边界。升级任一版本后，应重新核对 `NavHost`、entry lifecycle、状态恢复和预测返回实现。
