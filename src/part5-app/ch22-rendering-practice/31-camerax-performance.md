...
平台锚点为 Android 17 / API 37 / `android-17.0.0_r1`，稳定库基线为 CameraX 1.6.1。涉及跨设备缓冲区共享与同步时，kernel 源码锚点为 `android17-6.18-2026-06_r6`。重点是应用侧配置和诊断；HAL3 request-result、BufferQueue、fence 与显示管线的完整推导见 [18.14 Android 17 Camera 渲染管线](../../part2-performance/ch18-rendering-pipelines/14-camera-pipeline.md)。
...
| CameraX 预览版 | 1.7.0-alpha02，仅作观察 | 不进入生产基线，也不据此描述稳定能力 |
| Android common kernel | `android17-6.18-2026-06_r6` | dma-buf、dma-fence 与 sync_file 公共语义 |

截至 2026-07-29，AndroidX 官方版本页列出的 CameraX 稳定版是 1.6.1，预览版是 1.7.0-alpha02。没有可核验的 3.0 主版本发布物，因此不据此描述版本能力。

CameraX 与 Android API level 分开发布。Android 17 设备可以运行较旧的 CameraX，而旧库未必认识平台新增的 metadata。CameraX 1.5.1 及更早版本在部分 Android 17 设备遇到新的 dynamic range profile 时可能在绑定阶段崩溃；官方要求升级到 1.5.2、1.6.0 或更高版本。以下采用 1.6.1。
...
- CameraX 1.7 仍处于 alpha 阶段，其中的 GPU `ImageAnalysis`、QHD 录像等接口不属于稳定基线。

## 2. CameraX 1.6 的位置：CameraPipe 仍在 Camera2 与 HAL 之上

CameraX 1.6 的架构变化需要单独说明。`Camera2Config.defaultConfig()` 已返回基于 CameraPipe 的配置，CameraX 不再沿用早期那套 Camera2 内部实现。不过，CameraPipe 使用的仍是平台 Camera2 接口；它没有绕过 `cameraserver`、Camera HAL、sensor 或 ISP。
...
图里的分层也给出了归因规则：用例协商、quirk 与绑定行为属于 CameraX；请求提交和结果分发跨越 CameraPipe 与平台；曝光、ISP 和厂商多帧算法属于设备；分析、编码、封装、存储和显示各有独立消费者。

| CameraX 用例 | 主要下游 | 常见限制 |
|---|---|---|
| `Preview` | 输出 Surface、BufferQueue、SurfaceFlinger / HWC | 实际承载类型、裁剪、旋转、显示合成 |
| `ImageAnalysis` | ImageReader 类消费者、Analyzer 执行器 | 回压、格式转换、图像归还、模型耗时 |
| `ImageCapture` | still request、3A、ISP、编码、I/O | 画质模式、ZSL、flash、多帧处理 |
| `VideoCapture` | 相机输出、编码器、Muxer、文件 | 质量组合、dynamic range、持续吞吐 |
| Extensions | 扩展会话、厂商算法 | 支持范围、延迟、内存、热与设备差异 |

Android 17 的 `Camera3OutputStream` 仍从 `ANativeWindow` 取得缓冲区，处理 acquire fence，把 HAL 返回的 release fence 随缓冲区交给消费者。CameraX 的 Java/Kotlin API 不改变这条所有权链。Analyzer 长时间持有 `ImageProxy`、编码器消费变慢或显示端等待，都可能向上游施加压力。

## 3. 会话配置先于单个参数优化

CameraX 需要把所有同时运行的用例转换为一组底层输出。某个尺寸能够单独打开，不能证明它能和另外三路输出并发。分辨率、格式、帧率、dynamic range、稳定、扩展模式及设备 quirk 都会参与组合选择。

### 3.1 一次绑定完整用例集合

应用已知要同时运行的用例时，应在同一次 `bindToLifecycle()` 中提交。CameraX 会基于完整集合协商输出。先绑定 Preview，再多次加入或移除其他用例，可能反复触发会话配置、缓冲区准备和首帧等待。

CameraX 1.6 的 `SessionConfig` 可以把用例、帧率、视口、效果和特性组放在同一个对象中。对正在运行的生命周期再次绑定新 `SessionConfig` 仍可能暂时停止数据并重新初始化，不能把该调用当成无缝更新保证。
...
- CameraX artifact 版本、相机 id、前后摄与 logical / physical camera；
- 已绑定用例及其格式、最终尺寸、crop rect 与 target rotation；
- 目标帧率范围、实际 CaptureResult 帧时长与曝光时间；
- dynamic range、稳定、ZSL、Extensions、CameraEffect 与 Camera2Interop 配置；
- `PreviewView` 请求的实现模式及实际 View / layer；
...
Android 17 / API 37 新增的 `CameraCaptureSession.updateOutputConfigurations()` 是 Camera2 平台接口，用于替换既有同构输出的 Surface。CameraX 1.6 没有承诺把任意用例切换自动映射为该 API。需要精确控制 Android 17 动态输出更新时，应评估 Camera2。
...
- `TextureView` 把相机图像作为纹理进入宿主窗口，裁剪、旋转、动画和 View 变换更灵活，但每帧还要经过宿主 HWUI / GPU 路径；
...
这段配置只表达偏好。CameraX 仍可因 API level、LEGACY camera、设备 quirk 等条件选择 `TextureView`。性能报告要通过 View 层级、SurfaceFlinger layer 或实际内部 View 确认承载方式。

这里还有一个源码与说明文档之间的细节。CameraX 1.6.1 的 `PreviewView` 文档把 target rotation 与显示旋转不一致列为 `TextureView` 回退条件；同版本的 `shouldUseTextureView()` 对应逻辑尚未实现，实际判断包含 API 24 及以下、LEGACY camera 与 SurfaceView quirk。工程结论应以目标版本源码和设备观测为准，不能只根据枚举名称推断。
...
2. 用例绑定与底层会话配置；
3. 首个有效 camera output 到达预览消费者；
4. 预览 layer 被显示系统 latch 并 present。
...
预览分辨率只需覆盖显示和业务裁剪需求。超过屏幕与识别需求的输出会增加 ISP、缓冲区、内存带宽和 TextureView / 特效路径的 GPU 工作。降低分辨率也可能让 HAL 选择另一 sensor mode，因此效果要在同一设备上复测。
...
不要设置一条跨设备的“超过多少毫秒就是 CameraX 卡顿”规则。统计 source sensor timestamp、camera output、显示 present 三组间隔，才能区分相机产帧变化与显示漏帧。

## 5. ImageAnalysis：吞吐、等待时间和归还责任

`ImageAnalysis` 是应用最容易直接造成相机回压的位置。CameraX 1.6.1 的源码提供两种策略：

| 策略 | 行为 | 适合场景 |
|---|---|---|
| `STRATEGY_KEEP_ONLY_LATEST` | Analyzer 忙时替换待处理的旧帧；队列深度配置被忽略 | 扫码、姿态、预览叠加等关注最新结果的实时任务 |
| `STRATEGY_BLOCK_PRODUCER` | 按顺序保留帧；队列满后生产端等待，影响同一 camera device 的其他用例 | 每帧都必须处理且平均吞吐能跟上输入的任务 |

`KEEP_ONLY_LATEST` 是默认策略。`BLOCK_PRODUCER` 的默认队列深度为 6，深度包含正在分析的图像。增加深度只能吸收短时抖动，同时会增加内存占用和结果等待时间；平均处理速度低于输入速度时，队列迟早会占满。

下面的 Analyzer 适合实时识别。它保留最新帧，并用 ATrace 标出应用处理区间。
...
`YUV_420_888` 是默认输出。请求 `RGBA_8888` 时，CameraX 会在内部执行 YUV 到 RGBA 的转换，并把结果放进第一个 plane。这个时间不能从模型推理计时中消失。
...
5. 降低分析频率或依据场景变化触发分析；
6. 选择更轻的模型或合适的 NNAPI / GPU 执行路径；
7. 只有必须逐帧处理时才使用 `BLOCK_PRODUCER`，并对队列等待时间设监控。

## 6. ImageCapture：捕获模式只表达偏好

`ImageCapture` 的端到端延迟可能包含 3A、曝光、sensor readout、ISP、厂商多帧处理、JPEG / DNG 编码、应用回调和存储写入。`takePicture()` 到回调的单个数值无法说明是哪一段变慢。

### 6.1 三种捕获模式

- `CAPTURE_MODE_MINIMIZE_LATENCY`：默认值，优先减少捕获延迟；
- `CAPTURE_MODE_MAXIMIZE_QUALITY`：允许设备选择更重的画质路径；
- `CAPTURE_MODE_ZERO_SHUTTER_LAG`：实验能力，从候选历史帧中选择接近按键时刻的帧，再执行 reprocess。

ZSL 的名称不承诺文件立即可用。它要求 API 23 及以上和 `PRIVATE` reprocessing；不满足时回退到 `MINIMIZE_LATENCY`。flash 为 ON / AUTO、绑定 `VideoCapture` 或使用 Extensions 时，当前公开说明不支持 ZSL。使用前应查询 `CameraInfo.isZslSupported()`，运行日志还要记录查询结果、flash、用例组合与捕获模式。
...
Android 17 / API 37 新增 `ImageFormat.RAW14`，设备还必须公布相应输出能力。CameraX 1.6 的 RAW / RAW+JPEG 能力通过 `ImageCaptureCapabilities` 和 CameraX 输出格式表达。CameraX 支持 RAW 不能证明它把平台 `RAW14` 暴露给应用。
...
选择器给出了优先级和回退规则。绑定后仍要记录最终尺寸、帧率、dynamic range、编码器与输出 `MediaFormat`，不能把 `FHD` 当成固定 codec、码率或色彩格式。

### 7.2 丢帧按四段归因

| 段 | 观测 | 常见问题 |
|---|---|---|
| 相机输出 | sensor timestamp、camera output 间隔 | 曝光、HAL、ISP、会话配置、热限制 |
| 编码输入 | Surface queue、编码器输入节奏 | 输出组合、GPU 效果、编码器背压 |
...
Night、HDR、Bokeh、Face Retouch 等 Extensions 会建立扩展会话，并调用设备厂商实现。它们可能使用多帧采集、额外中间缓冲区和后处理线程，延迟、内存与热表现不能从普通 `ImageCapture` 推导。
...
Android 17 平台还增加 vendor-defined camera extension mode。这个 Camera2 平台能力不表示 CameraX 1.6.1 已为所有自定义 mode 提供等价的稳定接口。应用需要自定义 vendor mode 时，应核对 CameraX 版本 API；无法表达时转向 Camera2。
...
- GPU Analyzer 或效果会增加纹理导入、同步、shader 与输出 Surface 工作；
- NNAPI / GPU 模型可能与预览效果争用 GPU 或内存；
- 逐帧 YUV → RGB 与 Bitmap 分配会增加 CPU、内存和 GC 压力；
...
相机持续运行会同时使用 sensor、ISP、DDR、CPU、GPU、NPU、编码器、显示与存储。短时间测试可能处于高频状态，几分钟后 thermal policy 调整频率或设备算法，帧率和延迟分布随之变化。
...
- 录像编码设置与存储位置；
- 预热时间、总时长和 thermal status。

降级应由状态机控制，并保留滞回，避免温度在边界附近时反复重绑会话。只降低应用线程优先级通常不能解决 ISP、GPU、编码器或 DDR 的热限制。

公共 kernel 的 dma-buf 和 dma-fence 源码可解释缓冲区共享与同步等待，但相机 sensor、ISP、频率和带宽策略通常位于厂商驱动、固件与 HAL。缺少厂商轨迹时，不要把 kernel 调度等待直接写成 ISP 结论。
...
| 指标 | 起点与终点 | 需要关联的证据 |
|---|---|---|
| provider 初始化 | 请求 provider → provider 可用 | 应用 marker、Binder、线程调度 |
| 绑定耗时 | `bindToLifecycle` → 会话可出帧 | CameraX 日志、camera configure、HAL |
| 首个 camera output | 首个 repeating request → 首个有效输出 | camera ATrace、frame number、stream |
...
| 拍照延迟 | 用户触发 → shutter / 图像 / 文件 | capture callback、sensor timestamp、I/O |
| 录像稳定性 | 录制开始 → Finalize | camera、codec、Muxer、存储、thermal |

平均值会隐藏首帧、弱光、切镜头和热限制的长尾。至少报告 p50、p95、p99、最大值、失败数与样本场景；样本量不足时直接说明，不补造百分位。
...
- Binder、sched、CPU frequency：跨进程调用和线程调度；
- MediaCodec / Codec2：录像编码；
- thermal、CPU / GPU / memory frequency：持续运行；
- 应用 ATrace：页面动作、绑定、Analyzer、拍照与录制状态。

FrameTimeline 主要解释应用窗口帧。`SurfaceView` 预览是独立 layer 时，应用窗口 jank 不能替代 camera output 与预览 layer present 证据。`TextureView` 则还要追宿主 HWUI 和应用窗口提交。

[14.20 Camera 性能分析](../../part3-tools/ch14-other-tools/20-camera-performance-analysis.md) 给出了 Android 17 的 Perfetto 配置、CameraService open、stream 首缓冲区、请求延迟 histogram 与 ZSL 时间边界，可直接沿用其采集配置。

### 11.3 内存必须分域

相机内存可能分布在：

- Java / Kotlin heap：业务对象、Bitmap、模型输入输出；
- native heap：CameraX、图像转换、codec、厂商库；
- GraphicBuffer / dma-buf：Preview、Analysis、录像与拍照输出；
- GPU、ISP、codec 与厂商私有池；
- ZSL、Extensions 和多帧算法的候选图像。

进程 RSS 增长不能直接换算为 `ImageAnalysis` 队列大小。先确认未关闭的 `ImageProxy`、Bitmap 分配和 native heap，再结合 dma-buf、Surface 与厂商数据判断。
...
`Camera2Interop` 可以向 CameraX 配置加入部分 `CaptureRequest` 选项或读取 Camera2 信息。它不会把 CameraX 用例变成应用自管的 `CameraCaptureSession`，也不给应用直接管理 stream、Surface、request sequence 和回调顺序。
...
- 管理 vendor tag、vendor-defined extension 或细粒度手动曝光；
