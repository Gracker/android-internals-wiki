...
AVD 的表现同时受资源复杂度、宿主 Canvas、线程调度和同时播放数量影响。若 trace 显示 UI/RenderThread runnable 等待，再沿 `android17-6.18-2026-06_r6` scheduler 证据检查；不能从 AVD XML 复杂直接推断调度异常。
...
- `jank_context`：JankStats / FrameMetrics 给出的 jank、overrun、页面和用户操作标签，接入方式见 22.8。
...
上报策略要克制。Window 帧指标做窗口聚合，AVD 现场字段只在 jank、严重 overrun 或命中风险条件时采样上报。`frozen` 若采用 Android Vitals 口径要单独标识策略来源，不能与项目自定义阈值混用。

## 扩展：VectorDrawable 路径复杂度与 GPU 负载关系

VectorDrawable 的路径复杂度会影响两端成本：UI 端要维护属性动画状态，渲染端要处理路径栅格化、填充和混合。AVD 退化到 UI 线程后，属性推进、invalidate 和 display list 记录会进入主线程热路径。可变刷新率设备要按当前帧 deadline 判断，不能固定使用 16.6 ms 或 8.3 ms。
...
- [AOSP `android-17.0.0_r1` `AnimatedVectorDrawable.java`](https://android.googlesource.com/platform/frameworks/base/+/refs/tags/android-17.0.0_r1/graphics/java/android/graphics/drawable/AnimatedVectorDrawable.java)
