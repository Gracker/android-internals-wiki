...
任务优先级应来自用户可见性和截止时间。例如，当前加载界面即将结束的场景，优先级高于远处尚未加载的关卡。优先级不能从“着色器代码越短越高”推导；源码长度与驱动创建成本没有稳定对应关系。
...
“首次出现时 `RenderThread` 变长、第二次恢复”是缓存或初始化问题的线索；这种现象也可能来自字形栅格化、图片上传、图层分配、驱动初始化等工作。没有调用级证据时，结论应写成相关性。
