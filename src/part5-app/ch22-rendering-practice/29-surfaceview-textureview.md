...
last_review_finalize_at: "2026-08-07T14:06:07+08:00"
last_review_finalize_run_id: "20260807-140509-edd6f8ad"
...
- `TextureView` 由应用进程内的 HWUI 消费输入 buffer，再把像素画入宿主 App Window。
...
| TextureView 固定多一个 SF GPU pass | 宿主 HWUI 必须采样输入纹理；SurfaceFlinger 只看到最终宿主 layer，是否生成 client target 由整屏 HWC strategy 决定 |
...
默认 composition order 小于 0 时，内容 layer 位于宿主窗口下方。宿主 HWUI 通过透明区域与 `Canvas.punchHole()` 露出下方内容。控制条、字幕和其它 View 仍画进宿主 App Window；视频、预览或游戏帧进入独立 BLAST child。
...
SurfaceView 内容保持独立 layer，宿主 UI 仍有自己的窗口 buffer；TextureView 输入在 RenderThread 结束第一套 queue 周转，SurfaceFlinger 只接收已经包含该内容的宿主 buffer。
...
一块单平面、未压缩 buffer 的粗略下界可以按下面的式子估算：
...
- TextureView 的 `SkImage`/import cache；
...
| 维度 | 固定条件 | 输出 |
| --- | --- | --- |
| 内容 | 同一视频/相机配置/游戏场景 | Producer cadence、分辨率、format、HDR |
| UI | 相同遮罩、变换、动画与窗口状态 | layer 集合、crop、alpha、composition type |
| 设备 | 同型号、同系统 build、同刷新率 | thermal、频率、驱动与 HAL 版本 |
| 时序 | warmup 后多轮、相同持续时间 | median、P90/P95/P99、丢帧与 late present |
| 功耗 | 同亮度、网络、音频、温度区间 | rail/energy、GPU busy、带宽、CLIENT 时间 |
...
Activity resumed、View attached 与 Surface valid 是三个状态。Producer 只能在 `surfaceCreated()` 后使用当前 Surface，并且在 `surfaceDestroyed()` 返回前停止所有线程对它的访问。

下面的骨架强调 Surface 所有权；`FrameProducer` 是业务接口。
...
返回 `true` 后由 TextureView release 当前 SurfaceTexture；返回 `false` 表示调用方接管，后续必须自行 release。应用用 `Surface(surfaceTexture)` 创建的包装对象始终由应用 release。

`setSurfaceTexture(newTexture)` 会立即 release 旧对象，且不会为旧对象调用 destroyed callback，也不会为新对象调用 available callback。复用旧 SurfaceTexture 时要显式记录所有权和 GL context detach 状态。

### 黑屏按对象逐项检查

Android 17 没有一个通用的“SurfaceView 黑屏 bug”能解释所有场景。按下面顺序检查更可靠：
...
`setSecure(true)` 表达安全输出要求，不是修黑屏开关。protected buffer 需要端到端保护路径；设备、外接显示或 HWC 不满足条件时，可能拒绝显示。移除 secure 以换取普通 GPU 读取会破坏 DRM/安全模型。
...
`COMPATIBLE` 会选择 TextureView。若页面只做常规预览并关注功耗/延迟，应保留 `PERFORMANCE`，再在目标设备确认实际实现和 HWC strategy。
...
下面的 XML 仅在需要 TextureView 语义时切换。
...
选择 TextureView 后应重新验证 HDR、DRM、功耗和宿主 GPU。贴纸若只是普通 View/Compose overlay，SurfaceView 默认位于宿主下方时仍可让宿主 UI 覆盖其上；只有贴纸必须与视频像素一起经过 shader、mask 或统一纹理变换时，TextureView/GL 视频路径才更合适。
...
下面的 native 代码只展示 Vulkan WSI 的对象关系。
...
`vkCreateAndroidSurfaceKHR` 使用 `ANativeWindow`；SurfaceControl 负责 layer tree 与 transaction。引擎还要管理 swapchain 重建、pre-rotation、semaphore/fence 和 Frame Pacing，不能依赖 `vkAcquireNextImageKHR` 或 `vkQueuePresentKHR` 的阻塞行为同步无关 CPU 工作。
...
Perfetto 的 FrameTimeline 对 SurfaceView 内容流没有完整覆盖。宿主 App Window 的 SurfaceFrame 与最终 DisplayFrame 仍有价值，SurfaceView 主体还要补 layer、BufferTX、Producer queue 和 fence。

### SurfaceView 的对象表

| 对象 | 主要字段 | 目的 |
| --- | --- | --- |
| 宿主 App Window | layer id/name、host BLAST、FrameTimeline token | 观察控制层与 hole-punch |
...
| DisplayFrame | HWC strategy、present/release | 用户对应的显示周期 |
...
6. 给每个 acquire、release、present fence 标明 owner；
...
目标 SurfaceView child 走 DEVICE，只说明当前 display frame 的 HWC strategy 接受它；不能推出固定使用某个硬件 plane，也不能外推到下一帧或另一台设备。
...
| Host RenderThread dequeue | App Window BLAST | host release fence、buffer stuffing、display 反压 |

报告中只写“卡在 dequeueBuffer”没有足够信息。队列名、layer、slot、frame number 与 fence owner 都应记录。
...
`animate()` 不会因为方法名就固定新增一个 render pass。要从 RenderThread/GPU capture 确认该动画怎样改变 draw、clip 和 layer cache。动画期间还应检查 Producer 帧率与宿主刷新率是否不同。
...
| Android 17 / API 37 | `setBlurRegions()`；当前 container + BLAST + background 锚点 | 当前 `DeferredLayerUpdater` 继续处理 crop、transform、dataspace、HDR 与 slot image cache |
...
- [ ] 延迟起点、终点和目标 percentile 已定义；
- [ ] 两套 queue 的 slot、fence 和 backpressure 分开；
...
- [ ] 每个 fence 都标明 acquire/release/present 和 owner；
...
SurfaceView 把内容保留在独立 BLAST child，使 Producer cadence 与宿主窗口 cadence 分离，也让 HWC 能单独评估这块内容。它有机会降低大面积视频、相机和游戏内容的宿主 GPU 采样与功耗，同时增加 Surface 生命周期、几何、hole-punch、独立 fence 和跨 layer 同步工作。

TextureView 把外部 BufferQueue 交给应用内 HWUI。它能够使用普通 View 的变换、裁剪、alpha 和兄弟节点混合；代价是输入必须赶上宿主帧，并由 RenderThread 采样进 App Window。SurfaceFlinger 无法把其中一个矩形重新拆成独立视频 overlay。

选型时先确定视觉和安全约束，再在目标设备上测延迟、功耗、内存和 HWC strategy。Perfetto 分析从目标 present 反向查：SurfaceView 要拆宿主、container 与 BLAST child，TextureView 要拆外部 queue、宿主 acquire 与 host BLAST。控件名称只能指出候选路径，trace 和源码才决定结论。
