...
<!-- AIW-源码调研-2026-06-12 -->
### [自动发现] 同步屏障机制对 Choreographer 的影响

**源码发现：基于 android16-release (API 36) 的 CombinedMessageQueue 实现**

Choreographer 在设置帧回调时会使用同步屏障机制，确保输入事件、Vsync 回调等异步消息优先执行。通过分析 AOSP 源码，发现 Android 16 对同步屏障机制进行了重大重构：

#### 1. 双模式调度架构

Android 16 引入 `CombinedMessageQueue`，根据进程类型自动选择实现：

```java
// MessageQueue.java:63-1100
...
**适用场景：**
- LegacyMessageQueue：默认用于应用进程，兼容性好
- ConcurrentMessageQueue：用于 system_server、SystemUI 等系统进程，提供更高消息吞吐量

#### 2. 同步屏障核心机制

**屏障设置：**
```java
// CombinedMessageQueue.java:1108-1123
private int postSyncBarrier(long when) {
    if (mUseConcurrent) {
        // 原子生成屏障令牌
        final int token = mNextBarrierTokenAtomic.getAndIncrement();
        mNextBarrierToken = token + 1; // 兼容反射访问
        
        // 创建 target=null 的屏障消息
...
**消息选择逻辑：**
```java
// CombinedMessageQueue.java:716-740
/*
...
Choreographer 在帧渲染时使用同步屏障模式：

1. **帧回调设置**：通过 `postFrameCallback()` 注册帧回调
2. **屏障插入**：在开始帧处理前插入同步屏障
3. **异步执行**：输入事件、Vsync 回调等异步消息优先执行
4. **屏障移除**：帧完成后调用 `removeSyncBarrier()` 移除屏障

这种机制确保了 UI 渲染的及时性，即使在高负载情况下也能保持较好的响应性。

**验证信息：**
- 一手来源：android16-release frameworks/base/core/java/android/os/CombinedMessageQueue.java (2951行)
- 版本边界：Android 16 (API 36)，推断 Android 17 可能进一步优化
- 影响范围：所有使用 Looper 的线程，特别是主线程的 UI 渲染管道
