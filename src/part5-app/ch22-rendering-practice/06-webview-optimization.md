...
1. 宿主容器创建与 WebView 启动初始化（startup）；
...
- Android 平台固定为 Android 17 / API 37 / `android-17.0.0_r1`，用于解释 Android 框架、HWUI 和显示系统；
...
同为 Android 17 的设备可以安装不同 provider 版本。平台源码能说明 `WebViewFactory` 怎样选择和装入 provider，却无法替代对应的 Chromium 源码版本（revision）。更完整的线程与显示路径见 [18.9 Android 17 WebView 渲染管线](../../part2-performance/ch18-rendering-pipelines/09-webview-rendering.md)。
...
3. App 主线程在 View 树遍历中记录 WebView functor 与绘制边界；
...
销毁前，页面所有者还要取消协程、Bridge 回调表、文件选择、权限请求和业务监听器。已经决定销毁时，无需先导航 `about:blank` 再等待；这会启动一次额外导航并产生更多回调。`clearCache()`、Cookie 清理和 Web Storage 清理作用于共享的数据配置空间（profile），不应混进单实例销毁。
...
OOM（out of memory，内存不足）与恢复设计见 [20.5 OOM 治理与 WebView Renderer 恢复](../ch20-stability/05-oom-webview-renderer-recovery.md)。
...
7. Bridge 按线程、origin（来源）、批量、payload（载荷）、超时和取消能力检查；
