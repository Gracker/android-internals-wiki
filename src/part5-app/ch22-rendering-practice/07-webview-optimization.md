...
同为 Android 17 的设备可以安装不同 provider 版本。平台源码能说明 `WebViewFactory` 怎样选择和装入 provider，却无法替代对应的 Chromium 源码版本（revision）。更完整的线程与显示路径见 [18.13 WebView 渲染管线](../../part2-performance/ch18-rendering-pipelines/13-webview-rendering.md)。
...
OOM（out of memory，内存不足）与恢复设计见 [20.9 WebView Renderer OOM 恢复](../ch20-stability/09-webview-renderer-oom-recovery.md)。
