---
title: "Android 17 原生动态库的只读加载与安全发布"
chapter: "20.13"
section: "20.13"
status: "finalized"
...
last_verified_against: "Android 17 behavior changes; Android 14 DCL behavior changes; Android DCL security guidance; Android NDK JNI tips; AOSP android-17.0.0_r1 Runtime.java and VMRuntime.java; AOSP native library namespace docs"
...
本章的源码依据为 AOSP（Android Open Source Project，Android 开源项目）`android-17.0.0_r1`。16 KB 页面大小的结论沿用 20.11 的 Android 17 基线；这里不重复讨论 Linux 内核实现。
...
### 5.3 16 KB 页面大小要检查 ELF 程序头

ELF 程序头描述运行时如何把文件映射进内存；其中 `PT_LOAD` 条目代表需要加载的段，`p_align` 表示该段要求的对齐方式。运行时释放出的独立 `.so` 应按 20.11 的 Android 17 规则检查这些字段。APK ZIP 对齐只影响从 APK 直接 `mmap` 未压缩库的路径；`mmap` 是把文件内容映射到进程虚拟内存的系统调用。文件解压到私有目录后，ZIP 中的偏移对齐不再是这个独立文件的加载条件。ELF 段对齐与 APK ZIP 对齐应分别检查。
...
| 16 KB 不兼容 | ELF `PT_LOAD` 段对齐检查失败或链接器报告对齐错误 | 使用符合 20.11 基线的构建 |
...
| 4 KB 对齐库运行在 16 KB 环境 | 被 20.11 的构建检查或运行测试发现 |
...
- 1.58 讨论链接器命名空间、依赖解析与加载性能，本章只引用其中的加载条件。
...
- 20.11 讨论 16 KB 页面大小下的 ELF 与 APK 兼容，本章只使用对应检查结果。
- 1.15 讨论 JNI 注册与调用边界。库加载失败后仍调用对应原生方法，才会引出后续 JNI 故障。
