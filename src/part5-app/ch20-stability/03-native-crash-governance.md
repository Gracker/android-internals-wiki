...
sources:
  ...
  - type: aosp
    path: "art/runtime/signal_chain.cc"
  - type: aosp
    path: "system/core/debuggerd/tombstone.cpp"
  - type: aosp
...
[已验证: AOSP android-16.0.0_r1, bionic/linker/debuggerd/handler.cpp — 信号编号定义见 `asm-generic/signal.h`]
...
SignalChain 的拦截发生在 `sigaction()` 调用时：应用通过 JNI 调用的 `sigaction()` 实际执行的是 `art/runtime/signal_chain.cc` 中的包装函数，而非直接调用 libc 的 `sigaction`。这个包装函数将新的 handler 追加到链表末尾。

[已验证: AOSP android-16.0.0_r1, art/runtime/signal_chain.cc — `AddSpecialSignalHandlerFn()` 保证 special handler 在链头]

### 崩溃收集流程：debuggerd → crash_dump

当信号到达且没有被任何 handler 拦截（或 handler 选择传递），Android 的崩溃收集流程启动：

1. **linker 注册的信号处理器**（`bionic/linker/debuggerd/handler.cpp`）被触发
2. 通过 `socket` 向 `debuggerd` 守护进程发送崩溃通知
3. **debuggerd** `fork` 出 `crash_dump` 子进程
4. `crash_dump` 通过 `ptrace` attach 到崩溃进程，读取寄存器状态和内存映射
5. 使用 `libunwind` 回溯调用栈，收集所有线程的堆栈
6. 生成 tombstone 文件写入 `/data/tombstones/`
7. 将崩溃信息通过 `ActivityManagerService` 传递给应用层的 `UncaughtExceptionHandler`

`fork` + `ptrace` 的设计是关键：崩溃进程的内存空间可能已经损坏，如果在进程内部做堆栈回溯，可能二次崩溃。独立进程通过 `ptrace` 从外部读取，安全性更高。
...
- **API 30+**：`ActivityManager.getHistoricalProcessExitReasons()` 返回 `ApplicationExitInfo`，其中 `REASON_CRASH_NATIVE` 类型包含 tombstone 内容（通过 `getProcessExitInfo()` 获取）
- **dropbox**：系统将 tombstone 同时写入 `dropbox`（`adb shell dumpsys dropbox --print` 可查看）

[已验证: AOSP android-16.0.0_r1, system/core/debuggerd/tombstone.cpp — `_print_thread_info()` 和 `_print_memory_and_code()` 函数]
...
第一行 MODULE 描述符的四字段含义：**OS**（`Linux`）、**Arch**（`arm64`）、**Build ID**（16 字符 hex，来自 ELF 的 `.note.gnu.build-id` 段）、**Module Name**。

#### 符号文件格式

**源码位置**：`external/google-breakpad/docs/symbol_file_format.md`（AOSP 内置）
...
[已验证: AOSP android-16.0.0_r1, art/runtime/signal_chain.cc — SignalChain 保证系统 handler 不被跳过]
