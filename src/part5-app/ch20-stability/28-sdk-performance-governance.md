...
- 多个 Maven artifact 和一串传递依赖；
- Java/Kotlin 字节码、资源、Manifest 合并项与若干 ABI 的 `.so`；
- `ContentProvider`、`Service`、`Receiver`、`Activity`、独立进程和自定义权限；
- 初始化代码、线程池、定时任务、网络协议、数据库与本地缓存；
- consumer ProGuard/R8 规则、基线配置文件和注解处理器；
- 服务端开关、控制台配置、数据处理关系与供应商运维能力。

因此，“一个应用平均有 20～40 个 SDK”“SDK 代码占 30%～60%”不能当作项目结论。这类数字高度依赖统计口径：是按供应商、业务能力、Maven artifact、DEX 包名前缀，还是按最终安装字节计数？同一家供应商可能拆成十几个 artifact，一个基础库也可能被多个 SDK 复用。
...
调试变体和发布变体的混淆、资源压缩、动态特性交付与依赖选择可能不同。评估必须以准备发布的 variant 为准。
...
第一条命令给出直接依赖和传递依赖，第二条命令显示版本选择与依赖路径。它们仍然只是构建输入；还要检查最终合并 Manifest、APK/AAB 和动态交付模块。Android Studio 的 [APK Analyzer](https://developer.android.com/studio/debug/apk-analyzer) 可以比较两个 APK/AAB 的下载大小、文件组成、DEX 内容和最终 Manifest。
...
| 代码 | DEX 包、原生库、ABI、Build ID、consumer R8 规则 |
...
| 可观测性 | Trace 能力、日志、指标、Java mapping、原生符号 |
| 运维 | 负责人、供应商响应时限、禁用开关、回退版本、移除成本 |

Google Play 的 [第三方 SDK 使用责任说明](https://support.google.com/googleplay/android-developer/answer/10358880?hl=zh-Hans)强调，即使问题来自第三方代码，应用开发者仍对发布的应用负责。[Google Play SDK Index](https://support.google.com/googleplay/android-developer/answer/13326895?hl=zh-Hans)可作为版本风险信号，但不能代替本应用的实测与代码审查。
...
每次升级都应保存依赖图、Manifest、APK/AAB 组成和基准结果的差异。若供应商无法说明变更，应提高灰度门槛，而不是靠版本号猜测风险。
...
3. **重复采样**：保留分布、P50/P95 和异常样本，不用单次结果下结论。
4. **保留证据**：产物、版本、编译模式、Trace、符号、mapping 与实验脚本一并归档。

单 SDK 实验用于判断边际成本，全部 SDK 的集成实验用于识别竞争和叠加效应。两种实验不能互相替代。
...
| 内存增加来自 SDK | heap dump 中有 SDK 包名 | 同场景 A/B + retained path/分配栈 + 原生 Build ID |
| 网络由 SDK 发起 | 某域名看起来像供应商 | 请求 ID/调用栈/供应商钩子 + 端点和配置核对 |
...
- Binder 调用、锁等待和同步网络探测；
- 初始化时注册监听器、创建线程池或加载原生库；
- 多个 SDK 争用主线程、磁盘和类加载锁。

这也解释了为什么只给 `Application.onCreate()` 打点会漏掉早期成本。Provider 优化的完整路径见 [ContentProvider 启动优化](../ch21-startup/03-contentprovider-optimization.md)。
...
[Macrobenchmark](https://developer.android.com/topic/performance/benchmarking/macrobenchmark-overview) 能在应用进程外重复驱动冷、温、热启动，并保存 Perfetto Trace。目标应用需要可 profile。比较 SDK 版本时，release 配置、设备、启动模式和编译模式都要相同。
...
对于可控的集成代码，可用 `androidx.tracing` 给初始化入口增加命名切片；不透明的 Provider 只能依赖系统切片、调用栈或供应商提供的 Trace。切片应覆盖等待时间和结果，而非只包住一次异步任务提交。
...
- PSS 按映射共享者分摊物理页，适合进程级比较；
- RSS 统计驻留页，读取成本较低，但共享页会在多个进程重复出现；
- `smaps` 描述每个 VMA 的地址、权限、文件与页统计；
- Java heap dump 描述托管堆对象及引用；
- heapprofd 依据分配调用栈采样原生堆。
...
- 同一进程的 Java heap 与 native allocator 被宿主和各 SDK 共用；
...
4. Java 堆查看 dominator 与 retained path，不只按类名计数。
5. native 堆用带 Build ID 的符号解析 heapprofd 分配栈。
6. 把加载映射、活跃分配、线程栈和图形缓冲分开解释。
7. 保存每轮原始数据与分布，报告差值和置信范围。
...
嵌入式 SDK 与宿主共享 UID、进程状态、权限和后台限制。将任务提交到线程池、协程或 native thread，不会取得新的后台执行豁免。
...
`top` 可以发现进程或线程在某个采样窗口内占用 CPU，却不能说明谁安排了工作。simpleperf 与 Perfetto 的调用栈更接近归因证据，但仍受这些条件影响：
...
`TrafficStats` 的 UID 统计适合观察应用总体网络量，但同 UID 内无法拆分宿主和多个 SDK。应用 HTTP interceptor 只能覆盖经过该客户端的请求，SDK 的自建客户端、Cronet、WebView 或 native socket 可能绕开它。
...
- 后台音频限制收紧，音频相关 SDK 在不满足条件时可能拿不到音频焦点或启动播放失败。
- 后台调度、前台服务和权限仍由宿主应用共同承担合规与稳定性责任。
...
ANR 同样不能只看是否出现 SDK 帧。要找到主线程正在等待什么、锁或 Binder 的另一端是谁、后台线程是否持有资源，以及等待是否由宿主调用时机触发。

### 7.2 符号与版本是上线条件

每个 release 必须保存与产物一一对应的：

- R8 mapping 和 retrace 工具版本；
- 未剥离 native library 或可用符号文件；
- ABI、Build ID、SDK 精确版本和制品校验值；
- 上传到监控平台的符号处理结果。

[ndk-stack](https://developer.android.com/ndk/guides/ndk-stack)需要与崩溃二进制匹配的符号。若闭源 SDK 只提供 stripped `.so` 且供应商无法按 Build ID 符号化，团队就无法可靠定位 native crash；这应进入准入风险，而不是等事故发生后再补材料。

聚类键建议包含异常类型或 signal、归一化栈、SDK 版本、ABI、Android 版本、设备、进程和功能路径。[Android vitals](https://support.google.com/googleplay/android-developer/answer/9859174?hl=zh-Hans)可能标记“可能与 SDK 有关”的问题，这仍是线索；SDK 供应商可通过 [Google Play SDK Console](https://support.google.com/googleplay/android-developer/answer/12246095?hl=zh-Hans)接收聚合信息并上传反混淆文件。

对于没有 Java/Native 崩溃栈的异常退出，应结合 `ApplicationExitInfo`、系统日志、内存样本和前台状态分析。不要把低内存杀进程、用户停止、系统更新与 SDK crash 混成同一指标。
...
方法数可帮助发现 DEX 变化，但不能替代字节、启动类加载和运行时成本。
...
供应链记录还应包含来源仓库、制品校验、许可证、SBOM、漏洞响应与下架策略。远程下发未知代码或通用热修复并不是稳妥的 SDK 修复方案；优先级通常是关闭能力、回退已验证版本、发布修复包。

## 9. 准入：阈值来自应用预算

不存在适用于所有应用的“启动不得增加 X ms、内存不得增加 Y MB”。社交应用、车机、低内存设备与支付确认页的预算不同。阈值应从应用 SLO、关键场景、设备档位和当前余量推导，并为多个 SDK 的合计成本留出预算。
...
| 内存 | 稳态/峰值 RSS/PSS、Java retained、native 分配、低内存恢复 |
...
业务代码若到处暴露供应商类型，替换和禁用都会变成大规模改造。应用应以自己的领域接口包住 SDK，并把同意、初始化、状态和异常处理收在适配层。
...
这里的开关阻止后续调用，并只在供应商支持时执行停止操作。它不能卸载已经加载的 DEX/`.so`，也不能自动撤销静态初始化、未知监听器或已经排队的任务；SDK 必须提供成对的注册/反注册和可重复调用的停止契约。
...
- 读取开关不依赖待禁用 SDK；
- 冷启动早期即可得到安全默认值；
- 禁用路径不触发无限重试或 crash loop；
- 本地保留上次可信状态并设过期策略；
- 隐私同意撤回能停止采集并处理待上传数据；
- 服务端、客户端和供应商控制台状态可核对。

若 SDK 在进程创建时由 Provider 自动运行，远程开关读取往往已经太晚。高风险 SDK 应改为显式初始化，或至少让 Provider 只做不触碰网络、磁盘重活的轻量登记。
...
- 在用户同意和数据配置满足条件后初始化；
- 统一事件 schema、采样、批量大小、上传条件和重试；
- 限制 breadcrumb、附件、线程栈与本地队列上限；
...
- 只在广告场景临近使用时初始化或预热；
- 预取必须有数量、超时、内存和取消上限；
- 分离 SDK 初始化、请求、素材下载、WebView、视频解码和 View attach 成本；
...
- 双方没有共享 Java heap 或 native heap；
- 当前模型只支持 DEX SDK，不允许 SDK 自带 JNI 代码；
- SDK 可用的权限、API、存储与组件受到限制；
- 远端 UI 通过受控的 Surface/View 机制嵌入宿主；
- Runtime 进程可能死亡，宿主必须处理死亡、降级和重新加载。

Android 17 AOSP 的 [`SdkSandboxManagerService`](https://android.googlesource.com/platform/packages/modules/AdServices/+/refs/tags/android-17.0.0_r1/sdksandbox/service/java/com/android/server/sdksandbox/SdkSandboxManagerService.java)包含 sandbox 启动、绑定、SDK 加载/卸载与死亡处理，是判断系统端生命周期的源码入口。

### 12.2 隔离不自动带来性能收益

独立进程改善了地址空间隔离，也让宿主与 Runtime 进程的内存更容易分别观察；代价包括：
