...
在 `android-17.0.0_r1` 的 `soinfo::prelink_image()` 中，linker 解析 `DT_JMPREL` 和 `DT_PLTRELSZ`，但忽略 `DT_PLTGOT`，源码注释给出的原因是 Android 不支持 `RTLD_LAZY`。随后 `link_image()` 执行重定位，`process_relocation_impl()` 的 `R_GENERIC_JUMP_SLOT` 分支把解析后的目标地址写入 relocation target。

这段流程带来三个结论：

1. Android 上不能套用桌面 Linux 的“第一次调用时由 lazy resolver 回填 GOT”模型。
2. PLT Hook 改写的是 linker 已完成的重定位结果，并不依赖 `DT_PLTGOT`。
3. IFUNC/IRELATIVE resolver 也是装载期重定位的一部分。解析完成后，普通调用不会自动再次解析并覆盖 Hook；新装载的 ELF 拥有自己的重定位槽，需要单独处理。

### 3.2 RELRO 是权限边界，不是“无法 Hook”

`soinfo::link_image()` 完成 relocation 后调用 `protect_relro()`，把 GNU RELRO 段设为只读。待修改的函数地址可能位于 `.got.plt`、`.data` 或 `.data.rel.ro`，不能假定所有槽都处于同一种保护状态。

RELRO 的含义是：框架不能直接写入受保护的槽，需要先确认所在映射的权限和页范围，再进行受控的 `mprotect()` 与指针更新。它不是密码学保护，也不会让同进程代码永远无法修改该页。修改是否成功还受映射方式和进程策略影响，失败必须被当作正常结果处理。
...
Android 17 linker 在 64 位进程中遇到 `DT_TEXTREL` 或 `DF_TEXTREL` 会拒绝装载。这里约束的是 ELF 在装载重定位阶段修改不可写代码段的声明。
...
Android 17 的 debuggerd 在早期为 `SIGTRAP` 等致命信号注册处理函数，并设置 `SA_SIGINFO | SA_ONSTACK | SA_RESTART | SA_EXPOSE_TAGBITS`。`SA_EXPOSE_TAGBITS` 不是 Android 17 才加入这条路径的新能力。应用若正确安装并消费自己的 `SIGTRAP`，并不必然生成 tombstone；如果事件进入 debuggerd 的致命处理路径，debuggerd 会创建 pseudothread，启动 `crash_dump`，后者仍通过 `ptrace` 暂停和读取目标线程，完成后再按信号语义终止进程。
