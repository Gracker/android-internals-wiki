...
related_chapters: ['20.14', '20.19', '14.13']
...
本章以 `android-17.0.0_r1` 为平台锚点，重点分析普通应用进程内的 arm64 Native Hook。Android 17 没有提供公开的 Hook API，也没有让 Hook 框架获得额外权限。应用仍应优先使用公开 NDK API、编译期插桩、显式代理或系统诊断工具；只有无法改造调用方、且收益足以覆盖稳定性成本时，才考虑运行时 Hook。
...
| 低频、短期、实验室级指令探针 | Trap Hook | 可以承担信号与调试器兼容成本 |
...
在 `android-17.0.0_r1` 的 `soinfo::prelink_image()` 中（`bionic/linker/linker.cpp` line 2987-3018），linker 解析 `DT_JMPREL` 和 `DT_PLTRELSZ`，但 `DT_PLTGOT` 分支只有注释 `Ignored (because RTLD_LAZY is not supported)` 并直接 break [来源: DeepResearch/2026-07-14-android17-native-hook-three-schools-inlinhook-arm64.md]：
...
随后 `link_image()` 调用 `relocate()`，`process_relocation_impl<RelocMode::General>()`（`linker_relocate.cpp` line 165+）的 fast path 直接把解析结果写入 GOT slot [已验证: bionic linker_relocate.cpp android-17.0.0_r1]：
...
完整调用链为：`dlopen(name)` → `do_dlopen()` → `find_library()` → `soinfo::prelink_image()` → `soinfo::link_image()` → `relocate()` → `process_relocation()` → `process_relocation_impl<General>()` 的 `R_GENERIC_JUMP_SLOT` 分支 [来源: DeepResearch/2026-07-14-android17-native-hook-three-schools-inlinhook-arm64.md]。

这段流程带来三个结论：

1. Android 上不能套用桌面 Linux 的“第一次调用时由 lazy resolver 回填 GOT”模型。
2. PLT Hook 改写的是 linker 已完成的重定位结果，并不依赖 `DT_PLTGOT`。
3. IFUNC/IRELATIVE resolver 也是装载期重定位的一部分。解析完成后，普通调用不会自动再次解析并覆盖 Hook；新装载的 ELF 拥有自己的重定位槽，需要单独处理。具体而言，通过 `dlsym()` 取得的是绝对地址；若目标函数是 IFUNC（`STT_GNU_IFUNC`），linker 在 `R_GENERIC_IRELATIVE` 分支中会先调用 `call_ifunc_resolver()` 求出最终实现地址再写入 GOT slot [来源: DeepResearch/2026-07-14-android17-native-hook-three-schools-inlinhook-arm64.md]。

### 3.2 RELRO 是权限边界，不是“无法 Hook”

`soinfo::link_image()`（`bionic/linker/linker.cpp` line 3386+）在 relocation 完成后调用 `protect_relro()`，把 GNU RELRO 段设为只读 [已验证: bionic linker.cpp android-17.0.0_r1]：
...
Android 17 还引入了 MTE globals tagging：当 `should_tag_memtag_globals()` 返回 true 时，`linker_relocate.cpp`（line 343+）在 `R_AARCH64_RELATIVE` 重定位阶段对写入地址额外应用 MTE tag [来源: DeepResearch/2026-07-14-android17-native-hook-three-schools-inlinhook-arm64.md]：
...
这意味着 RELRO 内的 GOT 段在加载完成后即只读，且可能携带 MTE tag metadata。RELRO 的含义是：框架不能直接写入受保护的槽，需要先确认所在映射的权限和页范围，再进行受控的 `mprotect()` 与指针更新。它不是密码学保护，也不会让同进程代码永远无法修改该页。修改是否成功还受映射方式、SELinux 策略和进程策略影响，失败必须被当作正常结果处理。

实战中 GOT Hook 框架的时机选择变成"在 `dlopen()` 内 hook，但晚于 JUMP_SLOT 解析、早于 `protect_relro()`"——这个窗口非常短 [来源: DeepResearch/2026-07-14-android17-native-hook-three-schools-inlinhook-arm64.md]。
...
Android 17 linker 在 LP64（64 位进程）中遇到 `DT_TEXTREL` 或 `DF_TEXTREL` 时会直接 `DL_ERR` 拒绝装载。源码位置在 `soinfo::prelink_image()`（`linker.cpp` line 3183+）[已验证: bionic linker.cpp android-17.0.0_r1]：
...
这意味着 64 位 `.so` 一旦声明 `DT_TEXTREL`，整个 `dlopen` 直接失败。这里约束的是 ELF 在装载重定位阶段修改不可写代码段的声明。
...
ARM64 Inline Hook 在指令修复上比 ARM32 简化（无 Thumb 模式切换），但 `ADRP/ADR/LDR` 字面加载指令的 PC-relative 修复仍是工程难点。两种架构的核心差异如下 [来源: DeepResearch/2026-07-14-android17-native-hook-three-schools-inlinhook-arm64.md]：
...
其中步骤 4 的 PC-relative 重写是核心难点。以 `ADRP` 指令为例 [来源: DeepResearch/2026-07-14-android17-native-hook-three-schools-inlinhook-arm64.md]：
...
步骤 7 的 icache 同步不可省略：ARM64 采用 Harvard 架构变体，D-cache 写入不自动进 I-cache，必须调用 `__builtin___clear_cache()` 或 `cacheflush()` syscall [来源: DeepResearch/2026-07-14-android17-native-hook-three-schools-inlinhook-arm64.md]。
...
Android 17 的 debuggerd 在 `debuggerd_init()`（`system/core/debuggerd/handler/debuggerd_handler.cpp` line 884+）中为 `SIGTRAP` 等致命信号注册处理函数，并设置 `SA_SIGINFO | SA_ONSTACK | SA_RESTART | SA_EXPOSE_TAGBITS` [已验证: debuggerd_handler.cpp android-17.0.0_r1]：
...
`SA_EXPOSE_TAGBITS` 是 Android 17 在 sigaction 层面新增的 flag，使 GWP-ASan、MTE 故障可读 tag 字节 [来源: DeepResearch/2026-07-14-android17-native-hook-three-schools-inlinhook-arm64.md]。

`debuggerd_signal_handler()`（line 663+）处理 `SIGTRAP`、`SIGSEGV`、`SIGBUS`、`SIGFPE`、`SIGILL`、`SIGABRT` 等信号。关键步骤包括：设置 `PR_SET_DUMPABLE=1` 允许 `crash_dump` ptrace attach；通过 `clone()` 派生 pseudothread（`CLONE_THREAD|CLONE_SIGHAND|CLONE_VM|CLONE_CHILD_SETTID`），共享信号处理表和地址空间，`crash_dump` 进程再通过 `ptrace` 暂停和读取目标线程完成 tombstone 落盘；最后按信号语义终止进程 [来源: DeepResearch/2026-07-14-android17-native-hook-three-schools-inlinhook-arm64.md]。
...
以下为各 Hook 流派的写入时机和单次调用粗估开销 [来源: DeepResearch/2026-07-14-android17-native-hook-three-schools-inlinhook-arm64.md]：

| Hook 流派 | 写入时机 | 单次调用开销（纳秒级） | 注入开销 | 风险 |
|----------|---------|------------------------|---------|------|
| GOT/PLT (eager) | `dlopen` 期间 | <10ns/调用（GOT 直跳） | 0 | RELRO 锁定、SELinux |
| Trap Hook | 调用时 | 1-10μs（SIGTRAP + ptrace） | 一次 fork+ptrace | 进程被 debuggerd 截断 |
| Inline (ARM64) | 调用时 | 5-50ns（多一条 B 指令） | 一次 mprotect+icache flush | SELinux execmod、写时窗口、RELRO |
| IFUNC Hook (间接函数层) | `relocate()` 期间 | 0 | 0 | 仅适用于 `STT_GNU_IFUNC` 标记的函数 |

> 注：以上时间为本地测量范围的粗估量级，未引用第三方 benchmark；未经一手验证 [来源: DeepResearch/2026-07-14-android17-native-hook-three-schools-inlinhook-arm64.md]。

## 9.6 Android 16 与 Android 17 版本差异

以下维度对比 `android-16` (API 36) 与 `android-17` (API 37) 在 Hook 相关机制上的变化 [来源: DeepResearch/2026-07-14-android17-native-hook-three-schools-inlinhook-arm64.md]：
