...
last_verified_against: "android-17.0.0_r1 / Android 17 (API 37); kotlinx.coroutines 1.11.0 and AndroidX Lifecycle 2.11.0 public documentation/source links; no Android 18/API38+ claims"
confidence: medium-high
sources:
  - "kotlinx.coroutines 1.11.0 source and official API docs"
  - "Android Developers: lifecycle-aware coroutines, StateFlow/SharedFlow, Compose side-effects, tracing, background task scheduling"
  - "AOSP android-17.0.0_r1 / Android 17 API 37 platform boundary"
---
...
### 🔹 调试能力与线上边界
- Android 上不能依赖 `kotlinx-coroutines-debug` JVM agent 全局枚举
- 不把未公开的 CoroutineTracing / correlate / internal continuation hook 当成稳定 API
- `-Dkotlinx.coroutines.debug=on` 只作为调试与日志辅助
- Perfetto 只能看到线程、Looper 与应用主动写入的 trace slice

### 🔹 线上协程监控方案设计
- 从受控 root scope 与 operation 创建入口登记创建/完成事件
...
### 🔹 Android 17 与库版本边界
- Android 17 平台不改变 Kotlin `Job` / Flow 生命周期语义
- Jetpack Lifecycle 2.8+ `viewModelScope` 注入等库级演进
- Compose 的 LaunchedEffect / rememberCoroutineScope 泄漏边界
- Android 后台执行限制对协程载体选择的影响
...
本章的平台锚点是 Android 17 / API 37 / `android-17.0.0_r1`。协程与 Lifecycle 属于独立发布的库，不能把 Android 版本当成库版本；涉及库实现时分别以 `kotlinx.coroutines 1.11.0` 和 AndroidX Lifecycle 2.11.0 为验证基线。涉及 Linux task 调度时以内核 `android17-6.18-2026-06_r6` 为边界；协程本身没有可由内核直接识别的 task 类型。
...
在 `kotlinx.coroutines 1.11.0` 的公开 API 与源码中，也没有面向 Android 应用的稳定 `CoroutineTracing` 或 `correlate()` 接口。outline 中的这些名称不能作为“1.7+ 已提供”的版本结论。

因此，以下对象不能作为 Android 线上监控基础：

- `DebugProbes.dumpCoroutinesInfo()`；
- outline 中没有公开出处的 `DebugCoroutinesInfo`、`correlate()`；
- `CoroutineStackFrame`；
