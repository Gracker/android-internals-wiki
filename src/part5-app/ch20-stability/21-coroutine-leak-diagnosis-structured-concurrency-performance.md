...
last_verified_against: "android-17.0.0_r1 / Android 17 (API 37); kotlinx.coroutines 1.11.0 source/API docs; AndroidX Lifecycle 2.11.0 and Tracing 1.3.0 public docs; no Android 18/API 38+ conclusions"
...
协程挂起时不占用专属线程，却仍保留 continuation、`CoroutineContext`（随协程携带的 `Job`、dispatcher、名称等上下文元素）和被 lambda 捕获的对象。它恢复时也可能换到另一个 worker。因此，线程数和协程数没有一一对应关系。线程层的归因与阈值见 [20.19 线程泄漏与匿名线程监控](./19-thread-leak-anonymous-thread-monitoring.md)。
...
Default 与 IO 共享调度器线程，`withContext(Dispatchers.IO)` 从 Default 进入时不保证发生一次 OS 线程切换。dispatcher 切换的工程成本还包括队列等待、任务粒度和 context element（`CoroutineContext` 中随协程传递的元素），不能只用“协程切换比线程切换快”的固定数字评估。更完整的调度与背压（生产速度超过消费速度后形成的排队压力）分析见 [8.6 Kotlin Coroutine 性能实践](../../part2-performance/ch08-responsiveness/06-coroutine-performance.md)。
...
需要稳定线程属性的组件应使用有界的专用 executor，并在 `ThreadFactory`（创建和配置线程的工厂）中设置线程属性，再把它转成 dispatcher。相关线程必须由 owner 关闭并纳入 [20.19](./19-thread-leak-anonymous-thread-monitoring.md) 的 task/线程池监控。
