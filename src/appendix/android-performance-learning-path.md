...
| 主题 | 学习入口 | 实验目标 | 边界条件 |
|---|---|---|---|
| Macrobenchmark | [Jetpack Benchmark](../part3-tools/ch19-apm/11-jetpack-benchmark.md) | 固定启动模式、CompilationMode、迭代次数和设备状态，输出原始样本及分位数 | target app 需满足 Macrobenchmark 对 profileable/non-debuggable 构建的要求 |
...
| ProfilingManager | [ProfilingManager](../part3-tools/ch19-apm/13-profiling-manager.md) | 在 API 35+ 请求 system trace、heap dump、heap profile 或 stack sampling，记录回调与失败 | 请求受系统限流，系统可以拒绝；产品设计不能假定每次都有产物 |
