...
- 进程首次绑定应用时，当前进程声明的内容提供者会早于 `Application.onCreate()` 初始化；
...
ContentProvider 为结构化数据提供统一的 URI、权限和调用协议。`content://authority/path` 中，授权标识用于找到内容提供者；路径和查询参数由内容提供者自己解释。

一次访问涉及三方：

| 参与者 | 主要对象 | 职责 |
|---|---|---|
| 调用方 | `ContentResolver`、`ContentProviderClient` | 解析 URI、取得内容提供者、发起同步调用、管理引用 |
| `system_server` | `ContentProviderHelper`、`ContentProviderRecord` | 按授权标识查找内容提供者，必要时启动进程，跟踪发布和引用关系 |
| 提供方 | `ContentProvider`、内部 `Transport` | 校验传入身份与权限，执行查询、插入、更新、删除等实际逻辑 |

`ContentProvider.Transport` 是 Binder 服务端存根。跨进程调用会进入提供方的 Binder 线程；同进程调用可以取得本地内容提供者引用，直接在调用方线程执行。同一个 `query()` 因此可能采用完全不同的线程模型：
...
跨进程：
调用线程 → IContentProvider 代理 → Binder
...
内容提供者可以通过 `readPermission`、`writePermission`、路径权限和临时 URI 授权控制访问。`Transport` 会校验授权标识、调用方归因信息和读写权限，再把请求交给内容提供者实现。
...
如果任意内容提供者在 `onCreate()` 中做磁盘扫描、数据库迁移、同步网络等待或复杂依赖初始化，后续内容提供者和 `Application.onCreate()` 都会被推迟。远程调用方也可能正在等待这个进程发布内容提供者。

### `initOrder` 只保证数值优先级

`ComputerEngine.queryContentProviders()` 按 `ProviderInfo.initOrder` 降序排列，值大的先安装。Android 17 的比较器对相同值返回 0，因此源码只明确保证数值优先级；不能把同值内容提供者的实际顺序设计成依赖条件。

如果初始化组件之间存在依赖，应该使用显式依赖图，或由应用在可控入口中组织，而不是依靠两个库碰巧选择了不同的 `initOrder`。

### 远程内容提供者冷启动

当调用方请求的内容提供者进程尚未运行时，`system_server` 会启动它。调用方需要等待：
...
`Application.onCreate()` 位于发布之后，通常不在“等待内容提供者发布”的前置路径内。不过内容提供者一经发布，远程 Binder 请求就可能与提供方主线程的 `Application.onCreate()` 并发执行。若二者争用同一数据库锁、CPU 或 I/O，首次查询仍会被间接拖慢。

---

## 跨进程查询怎样返回游标

`query()` 是同步 API。调用方线程会等到内容提供者返回一个可用的游标接口；把它放在主线程上，远程进程冷启动、数据库锁和磁盘 I/O 都会直接变成界面卡顿。
...
窗口能减少大结果集的复制，但不能让无限数据一次返回。一个窗口装不下全部结果时，调用方移动到窗口外的行，`BulkCursorToCursorAdaptor` 会调用远端 `getWindow(newPosition)` 取得新窗口。若内容提供者要求接收所有移动事件，即使目标仍在当前窗口，也可能调用远端 `onMove()`。

“`moveToNext()` 永远不走 Binder”并不准确。通常在当前窗口内读取列值不需要远程取数；窗口失效、越界或内容提供者请求所有移动回调时，仍会发生 Binder 往返。
...
框架层没有把原 SQL 自动改写成高效的键集分页。访问很深的位置时，底层查询仍可能遍历大量前序结果才能填到目标窗口。数据量大且需要翻页的接口，应让内容提供者直接暴露分页条件，例如：
...
`CursorWindow` 中的行数据不直接塞进这块缓冲区，但下列内容仍会占用 Binder 事务：

- URI、`projection`、`selection`、参数和附加数据；
- `ContentValues`、`Bundle`、`ContentProviderOperation` 数组；
- 返回的元数据、异常和文件描述符；
- 同一进程并发进行的其他 Binder 请求。

`applyBatch()` 的批次并非越大越好。它减少往返次数，却可能让单笔序列化数据过大并触发 `TransactionTooLargeException`。批大小应通过真实数据分布测试。
...
`ContentResolver.acquireContentProviderClient()` 返回稳定客户端。系统会记录调用方对内容提供者的稳定依赖；调用方用完客户端后必须调用 `close()`。

`acquireUnstableContentProviderClient()` 适合调用方不信任提供方稳定性的场景。它关闭了内容提供者进程死亡时清理依赖进程的那套稳定引用语义，但调用者必须自己处理：
...
非稳定引用不会让慢查询自动变快，也不为增删改查增加超时。它解决的是内容提供者进程死亡时的故障边界。
...
|---|---:|---|---|
| 已附加的进程发布内容提供者 | 10 秒 | `CONTENT_PROVIDER_PUBLISH_TIMEOUT_MILLIS` | `system_server` 清理正在启动的内容提供者，并以初始化失败原因移除提供方进程 |
| 调用方等待新进程发布内容提供者 | 20 秒 | `CONTENT_PROVIDER_READY_TIMEOUT_MILLIS` | 获取等待结束，返回失败或进入相应清理 |
| 已取得内容提供者后的 `getTypeAsync()` 等回调 | 3 秒 | 私有 `CONTENT_PROVIDER_TIMEOUT_MILLIS` | `ContentResolver` 停止等待并返回失败结果 |
| 普通查询、插入、更新、删除 | 无统一内容提供者超时 | 同步 Binder 调用 | 后果由调用线程所处场景、应用 ANR 条件或系统 API 监测决定 |

如果 `ContentResolver.getType()` 不能直接取得内容提供者，Android 17 还会走 `system_server` 的异步后备路径；对应等待上限由就绪超时与 3 秒回调超时组合而来。它仍然不是增删改查超时。

### `setDetectNotResponding()` 不属于普通应用超时 API

`ContentProviderClient.setDetectNotResponding()` 是 `@SystemApi` / `@hide` 能力，并要求 `REMOVE_TASKS` 权限。调用者给它设置时长后，`NotRespondingRunnable` 才会调用 `appNotRespondingViaProvider()`，把已连接的内容提供者标记为无响应。
...
调用方还要用 `use` 或其他结构及时关闭游标，避免窗口和内容提供者引用长期占用。

---

## Binder 线程池如何耗尽

跨进程内容提供者的增删改查默认在提供方 Binder 线程执行。`ProcessState.cpp` 中 `DEFAULT_MAX_BINDER_THREADS` 为 15，但它是默认配置，不代表跟踪记录中必然只能看到 15 条 Binder 线程；线程池启动方式、已加入线程和产品代码都可能影响实际数量。
...
3. 可执行 Binder 线程逐渐用尽；
4. 新请求在驱动或线程池中排队；
5. 调用方同步等待，随后可能触发自身场景的 ANR 条件，或被框架层的内容提供者无响应监测处理。

这里不存在“system_server 心跳拿不到 Binder 线程，所以自动判 ANR”的通用机制。应按证据区分三种位置：

- 调用方主线程停在 `IContentProvider` 代理：正在等待远端回复；
- 提供方多条 Binder 线程停在同一锁或 I/O：提供方并发被串行瓶颈卡住；
- 提供方主线程停在 `handleBindApplication()` 或内容提供者 `onCreate()`：内容提供者还在冷启动或发布。

锁竞争是根因时，仅把内容提供者移到独立进程不会消除问题。它只能改变内存与线程池边界。

---

## 独立内容提供者进程的收益和代价

清单中的 `android:process=":provider"` 会把内容提供者放入应用私有的独立进程：
...
- 进程基础内存、类加载和页表成本增加；
- 调用全部变成 Binder IPC；
- 内容提供者进程被回收后，下次访问要重新启动。

内容提供者侧 GC 虽不会暂停主进程线程，却会延迟远程回复，因此调用方仍能感知到停顿。

如果多个进程访问同一个 SQLite 文件，WAL 通常能提高读写并发，但不是“多进程访问的必选开关”，也不能消除所有写串行和 `SQLITE_BUSY`。数据库结构、事务长度、连接配置和跨进程失效通知都要在目标设备上验证。
...
延迟初始化不等于把工作无条件丢到线程池。内容提供者的第一次查询可能马上到来，必要状态要用明确的并发协议保护，并允许取消或失败返回。

### 正确理解 Jetpack App Startup

App Startup 用一个 `InitializationProvider` 发现多个 `Initializer`，并按 `dependencies()` 构建依赖顺序。它可以减少各库各自声明初始化内容提供者的重复成本，但初始化代码本身仍运行在启动路径上。

如果某个组件不应自动初始化，需要从合并后的清单中移除对应元数据，再在需要时调用 `AppInitializer.initializeComponent()`。这才是 App Startup 的延迟初始化；不存在自动把重任务移出启动路径的“延迟标记”。

收益必须用目标应用实测。不能使用“每减少一个内容提供者固定节省 2 ms”或“必然提升 35%”这类没有设备、构建和样本条件的数字。
...
批量写入可以用 `bulkInsert()` 或 `applyBatch()`，但要控制每批序列化数据的大小：
...
Android 17 中，`ContentProvider.applyBatch()` 的默认实现只是逐个调用 `ContentProviderOperation.apply()`。它不会自动开启 SQLite 事务，也不保证失败时回滚前面已完成的操作。

若内容提供者要提供原子批处理，必须在自己的存储层实现事务：
...
调用方不能假定任意第三方内容提供者的 `applyBatch()` 都具有原子性，需要查阅目标内容提供者的契约。

---

## Perfetto 与线程跟踪怎样互相印证
...
采集时启用 Activity Manager atrace 分类，并同时记录 Binder、调度、进程和文件系统 I/O。诊断顺序是：
...
`ActivityThread.installContentProviders()` 在 Android 17 固定源码中没有可依赖的同名跟踪区段。不能预设 Perfetto 一定存在 `installContentProviders` 片段。
...
- simpleperf 或 Perfetto 调用栈采样；
- 提供方主线程的跟踪栈；
- `system_server` 的进程启动和内容提供者发布日志。
...
- 提供方主线程；
- 提供方全部 Binder 线程；
- `system_server` 中相关内容提供者记录；
- 同时段的 EventLog / 系统日志；
- Perfetto 中的 Binder、调度、监视器竞争和 I/O。
...
| 版本 | 已确认变化 | 使用边界 |
|---|---|---|
| Android 9（API 28） | 公开 `CursorWindow(String, long)`，旧的本地/远程构造语义废弃 | 可指定窗口容量，但不能替代分页和索引 |
| Android 11（API 30） | 固定标签中可见 `ContentProviderClient.setDetectNotResponding()` 的系统/测试能力 | 需要系统权限，不属于普通应用增删改查超时 API |
| Android 12（API 31） | `ContentResolver.getType()` 内部使用 `getTypeAsync()` 回调 | 公开 API 仍是同步 `getType()`；应用仍应自行选择线程 |
| Android 17（API 37） | 本章当前实现基线，保留 10 秒发布、20 秒就绪、3 秒已连接异步回调的分层语义 | 数值受 `Build.HW_TIMEOUT_MULTIPLIER` 影响，增删改查仍无统一内容提供者超时 |

版本迭代只说明能够由对应标签或官方 API 确认的变化。Photo Picker、分区存储等功能会改变数据访问方式，但不应被写成 ContentProvider Binder 或超时机制本身的版本断点。

---

## 常见误区

### “内容提供者一定运行在 Binder 线程”

同进程调用可以直接在调用方线程执行。只有跨进程调用才进入提供方 Binder 线程。

### “所有内容提供者都有 10 秒增删改查超时”

10 秒是进程附加后的发布窗口。普通增删改查没有这个统一计时器。
...
它默认仍在 `InitializationProvider` 中运行初始化器。实际的延迟初始化需要移除自动发现元数据，并在业务入口手动初始化。

### “批处理天然具有事务性”

框架层默认 `applyBatch()` 只按顺序应用操作。事务语义由具体内容提供者实现。
...
1. `ContentResolver.acquireProvider()`：调用方怎样向 AMS 取得内容提供者；
2. `ContentProviderHelper.getContentProviderImpl()`：授权标识查找、进程启动与就绪等待；
3. `ActivityThread.handleBindApplication()` / `installContentProviders()`：提供方怎样创建和发布内容提供者；
...
7. `ContentProviderClient`：稳定/非稳定引用和系统 API 无响应监测。

一次慢查询可以分成“等进程”“等发布”“等 Binder 线程”“等数据库”和“等新 CursorWindow”五类问题。分清阶段后，才能避免优化破坏安全、正确性或进程稳定性。
