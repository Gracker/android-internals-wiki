...
Android 17 使用 cgroup 管理 CPU 调度、CPU 集合、I/O、内存、进程冻结与进程组生命周期。平台通过 `libprocessgroup` 和 task profile 隐藏底层文件路径，使 framework 和 native service 只表达“后台”“top-app”“冻结”等意图。

理解这套机制时，最重要的事实是：`android-17.0.0_r1` 的 AOSP 默认配置仍是 cgroup v1/v2 混合拓扑。cgroup v2 的“统一层级”只约束 v2 控制器；它没有把 Android 的 v1 `cpu`、`cpuset`、`blkio` 自动并入同一棵树。

本章以 AOSP `android-17.0.0_r1` 和内核 `android17-6.18-2026-06_r6` 为锚点。设备厂商可以覆盖控制器版本、挂载点、子组和参数，因此本文先给出 AOSP 基线，再说明如何核对设备实值。
...
因此，Android 17 AOSP 中不存在统一的 `/dev/cgroot/cpu/top-app` 基线路径。CPU profile 写 `/dev/cpuctl/...`，cpuset profile 写 `/dev/cpuset/...`，freezer 和 v2 memory 则写 `/sys/fs/cgroup/...`。
...
v2 挂载会先尝试 `memory_recursiveprot`，使 `memory.min`/`memory.low` 的保护可以递归作用于子树；旧内核不支持时才退回普通挂载。Android 17 的内核锚点支持该选项。
...
### 2.3 `cgroup.subtree_control` 的准确边界
...
同名定义由后加载的文件替换，新增名称则加入集合。分析 OEM 设备时，AOSP 源文件只能充当起点；设备最终文件和运行时 cgroupfs 才是有效配置。

### 3.2 Profile 由哪些 Action 组成
...
原稿中的 `SetClamps` 不是 Android 17 `task_profiles.cpp` 支持的 action。UClamp 通过 `UClampMin`、`UClampMax`、`UClampLatencySensitive` attribute 或 cgroup 初始配置表达。

### 3.3 AOSP 默认 Profile 的真实映射
...
`MaxPerformance` 本身只移动 CPU controller。把 CPU 与 cpuset 一起切到 top-app 的是 aggregate profile：
...
这也解释了线程 API 名称的差别：只设置 scheduling group 的入口不会自动修改 cpuset；“group and cpuset”或进程级入口使用包含 `ProcessCapacity*` 的 aggregate profile。
...
`SetProcessProfilesCached` 中的 Cached 指文件描述符缓存，可减少反复打开固定 cgroup 文件的成本。它没有比较进程旧状态并跳过所有 profile action。带 `<uid>`/`<pid>` 的动态路径也不会使用同一套全局 fd 缓存。

进程级 `setProcessGroup` 还保留历史约束：不能直接传 `THREAD_GROUP_FOREGROUND`；默认组会按 API 语义处理。阅读 framework 调用时，应继续追踪最终 profile 名，不要只看 `THREAD_GROUP_*` 的整数。
...
`Service::Start()` 在子进程 exec 前应用这些 profile。Android 12 起，`task_profiles` 替代直接写 `writepid /dev/.../tasks` 的旧写法。profile 名稳定后，产品可以改变底层 controller 或路径而不修改 service 定义。
...
OomAdjuster 通过 `mProcessGroupHandler` 异步发送组变更，再由 callback 调整应用及相关子进程。profile 文件写入没有直接塞进 OomAdjuster 的计算循环。状态字段先更新、cgroup 迁移随后执行，因此短时间观察可能同时看到“新 sched group”与“旧 cgroup 路径”；不能预设固定的 100～200 ms 窗口。
...
cpuset 挂载使用 `noprefix`，所以实际文件名是 `/dev/cpuset/<group>/cpus` 和 `mems`，不是 v2 风格的 `cpuset.cpus`。
...
线程最终允许使用的 CPU 是 thread affinity 与有效 cpuset 的交集。出现“设置 affinity 后仍不上大核”时，应同时读取：
...
Android 17 AOSP 默认把 CPU controller 挂在 v1 `/dev/cpuctl`，所以不能用这两个 v2 文件解释平台基线。厂商若通过 `cgroups.json` 把 CPU 改为 v2，才需要核对 `cpu.weight`、`cpu.max` 与 subtree activation。具体产品是否这样配置，应从设备文件和 mountinfo 判断。
...
memory controller 在默认 `cgroups.json` 中标为 optional。设备不支持、没有启用或被 vendor 改为其他布局时，相应 profile action 不能按 AOSP 路径假定成功。
...
普通应用没有一个可依赖的公开“即将被冻结”回调。监控系统若需要区分冻结，应使用系统侧/外部观测、生命周期上下文与采样连续性共同判断，不能靠在已冻结进程里轮询 `cgroup.freeze`。也没有证据支持把后台 profiler 的 10 ms 周期统一换算成 50～100 ms。
...
这些值应和 `/proc/<pid>/cgroup` 指向的实际组对应。top-app 组存在不代表目标线程已经加入，profile 名存在也不代表 action 全部成功。
...
2. 读取 system、API-level、vendor、system_ext 的最终 JSON；
