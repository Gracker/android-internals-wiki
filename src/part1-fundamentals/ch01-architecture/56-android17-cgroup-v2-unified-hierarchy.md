...
Android 17 使用 cgroup 管理 CPU 调度、CPU 集合、I/O、内存、进程冻结与进程组生命周期。平台通过 `libprocessgroup` 和任务配置（task profile）隐藏底层文件路径，使框架和原生服务只表达“后台”“top-app”“冻结”等意图。
...
| 块 I/O | v1 | `/dev/blkio` | 后台 I/O 权重与优先级 |
| 冻结器 | v2 核心接口 | `/sys/fs/cgroup` | 冻结应用/系统的每进程 cgroup |
| 内存 | v2，可选 | `/sys/fs/cgroup` | 每进程统计、保护、限制与回收接口 |

Android 17 AOSP 中不存在统一的 `/dev/cgroot/cpu/top-app` 基线路径。CPU 配置写 `/dev/cpuctl/...`，cpuset 配置写 `/dev/cpuset/...`，冻结器和 v2 内存控制器则写 `/sys/fs/cgroup/...`。
...
`/proc/<pid>/cgroup` 会按层级列出这些归属。看到 v2 行中的 `0::/apps/...`，不能据此推断 CPU 与 cpuset 也已迁入 v2；必须结合 `/proc/<pid>/mountinfo` 或设备上的 `cgroups.json` 解释每一行。

## 2. init 如何建立层级

### 2.1 控制器挂载由 `CgroupSetup()` 完成

PID 1 在 init 的 `SetupCgroupsAction` 中调用 `CgroupSetup()`。`libprocessgroup/setup/cgroup_map_write.cpp` 根据描述符分别处理：

- v1：以 `cgroup` 文件系统挂载单个控制器；
- v2：以 `cgroup2` 挂载统一层级；
- v2 内存：在允许的层级深度写 `+memory` 到 `cgroup.subtree_control`；
...
源码以 `AID_APP_START` 为分界选择 `apps` 或 `system`，并不是根据包名、签名或进程名判断。创建过程会建立 UID 目录、按需激活内存控制器，再建立 PID 目录，并把初始 PID 写入该目录的 `cgroup.procs`。这一级目录同时承载冻结器、`cgroup.kill` 和可用的内存控制文件。

这种布局解决的是应用/系统进程隔离与生命周期管理。它不决定 top-app 或 background 的 CPU 组；CPU 和 cpuset 仍通过各自的 v1 层级管理。

### 2.3 `cgroup.subtree_control` 的作用边界

cgroup v2 控制器默认不会自动向子层级分发。父 cgroup 只能启用 `cgroup.controllers` 中列出的控制器，并且要满足自顶向下和无内部进程约束。Android 的 `NeedsActivation`、`MaxActivationDepth` 和 `ActivateControllers()` 封装了这部分操作。

冻结器需要单独说明。`cgroup.freeze` 是 cgroup v2 的核心接口，存在于非根 cgroup；它不作为 `+freezer` 写入 `cgroup.subtree_control`。AOSP 在 `cgroups.json` 中把它命名为 `freezer`，是为了让任务配置通过统一的控制器/属性抽象找到文件。
...
| 文件 | 作用 |
| --- | --- |
| `cgroups.json` | 定义控制器名称、版本、挂载点、权限、可选性和激活要求 |
| `task_profiles.json` | 定义属性、配置动作和聚合配置 |

Android 17 支持按产品首发 API、厂商和 `system_ext` 覆盖或追加配置。源码的加载顺序是：

1. 系统默认文件；
2. `ro.product.first_api_level` 对应的 API 级别文件；
3. 厂商文件；
4. 任务配置还会读取 `system_ext` 文件。

同名定义由后加载的文件替换，新增名称则加入集合。分析 OEM 设备时，AOSP 源文件只能作为起点；设备上的实际文件和运行时 cgroupfs 才是有效配置。

### 3.2 配置动作

Android 17 的动作不限于移动 cgroup：

- `JoinCgroup`：把线程或进程加入指定控制器的子组；
- `SetAttribute`：通过属性名查找进程/线程对应文件并写值；
- `WriteFile`：写指定路径；
- `SetTimerSlack`：写 `/proc/<tid>/timerslack_ns`；
- `SetSchedulerPolicy`：设置 Linux 调度策略和优先级/nice；
- `Compact`：通过内存 cgroup 的 `memory.reclaim` 发起回收；
- 聚合配置：按顺序执行多个配置。

`SetClamps` 不是 Android 17 `task_profiles.cpp` 支持的动作。UClamp 通过 `UClampMin`、`UClampMax`、`UClampLatencySensitive` 属性或 cgroup 初始配置表达。
...
| `Frozen` / `Unfrozen` | 将 `FreezerState` 写为 `1` / `0` | v2 每进程 `cgroup.freeze` |

`MaxPerformance` 本身只移动 CPU 控制器。聚合配置负责把 CPU 与 cpuset 一起切到 `top-app`：
...
线程 API 名称也反映了这项差别：只设置调度组的入口不会自动修改 cpuset；“group and cpuset”或进程级入口使用包含 `ProcessCapacity*` 的聚合配置。

## 4. Java/JNI 到任务配置的调用链
...
`SetProcessProfilesCached` 函数名中的 `Cached` 指文件描述符缓存，可减少反复打开固定 cgroup 文件的成本。它不会比较进程旧状态并跳过所有配置动作。带 `<uid>`/`<pid>` 的动态路径也不会使用同一套全局文件描述符缓存。

进程级 `setProcessGroup` 还保留历史约束：不能直接传 `THREAD_GROUP_FOREGROUND`；默认组会按 API 语义处理。阅读框架调用时，应继续追踪实际配置名，不要只看 `THREAD_GROUP_*` 的整数。
...
`Service::Start()` 在子进程执行 `exec` 前应用这些配置。Android 12 起，`task_profiles` 替代直接写 `writepid /dev/.../tasks` 的旧写法。配置名稳定后，产品可以改变底层控制器或路径而不修改服务定义。

## 5. OomAdjuster 如何决定应用的调度组

### 5.1 进程状态与调度组没有固定一一映射
...
同一个 `PROCESS_STATE_SERVICE` 可能因正在执行前台服务回调、普通后台服务或绑定传播而得到不同调度组。把所有 `PROCESS_STATE_SERVICE` 固定映射到前台组，或把所有缓存/空进程固定映射到受限组，都会丢失策略条件。

### 5.2 计算结果转换为调度组

`OomAdjuster.applyResultsLSP()` 对 scheduling group 的关键映射如下：

| 框架调度组 | `Process.THREAD_GROUP_*` |
| --- | --- |
...
OomAdjuster 通过 `mProcessGroupHandler` 异步发送组变更，再由回调调整应用及相关子进程。配置文件写入不在 OomAdjuster 的计算循环内执行。状态字段先更新、cgroup 迁移随后执行，因此短时间观察可能同时看到“新调度组”与“旧 cgroup 路径”；不能预设固定的 100～200 ms 窗口。

进入或离开 `top-app` 时，框架还可能单独调整主线程/RenderThread nice，或在配置允许时切换 FIFO 界面调度。这些动作和 cgroup 迁移有关联，但属于不同内核接口，排查时要分别取证。
...
init 只保证组和权限存在，并把根 CPU 掩码复制为初始值。具体 SoC 的 `top-app`、`foreground`、`background` CPU 掩码由设备配置写入。AOSP 不能证明“background 固定为 0-3”或“top-app 固定为 0-7”。

### 6.2 UClamp 的位置与含义

Android 通用内核把 `cpu.uclamp.min`、`cpu.uclamp.max` 和 Android 扩展的延迟敏感属性暴露在 `/dev/cpuctl/<group>/`。`task_profiles.json` 把它们声明为 `cpu` 控制器的属性；进程加入某个 CPU 组后，调度会受该组当前参数约束。

UClamp 钳制调度器看到的利用率上下界，会影响选核与调频决策输入。它不等于锁定频率，也不保证某个线程立即迁移到大核。热限制、CPU 亲和性、cpuset、调度类、负载和调频器仍共同决定执行位置与频率。

### 6.3 CPU 亲和性与 cpuset 取交集

线程允许使用的 CPU 是线程亲和性与有效 cpuset 的交集。出现“设置亲和性后仍不上大核”时，应同时读取：

- `/proc/<tid>/status` 的 `Cpus_allowed_list`；
- `/proc/<tid>/cgroup` 的 cpuset 层级；
...
`cpu.weight` 和 `cpu.max` 是 cgroup v2 CPU 控制器接口。内核 `6.18` 中：

- `cpu.weight` 以 1～10000 的权重在活跃兄弟 cgroup 间分配 fair-class CPU 时间；
- `cpu.max` 用 `$MAX $PERIOD` 限制带宽，`max` 表示不设上限。

Android 17 AOSP 默认把 CPU 控制器挂在 v1 `/dev/cpuctl`，不能用这两个 v2 文件解释平台基线。厂商若通过 `cgroups.json` 把 CPU 改为 v2，才需要核对 `cpu.weight`、`cpu.max` 与子树激活状态。具体产品是否这样配置，应从设备文件和 mountinfo 判断。

## 7. v2 内存控制器

### 7.1 AOSP 提供的属性

Android 17 的 `task_profiles.json` 为内存控制器定义了：
...
内存控制器在默认 `cgroups.json` 中标为可选。设备不支持、没有启用或被厂商改为其他布局时，不能假定相应配置动作会沿 AOSP 路径成功执行。
...
| `memory.high` | 超限后让该 cgroup 进入强回收/节流；极端情况下可暂时超过 | 设置过低会增加直接回收与分配延迟 |
| `memory.max` | 硬上限；回收无法满足时可能触发 cgroup OOM | 错误上限会导致进程被杀或分配失败 |
| `memory.oom.group` | 设为 1 时把 cgroup 作为不可分割的工作负载处理 | 可能扩大一次 cgroup OOM 的终止范围 |

`memory.low` 不能概括成“低于该值绝不回收”；它提供尽力而为的保护。`memory.high` 也不是普通告警阈值，超限会让该 cgroup 中的进程进入强回收和节流。

AOSP 默认配置暴露这些能力，但没有给所有应用统一写入 `memory.max`/`memory.high`。是否设置、设置多少属于产品策略。不要用假设的 system_server 上限、工作资料（Work Profile）上限或固定阈值解释未知设备。

### 7.3 与 LMKD 的边界

LMKD 主要根据 PSI、内存状态、`oom_score_adj` 和配置选择牺牲进程；内存 cgroup 的回收、`memory.max` 和 cgroup OOM 由内核执行。两条路径可以同时存在，但触发条件和受害者选择不同。

排查“进程因内存消失”时至少区分：

- LMKD 日志与杀进程原因；
- 内核 OOM/cgroup OOM 日志；
- `memory.events` 中的 `high`、`max`、`oom`、`oom_kill` 计数；
- tombstone、ApplicationExitInfo 和进程自身崩溃证据。

只看到 RSS 接近某个数值，不能判断是哪条机制触发。

## 8. cgroup 冻结器与 Binder Freezer
...
解冻使用 `Unfrozen` 并写 `0`。这里不使用文件描述符缓存路径，因为文件名包含 UID/PID，且进程 cgroup 会创建和删除。

向 `cgroup.freeze` 写 `1` 会发起该 cgroup 及其后代的冻结。冻结完成可能滞后于写入；内核通过 `cgroup.events` 的 `frozen` 字段报告完成状态。因此，“一次写入就原子完成冻结”不准确。

### 8.2 Binder Freezer 处理 IPC 边界

暂停调度不能解决 Binder 中已有事务、同步调用和异步队列的语义。`CachedAppOptimizer.freezeProcess()` 分两次检查 Binder 状态：

1. 调用 `freezeBinder(pid, true, timeout)`。返回值非零表示仍有未完成事务，此时不写 `cgroup.freeze`，交给失败处理逻辑；
2. Binder 侧满足冻结条件后，调用 `setProcessFrozen(..., true)` 写入 cgroup 冻结器，并记录框架冻结状态；
3. 写入后再调用 `getBinderFreezeInfo(pid)`。若两步之间又出现 `TXNS_PENDING_WHILE_FROZEN`，同样进入失败处理逻辑。

第二次检查用于覆盖 Binder 冻结与 cgroup 冻结之间的竞态窗口。解冻时，源码读取 Binder 冻结信息；若没有需要终止进程的同步事务，就解冻 Binder，随后写 cgroup 冻结器。cgroup 冻结器负责停止线程运行，Binder 驱动负责 IPC 边界；两者不会被一条 `cgroup.freeze` 写操作自动合并。

完整的 pending transaction、oneway 和同步调用规则见 **1.18 Binder Freezer 与缓存进程冻结性能**。

### 8.3 对进程内监控的影响

进程冻结后，其普通线程不再获得 CPU，进程内 Watchdog、采样线程和上传线程也会暂停。解冻后的时间间隔突增不能直接算作 ANR、CPU 饥饿或网络超时。

普通应用没有可依赖的公开“即将被冻结”回调。监控系统若需要区分冻结，应结合系统侧/外部观测、生命周期上下文与采样连续性判断，不能靠在已冻结进程里轮询 `cgroup.freeze`。也没有证据支持把后台分析器的 10 ms 周期统一换算成 50～100 ms。
...
文件可能通过根目录映射显示为 `/etc/...`，`vendor`/`system_ext` 也可能追加定义。命令失败还可能来自 SELinux 或 shell 权限，不能把“读不到”解释为控制器不存在。
...
这些值应和 `/proc/<pid>/cgroup` 指向的实际组对应。`top-app` 组存在不代表目标线程已经加入，配置名存在也不代表所有动作都已成功执行。

Perfetto 适合验证后果：

- `sched_switch` / 线程状态：线程何时可运行、在哪个 CPU 执行；
- CPU 频率/空闲：UClamp 和负载变化后的频率与空闲状态；
- Binder/冻结器事件：冻结、解冻和事务失败；
- 内存计数器/PSI：回收、压力与 cgroup 内存事件的时间关系。

Perfetto 中看到大核迁移或频率上升，只能作为结果证据。要证明 cgroup 切换，还需配合配置调用、cgroupfs 或 `/proc` 快照。
...
1. 查看 mountinfo，确认控制器版本和挂载点；
2. 读取系统、API 级别、厂商、`system_ext` 的实际 JSON；
3. 展开聚合配置，列出每个动作；
4. 对照目标 PID/TID 的 `/proc/.../cgroup`；
5. 读取目标组的当前文件值；
6. 用 Perfetto 观察调度、频率、冻结器、PSI 和用户可见延迟。

不要从 AOSP 配置名推断厂商参数。`MaxPerformance` 表达策略意图，具体 CPU 掩码、UClamp 与温控上限由产品配置决定。
...
- 框架计算进程重要性和调度组；
- `libprocessgroup` 将稳定的配置名转换为 cgroup、调度器、定时器松弛值或内存动作；
- 内核按 v1/v2 控制器的实际配置执行调度、限制、回收和冻结。

平台基线中，CPU、cpuset、blkio 仍在 v1，冻结器与可选内存控制器位于 v2。v2 的每 UID/每进程树提供进程隔离、冻结、终止和内存文件；`top-app`/`background` 的 CPU 差异仍来自 `/dev/cpuctl`、`/dev/cpuset` 及设备参数。

定位问题时，应从设备当前拓扑出发：确认控制器版本与路径，再确认配置动作和 PID/TID 归属，然后用跟踪数据验证性能后果。把“Android 17 支持 cgroup v2”理解为“所有控制器已迁入统一树”，会让后续 CPU、内存和冻结器结论全部偏离源码。

相关章节：CPU 亲和性、cpuset 与任务配置见 **5.1**；EAS/UClamp 见 **5.2** 与 **5.7**；Binder Freezer 见 **1.18**；OomAdjuster 见 **1.34**；LMKD/PSI 见内存管理章节。
