...
`verify`、`quicken`、`speed-profile`、`speed` 经常一起出现在安装、启动和 OTA 排障里。它们都通过 `dex2oat` 进入 ART 产物管理体系，但成本和收益完全不同：有的只做 DEX 验证，有的生成 profile-guided AOT 代码，有的属于 Android 11 及以下的解释器优化历史。
...
compiler filter 是传给 `dex2oat` 的策略参数。它决定本轮做多少验证、是否编译方法、是否依赖 profile。把它理解成“越高越好”的等级，会导致两个误判：为了启动收益强行用 `speed`，把安装和存储成本拉高；或者看到 `speed-profile` 就以为启动路径都已编译，忽略 profile 覆盖率。
...
Android 8-11 设备仍可能出现 `quicken`。这类设备上，不要把 `quicken` 当作 `speed-profile` 的低配版本。它的收益来自解释器快速路径，不能提供 AOT 机器码覆盖。
