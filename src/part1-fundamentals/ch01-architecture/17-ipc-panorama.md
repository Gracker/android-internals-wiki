...
- InputManager 通过 Binder 把 `InputChannel` fd 交给应用，后续输入事件走 Unix `SOCK_SEQPACKET`。
...
- mmap 何时 unmap，SharedMemory 何时关闭。
- DMA-BUF 围栏未完成前谁不能复用 buffer。
- FMQ 写入方或 reader 死亡后如何重建。
...
Binder 提供调用方 UID/PID、对象引用与 SELinux binder policy；socket 依赖 socket namespace、文件权限、peer credentials 和 SELinux；fd 传递则把内核对象的访问能力交给对端。共享内存若未降低 protection，对端可能获得超出预期的写权限。
...
Binder 不是端到端零拷贝。Android 8 的 scatter-gather 优化减少了额外编组复制，但驱动仍要把事务内容复制到目标进程地址空间；复杂 Parcelable 仍有编码、对象表、字符串和容器处理成本。

### 3.2 同步与 oneway

同步事务要等服务端返回。调用方看到的 wall time 可能来自：
...
多个单向调用只有在发往同一个 `IBinder` object 时，才保证按发送顺序一次分发一个；不同 Binder object，或单向与同步调用混用时，没有这项顺序保证。若调用是同进程直调，`oneway` 也不会把它自动变成异步。

### 3.3 1MB 是共享预算，不是安全负载上限

Java `TransactionTooLargeException` 文档把 Binder 事务缓冲区描述为当前 1MB，并强调它由进程所有进行中的事务共享。Android 17 native `ProcessState.cpp` 的 receive mapping 更具体：
...
AIDL 解决类型契约、版本与后端，不改变底层 transaction buffer 约束。把一个 5MB `byte[]` 换成 AIDL 参数，并不会自动变成共享内存。
...
- Message 负载仍受 Parcel 与 transaction buffer 限制。
...
- 需要更大空间时才扩展到 ashmem-compatible region。
...
它没有 Binder 的“事务共享 1MB receive mapping”口径，但仍受 socket send/receive buffer、单个分组、内存和协议分帧约束。大流可以分块持续发送，不等于单次 `send()` 没有上限。
...
Unix 套接字可用 `sendmsg()` / `recvmsg()` 和 `SCM_RIGHTS` 把 fd 交给对端。它传递的是对同一内核对象的新引用，不会把文件内容复制进 ancillary data。
...
`wake()` 向 eventfd 写入一个 64 位计数，`epoll_wait()` 被唤醒后再读取。Pipe 是更早实现的历史背景，不应拿来解释当前 MessageQueue 唤醒路径。

eventfd 主要传递计数/通知，不承载业务负载。跨进程使用时，双方仍需继承或传递 fd。

### 6.3 Signal：异步通知，不是数据通道

Signal 的交付仍受目标线程调度、signal mask 和 handler 行为影响，不能写成“内核直接中断，延迟为零”。标准信号通常只表达编号；带 `siginfo_t` 的机制能附带少量元数据，但依然不适合业务数据流。
...
| `SIGABRT` | 主动中止，进入 native crash 流程 |
...
`setProtect()` 只能移除权限，不能重新增加。它只约束之后创建的映射；已有可写映射保持原权限。因此“写入 → unmap → 降为只读 → 传给对端”是更清晰的 least-privilege 顺序。
...
满足后走 `memfd_create()`；否则回到 ashmem device。`sys.use_memfd=true` 仍可用于强制覆盖，源码注明未来会移除。
...
图像数据走 DMA-BUF，也不等于应用可以忽略格式、stride、usage 和围栏。
...
- 可变长复杂对象。
- 含 pointer、Binder interface 或任意嵌套 buffer 的负载。
- 需要每条消息独立权限检查的协议。
- 无界队列。

HIDL FMQ 从 Android 8 提供；AIDL NDK 后端从 Android 12 可使用 FMQ。Java 后端可以转交 descriptor，但没有 Java FMQ 库直接读写队列。
...
- 使用 `/dev/binder`。
- vendor native code 使用 NDK 或 Rust 后端等稳定 runtime。
...
Microdroid/pVM 不能直接使用 host 的 Binder driver。Binder RPC 保留 AIDL/Binder 对象模型，把协议改为 socket transport；pVM 场景通常使用 AF_VSOCK。
...
它不继承内核 Binder 的 `BINDER_VM_SIZE` receive mapping，也不能套用“Binder 驱动一次复制”的结论。开销要拆成：

- Parcel / Binder RPC framing。
- socket/vsock buffer 缓冲区与 virtio。
- VM 调度和服务端工作。
...
| --- | --- | --- | --- | --- |
| Binder sync | 方法调用 + reply | caller 等 reply；跨 Binder 对象可并发 | 传 fd /分页 | threadpool、嵌套调用、transaction buffer |
| Binder oneway | 无回复的远程调用 | 同一 Binder 对象串行有序；queue 有界 | 传 fd，避免 oneway 洪泛 | backlog、冻结进程 buffer overflow |
| Unix socket | stream 或分组 | kernel socket buffer；协议自行分帧 | 分块流或 `SCM_RIGHTS` fd | 自建认证、版本、生命周期 |
...
| eventfd | 计数/唤醒 | 计数累积 | 不承载 payload | 忘记排空、fd 生命周期 |
| Signal | 异步进程/线程通知 | signal semantics | 不承载业务数据 | handler 安全、调度、保留信号 |
| Binder RPC / vsock | 跨虚拟机 RPC | socket/vsock 与 RPC session | 专门共享/文件通道 | VM 调度、framing、连接安全 |
...
### 15.2 socket/pipe 不能只搜片段名

系统跟踪不保证每次 `sendmsg()` / `recvmsg()` 都自动产生有语义的 userspace slice。更可靠的方法：

- App/服务在协议请求与完成处加 trace。
- 必要时采集系统调用 ftrace 或 CPU callstack。
- 同时看线程状态、socket buffer/backpressure 日志与负载计数。
- InputChannel 结合 input category、InputDispatcher 和目标应用线程分析。
...
| Android 17（API 37） | 默认 ashmem-compatible memfd 路径增加厂商 API 202604 与目标 SDK 37 等门禁 | 当前 AOSP 源码统一锚定 `android-17.0.0_r1` |
...
不成立。映射后可避免重复 IPC copy，producer 写入、consumer 转换、page fault 和缓存同步仍有成本。
...
### “RpcBinder over vsock 仍受 Binder 1MB 缓冲区限制”
