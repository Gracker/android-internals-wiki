...
性能优化实战和 ANR 案例分别见 §8.14 与 §9.6。本章先建立可用于读源码、看 trace 和解释故障的结构模型。
...
| 组件 | 与通知发布相关的职责 |
| --- | --- |
| ActivityManager / `ActivityManagerInternal` | 处理 user、FGS 通知策略、应用前台状态、PendingIntent 临时 allowlist |
...
`NotificationUsageStats` 是 NMS 自己的统计类。它不是 `UsageStatsManager`，`getAppEnqueueRate()` 也不是一次对 UsageStats 系统服务的查询。把两者混为一谈，会误判通知更新被丢弃时的依赖关系。
...
这段代码说明两件事。第一，`notify()` 可能在应用侧直接丢弃一次高频更新，连 Binder 都不会发生。第二，大图缩放、对象克隆和 Parcel 写入都计入调用线程时间。把大图解码、文本拼装和 `Notification.Builder.build()` 全放在主线程，会先造成应用自身卡顿，然后才轮到 NMS。
...
如果启用了 Notification Assistant，NMS 会用 `DELAY_FOR_ASSISTANT_TIME` 延后 `PostNotificationRunnable`，给 assistant 提交 `Adjustment` 的时间。这个延迟属于设计行为，不能直接判定为线程阻塞。
...
这些容器并非各自独立的数据副本；同一记录可能同时出现在列表和索引中。源码中的 `mNotificationLock` 保护跨结构一致性。诊断“更新丢失”时，要先分清记录停在 enqueued、被淘汰，还是已经 post 后尚未到达 SystemUI。
...
`SystemServer` 在主线程依次启动服务，随后才调用 `Looper.loop()`。所以这里的 `Looper.myLooper()` 是 `system_server` 主 Looper。原先常见的“NotificationManager 专用 ServiceThread”模型不适用于这个版本。
...
初次 post 中的 `mRankingHelper.extractSignals()` 和 `mRankingHelper.sort()` 仍由 `PostNotificationRunnable` 所在线程执行。不能看到名为 `ranker` 的线程，就认定所有排序都已从主线程移走。
...
2. extractor 可以返回 `RankingReconsideration`，交给 `ranker` Handler 延后处理；
3. `sort()` 先用 `NotificationTimeComparator` 排预排序结果；
4. 为每条记录计算 group proxy 和 global sort key；
...
| 位置 | 默认阈值 | 触发条件 |
| --- | ---: | --- |
| 应用进程的 `NotificationManager` 实例 | 目标上限 5 次/秒 | 同一 `(user, package, tag, id)` 已知为 enqueued，且新旧 `getProgressState()` 相同 |
| NMS `checkDisqualifyingFeatures()` | 每包目标上限 5 次/秒 | 已有同 key 的 posted/enqueued 记录，进度状态相同，且不是 autogroup |

`getProgressState()` 只有 `NONE`、`ONGOING`、`COMPLETE` 三种内部状态。进度从 41% 变为 42% 时仍是 `ONGOING`，所以持续刷百分比会进入限流；从 `ONGOING` 变为 `COMPLETE` 的关键更新可以通过“状态改变”条件。这里的豁免依据是状态变化，不是对每个 progress 数值做特殊判断。

客户端的 `mUpdateRateLimiter` 是 `NotificationManager` 的实例字段，同一实例中的合格更新共用一个 `RateEstimator`；通知 key 用于判断这次调用是否属于更新和记录拒绝日志，不会为每个 key 建立独立的 5 次/秒桶。`eventExceedsRate()` 比较估算输出速率与 `5f`，所以它也不是按自然秒清零的固定窗口计数器。
...
这个上限的源码条件还排除了 system/phone、已注册 listener 包，以及 FGS、user-initiated job 和系统聚合 group 的特定记录。应用不应利用这些例外堆积通知；它们是系统防止关键任务被普通通知配额误伤的保护条件。
...
`createNotificationChannel()` 是可重复调用的，但重复调用不等于覆盖配置。Android 17 的 `PreferencesHelper.createNotificationChannel()` 对应用侧更新采用以下规则：
...
- sound、vibration 等多数行为在首次创建后由系统保存，后续同 ID 创建不会重置。
...
通知中的 channel ID 随 `Notification` 一起传给 NMS。NMS 在同步提交阶段查询 `PreferencesHelper`；普通应用发布到不存在的渠道时，记录会被拒绝，并在日志中出现 `No Channel found`。它不会把 API 26+ 应用的错误 channel 自动改成另一个默认渠道。
...
- 判断 user/profile 和敏感内容可见性；
- 按 listener 的 trim 级别准备 `StatusBarNotification`；
- 构造对应的 `NotificationRankingUpdate`；
- 更新 URI 授权与包可见性；
- 生成一个稍后执行的 callback runnable。

这些 runnable 再被投递到 `mHandler`，随后调用 `INotificationListener.onNotificationPosted()`。AIDL 声明是 `oneway interface`，NMS 不等待远端 listener 方法执行完毕。

`oneway` 不代表零成本。NMS 仍要创建快照、写 Parcel、把事务放进 Binder 队列；远端进程不消费时，异步队列也可能形成压力。区别在于远端业务回调时长不处于应用最初 `notify()` 的同步等待范围内。
...
`NotifCollection.onNotificationPosted()` 有 `Assert.isMainThread()`。列表事件、ranking 应用和重建请求会经过 SystemUI 主线程。视图内容随后由 `NotificationRowContentBinderImpl` 异步构建 `RemoteViews` 并 apply 到 row；主线程仍需接收结果、替换 view、执行 measure/layout 和提交绘制。
...
可调用的方法受 `RemoteViews` 规则限制。SystemUI 不会加载并运行应用自定义 View 类的任意代码。这个限制同时服务于安全、跨进程稳定性和版本兼容。

系统样式也会在 SystemUI 中恢复 `Notification.Builder` 并生成平台模板 `RemoteViews`。区别在于布局和操作由框架控制，兼容性、内存约束和不同设备形态更可预测。能用 `BigTextStyle`、`MessagingStyle`、`CallStyle`、`ProgressStyle` 或 `MetricStyle` 表达的内容，优先使用系统模板。
...
满足条件后可以执行 `reapply`，重新运行操作而不重新 inflate 根布局。它仍会处理文本、图片、点击动作和校验，不能视为免费更新。改变自定义 layout ID 会失去这条重用路径。
...
Android 17 还在 SystemUI 的 `NotificationCustomContentMemoryVerifier` 增加了第二层检查。`NotificationRowContentBinderImpl` apply 自定义视图后，验证器遍历视图中的 `ImageView`：`BitmapDrawable` 按 `allocationByteCount` 计入，其他 Drawable 按固有宽高乘以 4 估算。`CHECK_SIZE_OF_INFLATED_CUSTOM_VIEWS` 使用 `@EnabledAfter(BAKLAVA)`，所以强制拒绝从 target API 37 开始；较低 target 的应用在同样超限时只收到迁移 warning。这个检查针对展开后的 Drawable，因而覆盖了 URI 和资源图片未计入 Parcel 估算的情况。

应用应优先使用系统模板；确需自定义视图时，要在解码前限制图片尺寸，并给视图被 NMS 剥离或被 SystemUI 拒绝准备可接受的退化路径。AOSP 阈值适合用于理解源码和日志，不适合作为应用的图片预算。

共享 bitmap backing 只能减少重复拷贝的某些环节，不能消除解码、缩放、Parcel 元数据、SystemUI 图片解析和 GPU 上传。大图通知的成本需要在应用、NMS、SystemUI 和渲染四段分别观察，详见 §9.6。
...
`FLAG_IMMUTABLE` 的理由是防止其他进程改变封装的 Intent，不能声称它天然比 mutable token 更快。inline reply 和 bubble 需要系统填充远程输入或启动参数，必须按对应 API 要求使用 mutable；Android 17 的 NMS 会拒绝把 immutable `PendingIntent` 用于 bubble intent 或带 `RemoteInput` 的 action。
...
冷启动或 cached app 解冻没有可跨设备使用的固定毫秒数。它取决于存储、代码和资源量、设备负载、进程状态与首帧工作。诊断时应记录点击业务 ID，在 Activity 的 `onCreate()` / `onNewIntent()` 和首帧处加 trace，再结合 `am_proc_start`、Binder 和调度事件定位。
...
应用使用 `setGroup(groupKey)` 给相关通知稳定分组，并用一条相同 group key、设置 `setGroupSummary(true)` 的通知描述组摘要。`setSortKey()` 只控制组内相对顺序。

如果只希望摘要发声或振动，应在所有子通知上设置 `GROUP_ALERT_SUMMARY`。反过来，`GROUP_ALERT_CHILDREN` 会静音 summary。默认 `GROUP_ALERT_ALL` 可能让一批同时发布的通知连续提醒用户。

NMS 也会对未正确分组的多条通知执行自动聚合。自动 summary 是系统生成记录，因此服务端更新限流会排除 `isAutogroup` 路径。自动分组的阈值、延迟和显示效果属于系统策略，应用不能把它当作稳定 API。

消息应用通常应使用 `MessagingStyle` 表达会话内容，再用 group 表达多个会话。不要为同一会话的每条消息创建一个永久通知；用稳定 `(tag, id)` 更新会话通知，既符合用户预期，也能控制 NMS 和 SystemUI 的记录数量。
...
Android 13 / API 33 起，非豁免通知受 `POST_NOTIFICATIONS` 运行时权限控制。应用仍可以调用 `notify()`；NMS 会创建和检查记录，并在 post 阶段根据包权限与渠道状态决定是否抑制显示。原先“权限拒绝后通知完全不进入 NMS”并不准确。

对普通通知，权限被拒绝时通常不会因为这项拒绝向 `notify()` 调用抛出异常，通知也不会进入通知栏。参数非法、包身份错误、危险 PendingIntent 或无效 FGS 通知仍可能抛异常，不能把 `notify()` 当成永不失败。

前台服务仍必须提供通知。用户拒绝通知权限后，前台服务任务可能仍出现在系统的 Active apps / Task Manager 表面，但通知不会按普通方式出现在 notification drawer。media session 与满足条件的 self-managed call 通知另有豁免边界，不能扩大成整个应用豁免。
...
渠道的 `setShowBadge()` 决定该渠道是否允许出现在支持 badging 的 Launcher；`Notification.Builder.setBadgeIconType()` 只是建议 Launcher 使用 none、small 或 large icon。两者都可能被 Launcher 或用户设置忽略。

不同 Launcher 获取与聚合通知的实现并非公开 API 合同。应用只应设置渠道和通知元数据，不要假设“每次更新必然启动 Launcher 进程”或用 badge 作为可靠计数存储。未读数应由应用数据维护。
...
应用可以在构建与提交两侧分别加 trace。trace 名只放阶段和短哈希，不写消息正文、账号或完整业务 ID：
...
- 应用自定义 atrace section。

在 Android 17 AOSP SystemUI 中，可见的源码 trace 名包括：
...
`dumpsys notification` 能看到活跃、enqueued、snoozed、渠道、listener、assistant、包级统计和 NMS 配置。`--noredact` / `--reveal` 会暴露通知内容，只能在授权测试设备上使用。
...
1. 应用 `notif:submit` 很长：先分解 builder、图片、Parcel 和同步 Binder wait；
2. NMS 没有记录：查客户端限流、channel、权限、50 条上限和 NMS 日志；
3. NMS 已 post、SystemUI 尚未收到：查 listener dispatch、Binder 队列和 SystemUI 进程状态；
4. `NotifCollection` 收到但 row 很晚：查 shade list rebuild、inflation executor、RemoteViews apply；
...
`MetricValue` 提供固定整数、浮点、文本、日期、时间和计时差等类型。计时器优先使用 `TimeDifference`，系统可以在显示端推进时间，无需应用每秒调用 `notify()`。
...
`MetricStyle` 至少需要一个 metric；`setCriticalMetric()` 只表达在紧凑表面优先使用哪个指标。请求 promoted treatment 也不保证系统一定把通知提升。
...
`Notification.hasPromotableCharacteristics()` 检查通知对象本身的结构条件，不包含用户是否允许、渠道 importance 或 OEM 附加条件。`NotificationManager.canPostPromotedNotifications()` 用于检查应用当前是否获准发布 promoted notification。最终是否设置 `FLAG_PROMOTED_ONGOING` 仍由系统决定。

Live Update 应用于已经开始、由用户发起且对时间敏感的活动，例如导航、行程、配送和进行中的训练。普通促销、未来很久才发生的事件或没有明确结束点的状态不符合用途。
...
它可以用于 `Metric`、`ProgressStyle.Segment`、`ProgressStyle.Point`，也可通过 `createSemanticStyleAnnotation()` 标记部分文本。颜色由平台根据主题和显示表面选择；应用不应依赖某个固定 ARGB 值。文本去掉颜色后仍必须表达完整含义，无障碍信息不能只靠颜色区分。

这些语义样式只有在通知和显示表面符合条件时才会变成可见样式。AOSP 的 `Metric` 文档明确把语义显示限定在 promoted notification，文本 annotation 的文档也以 promoted notification 为典型资格条件；普通通知中保留语义数据，不代表界面一定着色。
...
9. Android 17 的 Live Update 是否逐项满足资格，而非只调用一个 setter。
