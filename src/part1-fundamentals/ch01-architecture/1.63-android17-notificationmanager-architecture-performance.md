...
`NotifCollection.onNotificationPosted()` 有 `Assert.isMainThread()`。列表事件、ranking 应用和重建请求会经过 SystemUI 主线程。视图内容随后由 `NotificationRowContentBinderImpl` 异步构建 `RemoteViews` 并 apply 到 row；主线程仍需接收结果、替换 view、执行测量与布局并提交绘制。
