...
- 对“同一个通知且进度状态未改变”的高频更新做 5 次/秒的客户端限流；
...
- 查询应用前台状态；
- 最后才 `mHandler.post(new EnqueueNotificationRunnable(...))`。
...
2. `PostNotificationRunnable` 再检查包和渠道是否被禁用，把记录写入 `mNotificationList` 与 `mNotificationsByKey`，提取信号、排序、决定提醒效果，最后准备监听器回调。
...
Android 17 对高频更新设置了两道包级保护：

| 位置 | 默认阈值 | 触发条件 |
|---|---:|---|
| 应用进程 `NotificationManager` | 5 次/秒 | 同一 `(user, package, tag, id)` 已知为 enqueued，且新旧 `getProgressState()` 相同 |
| NMS `checkDisqualifyingFeatures()` | 5 次/秒 | 已有同 key 的 posted/enqueued 记录，进度状态相同，且不是 autogroup |
...
服务端判断使用 `NotificationUsageStats.getAppEnqueueRate(pkg)`，阈值来自 `Settings.Global.MAX_NOTIFICATION_ENQUEUE_RATE`，AOSP 默认 `5f`。超限时记录日志和统计后返回 `false`。应用通常收不到异常或成功回调，因此业务层不能把每次 `notify()` 当成可靠消息投递。
...
`NotifCollection.onNotificationPosted()` 有 `Assert.isMainThread()`。列表事件、ranking 应用和重建请求首先经过 SystemUI 主线程。视图内容随后由 `NotificationRowContentBinderImpl` 异步构建 `RemoteViews` 并 apply 到 row；主线程仍需接收结果、替换 view、执行 measure/layout 和提交绘制。
...
这两个值是 framework resource，可被设备配置覆盖，不应作为应用可用预算。Android 17 对 target API 37 的自定义通知视图还加强了内存检查，关闭通过 URI 绕开既有限制的路径。应用要按“尽量不使用自定义视图，必须使用时保持图片和 action 很小”来设计，而不是贴着 AOSP 阈值试探。
...
Android 17 针对 target API 37 的 custom notification view 加强内存限制，并封住通过 URI 绕开既有图片统计的方式。迁移时应检查：
