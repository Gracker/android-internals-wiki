...
| 组件 | 与通知发布相关的职责 |
| --- | --- |
| ActivityManager / `ActivityManagerInternal` | 处理用户、前台服务（FGS）通知策略、应用前台状态、PendingIntent 临时 allowlist |
...
1. `EnqueueNotificationRunnable` 把记录放进 `mEnqueuedNotifications`，继承旧记录的排序信息，处理暂缓、分组和实例 ID，并把新通知交给 Notification Assistant；
...
这些容器并非各自独立的数据副本；同一记录可能同时出现在列表和索引中。源码中的 `mNotificationLock` 保护跨结构一致性。诊断“更新丢失”时，要先分清记录仍在 enqueued、已被淘汰，还是发布后尚未到达 SystemUI。
...
`WorkerHandler` 上会运行入队、post、listener dispatch runnable 以及多类 NMS 消息。耗时任务会与其他 `system_server` 主线程工作相互影响。NMS 仍有两个专用线程：
...
NMS 的全局顺序也不是屏幕上的最终顺序。SystemUI 收到 `RankingMap` 后，还会按自己的分区、filter、group 与 comparator 构建 shade list。两层排序解决的问题不同：NMS 提供系统权威排序，SystemUI 决定当前界面如何组织和显示。
...
| NMS `checkDisqualifyingFeatures()` | 每包目标上限 5 次/秒 | 已有同键的 posted/enqueued 记录，进度状态相同，且不是 autogroup |
...
- 可以更新名称、description 和 blockable 等少数字段；
...
高 importance 通知一旦满足悬浮条件，SystemUI 还要更新 shade、heads-up 容器、动画和相关图标。把普通状态更新都放进高 importance 渠道，会同时增加打扰和界面工作量。性能优化不能以提高 importance 为手段。
...
- 判断用户、资料与敏感内容的可见性；
- 按 listener 的裁剪级别准备 `StatusBarNotification`；
- 构造对应的 `NotificationRankingUpdate`；
- 更新 URI 授权与包可见性；
- 生成一个稍后执行的 callback runnable。

这些任务再被投递到 `mHandler`，随后调用 `INotificationListener.onNotificationPosted()`。AIDL 将接口声明为 `oneway`，NMS 不等待远端 listener 方法执行完毕。
...
`NotifCollection.onNotificationPosted()` 有 `Assert.isMainThread()`。列表事件、ranking 应用和重建请求会经过 SystemUI 主线程。视图内容随后由 `NotificationRowContentBinderImpl` 异步构建 `RemoteViews` 并应用到 row；主线程仍需接收结果、替换视图、执行测量与布局并提交绘制。
...
`NotificationManager.fixNotification()` 会先调用 `reduceImageSizes()`。NMS 的 `checkRemoteViews()` 随后用 `RemoteViews.estimateMemoryUsage()` 检查每个自定义折叠、expanded、heads-up 和 public view。AOSP `android-17.0.0_r1` 的默认资源值为：
...
`FLAG_IMMUTABLE` 用于防止其他进程改变封装的 Intent，不能声称它天然比 mutable token 更快。inline reply 和气泡需要系统填充远程输入或启动参数，必须按对应 API 要求使用 mutable；Android 17 的 NMS 会拒绝把 immutable `PendingIntent` 用于气泡 Intent 或带 `RemoteInput` 的操作。

`FLAG_UPDATE_CURRENT` 表示匹配到已有令牌时更新 creator 提供的 extras。它对 immutable token 同样有效，但不是通用性能开关。PendingIntent 的匹配依赖 Intent 的操作、data、type、component、categories 以及 request code 等身份字段；如果不同通知误用同一身份，后一次更新可能覆盖前一次点击携带的数据。

可以采用以下规则：

- 相同业务对象的通知更新：保持稳定的 request code 与 Intent 身份；
- 不同业务对象：用稳定且不冲突的数据 URI 或 request code 区分；
...
组内每个 child 仍是独立的 `NotificationRecord` 和 `NotificationEntry`。它仍要经过权限、渠道、排序、listener fan-out 和 SystemUI collection。SystemUI 是否提前展开子项、保留哪些 row，还受当前管线、缓存和可见状态影响。不能承诺“折叠时只 inflate summary”或“NMS 批量排序同组记录”。
...
前台服务仍必须提供通知。用户拒绝通知权限后，前台服务任务可能仍出现在系统的“活动应用”或任务管理器界面，但通知不会按普通方式出现在 notification drawer。media session 与满足条件的 self-managed call 通知另有豁免边界，不能扩大成整个应用豁免。
...
一次图标属性更新通常不是通知性能的主要部分。高频更新的问题来自整次 notification callback、ranking、shade list rebuild、内容 reapply 和可能的布局，不能只估算一个 `ImageView.invalidate()`。

### Launcher Badge（通知圆点）

渠道的 `setShowBadge()` 决定该渠道是否允许出现在支持角标的 Launcher；`Notification.Builder.setBadgeIconType()` 只建议 Launcher 使用 none、大图标。启动器或用户设置都可能忽略这些建议。

不同 Launcher 获取与聚合通知的实现并非公开 API 合同。应用只应设置渠道和通知元数据，不要假设“每次更新必然启动 Launcher 进程”，也不要用角标作为可靠计数存储。未读数应由应用数据维护。
...
`notif:submit` 的时长覆盖客户端限流、`fixNotification()`、Parcel 和 NMS 同步 Binder 阶段。它不覆盖 enqueue runnable、assistant delay、listener callback 与 SystemUI 渲染。
...
3. NMS 已发布、SystemUI 尚未收到：查 listener dispatch、Binder 队列和 SystemUI 进程状态；
4. `NotifCollection` 收到但 row 很晚：查通知栏列表重建、展开执行器和 shade list rebuild、inflation executor、RemoteViews apply；
5. row 已应用、首帧仍晚：查 SystemUI 主线程布局、RenderThread 和 GPU。

## Android 17 / API 37 新能力

### `Notification.MetricStyle`

Android 17 新增 `MetricStyle`，用于健康、计时、天气和出行等需要同时展示少量动态指标的场景。平台模板最多显示三个指标；每个 `Notification.Metric` 由标签、`MetricValue` 和可选 semantic style 组成。
...
`MetricStyle` 至少需要一个指标；`setCriticalMetric()` 只表达在紧凑界面优先使用哪个指标。请求 promoted treatment 也不保证系统一定提升通知。
...
这些语义样式只有在通知和显示界面符合条件时才会生效。AOSP 的 `Metric` 文档明确把语义显示限定在 promoted notification，文本注解的文档也把提升通知作为典型资格条件；普通通知中保留语义数据，不代表界面一定着色。

### 自定义通知视图限制

Android 17 针对目标 API 37 的 custom notification view 增加了展开后内存检查。迁移时应检查：
...
| Android 14 / API 34 | 多数 ongoing / FGS 通知允许用户单条划走，少数通话、media、DPC 等场景例外 |
...
4. custom `RemoteViews` 是否有系统样式替代方案；
5. PendingIntent 身份是否冲突，mutability 是否只按功能需要开放；
6. group 是否同时设置正确的摘要与 alert behavior；
7. 是否把通知显示当成业务成功确认；
