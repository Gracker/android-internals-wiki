...
Binder 需要专门处理。被冻结的线程无法处理 IPC：同步调用不能无限等待，`oneway` 事务也不能无限堆积。以下从 Android 17 框架层与 ACK 6.18 源码拆解这条链路。
...
Activity / Service / Broadcast / binding 等状态
                     │
                     ▼
OomAdjuster 计算 proc state、oom_adj 与能力
                     │
...
ActivityManagerService
  ...
  └─ unfreezeAppLSP()
                                      │
                                      ▼
CachedAppOptimizer 执行 Binder 冻结与 cgroup freeze
```
...
3. 再检查仍在等待回复的 transaction stack。
...
冻结不是忙等。task 不会在 CPU 上循环检查“能不能解冻”，所以 Java 线程、native worker、Handler 和协程都不会继续执行。
...
2. server 因失去重要绑定退到 cached 并被冻结。
...
1. 只会观察远程 Binder；local Binder 与当前进程同生共冻。
...
`ENQUEUE_ALL` 也有队列上限。Android 17 默认最大 1000 条；达到上限后丢掉最旧回调，Builder 可用 `setMaxQueueSize()` 显式设置。选择这个策略时，仍要写出容量、溢出和恢复方案。
...
官方 freezer 文档说明：当一个应用的所有进程都 frozen 时，系统会终止该应用的活动 TCP socket，避免 keepalive 唤醒 modem。需要长期可靠传递的业务应使用 FCM、JobScheduler、WorkManager 或适合其语义的系统设施，不能依赖缓存进程维持 socket。
...
| `SUBREASON_FREEZER_BINDER_ASYNC_FULL` | frozen 期间异步 Binder 缓冲区接近耗尽 |
...
Perfetto 未采集 ActivityManager 或调度数据时，“没看到事件”不能证明 freezer 未运行。
...
| 版本 | 已确认变化 | 工程含义 |
| --- | --- | --- |
| Android 11（API 30） | AOSP 支持 cached apps freezer；`ApplicationExitInfo` 公开 | 设备是否启用仍依赖内核与配置 |
| Android 13（API 33） | 公开 `ApplicationExitInfo.REASON_FREEZER` | 应用可把 freezer kill 与 LMK、ANR 分开统计 |
| Android 14（API 34） | cached 后默认 10 秒冻结；生命周期事件立即解冻；动态注册广播可排队；冻结前后增加 GC/压缩等配合 | freezer 从单点开关扩展为更完整的缓存进程策略 |
...
| Android 17（API 37） | 当前 `OomAdjuster` 以显式/隐式 CPU_TIME 能力统一冻结资格；Binder 监控与 freezer 重试链继续完善 | 源码判断锚定 `android-17.0.0_r1`，内核锚定 `android17-6.18-2026-06_r6` |
...
不成立。设备可能没启用 freezer，进程可能仍持有 CPU capability，也可能处于防抖、Binder 重试或实现级保护中。

### “frozen 就等于释放内存”

不成立。freezer 的直接作用是停止 CPU 执行；进程仍占地址空间和物理内存。GC、compaction、ZRAM 回写与 LMK 是另外的动作。
...
2. 同时记录 proc state、oom_adj、CPU_TIME 能力与 freezer cutoff。
3. 区分 pending freeze、请求冻结和 `cgroup.events frozen=1`。
4. 检查 Binder 冻结是否先于 cgroup freeze。
...
8. 检查固定频率任务、广播与 oneway 是否在解冻后形成突发。
