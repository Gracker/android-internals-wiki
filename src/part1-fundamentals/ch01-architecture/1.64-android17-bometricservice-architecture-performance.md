...
本文以 Android 17 / API 37 / AOSP `android-17.0.0_r1` 为平台锚点。这里讨论的是应用通过 `BiometricPrompt` 发起的认证，不把锁屏直接调用 `FingerprintManager`、录入流程或厂商私有解锁通道混成同一条路径。

先给出结论：`BiometricService` 是一次 BiometricPrompt 会话的仲裁者，但它并非应用直接访问的第一个 Binder 服务，也不执行图像采集和模板匹配。应用入口、系统策略、会话状态、传感器队列、认证 UI 与安全执行环境分属不同组件。定位“生物识别慢”时，必须先判断时间消耗落在哪一段。
...
这与“所有 App 认证请求在 BiometricService 排队”不同。队列存在于每个传感器的 `BiometricScheduler`；BiometricPrompt 层的新会话会替换旧会话。公开 API 文档也明确指出：已有认证进行时再次调用 `authenticate()`，旧客户端会收到取消。

应用不应在配置变化、重复点击或状态重组时快速执行“取消—重建—认证”。这种写法会制造 UI 重建、HAL 取消确认以及 scheduler 清理开销，还会让一次用户操作产生多份互相覆盖的回调。
...
API 37 增加了受角色和权限限制的 `BiometricManager.getBiometricSensorStrengths()`。它只向合格的前台钱包或设备策略管理角色暴露模态到强度的映射，并以 `LESS_THAN_STRONG` 隐去不应公开的细粒度等级。普通应用仍应使用 `canAuthenticate()`。
...
## `BiometricScheduler`：每个传感器一次一个非中断操作
...
3. 等当前 operation 的 finish callback 完成，再推进队列。

因此，“录入一定比认证优先”或“认证一定能抢占录入”都不成立。是否中断取决于具体 client 的属性、当前 operation 是否已经开始以及它是否可取消。

### HAL 也要求串行

Fingerprint 与 Face AIDL `ISession` 文档都规定：一个 session 同时只能执行一个 non-interrupting operation。`authenticate()`、`enroll()`、`detectInteraction()` 返回 `ICancellationSignal`，取消后 HAL 必须以取消错误结束该 operation。
...
`BiometricScheduler.startWatchdog()` 中虽有 10 秒清理定时器，但在本 tag 的 framework Java 源码中没有直接生产调用点，不能把它描述成每次 authenticate 自动启用的超时。
...
- 用于会话协调的 requestId、cookie 和 operationId。
...
只有 Strong sensor 可以在成功回调中返回可供 Keystore 使用的 HAT。Android 17 的 `AuthSession.onAuthenticationSucceeded()` 对 non-strong sensor 会丢弃 token。
...
这意味着 App 观察到的 success 时间包含 UI 收尾和 token 注入阶段。HAL 已经匹配成功，不代表 App callback 已经执行。

Gatekeeper 负责 PIN、图案和密码等设备凭据验证；biometric trustlet 负责生物识别匹配；Keystore2/KeyMint 消费合适的 AuthToken。三者共享安全认证协议，但职责不同。
...
| 结果 | 含义 | 会话行为 |
|---|---|---|
| `onAuthenticationFailed()` | 采集有效但未匹配 | App 收到 failed；人脸等模态可能暂停并允许重试 |
...
不要在文档中硬编码“失败 5 次锁 30 秒”作为所有设备的 framework 实现。精确要求来自当前 Android CDD，执行细节还受 sensor strength、HAL 与设备策略影响。诊断时以实际 error、duration、sensorId 和设备 CDD 版本为准。
...
| 标记 | 事件 | 可观测位置 |
|---|---|---|
| `T0` | App 调用 `authenticate()` 前 | App 埋点 |
| `T1` | `AuthService.authenticate()` 接收 | system_server 日志或定制 trace |
| `T2` | `PreAuthInfo` 完成并创建 `AuthSession` | `BiometricService` |
| `T3` | 各 sensor cookie 就绪 | scheduler / `onReadyForAuthentication()` |
| `T4` | SystemUI 开始显示 Prompt | `AuthController.showAuthenticationDialog()` |
| `T5` | UI 动画完成并允许启动指纹 | `onDialogAnimatedIn()` |
| `T6` | HAL 首个 acquired | sensor client callback |
| `T7` | HAL success/failure/error | sensor client callback |
| `T8` | SystemUI dismiss | `onDialogDismissed()` |
| `T9` | App callback 开始执行 | 调用方 Executor |

有了这些点，可以计算：

```text
入口与策略耗时      = T2 - T0
scheduler 等待      = T3 - prepare 时间
Prompt 准备耗时     = T5 - T4
指纹等待 UI 时间    = fingerprint start - T3
采集与匹配耗时      = T7 - T6
首个样本等待        = T6 - HAL start
UI 与 token 收尾    = T8 - T7
应用调度延迟        = T9 - T8
...
“HAL start”需要从 framework client 或 vendor trace 取得。仅有 App 的 `T0/T9` 时，只能得到端到端耗时，不能据此判断 TEE 或传感器慢。

### 系统自带诊断入口

以下命令用于查看当前 session、注册 sensor、provider 和 scheduler 状态：
...
tag 和详细日志受构建类型、日志级别及 OEM 修改影响。采集前应先在目标构建上确认实际 tag。
...
本次审阅的 `BiometricService`、`AuthSession` 和 `AuthController` 主路径没有稳定的专用 `Trace.traceBegin()` slice 可作为跨设备契约。不要把厂商 trace 名称写成 AOSP 保证；需要时在自有系统构建中给 T1～T9 增加成对 trace。

### 症状到证据的映射

| 症状 | 先看什么 | 常见原因 |
|---|---|---|
| 点击后迟迟不弹窗 | T0～T4、foreground/AppOps、PreAuthInfo | 主线程阻塞、策略检查失败、旧会话取消、SystemUI 忙 |
| 弹窗出现后指纹才慢慢亮 | T3～T5、UDFPS overlay/display mode | UI 协调或显示准备，不一定是 matcher |
| 有提示但迟迟不成功 | acquired 序列、T6～T7 | 样本质量、环境、HAL 或 secure matcher |
| 已显示成功，业务页面仍未继续 | T7～T9 | 成功动画、HAT 注入、App Executor 排队 |
...
| Android 17 / API 37 | 本文源码锚点；包含当前 Identity Check 资格处理、受限 sensor strength 查询和现代 AIDL session context 能力 |
...
| callback 跑在哪个线程 | `BiometricHandlerProvider.java`、`BiometricPrompt.java` |
...
一次慢认证可能卡在旧会话取消、预认证、scheduler、AIDL session 冷启动、Prompt 动画、UDFPS 显示协作、首个样本、secure matcher、HAT 注入或 App Executor。把 T0～T9 分段采齐，再对照当前 requestId、sensorId、operation 和 error，才能把问题归到负责的组件。
