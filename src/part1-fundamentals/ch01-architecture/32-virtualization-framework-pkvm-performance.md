...
Android Virtualization Framework（AVF）从 Android 13 开始提供受保护虚拟机能力。它面向需要抵御宿主 Android 被攻破的敏感工作负载：宿主仍负责创建、调度和终止虚拟机，但不能读取受保护虚拟机（pVM）的私有内存，也不能悄悄替换经过验证的 Microdroid 和载荷。

“运行在虚拟机里会慢多少”没有跨设备固定答案。AVF 的成本取决于 vCPU 调度、VM 退出、共享内存窗口、virtio I/O、验证启动以及载荷行为。可复现的测量必须建立在 Android 17 的组件关系和安全边界之上。
...
宿主应用 / 系统组件
        ...
        ▼
framework-virtualization Java 库
        │  创建进程 + Unix 域套接字上的 RpcBinder
        ▼
virtmgr 子进程（Rust）
...
(全局 Rust 惰性 Binder 服务)          │ /dev/kvm ioctl
        │ CID / 全局资源              ▼
        └────────────────────────► EL2 上的 pKVM
                                      │
                                      ▼
...
`android/virtualizationservice/src/main.rs` 则注册全局的 `android.system.virtualizationservice` 惰性 Binder 服务。它负责 CID、全局资源、统计和维护。它与每个客户端拉起的 `virtmgr` 不在同一进程，也不位于 `system_server`。
...
| 组件 | 所在位置 | 主要职责 | 不负责什么 |
|---|---|---|---|
| pKVM | ARM64 EL2，来自 ACK/KVM | 宿主/客户机 Stage-2 权限、页面所有权、vCPU 切换和 pVM 保护 | VM 配置、磁盘组装、载荷生命周期 |
| crosvm | 宿主用户空间 Rust 进程 | VMM、KVM ioctl、vCPU 线程、virtio 设备、VM 内存布局 | 判定 APK/载荷的业务可信性 |
| `virtmgr` | 宿主用户空间 Rust 进程 | AIDL 生命周期、镜像/FD 准备、启动和监控 crosvm | 在 `system_server` 中常驻 |
| pvmfw | pVM 首段固件 | 验证初始镜像、维护实例身份、派生每台 VM 的机密 | 提供 Android 框架 API |
| Microdroid | 客户操作系统 | 验证启动、SELinux、Bionic、原生载荷、Binder RPC | 完整 Android UI 和应用框架 |

Microdroid 是 AVF 提供的一种轻量客户操作系统，但不是 AVF 唯一支持的客户机。API 37 还允许自定义 VM 配置；Android 的 Linux 开发环境也是基于 AVF 的非受保护 VM 用例。

## 二、Microdroid 不是“小号完整 Android”

Microdroid 为原生载荷提供 Android 基础设施：Bionic、验证启动、SELinux、APEX、日志/崩溃调试能力，以及基于 vsock 的 Binder RPC。它明确不提供：
...
启用 ART APEX 后可以使用 `java.*` 核心 API，但这不等于拥有常规 Android 应用运行环境。载荷通常是 APK 内嵌的原生共享库，由 Microdroid 载荷启动器执行。

因此，下列推断在 API 37 中没有依据：

- Microdroid 默认含完整 ART、SystemServer 和服务管理器服务集；
- 受保护 Microdroid 可以直接使用 virtio-gpu 或 NPU HAL 加速通用 AI 推理；
...
传统 KVM 在宿主运行时通常不使用 Stage-2 限制，因此宿主内核可以访问承载客户机内存的物理页。pKVM 在宿主上下文也启用 Stage-2：

- 宿主 Stage-2 使用恒等映射，地址不重排，主要负责访问控制；
- 客户机仍有自己的 Stage-2，把客户机 IPA 映射到物理地址；
- EL2 维护页面所有者，并决定宿主、某台 pVM、虚拟机监控器或设备能否映射该页。

Android 启动之初，除虚拟机监控器保留区外的内存归宿主所有。创建 pVM 时，宿主把页面捐赠给客户机；EL2 随后从宿主 Stage-2 中撤销这些页的访问权限。crosvm 进程仍保留用于建立 KVM 内存槽的虚拟地址区间和内存记账关系，但对应物理页已不在宿主 Stage-2 的可访问映射中。宿主 CPU 或受宿主控制的设备不能凭借这段用户空间地址绕过 EL2 读取 pVM 私有页。
...
| `relinquish` | pVM → 宿主 | 是，重新归宿主 | 内存气球/长期 VM 归还不用的页 |
...
页面回收伴随权限变更和内容处理；配置给 VM 的内存也可以在 VM 销毁前归还。pKVM 提供 `relinquish` 超级调用，virtio 内存气球可以借此回收长期运行 VM 中不用的页。

### 3.3 共享窗口为什么影响 I/O

virtio 的常规设计假设宿主设备后端可以跟随 virtqueue 描述符访问客户机缓冲区。pVM 私有页不满足这个假设。若每次请求都临时共享一个小缓冲区，页面粒度共享还可能暴露同页中的无关数据。

AVF 的受保护客户机因此为 virtqueue 和数据缓冲区预留固定共享内存窗口，客户机在私有页与共享窗口之间进行中转复制。性能影响包括：

- 小 MMIO 控制访问可能触发客户机 → EL2 → 宿主 VMM → 客户机的往返；
- 大数据通常走共享 virtqueue，不需要每个字节都触发 VM 退出；
- 中转复制、缓存维护和唤醒次数会影响吞吐与尾延迟；
- 缓冲区大小、批量深度和 I/O 模式不同，结果会相差很大。
...
crosvm 通过 `mmap` 分配 VM 物理内存，再用 `KVM_SET_USER_MEMORY_REGION` 建立内存槽。Android 官方架构文档指出，这部分内存归属管理该 VM 的 crosvm 进程；宿主内存不足时，crosvm 可以被终止，整台 VM 也会随之停止。

对于已经捐赠给 pVM 的私有页：

- 宿主不能将其换出或执行 KSM 合并；
- 宿主不能把它当作普通匿名页读取；
- 客户机通过 `relinquish`/内存气球归还后，页面可以回到宿主；
- VM 停止时，虚拟机监控器清理并归还剩余页面。

“128 MiB 配置永久硬占 128 MiB，直到销毁”过于绝对；“宿主的 lmkd 能直接回收客户机内某几页”也不准确。内存压力的控制单元通常是 crosvm/VM，细粒度回收依赖客户机主动配合。
...
AOSP 的 `MicrodroidBenchmarks.canBootMicrodroidWithMemory()` 在 16～512 MiB 区间做二分尝试，只有载荷成功启动才记录结果。源码选择运行时探测，说明“内核 8 MiB + ART 24 MiB + 设备 4 MiB”这类静态加法不能作为产品容量结论。

### 4.3 AOSP 的内存测量方式

API 37 的基准测试同时采集三组数据：

1. 客户机 `/proc/meminfo`：`MemTotal`、`MemAvailable`、缓存、slab；
2. 宿主 `/proc/<crosvm-pid>/smaps`：区分 `crosvm_guest` 映射与 VMM 自身 RSS/PSS；
3. KVM 调试统计（设备支持时）：如受保护共享/虚拟机监控器相关内存。

这三组数的口径不同，不能相加后当作 AVF 总内存。应分别报告客户机配置量、客户机工作集、crosvm 宿主侧 PSS、pKVM/KVM 额外开销，以及回收前后的变化。

## 五、CPU 调度与 VM 退出

### 5.1 vCPU 是宿主调度器管理的线程

每个 vCPU 对应 crosvm 中的 POSIX 线程。线程调用 `KVM_RUN` 后进入客户机；出现需要 VMM 处理的 I/O、vCPU 执行 halt 或其他退出原因时，`KVM_RUN` 返回宿主用户空间。

宿主 Linux 调度器仍可抢占 vCPU 线程，并把客户机执行时间计入该线程。vCPU 线程可以使用常规 QoS 工具设置亲和性、cpuset、uclamp 和调度策略。客户机不能绕过宿主调度器获得物理 CPU。

因此，VM 内看到的慢任务至少可能来自三层：

1. 客户机内线程没有被客户机调度器选中；
2. 对应 vCPU 线程在宿主上处于可运行状态，但没有获得 CPU；
3. vCPU 因 MMIO/virtio/中断等事件退出，在 crosvm 或宿主内核等待。

只看客户机内的 Perfetto 无法区分后两层。性能分析需要把客户机时间线与宿主的 crosvm/vCPU 线程调度对齐。

### 5.2 每次 I/O 不等同于一次完整 VM 退出

virtio 用 MMIO 完成设备控制与通知，数据面主要通过共享 virtqueue 传输。一次高层文件读写可能拆成多个队列请求，也可能批量消费多个描述符。eventfd、epoll、中断合并和队列深度都会改变退出/唤醒次数。
...
没有设备型号、CPU 频点、负载、virtqueue 配置和统计分布时，`VM exit = 5–10 μs` 或“比系统调用慢 3–10 倍”都不能作为 Android 17 平台结论。

## 六、vsock 与 Binder RPC 的边界

### 6.1 vsock 是宿主与 pVM 的基础通信通道

`VirtualizationServiceInternal` 为运行中的 VM 分配 CID。CID 在 VM 存活期间唯一；VM 结束且相关 `IVirtualMachine` 引用释放后，数值可以复用。端口由客户机服务自行选择。
...
- `testVsockLatency()`：独立的回显/反转协议，同样在预热后循环测量；
- `testVsockTransferFromHostToVM()`：48 MiB 连续发送，报告吞吐；
- `testVirtioBlkSeqReadRate()`/`RandReadRate()`：区分顺序与随机读，并丢弃首次受宿主页缓存冷启动影响的样本。

这些用例没有把某一台实验设备的结果固化为平台常量。复用时应保留原始样本，报告 P50/P90/P99，并记录受保护模式、调试级别、内存、vCPU 拓扑、大页、uclamp、温度和 CPU 频点。

## 七、VM 生命周期语义

### 7.1 `run()` 返回不代表载荷已就绪

`VirtualMachine.run()` 完成启动请求后即可返回。VM 是否开始运行、操作系统是否启动、载荷是否就绪，要通过 `VirtualMachineCallback` 观察。常用节点包括：
...
API 文档把 `VirtualMachine.stop()` 比作拔电源：客户机软件不会收到正常关机通知，加密存储写入可能未持久化。需要正常关机时，应通过载荷的 Binder/vsock 协议请求退出，并等待 `onPayloadFinished()`。

“正常销毁 100–500 ms，强杀 50–200 ms”没有平台保证。应分别测量：

- 载荷收到退出请求到 `onPayloadFinished()`；
- `stop()` 调用到 `onStopped()`；
...
AVF 也没有完全替代 TrustZone。TEE 仍承载 KeyMint、Gatekeeper 等依赖安全世界的设备能力；pVM 提供更丰富、可动态创建的隔离执行环境。两者的信任根、设备访问和攻击面不同，不宜用固定的强弱关系或世界切换微秒数排序。
...
1. 以 `run()` 调用时刻为起点；
2. 以 `onPayloadStarted()` 或业务自定义就绪状态为终点，两者分开报告；
3. FULL 调试模式下再采集 vCPU、引导加载程序、内核、用户空间分段；
4. 至少区分首次实例创建、已有实例重启和页缓存冷热；
5. 报告分布，不只报平均值。

### 9.3 CPU 与 I/O

1. 客户机 Perfetto 观察载荷、客户机调度器和客户机 I/O；
2. 宿主 Perfetto 观察 crosvm、vCPU 线程、sched 和块 I/O；
3. 统计每单位请求的 VM 退出/通知次数；
4. 顺序/随机、吞吐/延迟、小消息/大流量分别测试；
5. 检查宿主页缓存、CPU 亲和性和 uclamp 是否改变结论。

### 9.4 内存与回收

1. 记录配置内存与客户机 `MemAvailable`；
2. 从 crosvm `smaps` 分开 VMM PSS 和 `crosvm_guest` 映射；
3. 设备允许时记录受保护共享/虚拟机监控器 KVM 统计；
4. 对内存气球/回收前后使用相同工作集；
...
3. 单独测量连接建立、单次 RPC、稳态吞吐和尾延迟；
4. 分开测量载荷正常退出与 `stop()` 强制终止；
