...
注册只让服务认识这个代理；调用 `markConnected()` 标记为 connected 后，
...
`makeNetworkMonitor()`。源码专门处理了 monitor 尚未返回而代理已继续更新的窗口，
...
对一个 `NetworkRequest`，`NetworkRanker` 先删除不满足能力、transport、
...
默认回调也不只在默认网络对象变化时触发。当前网络的能力、link properties
...
`bpf_traffic_account()` 后更新 UID、tag、interface 维度的映射；egress 也进入同一计数
...
除此之外，网络状态变化、上游变化、全局警报、注册 usage callback、强制更新、
...
布尔状态与 Data Saver、前后台状态等公开信息结合展示，但不能仅凭这个回调断定
...
AOSP 侧不只增加了清单声明。Connectivity 的 `BpfNetMaps` 维护 local-network
权限传播开关、UID/net/host allowlist 和缓存代次；这也解释了限制为何能作用于
受管套接字、原生套接字和上层网络库。
...
2. **agent 是否注册并连接**：netId、interface、capabilities 是否存在；
