...
- 🔹 **模块与进程边界**：Connectivity Mainline 模块、`system_server`、NetworkStack 独立进程与 `netd`
- 🔹 **网络的系统表示**：`NetworkAgent`、`NetworkAgentInfo`、`NetworkCapabilities`、`LinkProperties`
- 🔹 **注册与验证**：Binder 注册协议、`NetworkMonitor`、门户网络、部分连通与 Private DNS
- 🔹 **网络匹配与排序**：`NetworkRequest`、`NetworkScore`、`FullScore`、`NetworkRanker`
- 🔹 **应用回调语义**：监听、主动请求、默认网络、线程模型、回调顺序和资源上限
- 🔹 **流量统计与策略**：BPF 计数、历史归档、Data Saver、Doze、App Standby
- 🔹 **VPN 的两条实现路径**：应用 `VpnService` 与平台 IKEv2/IPsec
- 🔹 **Android 17 变化**：`ACCESS_LOCAL_NETWORK`、流媒体套餐速率、ECH

### 排障目标

- 🔸 分清“链路已连接”“声明可上网”“已验证联网”和“成为某 UID 默认网络”
- 🔸 判断问题发生在网络提供者、验证、排序、路由、策略、DNS 还是应用回调
- 🔸 用可测量指标替代固定耗时和固定 CPU 百分比
<!-- outline-end -->
...
`packages/modules/Connectivity/service/src/com/android/server/ConnectivityService.java` —
服务通过 `ModuleNetworkStackClient` 创建 `NetworkMonitor`]

[已验证: AOSP android-17.0.0_r1,
`packages/modules/NetworkStack/AndroidManifest.xml` —
`NetworkStackService` 的进程为 `com.android.networkstack.process`]
...
旧文章常写成 `AsyncChannel`。Android 17 的 agent 数据通道已经不是这套模型。
...
`markConnected()` 很重要：注册只让服务认识这个 agent；agent 标记为 connected 后，
...
`register()`、`markConnected()` 与 `INetworkAgentRegistry`]

[已验证: AOSP android-17.0.0_r1,
`packages/modules/Connectivity/service/src/com/android/server/connectivity/NetworkAgentInfo.java` —
created、connected、validated、partial connectivity 的独立时间状态]

## 从注册到可用：真实生命周期

可以把一次 Wi-Fi 或蜂窝网络上线拆成以下步骤：
...
验证状态机、DNS/HTTP/HTTPS/Private DNS 探测及退避重试]
...
specifier、UID 等条件的网络，再对候选集逐项筛选。Android 17 源码中的重要顺序是：
...
Android 17 对每 UID 的 outstanding requests 设有 100 个上限。这个计数由多种
...
[已验证: AOSP android-17.0.0_r1,
`packages/modules/Connectivity/bpf/progs/netd.c`；ACK
`android17-6.18-2026-06_r6`, `net/core/filter.c`]
...
`NetworkStatsCollection` 管理 bucket 化记录。`queryDetailsForUid()` 打开统计 session，
...
| Doze | 通过 dozable 等防火墙链约束未在允许列表中的 UID |
...
应用应把 blocked 当成状态输入，停止无意义的重试或把工作交给 JobScheduler/WorkManager，
而不是循环建 socket 验证限制是否解除。
...
TUN fd、`protect()` 与 underlying networks]

[已验证: AOSP android-17.0.0_r1,
`frameworks/base/services/core/java/com/android/server/connectivity/Vpn.java` —
应用 TUN 建立、VPN NetworkAgent、IKEv2/IPsec transform 与 MOBIKE]
...
Encrypted Client Hello。客户端库和服务端都支持时，TLS ClientHello 中的 SNI
...
`ACCESS_LOCAL_NETWORK` 为 dangerous permission]

[已验证: AOSP android-17.0.0_r1,
`frameworks/base/telephony/java/android/telephony/SubscriptionInfo.java` —
流媒体上下行 Kbps API 与 `BITRATE_UNKNOWN`]
