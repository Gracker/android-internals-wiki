...
- `setThreadPoolMaxThreadCount()` 可以改 native pool 上限；线程池启动后不能把上限调小。
...
优先级继承只能帮助 worker 更早获得 CPU，不能绕过互斥锁、磁盘 I/O、内存回收或下游慢服务。Perfetto 中若 worker 已经被调度但仍长时间 Sleeping，应继续查锁与 I/O，而不是继续调优 priority。
...
本文的 platform、libbinder 与 Perfetto 结论锚定 `android-17.0.0_r1`，Binder driver 结论锚定 `android17-6.18-2026-06_r6`。厂商内核 hook、服务线程池大小、SELinux 可见性和 user build 的调试权限仍需在目标设备确认。
