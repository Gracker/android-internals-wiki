...
1. Proxy 把接口令牌、方法事务码和参数写入 `Parcel`。
2. libbinder 通过 `BINDER_WRITE_READ` ioctl 把事务交给 Binder 驱动。
3. 驱动解析目标句柄，找到目标 Binder node 和宿主进程，为目标进程分配事务缓冲区。
4. 驱动选择目标线程，或把工作放入目标进程的待处理队列并请求补充工作线程。
5. 服务端线程收到 `BR_TRANSACTION`，进入 Stub 的 `onTransact()`，反序列化参数并调用实现。
6. 同步调用把结果与异常状态写入 reply；驱动再把 reply 投递给原调用线程。

Binder 还能在传输时转换 Binder 对象、句柄和文件描述符，并把内核可信的调用 PID、UID、SID 等身份信息交给服务端。服务端权限检查应在 IPC 入口完成，不能信任客户端写进 `Parcel` 的“身份字段”。
...
- `Proxy` 持有远端 `IBinder`，负责写入请求 `Parcel`、调用 `transact()` 并读取回复。
...
Android 17 的内核 Binder 在目标进程的 Binder 事务缓冲区中分配页面，再用 `copy_from_user()` 把发送方数据写入这些页面。目标进程已经把这片缓冲区映射到自己的地址空间，因此不需要再执行一次“内核缓冲区到接收方用户空间”的数据拷贝。
...
- Binder 对象、文件描述符和安全上下文需要驱动校验与修正。
...
所以 libbinder 实际请求的是 `1 MiB - 2 × page_size` 的虚拟映射。Android Developers 把它概括为当前约 1 MB。这个缓冲区属于进程，供该进程当前进行中的 Binder 事务共享，不是每次事务各有 1 MB：
...
内核把一半缓冲区空间作为异步事务的初始可用额度。大量 `oneway` 请求因此也会耗尽异步空间，并可能触发 `-ENOSPC`、`FAILED_TRANSACTION` 或 oneway spam 诊断。
...
不要用“单次不到 1 MB”作为安全线。并发量和对端 reply 同样占用各自进程的缓冲区。
...
`ProcessState::startThreadPool()` 会先创建线程池的首个线程；驱动在没有足够工作线程时返回 `BR_SPAWN_LOOPER`，libbinder 再按需创建线程，直到配置上限。手工调用 `joinThreadPool()`、Java 运行时的接入方式和服务自身配置还会影响最终可见线程数。诊断时应读取目标进程的实际线程与配置，不能只数到 15 就宣布池耗尽。
...
这条日志只能证明“输出日志的进程在该时间段内没有可用执行槽”，不能单独证明某个远端服务的线程池已满。
...
如果工作线程都在等同一把锁，增加线程只会制造更多等待者。应先缩短临界区、移出阻塞 I/O，或把长任务转交给受控的业务线程池。
...
- 缺少业务背压时，高频通知会堆积并挤占目标进程的异步空间。

Android 17 默认请求启用 `BINDER_ENABLE_ONEWAY_SPAM_DETECTION`。当某个发送 PID 占用过多目标异步缓冲区时，内核可以返回 `BR_ONEWAY_SPAM_SUSPECT`，libbinder 输出调用栈。它是问题信号，不是可靠的限流协议；接口设计仍需合并状态、去重、批量发送或明确丢弃策略。

### 冻结进程

对 cached/frozen 进程调用 Binder 还有额外语义：

- 同步事务会被内核以 `BR_FROZEN_REPLY` 拒绝；平台 freezer 策略可能终止被冻结的目标进程，调用方收到 `RemoteException` 或对应冻结错误。
...
同步 Binder 的调用方可能是高优先级线程，而服务端工作线程原本优先级较低。Binder 驱动会临时调整处理该事务的工作线程优先级，事务完成后再恢复，减少简单的优先级反转。

Android 支持三层规则：

- 事务优先级继承：同步事务继承调用方优先级；`oneway` 默认不继承。
- node 优先级继承：服务端通过 `BBinder::setMinSchedulerPolicy()` 为 node 设置最低调度优先级。
- 实时优先级继承：默认关闭，服务端 node 必须显式 `setInheritRt(true)`。

优先级继承只能帮助工作线程更早获得 CPU，不能绕过互斥锁、磁盘 I/O、内存回收或下游慢服务。Perfetto 中若工作线程已经被调度但仍长时间 Sleeping，应继续查锁与 I/O，而不是继续调整优先级。
...
接口名依赖 trace 中可解析的 `AIDL::*Server` / HIDL 切片；只抓 Binder ftrace 时，`interface` 与 `method_name` 可以为空。
...
4. 服务端执行得久：展开子切片，检查锁、I/O、GC、直接回收和下游 Binder。
5. 接口名缺失：不要猜方法，补抓 AIDL atrace 或用 transaction code 与服务端调用栈交叉确认。
6. 事务失败：检查 `FAILED_TRANSACTION`、缓冲区、进程死亡与 freezer 记录。

一个 Binder 工作线程空闲时也会睡在 `binder_thread_read` 或 ioctl 等待路径。看到该阻塞函数不代表卡死；只有承载关键事务的调用线程长时间等待，或目标端没有及时开始执行时，它才是需要解释的证据。
...
先确认工作线程在做什么。锁竞争和 I/O 不会因为等待者增多而变快；更大的线程池还会提高并发内存、锁竞争和下游压力。
...
- Android 14 及以后：平台会暂存发往 cached 应用的上下文注册广播，并在应用解冻后再投递；清单接收器仍会触发立即解冻。
- Android 17：原生 libbinder 仍以 `BINDER_VM_SIZE = 1 MiB - 2 × page_size`、`DEFAULT_MAX_BINDER_THREADS = 15` 为默认源码锚点；实际服务可以改变线程池配置。Perfetto `android.binder` 与 `android.binder_breakdown` 应以当前 Trace Processor 模块和实际录制事件为准。

本文的平台、libbinder 与 Perfetto 结论锚定 `android-17.0.0_r1`，Binder 驱动结论锚定 `android17-6.18-2026-06_r6`。厂商内核钩子、服务线程池大小、SELinux 可见性和 user build 的调试权限仍需在目标设备确认。
