...
范围限定在 Android 平台 API 与 AOSP 服务端。Google Play 服务的 `FusedLocationProviderClient`、`LocationCallback` 和地理围栏 API 是另一套客户端 API；它们可能使用系统的 `fused` provider，也可能在自己的服务中增加策略，但不能用来解释 `LocationManager` 的公开调用链。
...
| Provider | Android 17 框架层实现 | 请求是否主动驱动数据源 | 需要注意的边界 |
| --- | --- | ---: | --- |
...
Binder 注册成功不代表 Provider 马上运行，也不代表每个位置都能交付。权限、前后台和省电状态可以在注册存续期间改变，服务端会随之更新活动状态。
...
1. `LocationProviderManager` 把质量改为 `QUALITY_LOW_POWER`，并把间隔与 min interval 提高到内部的 10 分钟下限；
...
- 最大更新延迟取最小值；
- ADAS 绕过与 ignore-settings 标志做 OR；
- `lowPower` 做 AND；
- WorkSource 收集接近最短间隔、会影响底层工作量的注册。

如果 `maxUpdateDelay / 2 < interval`，framework 把合并后的 batching delay 置为 0。这样可以避免一个不具备批处理意义的延迟字段反复触发 Provider 重配。公开 `LocationRequest` 文档也把“最大延迟至少为间隔的两倍”列为可能批处理的条件。

例如，同一 `gps` provider 有两个活动请求：
...
同一 Provider 的活动请求共享底层工作。一条 1 秒 GPS 请求可能使 GNSS 持续工作；其他低频 GPS 注册虽然被单独限速，底层成本已经由最激进的请求决定。优化时应找出参与合并的最短间隔和最强质量，而不能只看某个业务模块自己的配置。
...
不要假设 Perfetto 中存在稳定的 `LocationManagerService` 或 `GnssLocationProvider` 自定义轨道。`android-17.0.0_r1` 的这些 Java 路径没有提供可依赖的统一 slice 名称。可用 Binder、线程调度、CPU frequency/idle、wakelock 和应用自定义 trace 组合还原延迟。
...
HAL 会上报调度、measurement、geofence、low-power mode 等能力。framework 据此启用对应路径。例如，没有调度能力时，`GnssLocationProvider` 可以通过闹钟在低频请求之间休眠和唤醒；具备调度能力时，可把周期直接交给 HAL。
...
`GnssLocationProvider.updateRequirements()` 从合并请求读取间隔、max update delay、low-power 提示和 WorkSource：

- 满足 batching 条件时，停止普通导航并启动硬件 batch；
- 已启动且 HAL 支持调度时，更新 `setPositionMode()`；
- 尚未启动时，设置 position mode 后调用 `start()`；
- HAL 不支持调度时，framework 用闹钟辅助长间隔轮询；
...
满足这些条件后，`GnssNative.startBatch()` 才会进入 `IGnssBatching.start()`。如果请求的 batch 长度小于硬件缓冲区容量对应的时长，framework 还会设置精确闹钟，按 batch 长度调用 `flushBatch()`。

batching 的收益来自减少 AP 唤醒和 Binder/Java 交付次数，但 GNSS 接收机仍可能按设定周期采样。它不能自动把射频跟踪成本降为零。评估收益要同时比较 AP 空闲时间、唤醒次数、GNSS 功耗统计和厂商电源轨数据。

### 8.3 GNSS 测量的 full tracking 是另一条请求

`GnssMeasurementRequest.Builder.setFullTracking(true)` 自 API 31 起用于原始 GNSS measurements。它要求关闭 duty cycling，以连续跟踪更多 GNSS 信号，通常会增加功耗。多个 measurement listener 合并时，只要任一活动请求要求 full tracking，合并结果就启用 full tracking；最短 measurement interval 也会胜出。

API 34 增加了 `GnssMeasurementsEvent.isFullTracking()`，调用者可以在 HAL 支持相应信息时判断事件是否处于 full-tracking 模式。

普通 `LocationRequest` 的 `QUALITY_HIGH_ACCURACY + 短间隔` 不等价于显式 `GnssMeasurementRequest.setFullTracking(true)`。`IGnssPowerIndication` 也不是 full-tracking 开关：它只提供 `setCallback()` 和 `requestGnssPowerStats()`，用途是报告 GNSS power capabilities/statistics。
...
HAL 通过 PSDS 回调请求 framework 下载数据。Android 17 的路径是：
...
- 网络、SUPL/PSDS 状态；
- 室外天空条件、静止/运动状态；
- 请求开始的 elapsed realtime 与首个有效定位；
- 多轮分布，而非只报一次最小值。
...
`GeofenceManager` 要求 fine location 权限。它为所有活动围栏合并一个位置请求，根据“当前位置到最近围栏边界的距离 / 假定最大速度”估算下次更新间隔，并受后台 proximity alert 节流值约束，最长不超过 2 小时。缓存位置超过 5 分钟就不用于这个估算。
...
位置问题常同时跨越应用、`system_server`、Provider 服务和厂商 HAL。只看一条回调耗时，很难区分请求未激活、Provider 没有定位、结果被过滤或 Executor 堵塞。
...
命令输出会随构建类型和厂商扩展变化。调试 user build 时，部分身份或 HAL 细节可能被裁剪。
...
| 回调频率低于请求 | 服务端改写后的间隔、单注册过滤 | coarse/后台节流、min interval、min distance、batching |
| GPS 长时间保持活动 | `gps` 的合并请求和调用者 | 另一条更短间隔请求、未取消注册 |
| TTFF 变长 | 多轮 TTFF、PSDS/SUPL/网络、天空条件 | 辅助数据、射频环境、HAL/固件变化 |
| 回调后系统仍持有 wakelock | App Executor 与完成时间 | Executor 拥塞或回调耗时 |
...
- 对 batching 同时验证 HAL 支持、batch size、请求比例和实际刷新；
