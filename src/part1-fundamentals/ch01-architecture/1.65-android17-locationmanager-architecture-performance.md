...
这一章只讨论 Android 平台 API 与 AOSP 服务端。Google Play services 的 `FusedLocationProviderClient`、`LocationCallback` 和 Geofencing API 是另一套客户端 API；它们可能使用系统的 `fused` provider，也可能在自己的服务中增加策略，但不能用来解释 `LocationManager` 的公开调用链。

> **源码锚点**
>
> - 平台：Android 17 / API 37 / `android-17.0.0_r1`
> - GNSS HAL 接口：同一平台标签下的 `hardware/interfaces`
> - 本章不涉及 Linux kernel 实现；不能把厂商 GNSS 驱动行为推断成 `LocationManagerService` 的固定语义
...
| Provider | Android 17 framework 侧实现 | 请求是否主动驱动数据源 | 需要注意的边界 |
|---|---|---:|---|
...
## 2. Provider 是调用者选择的，quality 不负责自动选源

平台 API 的核心规则是：**provider 决定请求交给谁，`LocationRequest.quality` 只是给该 Provider 的质量提示。**
...
`getLastKnownLocation(provider)` 通过 `ILocationManager.getLastLocation()` 读取指定 Provider 的服务端缓存。它不会为了这次调用启动 Provider，返回值可以为 `null`，也可能已经过时。业务必须结合 `Location.getElapsedRealtimeAgeMillis()`、accuracy 和数据完整性自行判断能否使用。
...
这解释了一个常见现象：Binder 注册成功不代表 Provider 马上运行，也不代表每个位置都能交付。权限、前后台和省电状态可以在注册存续期间改变，服务端会随之更新 active 状态。

### 4.3 coarse 权限会改请求，也会改结果

Android 12 起，用户可以在应用请求 fine+coarse 时选择 approximate location。Android 17 服务端对只有 coarse 权限的注册至少做两层处理：
...
同一 Provider 的活跃请求共享底层工作。一条 1 秒 GPS 请求可能使 GNSS 持续工作；其他低频 GPS 注册虽然被单独限速，底层成本已经由最激进的请求决定。优化时应找出参与合并的最短 interval 和最强 quality，而不能只看某个业务模块自己的配置。

`dumpsys location` 会列出 Provider 请求和注册，是定位“谁把 GPS 拉到高频”的首选证据。WorkSource/BatteryStats 用于能耗归因，但合并后的归因集合不表示每个应用承担相同比例。
...
HAL 会上报 scheduling、measurement、geofence、low-power mode 等能力。framework 据此启用对应路径。例如，无 scheduling 能力时，`GnssLocationProvider` 可以通过 alarm 在低频请求之间休眠和唤醒；有 scheduling 能力时，可把周期直接交给 HAL。
...
从 UNKNOWN/OUTSIDE 进入 INSIDE 时发送 entering；只有先处于 INSIDE，之后转为 OUTSIDE，才发送 exiting。原实现没有网格空间索引、loitering delay 或通用 hysteresis margin。
...
不能由此得出 `addProximityAlert()` 会自动把同一个围栏切换到 GNSS 硬件。前者在 `GeofenceManager` 中以位置更新做软件判断；后者服务于配置的硬件 geofence provider。Google Play services 的 Geofencing API 也不属于这条公开平台调用链。
...
建议在应用中用 elapsed realtime 写入自定义 trace 或结构化日志：

```text
T0  调用 LocationManager API
T1  system_server 注册变为 active / ProviderRequest 更新
T2  Provider 上报 location
T3  App Executor 开始回调
T4  App 完成轻量回调处理
```

判断方式：

- `T0→T1` 长：查 Binder、权限、位置开关、前后台与省电策略；
- `T1→T2` 长：查 Provider、GNSS TTFF、network/fused 外部服务和 HAL；
- `T2→T3` 长：查 Binder 调度、进程冻结/优先级、Executor 队列；
- `T3→T4` 长：查应用回调中的锁、I/O、计算和主线程工作。

Perfetto 可组合 `sched`、`binder_driver`、CPU frequency/idle、wakelock/power、network，以及设备提供的 vendor GNSS data source。没有自定义时间戳时，不要凭模糊的 “Gnss” 字符串把某段 slice 当成 TTFF。
...
本章在 `android-17.0.0_r1` 上确认了平台 API、服务端合并、GNSS HAL 兼容路径与两类 geofence 基础设施。厂商 Provider 算法、GNSS 芯片功耗和射频性能不属于 AOSP 可统一保证的范围，相关结论必须附具体设备与测量条件。
