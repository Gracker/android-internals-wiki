...
> - 本轮正文补充来自 DeepResearch 材料 `2026-07-17-android17-windowmanager-architecture-sourcecode.md`；材料中的行号仅作为 `android-17.0.0_r1` 源码定位线索，不用于推导固定锚点之外的行为。[来源: 2026-07-17-android17-windowmanager-architecture-sourcecode.md]
> - 本轮时效性巡检将正文范围重新锁定为 Android 17 / API 37 / `android-17.0.0_r1`；性能诊断命令、BLAST/Surface 所有权、Window tracing 与 input integration 均只按本章 frontmatter sources 所列 Android 17 锚点解释，不外推到未列入本书基线的平台版本。[来源: metadata/queue.json:freshness:src/part1-fundamentals/ch01-architecture/1.66-android17-windowmanager-architecture-performance.md；已验证: 本章 sources 与 `last_verified_against`]
