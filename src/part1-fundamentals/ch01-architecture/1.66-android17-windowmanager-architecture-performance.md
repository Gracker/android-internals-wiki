...
last_body_apply_at: "2026-07-26T07:15:29+08:00"
last_body_apply_run_id: "20260726-071529-abfc0d7c"
last_body_apply_source: "DeepResearch/2026-07-17-android17-windowmanager-architecture-sourcecode.md"
task2b_state: fixed
task6_state: reviewed
task9_state: reviewed
pipeline_stage: finalized
last_review_finalize_at: "2026-07-26T08:17:37+08:00"
last_review_finalize_run_id: "20260726-081439-8c646e9c"
---

# 1.66 Android 17 WindowManager 架构与性能优化

WindowManager 的难点不在某一个类，而在四套状态必须保持一致：
...
`WindowManagerService`（WMS）负责第 2 层，并把需要合成与输入系统执行的状态写进 `SurfaceControl.Transaction`。它不会绘制应用 View，也不负责逐个消费触摸事件。先分清这条边界，才能判断卡顿发生在应用遍历、WMS 全局锁、等待首帧、SurfaceFlinger 提交，还是 InputDispatcher。

> **源码锚点**
>
> - 平台：Android 17 / API 37 / `android-17.0.0_r1`
> - SurfaceFlinger、BLAST 与 InputDispatcher：同一标签下的 `frameworks/native`
> - 本章不使用设备无关的“固定省时”“固定 Surface 内存”结论；这些数据需要指定设备、刷新率、窗口数量和 trace 条件
> - 本轮正文补充来自 DeepResearch 材料 `2026-07-17-android17-windowmanager-architecture-sourcecode.md`；材料中涉及的行号仅作为 `android-17.0.0_r1` 源码定位线索，不扩展到 Android 18/API 38 结论。[来源: 2026-07-17-android17-windowmanager-architecture-sourcecode.md]
...
本轮材料进一步把入口阶段拆成三类可观测检查点：`WindowManagerPolicy.checkAddPermission()` 先返回窗口类型/AppOp/display 权限结果；进入 `mGlobalLock` 后再检查 display ready、session 是否死亡、重复 `IWindow`、子窗口父子关系和 token 类型；只有这些条件通过后才会创建 `WindowState` 并进入 input channel、焦点和层级更新。因此，`BadTokenException` 或 `ADD_*` 失败码通常应先映射到 addWindow 的校验阶段，而不是直接归因到 SurfaceFlinger 或 BLAST。[来源: 2026-07-17-android17-windowmanager-architecture-sourcecode.md]
...
所以，SurfaceFlinger trace 中的即时父子关系可能处于动画结构，不能只凭某一帧的 Layer parent 反推稳定的 WMS 容器树。应把 WindowManager trace 与 SurfaceFlinger layers/transactions 同时查看。
...
材料中的源码片段确认 `mAnimationHandler = new Handler(AnimationThread.getHandler().getLooper())`，其注释把 animation tick、layout、starting window creation 归为直接影响动画的任务；`mH` 则处理仍重要但不在动画关键路径上的消息。结合本章复核，正确表述应是“AnimationThread 承载 surface placement 等关键窗口任务并仍受 `mGlobalLock` 约束”，而不是把它描述成绕开 WMS 全局锁的独立实时通道。[来源: 2026-07-17-android17-windowmanager-architecture-sourcecode.md；已验证: AnimationThread/DisplayThread priority 语义见本章源码锚点]
...
- 已知 token 还会按 window type 检查，例如 IME token 不能拿来添加 wallpaper；
- 合法 `WindowContext` listener 可以让 WMS 以其 binder/options 构造 `WindowToken`；
- 其他允许自行建 token 的非应用窗口，可使用 `attrs.token` 或 `client.asBinder()` 创建。
...
如果 `displayContent.getWindowToken(attrs.token)` 查不到现有 token，Android 17 的材料显示还要区分 WindowContext 与兜底 binder 两条路径：合法 `WindowContext` listener 会通过 `WindowToken.Builder` 携带 `ownerCanManageAppTokens`、`roundedCornerOverlay`、`fromClientToken` 和 `options` 等语义标志构造 token；否则使用 `attrs.token` 或 `client.asBinder()` 继续按窗口类型约束创建/验证。这个 Builder 过程解释了为什么 token 解析不是简单的 HashMap miss 后直接 `new WindowToken`。[来源: 2026-07-17-android17-windowmanager-architecture-sourcecode.md]
...
应用侧收到 `BadTokenException` 时，先核对 window type、Context/token 来源、Activity 生命周期和 display，而不是先查 SurfaceFlinger；失败发生在 Layer 创建之前。
...
这条错误日志表达的是“client-owned surface 不应再由 server 创建”，并不表示 client-created surface 已废弃。

`ViewRootImpl.updateSurfaceControl()` 在该路径还会对 `INVISIBLE` 的 SurfaceControl 做缓存，后续重新可见时可复用有效句柄。它是有 flag 和可见性条件的客户端优化，不是 WMS 面向所有窗口的通用 Surface 池。
...
BLAST 的价值是让 buffer acquire 与 SurfaceControl transaction 有可表达的同步关系。它是否降低某台设备上的延迟、降低多少，需要 trace 比较；源码本身没有“每帧固定少一次 round trip”或“固定节省若干毫秒”的保证。

## 7. relayout：同步返回与异步发送的选择

`ViewRootImpl.relayoutWindow()` 在允许时先用本地 `WindowLayout.computeFrames()` 计算 frame，然后决定调用同步或异步 relayout。

### 7.1 `canRelayoutAsync()` 先检查是否具备本地计算条件
...
异步路径也分两种：server-created surface 调 `relayoutAsync()`，client-created surface 调 `relayoutAsync2()`。因此不能把 async relayout 写成只服务于 visibility/insets，或写成 client-surface 路径专属。
...
- `deferLayout()` 增加 defer depth；期间请求只记录 pending，最后 `continueLayout()` 再执行；
...
- `BLASTSyncEngine` 在 WMS 中等待一组 `WindowContainer` 达到 sync finished，再合并这组容器的 SurfaceControl transaction。
...
Transition 创建 SyncGroup
  ...
  → Transition 根据 ready groups 调 setReady()
...
Android 17 的 Shell Transitions 由 `TransitionController`/`Transition` 在 system server 收集参与者和起止状态，通过 `BLASTSyncEngine` 得到一致的 start transaction，再把 `TransitionInfo`、start transaction、finish transaction 交给 transition player。

`SurfaceAnimator` 则提供更基础的 Surface 动画机制：

1. 为被动画对象创建 leash；
2. 把目标 Surface reparent 到 leash；
3. `AnimationAdapter` 修改 leash 的 matrix、position、alpha、crop 等；
...
排查黑屏/闪屏时，应把“请求可见、已有 Surface、首帧已提交、Surface 已 show、Layer 已 present”分别确认。
...
但“窗口数量增加”不自动等于某种固定内存或帧耗。每个窗口的 buffer 尺寸、格式、buffer 数、是否持续更新、是否被硬件合成，以及显示器分辨率/刷新率都会改变结果。

fluid resize 的关键是减少不必要的同步 relayout，同时保证“尺寸和相对位置一起变”时仍取得 BLAST sync id。应用侧还应避免在拖拽期间对整棵 View 树做与尺寸无关的重复测量、昂贵图片解码或同步 I/O。
...
- 被识别的 leaked surface 或被杀进程；
- SurfaceFlinger layer/buffer 状态；
- 各可见窗口分辨率、格式和 buffer 数；
...
`WindowTracing` 可以按 transaction 或 frame 记录 WMS proto state；非 debuggable build 会拒绝启动。Android 17 默认实现写入 Perfetto 数据源，适合和 SurfaceFlinger transactions/layers 一起分析。
...
再配合 `sched`、`binder_driver`、`view`/`wm`/`gfx`/`input` atrace category、SurfaceFlinger transactions/layers 和应用自定义 trace，建立：
...
- overlay 使用正确 Context/token/type，并处理 display/lifecycle 变化。
...
本章基于 `android-17.0.0_r1` 复核了窗口添加、两种 Surface 所有权、BLASTBufferQueue、surface placement、BLASTSyncEngine、transition、visibility 与 input integration。设备侧合成能力、gralloc buffer 策略和显示时序由具体产品实现决定，性能结论应附 trace、设备和复现条件。
