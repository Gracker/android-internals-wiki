...
所以，SurfaceFlinger trace 中的即时父子关系可能处于动画结构，不能只凭某一帧的 Layer parent 反推稳定的 WMS 容器树。应同时查看 WindowManager 跟踪记录与 SurfaceFlinger 图层/事务。
...
应用侧收到 `BadTokenException` 时，应核对 window type、Context/token 来源、Activity 生命周期和显示器；失败发生在 Layer 创建之前，无需先查 SurfaceFlinger。
