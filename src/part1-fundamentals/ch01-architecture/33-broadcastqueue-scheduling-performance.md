...
Android 广播是一种受系统策略调度的事件分发机制。调用 `sendBroadcast()` 只表示系统接受了发送请求，不表示接收方会立刻执行。目标进程是否存活、是否处于 cached 状态、接收者类型、广播是否有序、调度槽是否空闲，都会改变投递时间。

Android 17 的实现与早期 Android 常见的“前台一条队列、后台一条队列”示意图已有明显差异。理解广播性能，需要从 `BroadcastRecord`、`BroadcastProcessQueue` 和进程调度槽三层入手。
...
这两个值还能通过 `Settings.Global.BROADCAST_FG_CONSTANTS` 和 `BROADCAST_BG_CONSTANTS` 中的 `bcast_timeout` 覆盖。文章或排障记录写“固定 10 秒/60 秒”时，应同时注明它是源码基准值，不是所有构建和设备都无法更改的常量。
...
内部类名在开发分支和历史版本中发生过变化，应用可依赖的是公开广播语义，不能根据某个旧类名判断设备一定运行 Legacy 或 Modern 模式。本章后续实现细节均锚定 `android-17.0.0_r1`。

## 二、三层数据结构分别解决什么问题

### 2.1 `BroadcastRecord`：一次发送及其全部接收者

`BroadcastController.broadcastIntentLockedTraced()` 查询 manifest receiver 和运行时注册的 receiver，把结果合成一份接收者列表，再构造 `BroadcastRecord`。
...
| `DELIVERY_DEFERRED` | 因 cached/deferral 策略延后 | 否 |
...
这些值来自 `BroadcastConstants`，并可由 `activity_manager_native_boot` DeviceConfig 调整。额外 urgent 槽用于在普通槽已占满时保证紧急广播前进，不是把所有广播并行度永久提高到 5。

## 三、从发送到 `onReceive()` 的实际路径
...
但源码仍对每个 receiver 分别调用 `scheduleRegisteredReceiver()` 或 `scheduleReceiver()`。这不是把多条广播序列化进一次 Binder 调用。文章中“同进程多个广播自动合并为一次 IPC”的说法没有源码依据。
...
对无序、没有 completion callback 的 context-registered receiver，`BroadcastRecord.isAssumedDelivered()` 返回 true。system_server 在成功发出 `scheduleRegisteredReceiver()` 后立即把该项标记为 delivered，不等待应用回报，也不会为它启动广播 ANR 定时器。

以下投递会等待 `finishReceiver()`，并受广播超时跟踪：

- manifest receiver；
- ordered broadcast 的 receiver；
- 带 completion callback、不能按 assumed-delivered 处理的投递。

这并不表示无序动态 receiver 可以长期占用主线程。它仍会阻塞该应用自己的 UI 和后续消息，可能触发输入、前台服务等其他类型 ANR；只是 BroadcastQueue 不等待这次无序动态投递的完成回执。
...
“无序广播并行”也不等于所有 receiver 同时执行。实际并行度仍受 running 槽、单冷启动槽、目标进程主线程和 cached 策略限制。
...
Manifest receiver 不走同样的无限延迟路径。重要的 manifest broadcast 可以让应用离开 cached 状态并启动接收进程。把“cached app 的所有广播都等其他原因拉起”写成统一规则是不准确的。
...
Deferral 解决“何时投递”，delivery group 解决“积压的多条是否都要投递”。没有显式 delivery group 策略时，系统不会因为接收者在同一进程就自动把任意广播合并。

`FLAG_RECEIVER_REPLACE_PENDING` 也只替换满足发送 uid、user、Intent 匹配、receiver 和其他条件的 pending 项。它不是针对整个 action 或整个进程的无条件去重。

### 5.4 freezer 与广播队列互相通知，但不能简化成“冻结时一律挂起”

广播队列会观察进程是否可冻结，并在状态变化时重新计算 runnable/deferred 状态。若接收进程已进入 running 投递，系统还会更新 OOM adj 和冻结状态，保证回调能够执行。

API 37 另有受 feature flag 控制的 outgoing broadcast 延迟：freezable 发送进程产生的广播可以暂存在它自己的进程队列，进程恢复后再入正式队列。这个“发送方延迟”与“接收方处于 cached 状态时延迟投递”不是同一条路径。
...
队列等待和广播触发的冷启动发生在启动此定时器之前。因此，一条广播端到端等待很久，不等于 receiver 已经执行超时；排障必须把 dispatch delay 与 finish delay 分开。

### 6.2 `goAsync()` 延长的是回调完成方式，不是无限时间

需要异步完成的 receiver 可以调用 `goAsync()` 取得 `PendingResult`，在短任务结束后调用 `finish()`。对需要 system_server 等待完成的投递，ANR 定时器不会因为 `goAsync()` 自动取消。

长时间网络、磁盘扫描、数据库迁移等工作不应留在广播完成窗口内。receiver 更适合做参数校验、去重和任务入队，再交给 `JobScheduler`、WorkManager 或明确生命周期的服务。
...
5. trace 中的长时间是 dispatch 前排队，还是 schedule 后执行。

## 七、应用侧容易写错的性能建议

### 7.1 不要把 manifest receiver 当成默认的省性能方案

Manifest receiver 的 IntentFilter 在安装阶段解析，但投递时仍要查询、做权限/可见性检查，而且它能拉起不存在的进程。Android 8 起，多数隐式广播还不能由面向 API 26+ 的应用随意静态注册。
...
官方文档明确不保证广播投递时延。需要同步响应、背压、调用结果或稳定低延迟的跨进程接口，应使用 bound service/AIDL。广播适合“一次发送、多个可能接收者”的事件通知。
...
- 需要去重时明确使用 delivery group，而不是期待系统猜测业务语义。
...
当前进程级实现把 `receive_delay_ms` 记为 0，因为“被进程队列选中”和“提交给应用”之间没有旧实现中的独立阶段。分析时重点看下面两个量：

- `dispatch_delay_ms = scheduledTime - enqueueTime`：包含队列等待及冷启动影响；
- `finish_delay_ms = terminalTime - scheduledTime`：包含应用侧执行与完成回执。

不要复制依赖不存在字段或别名的 SQL。先在目标 trace 中确认是否有 `broadcast_delivered`，再通过 `slice`/`args` 表查看该版本实际导出的参数名。没有启用 SDK category 时，仍可结合 `am_proc_start`、应用主线程 slice、sched 和 Binder 轨道还原延迟。
...
- Android 14 / API 34：cached app 的 context-registered broadcast 可以延后，公开 `BroadcastOptions` deferral 和 delivery-group API；
- Android 16 / API 36：跨进程 receiver priority 顺序不再保证，priority 只在同一应用进程内有效；
- Android 17 / API 37：本章源码结构以 `BroadcastQueueImpl`、`BroadcastProcessQueue` 和逐接收者 `delivery[]` 为准。
