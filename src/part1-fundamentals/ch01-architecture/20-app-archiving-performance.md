...
App Archiving 是 Android 15 把“归档应用”从应用商店能力上收到平台层后的实现。它处理的是存储空间回收：系统移除目标应用的 APK 与缓存文件，保留用户数据，并让 Launcher、Settings 和恢复流程继续知道这个包曾经存在。用户点灰显图标时，系统把恢复请求交回负责安装的应用商店。 [已验证: 官方文档, android-developers.googleblog.com/2024/04/the-first-beta-of-android-15.html#app-archiving]
...
- **存储回收**：APK、split APK 和 cache 是主要回收对象；用户数据目录保留，用于恢复后延续登录态、配置和业务数据。
...
公开 API 入口在 `PackageInstaller`。AOSP main 中 `requestArchive()` 的文档写明：归档过程中会移除应用 APK 与 cache，保留用户数据；归档应用仍会通过 `LauncherApps` 作为可展示应用返回，用户点击后进入恢复流程。 [已验证: AOSP main, frameworks/base/core/java/android/content/pm/PackageInstaller.java#L2418-L2438]
...
这个设计避免了把所有“类找不到”都当作归档恢复。普通坏 Intent、组件名写错、应用已彻底卸载，都不会被归档逻辑吞掉。
...
存储视角下，收益来自 APK 和缓存文件。用户数据目录保留，因此大数据账号、离线资源、数据库文件不一定被回收。空间治理策略要先区分 code / cache / data 三类目录：归档对 code 和 cache 更有效，对 data 的收益取决于安装器或应用是否另有清理策略。详见 6.1 节。
...
| Android 17 / API 37 | 当前公开资料未显示归档恢复口径发生根本变化；后续要以平台 release branch 复核 | [待验证] |

第三方应用商店要接入这套机制，至少要处理四件事：

- 作为 responsible installer 被 PMS 找到；否则 `requestUnarchive()` 会因为找不到安装器失败。
...
现有素材提到 Android 16 之后存在 `.sdm` 签名文件和恢复包完整性校验线索，但本轮没有在 AOSP main 的归档主路径中完成端到端验证。正文只记录一个待查方向：如果安装器通过签名委托或增量包恢复归档应用，需要确认 `.sdm` 与 APK 签名校验、update owner、installer identity 的交互点。 [待验证]
