...
Android 17 的 `PackageInstaller` API 文档明确要求同一组 split 满足以下条件：
...
这些约束也能在 `PackageInstallerSession.validateApkInstallLocked()` 中看到。该方法收集新增 APK，检查重复 split，通过 `assertApkConsistentLocked()` 核对包名、版本和签名，并将文件规范化为平台使用的名称。
...
## 二、PackageInstaller session 的真实执行过程
...
Android 10（API 29）起，父 session 可以通过 `setMultiPackage()` 关联多个子 session。提交父 session时，子 session 作为一个原子多包请求处理。它解决的是多个包需要共同成功或失败的问题，不等于并行下载，也不会取消每个子包各自的签名、策略和兼容性检查。
...
API 37 为 `PackageInstaller` 增加了开发者验证相关接口和结果信息，包括：
...
2. **session 写入**：`createSession()` 到最后一个输出流关闭，分别统计下载等待和本地写入；
...
调试时先标明问题属于下载、session 写入、平台验证、安装提交、Intent 解析还是 URI 读取。这个分类比笼统地说“分发慢”更容易得到可复现的结论。
