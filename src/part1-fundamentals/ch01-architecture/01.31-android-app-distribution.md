...
`validateApkInstallLocked()` 会收集新增 APK，检查重复 split，通过 `assertApkConsistentLocked()` 核对包名、版本和签名，并将文件规范化为平台使用的名称。换句话说，AAB 到 split APK 的选择可以发生在商店或 `bundletool`，但 base/split 集合一旦交给设备，设备端只接受满足 APK 一致性规则的结果。
