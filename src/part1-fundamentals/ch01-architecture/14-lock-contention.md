...
线程显示为等待（Waiting）或休眠（Sleeping），只能证明它没有在 CPU 上执行，不能直接证明发生了锁竞争。它可能在等 Java 监视器、原生互斥锁、条件变量、Binder 回复、I/O、定时器，也可能只是 Looper 正常睡在 `epoll_wait()`。

诊断锁问题时，先回答四个问题，不要从猜测“哪种锁更快”开始：

1. **谁在等**：主线程、RenderThread、Binder 工作线程，还是普通后台线程？
2. **等什么**：Java 监视器、原生同步原语、Binder 回复，还是正常事件等待？
3. **谁能让它继续**：实际持锁者或服务端线程是谁？
4. **持锁者为什么没有及时推进**：正在运行、排队等 CPU、阻塞在另一把锁，还是做了 I/O？
...
| 类型 | 常见入口 | 主要实现 | 首要证据 |
| --- | --- | --- | --- |
| Java 监视器 | `synchronized`、`Object.wait()` | ART Monitor / LockWord | `android_monitor_contention`、持锁者/等待者方法 |
| Java 并发包 | `ReentrantLock`、`Condition`、`LockSupport.park()` | AQS、挂起/唤醒、原生等待 | Java 调用栈、线程状态、相关业务埋点 |
| 原生锁 | `pthread_mutex`、`std::mutex`、条件变量 | bionic + futex | 原生栈、`blocked_function`、锁埋点 |
| Binder IPC | 同步 AIDL、服务端线程池 | libbinder + Binder 驱动 | Binder 事务/回复、客户端/服务端线程状态 |
| Looper 等待 | `MessageQueue.next()` | Java 队列 + 原生 Looper + epoll | 是否有到期消息、`nativePollOnce()`、MessageQueue 竞争 |

它们都可能在内核线程状态里表现为睡眠，但优化方式完全不同。

- Java 监视器要找持有同一对象监视器的线程。
- `ReentrantLock` 不属于 ART 监视器，不能指望 `android_monitor_contention` 自动给出持锁者。
- 原生 `futex_wait` 只说明某个 futex 慢路径在等待，无法据此确认它对应互斥锁或启用了优先级继承。
- 同步 Binder 调用的客户端等的是服务端回复；服务端内部可能又卡在 Java 或原生锁上。
- 空闲 Looper 睡在原生轮询中属于正常行为，无需消除。

## 2. ART 监视器：`synchronized` 在 Android 17 中怎样工作
...
这些枚举说明 LockWord 还要容纳对象标识哈希和转发状态。观察到 `synchronized` 时，不能把对象头恒定解释成一组持锁者/递归计数字段。

### 2.1 无竞争路径：轻量锁

对象未加锁时，ART 可以把当前线程的监视器线程 ID 和递归计数编码进 LockWord。线程通过原子更新获得轻量锁（thin lock）；同一线程重入时增加计数。

轻量锁的优势是不用为每个曾被 `synchronized` 的对象都分配完整监视器。无竞争时，路径短、没有线程休眠，也没有内核调度切换。

“无竞争的 `synchronized` 很便宜”不等于“任何 `synchronized` 都只做一次 CAS”。对象可能已经膨胀为重量级监视器，可能带有对象标识哈希，也可能发生递归和运行时状态变化。性能结论要落到实际对象和竞争形态。

### 2.2 竞争、`wait()` 和对象标识哈希：监视器膨胀

`art/runtime/monitor.cc` 的源码注释列出需要完整监视器的三类典型情况：

- 出现真实竞争。
- 在对象上调用 `wait()`。
- 一个需要加锁的对象同时带有对象标识哈希。

竞争线程面对被其他线程持有的轻量锁时，ART 会协调持锁者并尝试把锁膨胀为重量级监视器（fat monitor）。重量级监视器记录完整的持锁者、等待者、条件等待和诊断信息。

`Object.wait()` 的语义也不能简化成“睡一会”：

1. 调用线程必须已经持有该对象监视器。
2. `wait()` 把线程放入等待集合，并释放监视器。
3. `notify()` / `notifyAll()`、中断或超时使它具备继续条件。
4. `wait()` 返回前还必须重新获得同一个监视器。
...
也就是发布构建为 100ms，调试构建为 1000ms。这个常量服务于 ART 的长竞争告警逻辑，日志是否出现还受采样、持锁者方法是否可解析等条件影响。
...
- 可能来自条件变量。
- 可能来自 ART 或其他运行时内部同步。
- 可能是 Java 并发包最终触发的线程挂起。
- 只能证明线程在某个 futex 等待点睡眠，不能单凭函数名恢复锁对象和持锁者。

要定位原生锁，通常还需要原生调用栈、业务锁埋点、同进程线程状态，或在可复现环境中增加针对该锁的跟踪记录。
...
- 持锁者释放锁后撤销这次提升。
- 持锁者又阻塞在另一把 rt-mutex 时，提升可以沿依赖链传播。
- 等待者按优先级组织；同优先级使用 FIFO 顺序。

PI 缩短的是“高优先级线程被低优先级持锁者挡住，而持锁者又抢不到 CPU”的窗口。它不能缩短持锁者自己在临界区里做的 I/O、长计算或同步 Binder 调用。
...
只有互斥锁属性显式选择 `PTHREAD_PRIO_INHERIT`，bionic 才走 `__futex_pi_lock_ex()` / `__futex_pi_unlock()`，由内核 PI-futex 和 rt-mutex 处理持锁者优先级提升。普通互斥锁仍走普通 futex 路径。
...
客户端在等回复时没有持有“Binder Java 监视器”。驱动使用自己的锁和等待队列管理事务、线程与工作项；服务端的业务代码则可能再遇到 Java 监视器或原生互斥锁。
...
+ 服务端线程获得运行机会
+ 服务端业务执行
+ 服务端内部锁 / I/O / 嵌套 Binder 等待
+ 回复返回与客户端再次被调度
```

只看客户端睡在 Binder 相关内核函数上，无法判断哪一项占主导。
...
- `binder_transaction_priority()`：根据事务与 Binder 节点限制处理服务端线程的优先级。

这是 Binder 事务调度的一部分，不等同于 `PTHREAD_PRIO_INHERIT`，也不会自动提升“服务端线程正在等待的某个 Java 监视器持锁者”。如果 Binder 工作线程进入服务代码后又卡在应用锁上，仍要沿 Java 或原生持锁者链继续追。
...
初始化 Binder 驱动时，libbinder 通过 `BINDER_SET_MAX_THREADS` 把这个默认最大值设为 15。与此同时，`startThreadPool()` 会调用 `spawnPooledThread(true)` 主动启动一条主池化线程。
...
把这些机制压缩成“Binder 线程池固定 15 条”或“固定 16 条”都会误导跟踪分析。线程数只是容量的一部分；一个工作线程持大锁或做慢 I/O，仍可能让其他事务排队。
...
- 持锁者持锁执行长计算或磁盘 I/O。
- 持锁者在持锁状态发起另一个同步 Binder 调用。
- 多把系统锁形成长等待链。
- Binder 工作线程接近饱和，新事务迟迟没有线程处理。

分析顺序应是：从客户端事务跟到服务端回复，再看服务端线程的状态；如果它在等锁，继续找持锁者，不能停在“Binder 调用耗时”这个表面结论。
...
这里调用者已经受 `mServiceLock` 保护，再进入 `mProcLock`，符合全局锁到进程锁的顺序。`ProcessList.mLruProcesses` 也标注为组合锁保护。

这些源码能证明锁域和顺序，不能单独证明某个固定性能提升比例。锁持有时间必须来自具体设备与跟踪记录；没有可定位的一手基准时，不应写“从 25ms 降到 8ms”或“吞吐提升 3 倍”。

## 7. MessageQueue：Android 17 的一个针对性去锁案例

旧 MessageQueue 用同一个 `synchronized (this)` 保护有序链表，生产者入队、Looper 取消息和移除操作会在这把监视器上竞争。

对运行在 Android 17 且 `targetSdkVersion >= 37` 的应用，DeliQueue 默认启用。它把生产者提交改为 Treiber 栈上的 CAS，由 Looper 独占两个最小堆完成同步/异步消息排序。核心消息路径不再依赖旧的单一全局监视器，但空闲处理器和文件描述符记录仍有各自的小锁。

Google 公布的内部测试跟踪记录中，应用主线程花在锁竞争上的时间下降 15%。这个数字只描述其样本中的 MessageQueue 改造效果，不能外推到 AMS、Binder、原生互斥锁或业务锁。

DeliQueue 展示了一种有边界的无锁设计：

- 多生产者共享的提交路径无锁。
- 排序复杂度留给单一 Looper 持有者。
...
这张表能给出 Java 监视器的等待者、持锁者、双方方法、源码位置、是否主线程、锁名和时长。`android_monitor_contention_chain` 还能表达竞争的父子关系；配套线程状态表可以继续检查持锁者在持锁期间处于运行（Running）、可运行（Runnable），还是又阻塞在其他内核函数。

注意两个边界：

- 表中没有记录，不等于设备上绝对没有竞争；跟踪配置、运行时采样和可解析信息都会影响数据完整性。
- 这张表针对 ART Java 监视器，不覆盖所有 `ReentrantLock` 和原生互斥锁。
...
这一步只负责找候选。随后要结合原生栈或源码确认它对应互斥锁、条件变量还是其他等待，并找持锁者。不能根据 `futex_wait` 一个字符串直接断定发生了 Java 锁竞争。

### 8.3 Binder 客户端与服务端

Perfetto 的 `android.binder` 模块可以把事务和回复关联到客户端/服务端：
...
再用 `android_sync_binder_thread_state_by_txn` 或 `android_sync_binder_blocked_functions_by_txn` 分解客户端与服务端两端的线程状态。若服务端回复区间叠加 Java 监视器竞争，就继续查 `android_monitor_contention` 给出的持锁者。

“客户端时间长、服务端时间短”可能意味着排队、调度或回复返回延迟；“服务端时间长”也不能直接等同于 CPU 计算，服务端可能大部分时间在锁、I/O 或嵌套 Binder 上。

## 9. 三个常见现场怎样推理

### 现场一：主线程出现 `monitor contention with ...`

1. 在 `android_monitor_contention` 找等待者方法、持锁线程和持锁者方法。
2. 看持锁者在等待区间内的 `thread_state`。
3. 持锁者如果处于可运行状态但长时间未运行，检查 CPU 压力与优先级反转。
4. 持锁者如果睡在另一把锁或 Binder 上，继续沿依赖链追。
5. 回源码确认临界区，检查能否移出 I/O、IPC 或长计算。

### 现场二：主线程睡在同步 Binder 调用

1. 用 `android_binder_txns` 找服务端进程、线程和方法。
2. 比较客户端与服务端区间，确认时间花在哪一端。
3. 服务端工作线程若等监视器，用竞争表找持锁者。
4. 服务端工作线程若在原生 futex 中等待，结合原生栈找具体锁。
5. 多个事务都排队时，检查工作线程饱和和某个长事务是否占住线程。

### 现场三：线程停在 `futex_wait`

1. 先确认 Java 调用栈是否来自 ART 监视器、AQS/线程挂起，还是 JNI/原生代码。
2. 检查是否有对应的 `android_monitor_contention` 事件。
3. 没有 Java 监视器证据时，按原生锁或条件等待调查。
4. 找初始化代码，确认是否为 PI 互斥锁；不能根据函数名猜测。
5. 如果这是正常的条件变量等待，再查“为什么条件迟迟没有产生”，无需优化 futex 本身。
...
把代码移出锁前要重新审视不变量。盲目缩小范围可能制造“先检查后执行”竞态，性能改善不能以破坏正确性为代价。

### 10.2 再减少共享状态

- 用线程封闭或消息传递代替共享可变对象。
- 把一把全局锁拆成按对象、按分片或按子系统的锁。
- 读多写少时考虑不可变快照、写时复制或读写分离。
...
微基准要覆盖真实竞争形态。只测单线程获取/释放锁，无法预测主线程与多后台生产者同时竞争时的尾延迟。

### 10.5 Binder 线程数不是第一修复手段

增加工作线程可能缓解排队，也可能让更多线程同时争同一把全局锁，增加内存和调度压力。先找最长事务、持锁 I/O、嵌套同步 IPC 和锁依赖链，再判断容量是否不足。
...
| Android 9（API 28） | bionic 已具备 `PTHREAD_PRIO_INHERIT` 互斥锁属性接口 | 只说明可用；具体锁是否启用仍看初始化 |
...
Binder 默认线程配置在历史上容易被误传。本章只对当前 Android 17 锚点下结论：`DEFAULT_MAX_BINDER_THREADS` 为 15，`startThreadPool()` 另行启动主池化线程；这个数值并非进程的固定线程总数。
...
不成立。它们的功能、运行时路径和竞争行为不同。没有负载和跟踪记录，单凭类型无法判断。

### “看到 `futex_wait` 就找 Java 监视器持锁者”
...
大多数时候证据还不够。服务端工作线程可能卡在业务锁、I/O、CPU 调度或嵌套 IPC。先把客户端与服务端两端时间线连起来。

### “无锁一定更快”

不成立。CAS 重试、缓存行抖动、GC 和算法复杂度都可能成为新成本。DeliQueue 的价值来自针对具体队列拓扑的重构，不能只归结为“无锁”两个字。

## 结论

锁竞争分析要还原完整的等待链。Java 监视器用 `android_monitor_contention` 找等待者与持锁者；原生互斥锁从 futex 候选回到原生栈和初始化代码；Binder 用事务/回复连起客户端与服务端；MessageQueue 还要区分正常原生轮询与旧监视器竞争。

找到持锁者后继续问：它在 CPU 上运行吗，还是处于可运行状态却没被调度？它是否阻塞在另一把锁、I/O 或 Binder 上？追到无法推进的节点后，缩短临界区、拆锁、调整线程模型、启用 PI 或采用无锁结构才有明确目标。
