...
2. Zygote 预加载了“GPU driver”，每个应用使用的驱动就已经确定。
...
这是系统级摊销，并非没有代价。评估时要同时测 boot 与应用启动，不能只截取一张应用跟踪。
...
Zygote 是所有普通应用的父进程。把一个低覆盖率或依赖应用身份的 HAL 放进这里，会增加 boot、常驻映射和派生安全风险。因此，不能因为某个厂商 HAL 首次加载慢，就推导出“应该放进 Zygote 预加载”。
...
这段分支只触发所选图形后端的早期 loader 路径；它没有创建应用的上下文、表面或交换链。
...
`vkEnumerateInstanceVersion()` 查询 loader 支持的实例 API 版本，不需要 `VkInstance`。这条路径同样没有：
...
`Properties::initializeGlAlways()` 读取 `debug.hwui.initialize_gl_always`，默认值来自 HWUI flag。当 HWUI 使用 SkiaVulkan，但设备上仍有大量应用直接使用 GLES 时，这个分支会额外调用一次 `eglGetDisplay()`。
...
Skia、GL 或 Vulkan 首次创建 shader/pipeline、加载持久化缓存、上传纹理和字体图集，都是 per-App 或 per-driver-cache 成本。Zygote 没有应用资源，无法提前完成。
...
| RenderThread | `earlyPreloadGlContext`、EGL/Vulkan/Skia 初始化 | context 是否在首帧前完成，是否仍有驱动冷路径 |
...
下面的属性和全局设置共同描述 Zygote 预加载开关、HWUI 渲染管线以及可更新 driver/ANGLE 选择：
...
3. 等待 boot completed 后再启动测试应用。
4. 保持 ANGLE、updatable driver、HWUI 渲染器和 debug layer 配置一致。
5. 分别记录系统启动期间的 Zygote 预加载、应用绑定、RenderThread Zygote preload、App bind、RenderThread init 和首帧。
6. 报告中位数与高分位，不根据单次跟踪下结论。
7. 同时观察 PSS、共享页和 Zygote 常驻成本，避免只换来时间而忽略内存。

如果关闭预加载后系统启动变快、App 首帧却变慢，这属于成本转移；如果应用启动没有变化，可能有以下原因：
...
| Android 17（API 37） | `requireMapper4()` 可在设备 API ≥ 36 时跳过旧版 Gralloc 2/3；ANGLE 增加面向 Android 17 设备的 manifest opt-in 与设备门槛 | 当前结论锚定 `android-17.0.0_r1` |
