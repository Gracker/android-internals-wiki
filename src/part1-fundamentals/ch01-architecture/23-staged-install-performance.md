...
Staged Install 经常被概括成“更原子的 APK 安装”，这句话容易把两个不同问题混在一起。

Android 的普通安装本来就有事务边界。Android 17 的 `InstallPackageHelper.installPackagesTraced()` 把安装组织为 Prepare、Scan、Reconcile 和 Commit；前几个阶段完成检查，Commit 才修改 Package Manager 的系统状态。一个 multi-package session 还可以把多个 child session 作为一组提交。

Staged Install 解决的是另一件事：某批更新不能在当前运行中的系统里立即激活，需要先完整验证，再跨过一次重启，让 APEX 与 APK 在新一次启动中以一致状态生效。它的关键价值是**跨重启的激活协议**，不是给普通 APK 安装补上最基本的事务能力。
...
这三层可以叠加。一个 staged parent 可以包含多个 child，其中既有 APEX，也有 APK。此时既要满足 multi-package 的同组提交规则，又要满足 staged 的跨重启状态机。

普通安装的“原子”也有边界。`installPackagesTraced()` 要求可预见错误在 Commit 前被发现，并把系统状态修改集中到 Commit；但 `commitReconciledScanResultLocked()` 的源码注释同时警告，Commit 中途抛异常仍可能留下不一致状态。这是 Package Manager 的逻辑事务划分，不是数据库式或掉电安全的完整回滚。Staged Install 增加了持久化状态、apexd 协调和文件系统 checkpoint，覆盖的是需要跨重启激活的更新。
...
调用 `setStaged()` 后，这个 session 被安排到下一次重启安装。若它是 multi-package parent，所有 child 都必须采用一致的 staged 属性；rollback 属性也有同样的约束。任一 child 在激活时失败，整组不能按“其他 child 已经成功”处理。

这里还有一个容易混淆的边界：APEX 可以声明支持 rebootless update，但 rebootless APEX 不属于 staged session。命令行使用 `--force-non-staged` 走的是另一条路径。凡是明确设置为 staged 的 session，其安装语义就是等待重启。

## Android 17 的完整状态机

### 提交不是直接交给 StagingManager
...
`StagingManager.commitSession()` 接到的是已经走完当前验证流程的 session。它负责把 session 纳入重启恢复管理；它不是 `commit()` 后第一站，也不负责替代前面的解析、Developer Verification 或 `PackageSessionVerifier`。
...
三者会以 `isReady`、`isApplied`、`isFailed` 属性写入 `/data/system/install_sessions.xml`。较大的 session 数据存放在 `/data/system/install_sessions/`。`PackageInstallerService` 重启后从 XML 恢复状态，而不是依赖调用方重新提交。

对 staged 或 APEX session，stage 目录由 `buildSessionDir()` 放在数据分区的 staging 目录，内部存储通常表现为：
...
### 为什么先写 ready，再通知 apexd

`PackageSessionVerifier.endVerification()` 对包含 APEX 的 staged session 有一个很关键的顺序：

1. `session.setSessionReady()`，让 Package Installer 先持久化 ready。
2. `mApexManager.markStagedSessionReady(sessionId)`，再告诉 apexd 可以激活。

如果设备在第 1 步后、第 2 步前重启，apexd 没收到 ready，不会激活这批 APEX；重启恢复阶段可以把 session 判为失败。反过来，如果先通知 apexd、后写 Package Installer 状态，窗口期重启就可能出现 APEX 已激活、APK 侧却不知道该继续哪一个 session 的不一致状态。

这段顺序是理解 staged 原子性的最好源码证据：可靠性来自持久化状态与 apexd 状态之间经过设计的提交次序，而不是一句笼统的“重启后 promote”。
...
## 重启后：APEX 已先于 system_server 激活

设备启动时，apexd 会在 system_server 恢复 Package Installer session 之前验证并挂载 APEX。随后 `PackageInstallerService.restoreAndApplyStagedSessionIfNeeded()` 从持久化数据中找出：

- staged；
- 已 commit；
- 尚未 applied 或 failed；
- 没有 parent，或者是可恢复的顶层 parent。
...
开机早期
  ...
  └─ PackageInstallerService 恢复 session XML
       └─ StagingManager.restoreSessions(...)
            ...
            ├─ 清理 destroyed 与 dangling APEX session
            ├─ 对照 apexd session 状态
            └─ resumeSession(...)
                 ├─ 检查 APK-in-APEX 与重复包
                 ├─ 处理 rollback 用户数据快照
...
`restoreSessions()` 在发现 build fingerprint 与提交时记录不一致时，会让待恢复 session 失败。原因很直接：系统镜像已经改变，重启前完成的验证不能继续被当作当前系统上的有效结论。

非 ready session 也不会被当作 ready 强行安装。Android 17 会把它移出本轮立即恢复集合，并在 `BOOT_COMPLETED` 之后重新触发 verification。这通常意味着“这次开机没有应用”，而不是“绕过检查继续装”。

## checkpoint 不是 Virtual A/B 的同义词
...
包含 APEX 且设备支持 checkpoint 时，`resumeSession()` 不会过早把 apexd session 宣告为永久成功。session ID 会保留到 `PHASE_BOOT_COMPLETED`，届时 `markStagedSessionsAsSuccessful()` 才通知 apexd。本次启动能否走到 boot completed，本身就是健康检查的一部分。
...
APK-only session 在重启后的安装失败，通常走 `setSessionFailed()` 并清理自己的 stage。包含 APEX 的失败更严重：系统可能调用 `revertActiveSessions()`，再通过 checkpoint 回退；回退本身失败时还可能触发重启，以免继续运行在无法确认一致性的系统状态中。
...
旧实现常被描述为：PMS 调 installd，installd 从 `run_dex2oat.cpp` 读取 `restore-dex2oat-threads`，再直接 fork dex2oat。把这套描述用于 Android 17 staged APK，会得到错误的性能模型。

Android 17 的应用安装 dexopt 已由 ART Service 负责。staged APK 在重启后进入普通 APK 安装事务，`InstallPackageHelper` 准备 dexopt 请求，`DexOptHelper.performDexoptIfNeededAsync()` 再调用 `ArtManagerLocal.dexoptPackage()`。ART Service 通过 artd 管理编译请求和产物；底层仍可能运行 dex2oat，但调度入口不再是旧文章里的 PMS → installd dexopt 主线。
...
另一个重要边界是：APEX 本身不会作为普通 app 交给这条 dexopt 路径。mixed session 中需要 app dexopt 的是 APK child。

安装时 dexopt 也是 best effort。Android 17 的 `DexOptHelper` 明确避免仅因 dexopt 步骤失败就让应用安装失败。因此：

- “session failed”不能自动归因于 dex2oat；先看 Package Installer 保存的错误码和错误信息。
- dexopt 失败可能表现为安装后首次运行需要解释执行或等待后续编译，而不是 staged session 回退。
- `INSTALL_FAILED_DEXOPT`、进程被杀或空间不足等日志必须和当前代码路径核对，不能拿旧版行为直接套用。

## 性能测量：按三个时间窗口拆开

Staged Install 中间跨过一次 reboot。`commit()` 与 `isApplied()` 不在同一个连续时钟域，也不可能只靠一份短 Perfetto trace 得到可信的端到端耗时。应分成三个窗口测量。
...
- APEX 提交和验证；
- checkpoint 建立与 ready 持久化。

这部分结束条件是 `SessionInfo.isStagedSessionReady()`，不是“`commit()` 返回”。验证中含异步步骤，调用方必须通过回调或查询 session 状态等待 ready/failed。
...
- 文件系统 checkpoint 状态；
- system_server 启动到 Package Installer 开始恢复 session 的时间。

这一段通常需要 boot trace，并同时保留 apexd 日志。只采 system_server 启动后的 atrace，会漏掉 APEX 已经完成的 early-boot 工作。
...
不要预设 `verifyPackage`、`collectCertificates`、`commitPackageSettings`、`relabel` 一定是稳定存在的 slice 名。不同版本和厂商分支可能没有这些名称，先在目标 trace 中确认再写 SQL。
...
不要把“commit 命令已返回”当成 ready。命令行测试可使用 `--staged-ready-timeout` 等待 ready/failed；超时只说明等待窗口结束，不自动证明系统死锁。
...
- `restoreSessions` 与 `installApksInSession` slice 是否出现。
...
- `dexopt` 墙钟时间与 CPU 时间都长：看 compiler filter、DEX 规模、profile 和核心占用。
- `dexopt` 墙钟长但 CPU 少：看调度、I/O、thermal 或内存压力。
...
目录读取受 build 类型和 SELinux 权限限制。看不到 `/data/app-staging` 不等于目录不存在，也不能因此排除空间问题。mixed/APEX session 还要结合 apexd 自己的日志和 `dumpsys apexservice`，不要把 `/data/apex/active` 当成 Package Installer 写 APK 的 staging 目录。
...
`pm install-create` 之后还要用 `pm install-write` 写入 APK，并在 multi-package 场景把 child 加入 parent。具体脚本应检查每条命令的返回值和 session ID，不要靠 `grep Success` 推测 parent/child 已正确关联。
...
| Android 17 / API 37 | 本文当前锚点：Developer Verification 参与适用的 session 验证；StagingManager 与 ART Service 行为按 `android-17.0.0_r1` 核对 |

回看任何旧版本问题时，都要同时锁定 framework、ART 和 apexd 的版本。只把 framework 文件换成 Android 17、却继续引用旧 installd dexopt 或旧 apexd 状态机，会得到表面完整、实际跨版本拼接的结论。
