...
当 heapprofd 或 malloc debug 生效时，中间会多一层拦截。性能分析必须确认当前 dispatch，再判断耗时来自采样、回溯、Scudo 元数据操作、锁竞争、缺页，还是内核映射与回收。
...
Scudo 是面向堆漏洞的强化分配器。它能检测部分损坏的 chunk header、double free、无效状态、未对齐指针和分配/释放类型不匹配。应用没有触发检查的越界或 use-after-free，官方也把 Scudo 定义为 mitigation，不属于 ASan/HWASan 那类完整错误检测器。出现 `Scudo ERROR:` 时，应把短错误摘要作为排查入口，再结合 tombstone、HWASan、MTE 或可复现测试定位第一次非法访问。
...
设置自有栈时，还必须满足运行时页大小对齐。递归、较大的栈上数组、复杂 unwind、信号处理和 Sanitizer 都可能让“空载测试可用”的小栈在压力场景溢出。调优应使用目标构建测量高水位，再预留 guard 与故障处理余量。
...
“所有大于 64 字节的拷贝都走 NEON”不符合源码。具体汇编内部可能使用 pair load/store、SIMD、prefetch 或 non-temporal store，但阈值和收益属于被选实现及 CPU 微架构，不能从 Bionic API 层统一推导。
...
2. **分开统计分配次数、allocated bytes、RSS。** Scudo cache、匿名页、文件页和内核回收会让三个指标变化不同步。
