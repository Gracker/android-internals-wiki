...
NNAPI 从 Android 8.1 开始提供 C API，让上层推理框架把计算图交给 CPU、GPU、DSP 或专用加速器。它包含模型、编译、内存和执行等抽象，也支持同步/异步执行与编译缓存。

Android 15（API 35）起，NNAPI NDK API 已弃用。官方迁移说明还提醒：未来多数设备可能使用 CPU 后端，性能敏感工作负载应迁移到其他方案，例如 LiteRT GPU 运行时。因此，新项目不应再把 `ANeuralNetworks*` API 当作 Android 17 的推荐应用接口。
...
- **设备与功能可用性**：不同 API、设备和 Gemini Nano 版本的支持范围不同；调用前使用对应 API 的功能状态检查。
...
不能仅按“模型小于 100 MB”或“模型大于 1 GB”决定本地还是云端。参数量、上下文长度、KV 缓存、数据类型、峰值临时张量、内存带宽和散热条件都会改变可运行性。最终选择要以目标设备上的测量结果为准。
...
回调从主线程进入，耗时 I/O、数据库和模型推理必须切到工作线程，并响应 `CancellationSignal`，最终通过回调返回成功或错误，否则 agent 调用会直接造成应用主线程卡顿。

Android 17（API 37）还增加了运行时注册、Activity/全局作用域、函数状态观察等能力。运行时注册只在相应进程和 `Context` 生命周期内有效，调用者必须保存 `AppFunctionRegistration` 并在不再需要时 `unregister()`。
...
1. **能力检查与模型准备**：功能状态、模型下载或首次解压。
2. **运行时初始化**：创建会话或 `CompiledModel`、选择 delegate、编译计算图。
3. **输入处理**：图像解码与缩放、音频分帧、分词和缓冲区填充。
4. **推理**：首个结果延迟、单次推理延迟，或生成模型的首 token 与持续吞吐。
5. **输出处理**：解码、过滤、结构化、持久化与 UI 更新。
6. **稳态影响**：PSS/RSS、原生堆、GPU/NPU 缓冲区、电量、温度和降频后的尾延迟。
...
对编译、推理和后处理分别添加 `section`，再与线程调度、Binder 调用、内存和热状态轨道对齐。Perfetto 可以证明 CPU 线程是否被抢占、主线程是否等待或 AICore Binder 调用持续多久；能否看到 NPU 内部执行，要看设备厂商是否提供相应数据源。缺少 NPU 轨道时，不要仅凭 CPU 空闲推断推理一定在 NPU 上。
...
复用 tokenizer、会话、`CompiledModel` 与输入输出缓冲区通常能减少抖动。磁盘上的编译缓存还必须绑定模型内容、运行时、delegate 和设备；版本不匹配时应安全失效，不能把旧二进制产物长期复用。
...
普通应用不能可靠地指定 NPU 频率，也不应假设可以通过某个公开 API 固定 DVFS。频率和热节流由内核、Power HAL、热管理服务与芯片固件共同决定。

应用可做的是控制工作量：

- 通过 `PowerManager` 热状态监听器观察热状态，降低采样率、输出长度或任务并发。
...
| 设备覆盖 | 受设备、功能和模型版本限制 | 由 minSdk、模型和后端决定 | 主要受网络与服务可用性限制 | Android 16+，仍处实验/预览阶段 |
