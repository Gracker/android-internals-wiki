...
对编译、推理和后处理分别添加 `section`，再与线程调度、Binder 调用、内存和 thermal 轨道对齐。Perfetto 可以证明 CPU 线程是否被抢占、主线程是否等待或 AICore Binder 调用持续多久；能否看到 NPU 内部执行，要看设备厂商是否提供相应数据源。缺少 NPU 轨道时，不要仅凭 CPU 空闲推断推理一定在 NPU 上。
