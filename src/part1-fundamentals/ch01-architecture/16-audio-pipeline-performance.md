...
它们是兼容性能力声明，不是对当前蓝牙耳机、当前音效配置或当前系统负载的实时测量。专业音频应用仍应在目标设备和目标路由上做回环测试。
...
AudioFlinger 负责创建轨道、维护共享缓冲区状态、混音或转交数据、与 HAL 交互，并记录欠载等状态。AudioPolicyService 做出的选路决定与 AudioFlinger 的轨道接纳条件共同决定最终路径。
...
API 37 的 Java `AudioTrack` 增加按帧位置丢弃已写数据的能力，只允许用于：
