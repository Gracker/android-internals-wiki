...
Android 没有一个公共 API 能返回任意路由的完整端到端延迟。可用的 feature 只表达设备声明的能力：
...
低延迟和低功耗是两套目标。MMAP / Fast 为交互式音频缩短排队；offload 允许硬件队列承接更多数据，让 CPU 与 framework data pipe 更长时间休眠。不能因为两者都“绕过了部分普通 mixer 工作”就把它们视为同一路径。

## 4. FastMixer：请求是 hint，接纳才是结果
...
它省去的主要是每条 fast track 的重采样、per-track effects 和 per-mix effects，而不是“完全不混音”。Normal Mixer 先把普通 tracks 混成 submix，再通过 index 0 送给 FastMixer；FastMixer 再把它与 client fast tracks 合并后写向 HAL。
...
现稿常见的另一种误写是“frame count 不匹配就一定拒绝 FAST”。Android 17 对流式 fast track 会把 frame count 至少抬到 `mFrameCount * fast_track_multiplier`；它会改变实际缓冲配置，但不是上述第一层硬拒绝条件。打开 track 后应读取实际配置，不要假定 builder 请求原样生效。

### 4.2 为什么默认最多看到 7 条 client fast tracks
...
FastMixer 使用提升后的 `SCHED_FIFO` 优先级，目标是减少唤醒抖动。它仍然会在 HAL `write()` 处等待，并不等于“永远不阻塞”。
...
不能把 Oboe 简化成一个固定的“MMAP EXCLUSIVE → MMAP SHARED → FAST → Normal”决策表。最终路径仍受 API level、builder 参数、设备 profile、endpoint 占用和厂商实现影响。
...
低延迟输出通常让 buffer size 保持为 burst 的整数倍。太大增加排队，太小则在一次调度延迟后立刻 underrun。常用调优方法是：
...
### 6.2 callback 线程不能做可能阻塞的事

AAudio / Oboe 的 data callback 运行在高优先级线程上。callback 内避免：

- `malloc` / `new` 和不可控的对象构造。
- 文件、网络或 Binder I/O。
- mutex、condition variable、sleep。
- 停止、关闭当前 stream。
- 在触发 callback 的同一 stream 上再次 `read()` / `write()`。
- 日志洪泛和复杂 trace 字符串拼接。
...
对 targetSdk 37+ 的后台 App 还要满足更严格条件：
...
这不是“AudioFlinger 被系统调度器降频”的性能问题。更常见的证据是 App 供数停止、write 返回错误、callback 不再推进或 focus 请求失败。
...
`throw` 模式下，volume/focus 交互可抛 `IllegalStateException`；显式播放 write 会持续返回错误，某些无显式 write 的播放模式可能直接让 App 崩溃。测试模式比默认发布行为更严格，报告里必须写明使用了哪个开关。
...
- 可在短时间写入数秒数据到 hardware buffer。
- framework data pipe 随后可暂停，CPU 获得更长 sleep 时间。

它服务长音频省电，不服务交互式低延迟，也不能与 `LOW_LATENCY` 同时成立。成功打开后仍应读取实际 performance mode，并通过 dumpsys 确认 output 类型；“offloaded”不自动证明具体 DSP 型号或硬件解码实现。
...
`getCodecProvenance()` 返回的是配置时保存的这个 media type；未设置时返回空字符串。它不返回 Codec2 component 名、厂商模块名，也不能证明软件解码、硬件解码或 offload 路径。
...
- build fingerprint、Android 版本、是否 userdebug。
...
设备支持的 atrace category 会随版本和产品变化，可先运行 `adb shell atrace --list_categories`。不要假定所有 vendor audio slice 都有统一名称。
...
5. 路由、standby、stream start/stop 是否刚好与缺口重合。
6. CPU 频率、thermal throttling 或更高优先级 RT 线程是否改变调度窗口。

App 代码可用 `android.os.Trace` 或 `<android/trace.h>` 标记生产、callback 与消费区间，但一个 trace event 本身有开销；不要在每个 sample 或极细循环里插桩。
...
如果 counter 增长，同时 callback 在 deadline 前长期 runnable 未执行，优先查调度；callback 已按时完成但 counter 仍增长，再查 buffer、HAL write、route 和设备侧。
...
| 现象 | 先查 | 进一步验证 |
| --- | --- | --- |
| 点击后很久才发声 | warmup、queued frames、实际 output path | 首次与稳态分开测；查 standby/start 与 buffer size |
| 持续延迟大但无杂音 | Normal/deep-buffer/offload、重采样、effects | dumpsys 核对 flags/rate/output thread |
| 间歇爆音或断续 | xrun counter、callback deadline | Perfetto 看 runnable delay、锁、I/O、频率 |
| 同时录放越久越不稳 | input/output timestamp、FIFO 水位 | 检查 clock drift 与异步重采样 |
| LOW_LATENCY 请求无效 | 打开后的 mode、rate、sharing、FAST flag | 查 slot、effect chain、profile、MMAP fallback |
...
3. 用高速摄像、外部采集或声学 loopback 测端到端延迟。
...
不成立。AAudio 可以回退到 legacy AudioFlinger path。API 36+ 用 `AAudioStream_isMMapUsed()` 核对。

### “请求 LOW_LATENCY 就一定是 fast track”

不成立。它是 hint；sample rate、channel、FastMixer、slot 和 effects 都会影响接纳。
...
### “`getCodecProvenance()` 能看硬解还是软解”

不成立。它返回配置的 codec MIME media type，不是 codec component 或执行路径探针。
