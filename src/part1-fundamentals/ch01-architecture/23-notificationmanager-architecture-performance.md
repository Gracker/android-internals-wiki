...
# Android 17 NotificationManager 架构与性能优化
...
共享 bitmap backing（多个对象共用的像素存储）只能减少部分重复拷贝，不能消除解码、缩放、Parcel 元数据、SystemUI 图片解析和 GPU 上传。大图通知的成本需要在应用、NMS、SystemUI 和渲染四段分别观察，详见 §9.5。
...
- **§1.10 Binder 线程池与优先级继承**：解释同步 `notify()` 中的 Binder 调度和等待。
- **§1.12 ActivityManagerService**：NMS 使用的 user、进程状态、FGS 与 PendingIntent 能力。
