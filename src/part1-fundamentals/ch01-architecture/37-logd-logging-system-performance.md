...
文本日志进入 `__android_log_buf_write()` 后会再次执行 `__android_log_is_loggable()`。`Log.d()` 的参数在进入原生方法前已经求值，所以原生层的级别过滤能省去套接字写入，却省不掉调用方已经完成的字符串拼接、对象 `toString()` 或 JSON 序列化。

Android 17 的 liblog 使用 `writev()` 发送头部和有效载荷。旧版内核日志驱动的 `/dev/log_main` 等节点只适合解释早期版本，不能用来描述当前路径。

`/dev/socket/logdw` 是数据报套接字。logd 为它启用凭据传递，`LogListener` 从 `SCM_CREDENTIALS` 取得发送方 PID、UID、GID；客户端不能靠伪造有效载荷冒充另一个进程。SELinux 和文件、套接字权限约束谁能连接与写入，并不表示每条日志都要在 liblog 中执行一次 SELinux 判定。
...
`LogdWrite()` 为普通日志使用 `SOCK_NONBLOCK`。这里的普通日志包括 `main`、`system`、`radio`、`events`、`stats`、`crash` 等合法缓冲区；`security` 是受权限控制的特殊二进制缓冲区，liblog 为它另设阻塞套接字。

当 logd 来不及接收、套接字返回 `EAGAIN` 时，liblog 记录一次丢弃并返回。后续写入恢复且 liblog 内部日志允许输出时，它会尝试写入事件标签 `1006`（`liblog`），报告此前丢弃的数量。这与旧版 `chatty` 行属于不同机制。

因此，普通应用日志风暴的风险主要是：

- 调用方持续做格式化、JNI 转换和系统调用，消耗 CPU 与电量；
- 套接字过载后新日志丢失，关键现场被噪声覆盖；
- logd 忙于接收、压缩、裁剪和服务读者，增加系统负载；
- 日志对象和中间字符串增加分配压力。

把普通日志描述成“套接字满后 `write()` 阻塞主线程并导致 ANR”不符合 Android 17 的 liblog 实现。若跟踪数据显示线程长期停在日志相关调用，应继续检查自建日志框架的锁、磁盘输出端、格式化、崩溃收集器或厂商改动，不能直接归因于 AOSP `logdw` 反压。

也不要对 `EAGAIN` 做无间隔重试。这样会把允许丢日志的保护机制变成 CPU 自旋。

### 1.3 有效载荷上限与长消息

Android 17 的 `LOGGER_ENTRY_MAX_PAYLOAD` 为 4068 字节。`LogdWrite()` 会把超出上限的直接有效载荷截到可发送范围。Java `Log.printlns()` 处理长文本和堆栈时，会按有效载荷预算和换行位置拆成多条，再逐条调用原生写入。
...
logd 从 init 继承 `logdw` 套接字。Android 17 的 `LogListener` 有两种接收方式：
...
输出会列出各缓冲区的环形缓冲区容量、已消费量、可读量以及单条上限。`logcat -G` 可以按 `-b` 选择修改运行时大小，但通常需要相应权限；调大容量只能延长保留窗口，不能消除写入成本或套接字丢包。

### 2.3 默认实现使用序列化数据块

Android 17 的 `logd.buffer_type` 默认值是 `serialized`，另一个可选值为 `simple`。默认实现不是定长的 `LogBufferEntry[]` 数组。

`SerializedLogBuffer` 为每个日志 ID 维护 `SerializedLogChunk` 列表。当前数据块以可追加形式保存；完成的数据块使用 Zstd 1 级压缩。总计费大小超过目标容量时，logd 从较老的数据块开始裁剪。读取客户端需要访问压缩数据块时再解压，并通过引用状态避免正在读取的数据被直接释放。

这套设计的容量语义仍接近“保留有限窗口、淘汰旧数据”，内部单位则是数据块和序列化条目。分析裁剪成本、内存占用或慢速读取客户端行为时，需要以该实现为准。

### 2.4 `chatty` 属于历史机制

`system/logging/logd/README.compression.md` 的标题直接说明了 Android S 的变化：使用日志压缩取代 Chatty。Android 17 的 logd 源码没有当前默认路径所需的 `ChattyLogBuffer.cpp`，默认的序列化缓冲区也不会按旧说明插入 `uid=... expired ... lines` 来表示重复日志。

旧设备抓取中仍可能看到 `chatty`，AOSP 的事件标签表也保留了历史名称，但这不足以证明 Android 17 默认使用旧的重复行合并策略。当前版本有两个相关但不同的现象：

- 缓冲区超限：logd 裁剪较老的数据块；
- 写入套接字过载：liblog 遇到 `EAGAIN` 丢弃新写入，并可能用事件标签 `liblog` 报告数量。

## 3. logcat 的读取与过滤边界

### 3.1 读取套接字与每个客户端的线程

logcat 连接 `/dev/socket/logdr`。该读取套接字使用 `SOCK_SEQPACKET`，logd 接受连接和读取命令后，为客户端创建独立的 `LogReaderThread`。服务端命令支持的主要筛选条件包括：
...
安全缓冲区和跨 UID 读取还受凭据与权限约束。面向应用公开的 NDK 日志 API 通常写入 `main`；读取全局 logcat 所需的 `READ_LOGS` 也只授予受信任的特权组件。

### 3.2 标签与正则表达式在 logcat 客户端执行

`*:S MyApp:V` 一类标签/优先级规则由 logcat 进程的 `android_log_shouldPrintLine()` 判断。`--regex` 也由 logcat 的 `std::regex_search()` 执行。它们不会作为标签表或正则表达式交给 logd。

这个边界会直接影响性能判断：

- `--pid` 能下推给 logd，减少服务端发送的数据；
- `-b` 能缩小服务端读取的缓冲区集合；
- 标签/优先级规则和 `--regex` 主要减少终端输出，不一定减少 logd 到 logcat 的传输；
...
这里 `-b main` 和 `--pid` 控制服务端数据范围，后面的过滤规则决定 logcat 打印哪些标签。命令替换应在主机 shell 中执行；多进程应用还要确认 `pidof -s` 选中的进程是否符合排查目标。

### 3.3 慢速读取客户端与多个读取客户端

每个读取客户端都有独立线程和读取状态。多开 logcat 会增加线程、解压和套接字发送工作，但 AOSP 没有给出“每个客户端固定占用多少 KB”或“超过多少个必然退化”的通用阈值。

当读取客户端落后于裁剪位置时，序列化缓冲区会让它跳过已被回收的旧数据块，并记录相应警告。读取套接字也配置了发送超时，避免一个停滞客户端无限拖住发送。是否达到瓶颈取决于日志速率、缓冲区容量、读取客户端数量、输出介质和设备性能，需要在目标设备上测量。

## 4. 容易混淆的两条旁路

### 4.1 EventLog 与 StatsD 原子事件

`LOG_ID_EVENTS` 保存 `EventLog` 风格的二进制事件，其标签可由事件标签表解析。现代 StatsD 原子事件的主传输路径不同：Android R 及以后，`packages/modules/StatsD/lib/libstatssocket` 通过独立的非阻塞 Unix 域数据报套接字 `/dev/socket/statsdw` 向 statsd 写入。

因此，高频 StatsD 原子事件不能描述成“先写满 logd 的 `events` 缓冲区”。StatsD 有自己的套接字、丢失报告和限流语义；logd 仍定义 `LOG_ID_STATS`，也不能由名称推导出所有原子事件都经 logd 保存。分析原子事件丢失应查看 `libstatssocket` 与 statsd 的指标；分析 EventLog 保留窗口才需要查看 `events` 缓冲区。
...
`system/logging/rust/` 包含 Rust 日志 API、结构化日志和 liblog 绑定等客户端能力。它的存在不代表 logd 守护进程已经改写为 Rust。
...
这项条件判断可以跳过字符串插值和 `encodeLargePayload()`。如果只在调用外再包一层普通函数，但仍传入已构造的 `String`，就无法取得同样效果。自建日志组件若支持 lambda 延迟求值，应确认 lambda 只在级别允许时执行。

### 5.2 用 R8 按级别删除发布版日志

当前 R8 文档提供了专门的 `-maximumremovedandroidloglevel`。例如，发布版构建可以删除 verbose、debug 和 info 日志：
...
R8 会处理匹配的 `Log.*` 与 `Log.isLoggable()` 调用。这个规则近年才进入正式文档，接入前要确认项目实际使用的 R8/AGP 版本支持它。规则应只进入发布版变体，并通过反编译、R8 映射/使用情况输出和自动化测试确认结果。若只想作用于特定包或方法，可追加类规范（class specification）。
...
`-S` 输出 logd 统计，可结合 `--pid` 查看特定进程；具体字段和可见范围受构建与权限影响。不要从 `/proc/<pid>/fdinfo` 推断 logd 的逻辑缓冲区水位，它反映不了序列化数据块的占用与裁剪状态。
...
AOSP 自带 `system/logging/liblog/tests/liblog_benchmark.cpp`，其中有轻载写入和高压写入基准。结果取决于 SoC、内核、构建类型、缓冲区配置和 logd 负载，不能据此给出跨设备固定的“单条若干微秒”。比较优化前后的结果时，应固定消息长度、日志级别、CPU 状态和读取客户端配置，并同时记录丢失量；只统计成功循环次数会把丢弃误判为吞吐提升。

### 6.3 一个实用的判断顺序

- 跟踪数据中调用方在编码或字符串构造上耗时：减少参数工作；
- 应用 `writev()` 很密集且出现 `EAGAIN`：降低写入率，保留错误码和聚合结果；
- logd CPU 高：检查全机日志源、缓冲区裁剪与读取客户端数量；
...
| Android S | 序列化压缩取代旧 Chatty 方案，详见 `README.compression.md` |
| Android R 起 | StatsD 的原生侧原子事件经独立 statsd 套接字传输，避免与 EventLog 路径混写 |
| Android 17 / API 37 | logd 核心仍为 C++；默认使用序列化/Zstd 缓冲区；接收端具备受标志控制的 io_uring 路径；Rust 日志代码属于客户端能力 |
...
- 缓冲区：`LogSize.cpp`、`SerializedLogBuffer.cpp`、`SerializedLogChunk.cpp`、`CompressionEngine.cpp`
- 读取端：`LogReader.cpp`、`LogReaderThread.cpp`
...
这些锚点均按 `android-17.0.0_r1` 核查。产品分支若修改了 liblog 套接字标志、logd 缓冲区类型或权限策略，应以设备对应源码和运行时命令输出为准。
