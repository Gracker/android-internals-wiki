...
`/dev/socket/logdw` 是 datagram socket。logd 为它启用凭据传递，`LogListener` 从 `SCM_CREDENTIALS` 取得发送方 PID、UID、GID；客户端不能靠伪造 payload 冒充另一个进程。SELinux 和文件/socket 权限约束谁能连接与写入，并不表示每条日志都要在 liblog 中执行一次 SELinux 判定。
...
当 reader 落后于裁剪位置时，serialized buffer 会让它跳过已被回收的旧 chunk，并记录相应警告。读 socket 也配置了发送超时，避免一个停滞客户端无限拖住发送。是否达到瓶颈取决于日志速率、buffer、读取客户端数量、输出介质和设备性能，需要在目标设备上测量。
...
这项条件判断可以跳过字符串插值和 `encodeLargePayload()`。如果只在调用外再包一层普通函数，但仍传入已构造的 `String`，就无法取得同样效果。自建 logger 若支持 lambda 延迟求值，应确认 lambda 只在级别允许时执行。
