...
- 产品政策：开发者验证官方文档在 2026-07-25 的公开口径。
...
| 层次 | 解决的问题 | 载体 | 版本边界 |
|---|---|---|---|
| Google 的开发者验证政策 | 哪些设备、区域、渠道和用户路径需要验证 | Android Developer Verifier、Google Play 服务、开发者管理中心 | 官方称覆盖 Android 7 及以上的认证 Android 设备；分阶段启用 |
| Android 平台机制 | 安装会话如何请求验证器、等待结果、触发用户动作并回传错误 | `PackageInstallerSession`、`DeveloperVerifierController`、`PackageInstaller` API | 本章锚定 `android-17.0.0_r1`；公开结果 API 自 36.1 提供 |
| APK 与安装信任链 | APK 是否可解析、签名是否有效、升级证书是否兼容、权限与设备策略是否允许 | APK 签名方案、PMS、PackageInstaller、DPM、软件包验证器 | 由各自的平台版本和策略决定 |

Android 17 AOSP 提供验证器接入框架，却没有把 Google 的身份数据库和判定逻辑开源在 `frameworks/base` 中。实际验证器是系统指定的服务提供者；在 Google 认证设备上，官方把 Android Developer Verifier 描述为一项新的 Google 系统服务，并说明 Android 7 及以上设备通过 Google Play 服务接收相关更新。

AOSP 中存在这些类，也不表示任意 AOSP 构建都会自动执行 Google 的政策。Android 17 的 `PackageInstallerService` 默认策略是 `DEVELOPER_VERIFICATION_POLICY_NONE`；设备没有配置验证器，或功能开关没有启用时，安装会话会跳过这一步。

`android.content.pm.verify.developer` 下的 `DeveloperVerifierService`、`DeveloperVerificationSession` 和 `DeveloperVerificationStatus` 都标有 `@SystemApi` 与 `@hide`。它们是平台与受信验证器的协议，普通应用无法通过公开 SDK 直接实现或调用。普通安装器应使用 `PackageInstaller` 公开的附加字段、原因码和扩展接口。
...
首批执行的设备形态也有一层细节：Google Play 分发的应用需要登记所有设备形态；Play 以外的 2026 年首批强制范围当前落在所选四国的手机与平板设备。不要把手机上的实验结果直接外推到 TV、Auto 或 Wear。
...
| 高级流程（Advanced flow） | 2026 年 8 月面向高级用户推出；一次设置后可安装未注册应用 | 用户明确接受风险的旁加载路径，不是商店静默绕过接口 |
| 有限分发（Limited distribution） | 免费、无需政府签发身份证件，最多分享给 20 台经最终用户明确授权的设备 | 适合学习、课堂、家庭和非商业小范围分享 |
| 托管设备 + 组织商店 | 由 IT 管理员审核的组织内应用不要求完成验证 | 同一 APK 离开托管商店或进入非托管设备后，不能继续假设豁免 |

高级流程不是安装器可自行打开的开关。官方常见问题说明的流程包含启用开发者模式、反诱导确认、重启与重新认证、24 小时等待，以及再次使用生物识别或 PIN 确认。ADB 不受这段等待时间影响。
...
1. 开发者验证不能让伪造 APK 获得合法签名。
2. APK Signature Scheme 校验仍会发现签名无效。
3. 若设备上已有正版应用，升级安装还要通过签名谱系/证书兼容性检查。

一个 APK 的 v2/v3/v4 签名有效，只说明 APK 由对应密钥签发且内容未被篡改，不能自动证明开发者已完成身份验证，或包名已注册到对应账号。

发版资产至少要同时维护包名、当前与历史签名证书、验证账号与状态，以及各渠道最终交付 APK 的证书指纹。渠道重签会同时破坏升级兼容性和包名注册证明。

## 4. Android 17 对普通安装器公开了什么

### 4.1 API 36.1 与 API 37

`PackageInstaller` 文档把开发者验证的结果字段标为“36.1 版本加入”。Android 17 / API 37 继续提供这些接口，并把安装器目标 SDK 大于 36 作为新的回调行为边界。

36.1 是次要 SDK 版本，不应只检查 `Build.VERSION.SDK_INT >= 36`。需要区分次要版本时，使用：
...
| 常量 | 值 | 含义 |
|---|---:|---|
| `DEVELOPER_VERIFICATION_FAILED_REASON_UNKNOWN` | 0 | 验证器超时、连接失败，或报告未知原因导致无法完成 |
| `DEVELOPER_VERIFICATION_FAILED_REASON_NETWORK_UNAVAILABLE` | 1 | 验证器明确报告网络不可用 |
| `DEVELOPER_VERIFICATION_FAILED_REASON_DEVELOPER_BLOCKED` | 2 | 验证器已完成判断，但开发者未通过当前策略 |

`UNKNOWN` 不代表“未提供原因”。读取前必须检查附加字段是否存在，不能只使用默认值 0。

### 4.3 结果附加字段

| Extra | 类型 | 用途 |
|---|---|---|
| `EXTRA_DEVELOPER_VERIFICATION_FAILURE_REASON` | `int` | 安装最终因开发者验证中止时解释原因 |
| `EXTRA_DEVELOPER_VERIFICATION_LITE_PERFORMED` | `boolean` | 在失败结果中标记验证器是否只做了轻量验证 |
| `EXTRA_DEVELOPER_VERIFICATION_EXTENSION_RESPONSE` | `PersistableBundle` | 验证器对安装器扩展参数的响应 |
| `Intent.EXTRA_INTENT` | `Intent` | 需要用户动作，或提供可延后展示的系统解释页 |

`SessionParams.setExtensionParams()` 与 `getDeveloperVerificationServiceProvider()` 允许安装器和验证器扩展私有协议。`Bundle` 的结构由验证器实现决定，并非跨设备通用协议。安装器只有确认服务提供者后才能解释它；遥测也不应默认原样上传未知 `Bundle`。

## 5. 一次提交可能产生多次回调

`Session.commit(IntentSender)` 只是把封存后的会话交给系统异步处理。方法成功返回不代表安装已经完成。

开发者验证失败时，`EXTRA_STATUS` 取决于安装器的目标 SDK、权限和失败是否允许用户处理：

| 安装器 | 首个回调 | 后续结果 |
|---|---|---|
| 目标 SDK ≤ 36，只有 `REQUEST_INSTALL_PACKAGES` | 先收到不带原因的 `STATUS_PENDING_USER_ACTION` | 用户允许绕过则继续；否则返回 `STATUS_FAILURE_ABORTED` 和原因 |
| 目标 SDK ≤ 36，持有特权 `INSTALL_PACKAGES` | 通常直接收到 `STATUS_FAILURE_ABORTED` 和原因 | 不依赖普通用户确认流程 |
| 目标 SDK ≥ 37，需要用户输入的非阻断问题 | 先收到不带原因的 `STATUS_PENDING_USER_ACTION` | 用户重试/允许后继续，或最终返回中止状态和原因 |
| 目标 SDK ≥ 37，其余阻断结果 | `STATUS_FAILURE_ABORTED` 和原因 | 可能带 `Intent.EXTRA_INTENT`，供系统解释原因 |
...
Android 17 把新机制直接接入 `PackageInstallerSession`，没有把 Google 的业务判定写进旧的软件包验证器。

### 6.1 创建会话：冻结默认策略并预热验证器

`PackageInstallerService.createSessionInternal()` 从逐用户策略表读取默认值，作为会话的初始策略和当前策略。策略会写入会话 XML，因此进程重启不会改变已创建会话的初始边界。

满足以下条件时，构造 `PackageInstallerSession` 会提前绑定验证器：
...
若 `SessionParams.appPackageName` 已提供包名，控制器还会调用 `onPackageNameAvailable()` 让验证器预取数据。此时尚未开始验证，只是在预热验证器。

### 6.2 提交：封存、流式校验，再进入安装消息

下面的调用序列用于定位 `commit()` 之后、开发者验证之前的会话状态转换：
...
`streamValidateAndCommit()` 会准备 DataLoader、验证 APK/APEX 会话的基本结构并标记为已提交。执行到这里仍未安装成功。

### 6.3 `handleInstall()`：解析 APK 后先做开发者验证
...
单包会话使用一个 `CompletableFuture`。多包会话不验证父会话，而是分别验证每个子会话；任一子会话失败，父会话整体失败。
...
验证器通过 `DeveloperVerificationSession` 回报：
...
完整结果包含 `isVerified`、轻量验证状态、应用元数据状态和可选失败说明。未完成结果当前只区分未知原因与网络不可用。

### 6.5 结果怎样回到原安装链

下面的分支图说明验证器结果怎样完成 future，并决定恢复原安装链还是结束会话：
...
开发者验证位于 APK 已解析、原有会话验证继续运行之前。通过后，包仍可能因为签名、设备策略、存储、共享库、APK 格式或后续安装事务失败。

## 7. 策略决定失败后的处理方式

Android 17 AOSP 定义了四种系统 API 策略：

| 策略 | 开发者被阻止 | 未知 / 连接失败 / 超时 | 网络不可用 |
|---|---|---|---|
| `NONE` | 不阻断 | 不阻断 | 不阻断 |
...
逐用户默认策略由验证器或系统指定的策略委托方设置。验证器还可通过会话的 `setPolicy()` 覆盖当前请求。业务代码不能只凭国家码模拟最终判定，必须以系统回调为准。

## 8. ADB 的绕过边界

官方面向开发者的承诺是 ADB 工作流不变。Android 17 源码把这个产品承诺拆成机制与结果两层：旧的非强制路径可以直接跳过；启用 `verificationServiceAdb` 后，ADB 请求也可送到验证器，并通过 `FLAG_VERIFICATION_IS_ADB` 表明来源。验证器可用 `DEVELOPER_VERIFICATION_BYPASSED_REASON_ADB` 明确回报绕过结果。

若内部会话还设置了 `forceVerification`，平台会追加 `FLAG_VERIFICATION_FORCED_ON_ADB`；即便如此，源码注释仍要求只有阻断策略才能阻断。这些都属于命令行、测试或系统管理边界，不是普通第三方安装器 API，也不改变常规 ADB 用于开发测试的产品承诺。
...
控制器会在会话创建时尝试预热验证器，正式请求则在 `handleInstall()` 解析 APK 后发送。只看“创建会话到成功”无法得到纯验证耗时。
...
| 时间点 | 普通安装器是否可见 | 含义 |
|---|---|---|
| 会话创建、文件写入 | 可见 | 安装器自身 I/O |
| `commit()` | 可见 | 系统异步处理起点 |
| 首次 `STATUS_PENDING_USER_ACTION` | 可见 | 需要用户介入，不一定来自开发者验证 |
| 开发者验证失败原因 | 仅失败且系统提供时可见 | 可确认验证层失败 |
| 最终成功/失败 | 可见 | 安装会话结束 |
| 验证器绑定、请求、响应的精确时间 | 普通应用不可直接取得 | 需要系统指标、跟踪记录或受控设备日志 |
...
`STATUS_FAILURE_BLOCKED` 的公开定义覆盖设备策略、软件包验证器和系统关键包保护等来源。只有 `STATUS_FAILURE_ABORTED` 同时带开发者验证原因时，才有充分证据归入本机制。

`EXTRA_STATUS_MESSAGE` 是调试文本，可能随系统版本、语言和 OEM 改变。它适合保留样本，不适合作为监控聚合键。

## 11. Android 17 r1 的轻量验证字段类型不一致
...
- 安装器目标 SDK、是否持有特权安装权限；
- `SDK_INT`、`SDK_INT_FULL`、会话是否为多包/分阶段安装；
- `EXTRA_STATUS`、原因字段是否存在及枚举值；
- 是否出现待用户操作、停留多久、最终继续还是取消；
- 托管/非托管、ADB/非 ADB；
- 会话写入、提交到首回调、提交到终态的耗时；
- 服务端版本化的政策区域和渠道批次。

不建议默认记录未知验证器扩展 `Bundle`、系统解释页文本、身份文件、账号凭据或签名私钥，也不要用自由文本状态消息作为高基数主维度。
...
- 后台收到待用户操作状态时通过通知恢复，不盲目拉起 Activity。
...
| 设备 | Google 认证 Android 设备、无对应验证器的 AOSP/企业设备 |
| 安装器目标 SDK | ≤ 36、37 |
| 权限 | `REQUEST_INSTALL_PACKAGES`、特权 `INSTALL_PACKAGES` |
| 结果 | 已验证、开发者被阻止、网络不可用、超时 |
| 用户路径 | 待用户操作后继续、重试、取消 |
| 分发 | 参与商店、非参与商店、直接旁加载、ADB、托管商店 |
| 会话 | 单包、多包 |
...
| 旧软件包验证器 / Play Protect | 对应用内容、恶意行为或包风险做判断 | 否 |
| 开发者验证 | 开发者身份、包名与密钥注册不满足当前策略 | 本章机制 |
| SDM / dex metadata | 安装附带的执行优化元数据是否可信 | 否 |
| dexopt / ART profile | 安装后代码编译状态和启动性能 | 否 |

可以按下面的职责模型理解：

> 开发者验证只是安装继续条件中的一项，无法取代 Android 现有的安装安全模型。

它通过 Android 17 的 `PackageInstallerSession` 接入统一安装事务；Google 的实际验证规则由设备上的验证器和当前政策控制；普通安装器只应根据公开回调处理用户动作与终态，不应猜测系统内部结论。
...
- [DeveloperVerifierService：系统验证器的回调入口](https://android.googlesource.com/platform/frameworks/base/+/android-17.0.0_r1/core/java/android/content/pm/verify/developer/DeveloperVerifierService.java#48)
...
- [PackageInstallerService：逐用户策略与会话创建](https://android.googlesource.com/platform/frameworks/base/+/android-17.0.0_r1/services/core/java/com/android/server/pm/PackageInstallerService.java#298)
...
- [PackageInstallerSession：开发者验证异步链路](https://android.googlesource.com/platform/frameworks/base/+/android-17.0.0_r1/services/core/java/com/android/server/pm/PackageInstallerSession.java#3244)
- [PackageInstallerSession：结果、用户动作与失败附加字段](https://android.googlesource.com/platform/frameworks/base/+/android-17.0.0_r1/services/core/java/com/android/server/pm/PackageInstallerSession.java#3491)
