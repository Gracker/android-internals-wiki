...
完整的 pending transaction（待处理事务）、oneway（异步单向调用）和同步调用规则见 **1.12 Binder Freezer 与缓存进程冻结性能**。
...
相关章节：CPU affinity、cpuset、task profile、EAS（Energy Aware Scheduling，能耗感知调度）与 UClamp 见 **5.1**；Binder Freezer 见 **1.12**；OomAdjuster 见 **1.6**；LMKD/PSI 见内存管理章节。
