...
编译器不知道线上设备最常执行哪条路径，只能先用静态启发式规则决定函数内联、基本块顺序和热点/冷点布局。AutoFDO 把真实工作负载中的执行采样转换成 LLVM 采样配置文件，再用这份配置重新构建二进制，让编译器根据运行时证据做这些决策。
...
- 平台用户空间的原生库/可执行文件由 Soong `afdo` 能力接入；
- ACK `android17-6.18-2026-06_r6` 携带 GKI `kernel.afdo`，Kleaf 在构建 `vmlinux` 时引用。

本节以 AOSP `android-17.0.0_r1` 和 ACK `android17-6.18-2026-06_r6` 为当前基线。Android 15/16 的分支只用于说明内核 AutoFDO 的推广过程。

---

## 采样 PGO 与 AutoFDO

PGO（基于配置文件的优化，Profile-Guided Optimization）按配置文件的产生方式大致分两类：

| 类型 | 数据来源 | 优点 | 主要代价 |
|---|---|---|---|
| 插桩 PGO | 编译器插入计数器，运行后产生精确计数 | 基本块和边信息完整 | 需要插桩构建，运行开销与代码布局都会变化 |
| 采样 PGO / AutoFDO | PMU、分支跟踪或采样分析器记录运行位置 | 不必改源代码或插入计数器，适合生产型工作负载 | 采样会丢失、偏斜，地址还原和工作负载代表性更难保证 |

“自动”指配置文件从采样数据自动转换并进入反馈构建，不表示系统能省略工作负载、符号、工具链和验证。

采样也有开销。Coresight 跟踪会产生 AUX 数据，需要缓冲、压缩和后处理；数据量过大时还可能溢出或被 simpleperf 限流。它没有插桩计数器带来的代码侵入，但采集仍会影响设备。
...
最终变化的是机器码，不是 C/C++ 源码语义。配置偏差可能造成代码膨胀或性能回退，编译器和链接器也可能有缺陷。Android 的发布流程仍要比较配置文件、二进制代码段、基准性能和稳定性。

---

## Android 17 的用户空间接入

Soong 的 `build/soong/cc/afdo.go` 负责 C/C++ 目标的 AFDO 属性、配置文件查找和编译参数。模块用：

```bp
afdo: true,
```

声明希望使用由构建环境提供的采样配置文件。Android 17 固定标签中，可以直接看到：
...
这只能证明目标具备 AFDO 构建接入，不能证明任意本地构建都一定拿到了有效配置文件。是否使用，还取决于配置、目标架构、构建变体和产物日志。验证时应在详细构建日志中查找 `-fprofile-sample-use=`，再核对该路径对应的配置文件。

### 与 ART Baseline Profile 的区别

| 维度 | AutoFDO | Baseline / Runtime Profile |
|---|---|---|
| 主要代码 | C/C++ 原生二进制、库、内核 | 应用 DEX 中的 Java/Kotlin 方法 |
| 编译器 | Clang/LLVM | ART / dex2oat |
| 配置单位 | 地址、源码位置、调用与分支样本 | DEX 类和方法热点 |
| 生效时机 | 系统或内核构建 | 安装、更新或后台 dexopt |
| 谁负责 | Android 平台、内核、OEM 构建团队 | 应用开发者、分发渠道和 ART |

Baseline Profile 决定应用中哪些 DEX 路径值得提前编译。AutoFDO 让 `libhwui`、`libart`、Bionic 或内核等原生目标按照真实系统热点重新生成机器码。两者可以同时改善启动，但不能互换。
...
2. 同目录的说明文档记录配置文件来源、工作负载、转换命令和基准；
...
当前标签已经把内核 AutoFDO 接入 GKI 构建。说明文档指出，当前配置文件针对 AArch64 内核 6.18.21 采集，并会在对应滚动分支持续更新。标签固定了某次配置内容，分支最新提交仍会继续变化；复现实验必须记录使用的是标签、配置文件二进制内容还是分支提交。

### Android 17 说明文档中的性能数据

固定标签的说明文档给出 Pixel 8 结果：
...
这些数字不能直接当成所有 Android 17 设备的收益。说明文档同时注明：

- 结果仍是初步数据；
- 当时 Pixel 对 6.18 的功耗、CPU 频率调节和调度尚未完全调优；
- Binder 三项是多轮测试中的最佳结果，用于处理方差。

这些结果只说明该配置文件在特定 Pixel 8 实验中观察到正向变化。OEM 仍要在自己的 SoC、调度配置、厂商模块和关键用户场景上重新进行 A/B 测试。
...
PMU 是 ARM CPU 性能事件的基础设施，simpleperf 通过 Linux perf 接口访问。要生成高质量 AutoFDO 配置文件，仅有周期采样未必足够；Android 内核流程使用 Coresight 分支跟踪还原已执行的指令流。

硬件实现随 SoC 变化：

- ETM（嵌入式跟踪宏单元，Embedded Trace Macrocell）是常见的 Coresight 指令跟踪源；
- ARMv9 平台可以使用 ETE（Embedded Trace Extension）；
- TRBE（跟踪缓冲扩展，Trace Buffer Extension）把跟踪数据写入内存缓冲区。
...
配置文件只描述采集期间实际执行的代码。只跑一次开机，会过度偏向启动路径；只启动一个应用，会遗漏 Binder、文件系统、网络、内存回收和后台工作。

Android 内核说明文档使用的代表性流程包括：

- 对前 100 个应用执行应用爬取器；
- 单应用爬取器运行 3 分钟、执行两次；
- 单应用启动 3 秒、执行 15 次；
- 每次启动后终止进程并清理缓存，覆盖冷启动。

Android 官方博客还报告实验室工作负载与内部设备群执行模式约有 85% 相似。这个数字只说明该套 Google 工作负载的验证结果，不是 OEM 自建工作负载的天然质量分。

---

## 从 `perf.data` 生成 `kernel.afdo`

Android 17 simpleperf 文档和当前 ACK 说明文档给出的内核流程可整理成四步。
...
用另一次构建的 `vmlinux` 强行还原地址，会让配置文件映射到错误源码位置。`--allow-mismatched-build-id` 只是工具提供的容错开关，不能用来忽略来源信息。
...
`perf.data` 是原始采集容器；`branch01.data` 是压缩后的中间分支列表。文档建议多次录制，因为 ETM 指令流在高负载下可能因溢出和速率限制而丢失。多份配置文件能增加覆盖，但也要防止放大异常工作负载的噪声。

### 3. 主机侧生成 AutoFDO 文本配置文件

把所有分支列表与对应未剥离二进制放到主机：
...
`kernel.autofdo` 是按内核二进制聚合并符号化后的文本配置文件。若采集覆盖多个二进制，必须按目标分开处理；不能把同一份未区分目标的地址样本同时用于 `vmlinux`、`.ko` 和用户空间库。

### 4. 转为 LLVM 采样配置文件

当前 Android 17 ACK 说明文档使用：
...
- `--use_fs_discriminator` 是当前 GKI 说明文档的转换要求；
- `--prof_sym_list=false` 避免 Clang 把未列入配置文件的内核函数都视为冷函数。

内核配置文件不可能覆盖所有错误处理、中断和低频管理路径。保留未采样函数的标准优化策略，可以降低覆盖不足造成的去优化风险，也能避免某些热/冷代码段与初始化代码段不匹配。
...
固定标签能保证复现边界。分支最新提交中的 `kernel.afdo` 会持续刷新，不记录配置文件内容就无法解释两次构建差异。

### 覆盖率不等于代表性

配置文件覆盖更多函数，不一定更好。后台压力测试、开机和交互启动的权重若与真实产品不一致，编译器可能把有限的指令缓存和内联预算分给错误路径。
...
先从详细构建日志和反汇编确认 A 组没有使用配置文件、B 组已经使用。否则“无差异”可能只是两组都没接入，所谓“提升”也可能来自工具链或配置漂移。

### Perfetto 看系统结果

Perfetto 适合比较：

- 冷启动的 `system_server`、Zygote、应用和首帧分段；
- Binder 请求的调用方等待与服务端执行；
- 可运行状态、CPU 频率、空闲状态和调度迁移；
- 文件系统 I/O 和缺页；
- 系统启动时间线。

同一关键用户场景至少运行足够轮次，并采用相同的缓存和进程状态。只比较单次截图没有统计意义。
...
硬件事件是否可用、怎样归因取决于 PMU 和权限。函数级变化可用 `simpleperf record/report` 验证热点是否移动；内核目标还要区分 GKI `vmlinux`、GKI 模块和厂商模块。

如果冷启动变快但周期数上升，可能是调频、并行度或 I/O 改变；如果微基准变快而整机关键用户场景不变，说明该路径不是当前瓶颈。两类结果都需要解释，不能只挑正向指标。
...
直接使用 Android 17 GKI 配置文件只是起点。OEM 还应：

1. 确认实际 GKI 构建引用了固定标签的 `kernel.afdo`；
2. 为自研内核差异和产品关键用户场景采集代表性配置文件；
3. 为 GKI 模块/厂商模块分别保留未剥离 ELF 和配置文件；
4. 用同版本工具链生成每个目标的采样配置文件；
5. 建立性能、体积、功耗和稳定性门禁；
6. 随代码漂移定期刷新，避免长期复用旧配置文件。

当前 ACK 说明文档聚焦 `vmlinux`。模块需要单独采集、符号化和构建接入；不能因为内核主体有 `kernel.afdo` 就认定厂商驱动已经得到相同优化。
...
- 原生库自身的 PGO/LTO（如果掌握构建与代表性工作负载）；
- 用 Perfetto 和 simpleperf 确认瓶颈层次；
- 不把 OEM 或 Pixel 内核 AutoFDO 数据承诺为应用自身收益。

应用包含大型 C/C++ 库时，也可以建立自己的采样 PGO 流程，但那是该原生目标的构建优化，不等于 Android GKI AutoFDO。
...
| Android 15 / 16 GKI | 2026 官方博客说明先向 6.6、6.12 分支持续发布内核配置文件 | 作为内核推广历史，不把分支最新提交的数据当作 Android 17 固定结果 |
| Android 17 / `android17-6.18-2026-06_r6` | 固定标签含配置文件和说明文档，并在 `BUILD.bazel` 接入 | 本章当前内核基线；性能数字按说明文档的初步数据条件解释 |

官方博客发布时把 Android 17 对应的 6.18 GKI 写作后续扩展方向；当前固定标签已经包含对应配置文件。版本判断应以所选标签的文件与构建规则为准，不能停留在更早的路线图。
...
不会。采集、转换、重新编译和验证发生在研发/构建流程，用户设备运行的是已经用配置文件编译好的产物。

### “`afdo: true` 证明本地二进制已优化”

它只打开模块的构建接入。还要从构建日志、配置文件路径和编译参数确认实际使用。

### “ETM 采集没有性能开销”

它不需要插桩计数器，但跟踪数据仍消耗硬件缓冲、带宽、存储和后处理时间，也可能丢失。

### “Pixel 8 的 6.6% 冷启动收益会复制到所有设备”

该数字来自 Android 17 说明文档中的特定初步实验，且平台当时尚未完全调优。不同 SoC、内核配置、厂商模块和工作负载都会改变收益。

### “同一个 `kernel.afdo` 能优化所有模块”

采样配置文件必须对应具体二进制和源码位置。`vmlinux`、GKI 模块、厂商模块与用户空间库要分别处理。

### “AutoFDO 替代 Baseline Profile”

前者优化原生代码/内核构建，后者指导 ART 编译应用 DEX。它们作用层不同。

---

## 源码阅读顺序

1. `system/extras/simpleperf/doc/collect_etm_data_for_autofdo.md`：采集、分支列表、符号化和转换；
2. ACK `gki/aarch64/afdo/README.md`：Android 17 配置文件来源、命令与基准限定；
3. ACK `BUILD.bazel`：`clang_autofdo_profile` 怎样进入 GKI 构建；
4. `build/soong/cc/afdo.go`：用户空间 C/C++ 模块怎样查找和应用配置文件；
5. `libhwui`、`libartbase`、`libart` 的 `Android.bp`：真实模块怎样声明 `afdo: true`；
6. Coresight ETM/TRBE 驱动：设备侧跟踪能力从哪里来。

完整证据需要回答四个问题：配置文件采了什么工作负载、对应哪个二进制、由哪个构建规则消费、收益用什么 A/B 结果确认。缺少任何一项，AutoFDO 都只是一个已开启但缺乏验证证据的编译选项。
