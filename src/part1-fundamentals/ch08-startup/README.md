...
目录中保留了一批 `status: deprecated` 的重复稿、错题提纲和失效来源。它们用于记录“为什么某个说法不能继续引用”，默认不进入学习主线；本页明确列出的 8.34、8.38 只承担边界核查。文件顶部的 `task6_*`、`task9_*`、`status`、review 时间和 OpenClaw/Hermes 字段是流水线状态，正文审阅不得删除或擅自重置。
