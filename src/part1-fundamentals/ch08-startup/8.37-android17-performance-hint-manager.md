...
target 表示一次周期希望完成的总时长，actual 表示刚完成周期的实测总时长。两者必须使用同一时间基准；Java 源码要求采用 `SystemClock.uptimeNanos()`。不能用“帧间隔”作为 target，却把某个子函数耗时当 actual，也不能只在慢帧时上报。这样的样本会让反馈失去含义。
...
final class PeriodicHintWorker implements AutoCloseable {
    ...
    private void runOneCycle() {
        ...
        runPeriodicWork();
        long actualNs = SystemClock.uptimeNanos() - startNs;
...
能效模式适合有余量的稳定负载。接近 deadline 的交互循环若盲目开启，可能扩大尾延迟；设备温度高或全局功耗预算收紧时，系统也可能覆盖应用期望。测试时应同时记录 thermal status、功耗、帧/任务耗时与 CPU 调度数据，不要只比较平均频率。
...
3. 用业务 deadline 定义 target，用 `uptimeNanos()`/`CLOCK_MONOTONIC` 测量同一周期 actual。
...
- [2.12 ADPF、刷新率与缓冲队列](../ch02-rendering/2.12-adpf-refresh-rate-bufferqueue.md)：继续阅读 ADPF 在渲染路径中的位置。
