...
tags: ["android17", "ch08", "performance", "optimization"]
related_chapters: []
created_by: "task2a-knowledge-gap"
created_date: "2026-07-08"
gap_source: "AOSP结构"

# 8.33 Android 17 应用启动边界与 AndroidX App Startup 依赖图

> [!WARNING]
> 下面的受保护提纲误用了 GPU 主题，和本章标题无关。它仅作为历史记录保留；有效正文从提纲之后开始。
...
`ActivityThread.installContentProviders()` 会逐个安装 Provider，把得到的 `ContentProviderHolder` 放进列表，最后用一次 `IActivityManager.publishContentProviders()` 将该列表发布给 system_server。因而，“N 个 Provider 必然对应 N 次 `publishContentProviders` Binder 调用”不符合 Android 17 源码。合并 Provider 仍能减少组件实例、类加载和各 Provider `onCreate()` 的固定开销，只是不能把收益错误归因于 N 次发布调用变成一次。
...
manifest 主动发现会直接对 metadata key 执行 `Class.forName()`。这意味着 Provider 运行时，声明的初始化器类必须已经可以由当前进程的 ClassLoader 加载。
...
同理，“类已经被 ClassLoader 缓存”不能等价为零成本。每个进程仍可能承担类解析、校验、静态初始化和首次代码执行成本，应以目标设备的启动测量为准。
