...
AOT 的价值是把部分设备端编译工作移到分发阶段。它通常更适合模型固定、目标 SoC 已知、首次响应敏感的场景。JIT 更适合设备覆盖广或模型动态更新的场景，但第一次创建模型可能承担编译成本。
...
CPU 还要承担预处理、后处理、图中未委托节点和提交等待。检查 Android 17 feature 声明、设备能力、delegate 创建结果、分区日志与模型支持列表，再判断是正常的混合执行还是回退。
