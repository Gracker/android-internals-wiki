...
这章先纠正一个会影响后续分析的名称：在 `android-17.0.0_r1` 中，`frameworks/base` 和 `packages/modules/NeuralNetworks` 都没有 `FoundationModelManager` 类。Android 17 的系统入口是 `OnDeviceIntelligenceManager`。平台源码的另一组、仍受发布开关控制的接口中可以看到 `OnDeviceModel`、Embedding 和 Image Description 类型，但它们也不叫 `FoundationModelManager`。

因此，本文把文档标题中的 `FoundationModelManager` 视为早期素材留下的概念名，正文统一讨论 Android 17 的实际实现。阅读时还要记住一条边界：OnDeviceIntelligence（下文简称 ODI）负责权限检查、服务发现、IPC 转发、只读数据校验、连接管理和部分调用方优先级处理；模型格式、运行时、NPU 选择、批处理、抢占和内存复用由设备厂商的服务实现。
...
- 本章不涉及 Linux 内核实现；全书内核锚点 `android17-6.18-2026-06_r6` 在这里没有可直接验证的 ODI 专用调度代码。
...
| `SharedMemory` 是否等于零拷贝 | 不等于；源码只保证允许传递并收紧为只读 |
...
这里的权限名包含 `BIND_ON_DEVICE_...`，不能写成旧文中的 `BIND_ONDEVICE_...`。system_server 会检查配置组件的绑定权限，并要求推理 Service 带 `FLAG_ISOLATED_PROCESS`、不能是 external service。产品 manifest 还需与 framework resource 中配置的 ComponentName 对上。
...
`processRequest()` 名字容易让初学者误以为它是同步调用。Manager 只负责发起异步 Binder 请求，完成结果通过 `ProcessingCallback` 返回；流式版本通过 `StreamingProcessingCallback` 多次返回 partial result，再以成功或失败结束。
...
AOSP 只证明第一项机制可用，并对访问权限做约束。文章不能从 `ParcelFileDescriptor` 或 `SharedMemory` 推导出模型权重必然 `mmap`，也不能承诺 CPU、NPU 之间没有拷贝。
...
这里需要撤回“ODI 已把 NPU rail 功耗自动分摊给调用 UID”的结论：
...
旧文给出的 TTFT、token/s、内存和并发数字没有设备、模型、输入长度、温控状态或 runtime 证据，不能作为 ODI 基线。更可靠的做法是先定义场景，再记录分布。
...
- 回调 Executor 不承担长时间 I/O 或 UI 重任务。
...
本章的当前结论只以 `android-17.0.0_r1` / API 37 为准：
