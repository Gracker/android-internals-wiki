...
权限名包含 `BIND_ON_DEVICE_...`，不能省略 `ON_DEVICE` 中间的下划线。system_server 会检查配置组件的绑定权限，并要求推理 Service 带 `FLAG_ISOLATED_PROCESS`、不能是 external service。产品 manifest 还需与 framework resource 中配置的 ComponentName 对上。
