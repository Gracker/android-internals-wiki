...
Linux thermal core 位于 `drivers/thermal/`。三个基本对象分别承担不同职责：
...
这解释了某些设备上逐级收紧的曲线，但 Perfetto 里出现阶梯频率仍不足以证明正在运行 `step_wise`。OPP 本身是离散的，schedutil 与固件也会生成阶梯变化。
...
3. policy 上限、capacity 或设备功率约束同期收紧。
