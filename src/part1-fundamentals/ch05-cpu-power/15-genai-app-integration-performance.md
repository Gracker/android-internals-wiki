...
不要根据“系统服务”身份猜测 LMK（Low Memory Killer，低内存回收机制）的回收顺序，也不要假定调用方进程终止后推理一定继续。若要分析回收原因，应把 lmkd 事件、进程状态、PSI 和请求时间线放在一起。§4.4 说明了 Android 17 的 LMK 机制，§5.14 说明了 Cache 与内存统计的层级边界。
...
对于持续时间短、到达时间又不固定的请求，也不要为了“用了 ADPF”临时创建线程和会话。AOSP 源码要求 hint session 面向一组相互关联、长期存在的线程；周期性工作应报告目标时间和实际工作时间。具体用法见 §5.9。
