...
标题中的 Linux 6.10 只表示版本演进背景。sched_ext 在上游主线 Linux 6.12 才合入；6.10 时期的同名材料可能描述合入前 patch set，不能直接代表 Linux 6.10 主线，更不能代表 Android 17 GKI。
...
`scx_bpf_cpuperf_set()` 最终仍走 schedutil 和 CPUFreq core，因此不会绕开具体 SoC driver。6.18 r6 提供 `qcom-cpufreq-hw`、`mtk-cpufreq-hw`、SCMI CPUFreq 等驱动，它们对 fast switch、频率表、policy 范围和硬件写入方式有不同实现。
