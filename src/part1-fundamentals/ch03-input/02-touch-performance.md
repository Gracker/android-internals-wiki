...
触摸驱动向 Linux 输入核心上报事件，evdev 再通过 `/dev/input/eventX` 暴露 `input_event` 流。Android 的 EventHub 用 inotify 发现设备节点增删，用 epoll 等待已打开文件描述符的数据；`getEvents()` 被唤醒后批量读取事件。
...
`readTime - when` 可用于判断事件从驱动/evdev 到 EventHub 读取之间是否停留过久。这个差值应从目标设备的轨迹或日志取得；源码没有承诺它必然小于 1ms。
...
这些判断都要结合持续时间：队列在事件发布与 ACK 之间短暂变为 1 是正常状态，连续增长或长时间不归零才提示背压。`wq` 增长也不能单凭计数器断言“主线程阻塞”，还要查看应用主线程、异步 InputStage 和套接字回写时序。
...
InputDispatcher 通过 `InputChannel` 将事件发送给目标进程。Android 17 的通道由 `socketpair(AF_UNIX, SOCK_SEQPACKET, ...)` 建立并设为非阻塞；`InputChannel::sendMessage()` / `receiveMessage()` 负责消息收发。应用的 `NativeInputEventReceiver` 把文件描述符注册到主线程 Looper，数据可读后消费消息。
...
`NativeInputEventReceiver` 把事件转成 Java `InputEvent` 后，`WindowInputEventReceiver` 将其加入 `ViewRootImpl` 的待处理队列。Android 17 用 `aq:pending:<window>` 计数器记录队列长度，并以 `deliverInputEvent` 同步/异步轨迹标记整段处理。触摸这类指针事件通常从 IME 后置链进入 `EarlyPostImeInputStage`、`NativePostImeInputStage` 和 `ViewPostImeInputStage`，再由 View 树处理。

View 的触摸分发沿命中的目标分支和已建立的 `TouchTarget` 关系进行，并非每个 MOVE 都遍历整棵 View 树。事件可能只更新滚动偏移或状态，也可能触发 `invalidate()`、`requestLayout()` 或动画。前者通常只要求重绘，后者可能让下一次遍历执行测量、布局与绘制。

### 7. 渲染上屏

触摸引起的状态改变要进入某一帧，随后经过应用遍历、RenderThread/GPU、BufferQueue、SurfaceFlinger 和显示输出。任何一段错过当前帧截止时间，都可能让可见反馈顺延一个或多个刷新周期。不能预设渲染一定是最长阶段：主线程调度、输入处理、GPU 和合成都要用同一份轨迹逐段排除。
...
当触摸采样率高于渲染帧率时，一个 VSync 周期内可能到达多个 `ACTION_MOVE` 样本。若每个样本都立即唤醒应用并执行完整的 View 分发，会增加 Looper 和业务回调压力；即使多次失效请求合并到一帧，前面的 CPU 工作也可能重复。批处理用较少的应用交付次数保存这些样本。
...
下面的代码按时间顺序处理一枚批量 `MotionEvent`，适合画笔和轨迹记录；列表滚动通常只需使用当前坐标。
...
重采样坐标会作为批次中的新样本加入，事件里至少保留一枚来自设备的真实样本。非缓冲分发使用 `consumeBatchedInputEvents(-1)` 立即取走所有待处理样本，旧 `InputConsumer::consumeBatch()` 在 `frameTime < 0` 时直接返回，不执行重采样。
...
这条分叉决定 MOVE 是立即送达，还是贴近下一帧的输入回调消费。普通滚动使用缓冲路径，可以减少 Looper 唤醒和 View 分发次数；笔迹、绘图、签名可在确认命中目标后调用 `View.requestUnbufferedDispatch(event)`。它只影响当前手势序列，应用仍要逐个处理事件，CPU 调度和回调压力也会增加。

### 批处理与 WaitQueue 在 Perfetto 中的表现

批处理与应用处理能力是两组不同的证据，在 Perfetto 中对应不同的轨迹和切片。

**InputDispatcher 侧 — 派发/ACK 背压**：

1. 找到 `system_server` 进程的 InputDispatcher 线程
2. 观察 `iq`、`oq:<channel>`、`wq:<channel>` 的 ATRACE_INT 计数器（队列长度，不是切片）
...
1. 切换到应用进程的主线程轨迹
2. 查找原生 `dispatchInputEvent MotionEvent ... historySize=N`，确认应用收到的 `MotionEvent` 是否包含历史样本
...
5. 将事件 ID 与 FrameTimeline/应用帧关联，确认输入更新落入哪一帧以及何时送显
...
2. **检查 InputDispatcher**：观察结构化输入事件、`iq`、目标连接的 `oq:*` / `wq:*`，确认窗口选择、发布和 ACK。
...
分析触摸响应时，可检查以下轨迹、计数器和切片：

**system_server 进程：**
- **InputReader 线程**：结合 `android.input.inputevent` 的 evdev/输入事件时间和线程调度，统计相邻采样间隔；线程唤醒间隔不等于硬件固定采样率。
- **InputDispatcher 线程**：观察 `iq`、`oq:<channel>`、`wq:<channel>` 的长度与持续时间。
  ...
  - `wq` 长时间不降：已发布事件尚未收到完成反馈，需要转到目标应用继续分析。
...
- EventHub/InputReader 的设备、映射器和配置状态。
- 焦点应用/窗口、当前触摸状态与指针捕获。
...
- WaitQueue（wq）持续堆积
- 最早的 WaitQueue 条目越过该连接的分发超时时间后，InputDispatcher 判定连接无响应
...
批处理后的一枚 `MotionEvent` 只做一次 View 分发；只有应用主动遍历历史样本，或使用非缓冲分发收到更多事件时，才会按更多样本执行自身的轨迹逻辑。不要用“采样点数量 × 整棵 View 树深度”估算开销。

优化思路：
- 用轨迹或方法级采样确认 `dispatchTouchEvent()`、手势检测器和业务监听器的耗时。
- 删除没有布局或语义价值的包装层；是否改用 ConstraintLayout 要以测量/布局数据为依据。
...
不少设备实现了输入升频、触摸升频或类似策略，在输入到来后短时间调整 CPU/GPU 性能状态或线程放置。这些名称、触发条件和持续时间属于 SoC/厂商实现，不属于 Android 17 框架的统一契约。
...
这种情况在 Perfetto 中表现为：
- 应用 FrameTimeline 标记错过截止时间，且主线程的输入/遍历已按时结束。
- RenderThread `DrawFrame`、GPU 队列/完成或栅栏等待延伸到截止时间之后。
- `dequeueBuffer` 长时间等待可用缓冲区；具体原因还要结合 BufferQueue 和消费者状态。
- SurfaceFlinger FrameTimeline 显示合成帧错过截止时间，或目标图层的缓冲区没有赶上锁存。

`GPU Completion` 不表示用户看到画面的时间，显示侧边界应使用送显时间。判断“慢了两帧”也要按当时的实际刷新周期计算，不能固定写成 32ms。
...
这几层可以组合，但不能互相替代。轨迹预测降低的是“已知轨迹落后于手”的感知差距；它不会修复主线程阻塞、丢帧或 SurfaceFlinger 合成延迟。
...
下面的 Java 骨架展示框架 API 的最小调用顺序。目标时间必须使用 uptime 时基的纳秒值，而且同一个实例在一次手势中只记录一个设备的事件流。
...
void onStylusEvent(MotionEvent event, long targetTimeNanos) {
    ...
    if (predicted != null) {
        drawTemporaryPrediction(predicted); // 同时消费 predicted 的历史样本
        predicted.recycle();
    }
}
```

实际代码还要在 `ACTION_UP` / `ACTION_CANCEL` 清理临时预测层，并处理多指针、重采样点和模型不可用。普通按钮点击没有连续轨迹可预测；列表滑动也应先修复调度、主线程和帧截止时间问题，再评估轨迹预测是否适合产品交互。
...
更高的刷新率会缩短刷新周期：60Hz 约 16.67ms，120Hz 约 8.33ms，144Hz 约 6.94ms。应用和系统能按期生产帧时，它能缩短等待显示机会的时间。VRR 设备可能根据内容、触摸、功耗和温控状态切换刷新率，因此分析必须读取轨迹中该时段的实际帧时间线，不能只采用设置页或规格表的最高值。
...
- `libs/input/InputConsumerNoResampling.cpp` + `libs/input/Resampler.cpp`：把批处理/传输与 `LegacyResampler`、`FilteredLegacyResampler` 拆开，Android 17 源树已经编译和测试这些实现，但常规 ViewRoot JNI 代码没有改用它。
