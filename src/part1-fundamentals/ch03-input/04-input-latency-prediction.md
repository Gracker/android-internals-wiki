...
### 🔹 调度、提频与硬件采样策略
把 Input Boost、CPU 调度、触控采样率、ARR（adaptive refresh rate）等因素放进同一条分析路径，避免只盯 App 代码。
...
输入延迟最容易混淆的是测量口径。InputDispatcher 的 `total_latency_dur` 只能说明事件派发到 ACK 的耗时；Frame Timeline 说明一帧是否按时 present；手写笔应用关心的则是笔尖和墨迹之间的距离。三者都叫输入延迟，但测到的不是同一段时间。

分析跟手性时，先统一口径，后面才谈得上优化。

除专门说明的版本沿革外，本节的平台实现以 AOSP `android-17.0.0_r1` 为源码锚点。
...
在 App 侧，事件到达和 batched MOVE 的消费要分开看。普通未批处理事件由 `WindowInputEventReceiver#onInputEvent()` 接收，随后进入 `ViewRootImpl` 的输入阶段链；积累中的 MOVE batch 则通常由 `ViewRootImpl` 注册 Choreographer 的输入 VSync callback，在目标帧时间到来时调用 `consumeBatchedInputEvents(frameTimeNanos)`。因此，不能把每个输入事件都描述成“等待 `CALLBACK_INPUT` 后才分发”。
...
[已验证: AOSP android-17.0.0_r1, frameworks/native/libs/input/InputTransport.cpp + frameworks/base/core/java/android/view/ViewRootImpl.java]
...
这里的 5ms 是让坐标贴近目标帧的采样偏移，不是额外附加到端到端链路上的固定 5ms 延迟。Native 层还并存 `InputConsumerNoResampling`、`Resampler.cpp` 和 `LegacyResampler` 等组件，但它们不是这个 tag 下 ViewRoot JNI 直接构造的消费路径。
...
[已验证: AOSP android-17.0.0_r1, frameworks/base/core/jni/android_view_InputEventReceiver.cpp + frameworks/native/libs/input/InputConsumer.cpp] [已验证: 官方 API, MotionEvent.PointerCoords.isResampled()]
...
[已验证: 官方文档, developer.android.com/reference/android/view/MotionPredictor] [已验证: 官方文档, developer.android.com/reference/androidx/input/motionprediction/MotionEventPredictor] [已验证: AOSP android-17.0.0_r1, frameworks/native/libs/input]

### Android 17 源码边界

`MotionPredictor(Context)` 先读取设备资源 `config_enableMotionPrediction` 和 `config_motionPredictionOffsetNanos`。AOSP 基础配置分别是 `false` 和 `0`，所以 API 存在不等于所有设备都默认启用；调用前必须用 `isPredictionAvailable(deviceId, source)` 判断。
...
[已验证: AOSP android-17.0.0_r1, frameworks/base/core/java/android/view/MotionPredictor.java + core/res/res/values/config.xml + frameworks/base/core/jni/android_view_MotionPredictor.cpp + frameworks/native/libs/input/MotionPredictor.cpp + TfLiteMotionPredictor.cpp]

### Framework API 的使用方式

Framework API 需要跨整段 stylus stream 复用同一个实例。下面的封装负责记录真实事件并返回可空的预测事件：
...
[已验证: 官方 API, androidx.graphics.lowlatency.CanvasFrontBufferedRenderer + LowLatencyCanvasView] [已验证: 官方文档, developer.android.com/develop/ui/views/touch-and-input/stylus-input/advanced-stylus-features]
...
`android_input_events` 主要关联 input atrace slice（如 `sendMessage`、`receiveMessage`、`deliverInputEvent`）与 Frame Timeline。`android.input.inputevent` 是只在 debuggable build 上可用的结构化数据源，它用于填充 `android_motion_events`、`android_key_events`、`android_input_event_dispatch` 等表，不是查询 `android_input_events` 的前置条件。

`end_to_end_latency_dur` 需要成功关联输入与 present 帧；关联不到时会是 `NULL`。对于未 batch 的事件，标准库可能用下一帧作推测关联，此时 `is_speculative_frame = 1`。因此，空值表示证据不足，推测值也要与界面行为和 Frame Timeline 一起核对。

[已验证: Perfetto android.input stdlib, android_input_events / android_motion_events / android_input_event_dispatch]
