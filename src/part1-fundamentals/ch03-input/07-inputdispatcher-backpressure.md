...
- `waitQueue` 为空却写不进去，源码把它视为异常：通道理应没有未完成事件却仍不可写。系统调用 `abortBrokenDispatchCycleLocked()`，清空分发队列，把连接标为 `BROKEN`，并异步通知策略层。
- `waitQueue` 非空时，说明已有事件发布成功但仍未收到完成回执，通道此刻也不可写。InputDispatcher 保留 `outboundQueue` 的队首事件并退出本轮发送，等待客户端推进消费与回执。

这是输入管道的反压点，由通道的写入结果触发，不需要根据主线程状态猜测应用是否繁忙。针对该连接的新分发项会积在 `outboundQueue`；如果前面的待处理事件还在等待焦点或策略条件，系统级 `inboundQueue` 也可能增长。两者需要分开观察，不能把某个连接不可写等同于所有输入都停止。
...
`startDispatchCycleLocked()` 在尝试发布前，以当前时间填写 `deliveryTime`，并根据目标连接的分发超时时间计算 `timeoutTime`。只有发布成功，条目才会从 `outboundQueue` 移进 `waitQueue`，响应中的连接也才会把该超时点加入 `mAnrTracker`。客户端回传 `Finished(seq)` 后，`doDispatchCycleFinishedCommand()` 按 `seq` 删除 `waitQueue` 与跟踪器中的对应条目。
...
2. **存在焦点应用，但需要焦点的事件找不到焦点窗口**：事件留在 `mPendingEvent`，计时依据来自 `mNoFocusedWindowAnrState`。触摸可以按坐标命中窗口；单凭一次普通触摸不会触发这类无焦点窗口 ANR。
...
`processAnrsLocked()` 读取 `mAnrTracker` 最早到期的令牌。超时后，它先把连接的 `responsive` 置为 `false`，再移除跟踪器中该连接的所有唤醒点。`onAnrLocked(connection)` 若发现 `waitQueue` 已经清空就停止上报；否则用队首条目的发布等待时间生成原因，通过 `processConnectionUnresponsiveLocked()` 通知策略层，并由 `cancelEventsForAnrLocked()` 为该连接合成取消事件。

隔离不会停止整个输入系统。Android 17 的 `addFocusInputMonitoringTargetsLocked()` 会跳过无响应的焦点输入监视器；触摸目标解析中的 `canWindowReceiveMotion()` 也会拒绝向 `responsive=false` 的连接发送新的触摸。`canReceiveForegroundTouches()` 只决定目标能否带 `FOREGROUND` 标记，不负责响应性判断。源码注释还指出，焦点事件仍可能继续积压，因此“连接无响应”也不等于该令牌的所有队列立刻清空。
...
在无焦点窗口等待期间，如果出现新的指针 `ACTION_DOWN`，`shouldPruneInboundQueueLocked()` 会对其位置做命中测试。目标窗口属于另一个应用令牌，或该位置存在可响应的监视窗口时，InputDispatcher 把这个新事件记录为 `mNextUnblockedEvent`。在队列推进到它之前，按键等按焦点分发的事件会按 `DropReason::BLOCKED` 丢弃；指针类运动事件仍走自己的按坐标分发路径。

这项裁剪不适用于普通的“某连接 `waitQueue` 超时”，也不会清空整个 `inboundQueue`。它专门解除无焦点窗口路径对新目标的阻塞，触发条件和被丢弃的事件类型都受源码约束。

这也解释了为什么一个应用启动阶段迟迟没有焦点窗口时，用户触摸另一个应用或可响应的监视窗口，新的目标仍有机会继续处理输入。
...
1. 应用主线程正在执行长任务，没有进入 `NativeInputEventReceiver` 的消费回路。
2. 应用主线程被 Binder、锁、I/O、GC 或同步等待卡住，导致已收到的事件无法完成处理。
3. `system_server` 或目标进程调度异常，InputDispatcher 写入、应用读取、回执返回中的某段被 CPU 饥饿、锁竞争或内核等待拖慢。

建议按以下顺序定位：先用 `dumpsys input` 或 Perfetto 计数器确认 `wq/oq/iq` 的卡点；再看同一时间窗口内目标应用主线程的调用栈和调度状态；如果主线程在等待 Binder，沿 Binder 事务查对端线程；如果主线程看起来空闲但 `wq` 不退，检查应用进程是否得到调度、通道是否为 `BROKEN`、窗口是否已移除，以及该条目采用的窗口或应用分发超时时间。
