...
InputDispatcher 用 stale event 机制阻止过老的按键或新手势起始事件继续进入目标窗口。它判断的是事件年龄，默认条件为：

```text
currentTime - eventTime >= 10 秒 × HwTimeoutMultiplier()
```

这个条件没有表达“应用已经 ANR”，也没有表达“某个窗口一定没有回复 ACK”。事件可能在 InputDispatcher 内部等待，也可能带着过老的时间戳刚进入队列。排查 stale 日志时，第一步应确认事件老在哪里，而不是先给应用主线程定责。
...
- 单个窗口不回 ACK 可能间接改变后续派发，但它不是 stale 的定义，也不是唯一原因。

## Android 17 的判定来自 policy
...
Android 17 的生产源码中没有另一份 `isStaleEvent()` override；测试用 `FakeInputDispatcherPolicy` 可以替换阈值。厂商分支仍可改 policy 或常量，所以实机分析要同时记录构建版本。
...
| Focus / TouchModeChanged / DeviceReset | 没有 stale 分支 | 主循环明确令这三类不按该 drop reason 丢弃 |
...
对 Key，过期事件可以单独清理。Pointer Motion 是状态序列：
...
它也解释了 stale 机制的主要保护对象：避免启动一个已经失去时效的新动作，同时尽量维持既有输入状态的一致性。
...
connection ANR 的 deadline 记录在 `AnrTracker` 中；tracker 保存 `(timeoutTime, connectionToken)`，每轮取最早 deadline。应用及时回复后，对应 entry 从 `waitQueue` 删除，tracker 也删除该 deadline。
...
1. `DispatchFrozen=true`：先找谁冻结了分发，以及持续多久。
...
`RecentQueue` 同时保存已经派发和已经丢弃的事件，只显示描述与 age，不保存 drop reason。SensorEntry 为避免刷满队列，不进入 RecentQueue。因此，事后只有一份 `dumpsys input` 时，不能仅靠 RecentQueue 证明某个 event 是 stale。

## logcat 与 Perfetto 怎样配合

stale 的 INFO 日志只有固定文案，没有 event id、窗口名或事件描述，而且每个 `STALE` 都会打印。它是时间锚点，不是完整证据：
...
- stale 阈值属于 frameworks/native 与 libbase；本章涉及的 Linux event timestamp 路径按 `android17-6.18-2026-06_r6` 理解。
