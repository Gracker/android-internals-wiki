...
软键盘的显示会跨越应用、`system_server` 和 IME 三个进程，还要等待 IME 窗口具备可动画的 Surface。只测 `showSoftInput()` 的调用耗时，几乎看不到用户等待的那一段时间；只看应用主线程，也可能漏掉 IME 冷启动、会话创建和窗口控制权迟到。

本章以 Android 17 / API 37 / `android-17.0.0_r1` 为源码锚点，回答四个问题：

1. 显示请求在 Android 17 中如何从应用到达 IME；
2. `startInput`、IME 绑定和显示窗口为什么不能混为一谈；
3. 应用布局与 `WindowInsetsAnimation` 在什么位置影响帧时间；
4. 如何用 ImeTracker、`dumpsys input_method` 和 Perfetto 定位等待发生在哪个进程。

文中的“冷”“热”描述的是状态，不预设固定时延。IME 实现、设备性能、进程驻留、词库初始化和当前系统负载都会改变结果，性能结论应来自同一设备、同一构建和同一输入法上的分位数数据。

## 1. 先分清三件事

### 1.1 输入连接、显示请求与屏幕可见是三个事件

一个可编辑 View 获得 View focus 和 window focus 后，`InputMethodManager` 才能把它作为 served view，并为它建立或重启输入连接。IME 侧的生命周期回调与这个过程对应：
...
`showSoftInput()` 只表达“希望显示当前 IME”。如果输入连接已经存在，显示一个暂时隐藏的 IME 可以直接进入 `onShowInputRequested()` 和 `showWindow()`，不必再次调用 `onStartInput()`。排查重复初始化时，先确认初始化代码放在了哪个生命周期回调中。

### 1.2 API 返回值只说明请求有没有被接纳

Android 17 的 `InputMethodManager.showSoftInput(view, flags)` 要求：

- `view` 是当前 served view；
- `view` 自身有焦点；
- 它所在的窗口也有焦点；
- 当前没有需要保留控制权的用户驱动 IME 动画，预测返回正在隐藏 IME 的特殊情况除外。

返回 `true` 表示请求已进入后续处理，不表示 IME 已显示。带 `ResultReceiver` 的重载在 API 36 已弃用，因为回调同样不能可靠代表屏幕上的最终状态。`hideSoftInputFromWindow()` 对目标版本为 API 36 及以上的应用还存在兼容行为：即使最终状态没有改变也会返回 `true`。显示和隐藏都应通过 `WindowInsets.Type.ime()` 的可见性确认。
...
应用收到 `WindowInsetsAnimation.Callback.onStart()`，说明客户端已经取得 Insets 动画控制并进入动画阶段。它不等于 IME 第一帧已经提交，也不等于显示屏已经扫描出像素。根据目标选择时间终点：
...
Android 17 中，应用侧显隐请求以 Insets 为主线。`showSoftInput()` 在完成 served-view 检查后，也会转交当前 `ViewRootImpl` 的 `InsetsController`。
...
flowchart TD
    ...
    N --> O["WMS 等待 IME Surface 与控制 leash"]
...
第一，`InsetsController` 通过 `IWindowSession.updateRequestedVisibleTypes()` 把请求的可见类型交给 WMS。WMS 更新目标窗口的状态后，`ImeInsetsSourceProvider` 再把 IME requested-visibility 变化通知 IMMS。将现代路径概括为“应用直接 Binder 调 IMMS”会漏掉 WMS 的状态机。

第二，IMMS 的 `showCurrentInputLocked()` 只有在以下对象同时存在时才调用 IME：

- `InputMethodBindingController.getCurIme()` 非空；
- 当前 client 非空；
- 当前 client 的 `mCurSession` 非空。

条件不足时，ImeTracker 进入 `PHASE_SERVER_WAIT_IME`，请求 token 被保留，待绑定或 session 就绪后继续。条件齐备时才进入 `PHASE_SERVER_HAS_IME`，再由 `performShowIme()` 调用 `IInputMethodInvoker.showSoftInput()`。

第三，IME 接受显示并执行 `showWindow()` 后，应用也不一定立刻开始动画。WMS 的 `ImeInsetsSourceProvider` 需要等 IME Surface 已绘制、Insets 几何信息可用，并且可把带 leash 的控制权交给目标窗口。没有 control 或 control 中没有 leash 时，客户端 `InsetsSourceConsumer` 会继续把 IME source 当作不可见处理。
...
与其给“冷启动 300 ms”这类脱离环境的数字，不如先记录请求时的状态：

| 状态 | Android 17 证据 | 后续工作 |
|---|---|---|
| 显示热路径 | `mCurIme`、当前 client、`mCurSession` 均存在 | IMMS 可直接向 IME 发送 show |
| Service 已连、session 未就绪 | `mCurIme` 存在，`mCurSession` 为空 | 等 session 创建和 client 绑定 |
| 正在绑定 | `mMainConnection` 存在，`mCurIme` 为空 | 等 `onServiceConnected()`、IME 初始化和 session |
| 未绑定 | `mCurToken`、`mMainConnection` 为空 | bind service，必要时启动 IME 进程 |
| 绑定疑似卡住 | 绑定已发出但长时间没有 `mCurIme` | Android 17 在 3 秒阈值后重连 main connection |

`TIME_TO_RECONNECT_MS = 3000` 是框架判断“绑定长期没有连上”的恢复阈值，不能当作正常冷启动预算。性能报表中应把触发重连的样本单独列为异常。

### 3.2 可见绑定会提高 IME 的进程重要性

`InputMethodBindingController` 使用多组 Service binding：

- main connection 负责维持 IME 接口；具体 flags 受 `lowerImeOomImportance` 开关影响；
- visible connection 在 IME 要显示时增加，包含 `BIND_TREAT_LIKE_ACTIVITY`、`BIND_FOREGROUND_SERVICE`、`BIND_INCLUDE_CAPABILITIES` 和 `BIND_SHOWING_UI`；
- Android 17 还为特定多用户/工作资料场景准备 background connection。

这套设计说明“已绑定”和“正在显示”具有不同的进程重要性。它不能推出统一的冷、热耗时，更不能证明某个 IME 一定常驻。进程是否仍在、资源是否已加载、session 是否可复用，都需要从现场数据判断。

### 3.3 启动抑制配置不代表 isolated process

AOSP `android-17.0.0_r1` 中：

```xml
<bool name="config_preventImeStartupUnlessTextEditor">false</bool>
```

这个资源允许 OEM overlay 改为 `true`。启用后，IMMS 可在当前焦点没有落在文本编辑器时避免启动 IME，并为配置中的输入法或应用保留例外。它控制“何时启动 IME”，与 IME 是否运行在 isolated process 没有直接关系。

`AutofillSuggestionsController` 在 IME 尚未连接时可暂存 inline suggestions 请求，并在 main connection 的 `onServiceConnected()` 后继续处理。源码能证明请求被延后，不能单凭这一点声称它减少了多少次冷启动或节省了多少毫秒。

## 4. 应用侧如何可靠显示和确认 IME

Activity 刚创建时，View focus 和 window focus 可能尚未同时成立。下面的 AndroidX 写法会把显示请求交给窗口的 Insets controller：
...
`post` 的作用是把调用排到 View 所在线程，并让 served-view 状态有机会完成更新。它仍需满足当前窗口和 View 的焦点条件。
...
下面的原则比“用了 Insets 动画就会卡”更接近问题本身：
...
- 用 trace 包住自己的回调，确认耗时位于应用代码还是系统调度。
...
Perfetto 中这段 slice 的耗时只代表应用回调。若它很短但仍掉帧，还要继续检查同一帧的 traversal、RenderThread、IME 进程和 SurfaceFlinger。

### 5.3 `adjustResize`、`adjustPan` 与 `adjustNothing`

| 模式 | 平台行为 | 性能检查重点 |
|---|---|---|
| `adjustResize` | 缩小窗口可用区域，使内容在 IME 上方重新布局 | measure/layout 范围、滚动容器和每帧布局成本 |
| `adjustPan` | 平移窗口以尽量保持输入目标可见 | 平移后的交互与可见区域是否符合产品要求 |
| `adjustNothing` | 平台不为 IME 调整窗口内容 | 应用是否正确消费 Insets、滚动焦点并处理动画 |

模式名称不能直接换算成耗时。一个浅层 View 布局使用 `adjustResize` 可能很稳定，一个手动处理 Insets 的复杂页面也可能每帧反复布局。选择方式是先满足可访问性和焦点可见性，再用 FrameTimeline 与 traversal slice 验证。

Android 15 起，目标 SDK 35 及以上的应用默认使用 edge-to-edge；在 Android 17 上也应按这一边界处理系统栏和 IME Insets。Edge-to-edge 不会取消 IME Insets，应用仍需避免内容被键盘遮挡。为兼容旧系统和 AndroidX 的 Insets 分发，Views 页面通常仍会配置 `adjustResize`。

Compose 的 `WindowInsets.ime`、`imePadding()` 和 Insets consumption 属于 AndroidX Compose UI。它们的重组与测量行为取决于应用使用的 AndroidX 版本，不能用 `frameworks/base` 的 Android 17 tag 证明。排查时记录 Compose BOM/库版本，并缩小读取动态 Insets 的 composable 范围。
...
- Diff 计算可以离开主线程，View 变更仍回到主线程；
- 控制候选项中的图片解码、字体加载和对象分配；
- 分别跟踪 IME 主线程、RenderThread 和 Binder 线程，避免把 app 侧空闲误判成系统等待。

系统的 visible connection 会在显示期间提高 IME 的绑定重要性，但不会替 IME 缩短初始化路径。首帧优化仍取决于输入法自己的服务和渲染实现。
...
| `PHASE_CLIENT_REPORT_REQUESTED_VISIBLE_TYPES` | 客户端准备上报 requested-visible types |
| `PHASE_SERVER_UPDATE_REQUESTED_VISIBLE_TYPES` | WMS 已接收窗口请求 |
| `PHASE_SERVER_UPDATE_CLIENT_VISIBILITY` | IME source 的客户端可见性更新 |
| `PHASE_SERVER_WAIT_IME` | IME 接口、client 或 session 尚未齐备 |
| `PHASE_SERVER_HAS_IME` | IMMS 可以向 IME 发请求 |
| `PHASE_IME_SHOW_SOFT_INPUT` | IME wrapper 正在处理 show |
| `PHASE_IME_ON_SHOW_SOFT_INPUT_TRUE` | `onShowInputRequested()` 接受显示 |
| `PHASE_IME_SHOW_WINDOW` | IME 进入 `showWindow()` |
| `PHASE_SERVER_GET_CONTROL_WITH_LEASH` | WMS 可下发带 leash 的控制 |
...
`mCurRootView` 属于应用进程中的 `InputMethodManager`，可能出现在 IMMS 触发的 client 异步 dump 中；IMMS 在 `system_server` 内维护的核心状态不包含这个字段。阅读 dump 时要保留缩进和进程来源，否则容易把客户端焦点状态归到服务端。

## 9. IME tracing 与 Perfetto
...
命令会通知 IMMS、当前 client 和 IME 更新 tracing 状态。它适合查看输入相关状态转移；CPU 调度、帧截止时间和 Surface 合成仍要由 Perfetto 补充。

### 9.2 Perfetto 的进程与 slice

录制时至少覆盖 app、`system_server`、IME、RenderThread 和 SurfaceFlinger，并启用与调度、Binder、View、WindowManager、graphics、FrameTimeline 相关的数据源。Android 17 源码中可直接搜索的 slice 包括：
...
FrameTimeline 的 jank type 用于描述 missed deadline、high latency、buffer stuffing 等帧结果，不包含 `LAYOUT` 或 `MEASURE`。要判断布局是否拖慢一帧，应展开应用主线程的 traversal、measure/layout 相关 slice，再与该帧的 deadline 对齐。

IME 和应用通常是两个独立 Surface。一次过渡中可能出现四种不同结论：

1. app 帧稳定，IME 窗口掉帧；
2. IME 帧稳定，app 因 Insets 回调掉帧；
3. 两边提交都及时，SurfaceFlinger 或 GPU 未按时合成；
4. 动画前已经在绑定、session 或 leash 阶段等待。
...
- 设备型号、刷新率和电源/温控状态；
- IME 包名、版本、subtype 和主题；
- app 版本、目标 SDK、View 或 Compose 及对应 AndroidX 版本；
- 第一次显示、进程驻留、Service 已连、session 已就绪等起始状态；
- 是否外接硬件键盘，是否启用无障碍服务；
- 至少 P50、P90、P95 和异常样本，而非单次结果。
...
| 服务端路由 | requested-visible types 上报 | `SERVER_HAS_IME` 或 `SERVER_WAIT_IME` |
...
冷热样本要分组。若把首次进程启动与已建立 session 的样本混在一起，平均值既不能代表常用路径，也无法指导优化。

## 11. 版本边界

- Android 11 / API 30 引入可公开使用的 `WindowInsetsAnimation` 协同能力，是后续 IME 动画分析的历史起点；
- Android 12—14 延续 Insets 控制模型，具体修复应按对应 tag 查证；
- Android 15 / API 35 对目标 SDK 35 及以上应用强制 edge-to-edge，IME Insets 处理成为页面布局的必要部分；
- Android 16 / API 36 起，传统 show/hide flags 不再生效，带 `ResultReceiver` 的 show 重载被弃用；
- Android 17 / API 37 的 `android-17.0.0_r1` 使用通用 `InsetsSourceConsumer`，并通过 requested-visible types、`ImeInsetsSourceProvider`、IMMS 和 ImeTracker 串起显示路径。

本章不采用 API 37 后续小版本或 AndroidX 新版本才提供的动画行为解释 `android-17.0.0_r1`。遇到平台季度版本、OEM 修改或 Compose 行为时，应切换到对应源码与依赖版本复核。
