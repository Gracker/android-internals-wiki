...
`onTrimMemory()` 释放的是引用与产品缓存。它不会直接触发 ART GC、heap compaction、内核直接回收或 kswapd。ART 会按分配压力、堆占用与进程状态独立安排 GC；GC 完成后可以另行安排 HeapTrimTask。详细边界见 [4.49 trimMemory API 演进与 ART Heap Trim](4.49-android17-trim-memory-api-evolution.md)。
