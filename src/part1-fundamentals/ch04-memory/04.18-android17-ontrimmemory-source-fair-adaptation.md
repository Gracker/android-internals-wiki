...
级别数值允许未来插入中间状态，应用应按 `>=` 比较。旧系统、厂商代码或 shell 测试仍可能传入其他数值，兼容分支可以保留，但 Android 17 的资源治理不能依赖旧级别到达。
...
2. 多 Activity、画中画、前台服务等复杂状态下，回调时机取决于进程状态与 `pendingUiClean`，不能用单个 Activity 生命周期机械推导。

应用可以把级别 20 当作“可见界面资源可以缩减”的信号。适合释放的对象包括全尺寸预览、只服务于当前页面的预加载结果、不可见动画资源和可快速重建的界面缓存。正在播放的音频、下载任务或用户可感知的后台工作不能仅因界面隐藏就中止。

## 3. `BACKGROUND` 与冻结器的关系
...
代码先发送内存缩减通知，再安排冻结消息，但二者之间没有完成确认。目标应用接收 Binder 事务、主线程处理 Runnable 和系统冻结处理器分属不同调度上下文。可确认的边界如下：

- 系统尽力在冻结前给应用一次缩减缓存的机会；
- 发送方不会等待 `onTrimMemory()` 返回；
- 主线程拥塞时，回调可能很晚才运行；
- 进程被冻结后，尚未执行的工作只能等解冻；
- 应用拿不到稳定的“剩余处理时间”。

lmkd 不在这条路径中。lmkd 根据全局 PSI、水位、交换空间、抖动与 `oom_score_adj` 选择进程并发送终止信号，不会先调用 `scheduleTrimMemory()`。应用也可能在没有新缩减通知的情况下被终止。

## 4. Binder 到主线程的分发

`IApplicationThread.aidl` 把整个接口声明为 `oneway`。`system_server` 调用 `scheduleTrimMemory(level)` 时不等待应用处理完成；oneway 只表示异步调用，不代表事务队列容量无限或消息会在冻结前执行。
...
1. 创建 `trimMemory: <level>` 轨迹切片。
2. 若 `skip_bg_mem_trim_on_fg_app` 开启，当前进程仍为重要前台，且级别至少为 40，直接返回。
3. 收集当前进程中的组件回调。
4. 调用每个组件的 `onTrimMemory(level)`。
5. 正常路径最终调用 `WindowManagerGlobal.trimMemory(level)`。

第 2 步的早退也会跳过 `WindowManagerGlobal.trimMemory()`，但 `finally` 仍会结束轨迹。因此，看到 `trimMemory: 40` 切片不足以证明应用组件收到回调。
...
`WindowManagerGlobal.trimMemory()` 直接调用 `ThreadedRenderer.trimMemory(level)`。它表示缩减应用进程图形栈的缓存，不能扩大成“直接清理 SurfaceFlinger 的 BufferQueue 空闲缓冲区”。

## 6. 应用回调怎样写

以下实现先判断较高级别，并让清理动作保持低成本：
...
- 用户编辑、数据库事务和任务进度按业务时机持久化，不等待内存缩减回调；
- 不主动调用 `System.gc()`；
- 不在回调中同步写盘或等待网络；
- 不直接回收仍可能被 View、渲染器或其他线程使用的 Bitmap；
- 缓存在常态运行时就要有容量上限；
- 清理后记录条目数和估算字节，方便验证收益。

`onTrimMemory()` 释放的是引用与产品缓存。它不会直接触发 ART GC、堆规整、内核直接回收或 kswapd。ART 会按分配压力、堆占用与进程状态独立安排 GC；GC 完成后可以另行安排 HeapTrimTask。详细边界见 [4.49 trimMemory API 演进与 ART Heap Trim](4.49-android17-trim-memory-api-evolution.md)。

## 7. lmkd、冻结器与 MemoryLimiter 要分开看

| 子系统 | 主要输入 | 主要动作 | 向应用发送缩减通知 |
| --- | --- | --- | --- |
| `AppProfiler` | 进程状态、`pendingUiClean` | 发送 `UI_HIDDEN` | 是 |
| `CachedAppOptimizer` | 缓存进程/冻结器状态 | 发送 `BACKGROUND`、安排冻结 | 是 |
| lmkd | 全局 PSI、水位、交换空间、抖动、adj | 选择并终止进程 | 否 |
| MemoryLimiter | 进程状态、cgroup 用量/事件、配置值 | 写限制、检查越界、诊断/终止 | 否 |
| 内核回收 | 水位、分配与 cgroup 压力 | 回收、换出、节流 | 否 |
...
首次 `memory.high` 事件、红区检查和后续进程终止属于 MemoryLimiter 自己的状态机。它没有为目标应用派发专属 `onTrimMemory()`；30 秒的性能分析/终止延迟也不是应用可依赖的自救窗口。源码与监控方法见 [4.17 Android 17 MemoryLimiter 与内存监控影响](17-android17-MemoryLimiter-与内存监控影响.md)。

“公平运行内存”可以作为产品目标，不能写成 Android 17 的公共 API。公平性的证据应包括目标应用释放量、恢复成本、其他应用留存、系统终止进程次数与交互响应，不能只看本进程 PSS。
...
- 远程服务进程没有界面时，通常没有 `pendingUiClean`，不能假设它会收到级别 20；
- 服务进程只有进入缓存/冻结条件后，才可能沿冻结器路径收到级别 40；
- 隔离进程也不能依赖宿主进程替它清理。

多进程应用应为每个进程建立自己的内存预算。公共数据如果通过文件映射、共享内存或 DMA-BUF 使用，还要分别观察每个进程的 PSS/RSS 和资源所有权；“多个 cgroup 一定重复计费”需要结合具体控制器与映射类型验证，不能一概而论。
...
目标进程必须处于允许发送该后台级别的状态；`ActivityManagerService.setProcessMemoryTrimLevel()` 会拒绝向重要前台进程注入级别 20 及以上。

shell 还接受旧级别和原始整数。这只验证应用兼容分支，不能证明正常系统路径会再次发送旧级别。
...
系统轨迹只说明 `handleTrimMemory()` 开始和结束。为了确认组件代码执行，应用还应记录：
...
PSS/RSS 测量应在同一设备状态下重复多轮，并按 Java、原生、图形、文件后备与交换空间分类。一次 PSS 下降不等于内存缩减已经把物理页交还系统，也不能证明 MemoryLimiter 或 lmkd 已经触发。
...
| lmkd 在终止进程前发送级别 40 | 级别 40 来自 `CachedAppOptimizer` 的冻结器路径 |
| oneway Binder 不占缓冲且保证到达 | 发送方不等待完成；队列、调度与冻结仍会影响执行时机 |
| 回调组件顺序是 Application→Provider→Activity→Service | 源码顺序是 Application→Activity→Service→Provider |
| 内存缩减会触发 Major GC 和规整 | `handleTrimMemory()` 没有调用 VMRuntime 或 GC |
| MemoryLimiter 使用 `memory.swap.high` | 执行代码使用 `memory.swap.max` |
| 游戏/播放器有统一 2GB 或 4GB 配额 | 阈值由平台配置与进程状态决定 |
| 缺少轨迹切片表示消息丢失 | 还可能是未发送、进程状态不符、抓取配置不足或时间窗不匹配 |

## 11. 复核清单

- [ ] 只把级别 20/40 当作 Android 17 的常规应用通知；
- [ ] 把 `AppProfiler`、冻结器、lmkd 与 MemoryLimiter 分开解释；
- [ ] 不依赖回调保存关键状态或预告进程死亡；
- [ ] 回调只做主线程可承受的可重建资源清理；
- [ ] 多进程分别设置预算与回调；
- [ ] 用应用自定义证据确认回调执行，而非只看框架轨迹；
- [ ] 同时验证释放收益与回前台重建成本。
...
平台源码以 `android-17.0.0_r1` 为锚点。涉及 cgroup 语义时，以内核 `android17-6.18-2026-06_r6` 的 `Documentation/admin-guide/cgroup-v2.rst` 与 `mm/memcontrol.c` 为准。
