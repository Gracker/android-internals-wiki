...
本章以 AOSP `android-17.0.0_r1` 为平台源码锚点，以 `android17-6.18-2026-06_r6` 为内核锚点。讨论范围包括应用 native heap、stack、globals，以及 Android 如何把 manifest 配置传给 bionic 和 Scudo。
...
两者相同，访问可以继续；两者不同，处理方式由当前线程的 tag-check fault mode 决定。一个 granule 是 16 字节，原文所写的 32 字节不正确。32 字节只是在把两个 4-bit tag 紧凑存入一个字节时对应的数据量。
...
这里的“Android 17”表示当前源码锚点能确认的实现，不表示所有能力都首次出现在 Android 17。Stack MTE 的官方应用支持从 Android 14 QPR3 开始；globals 的首次版本需要逐个历史 tag 核实，本章不写未经验证的首发版本。
