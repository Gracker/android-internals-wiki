...
last_deep_review_at: "2026-08-03T20:38:07+08:00"
last_deep_review_run_id: "20260803-203556-deep-review-44dab4a0"
...
> 本节锚定 AOSP `android-17.0.0_r1` 与内核 `android17-6.18-2026-06_r6`。§4.17 关注 MemoryLimiter 对应用监控的影响；本节沿着源码调用链解释它怎样启用、怎样写入 memcg、怎样从事件监听切换到轮询，以及联合超限后为什么会延迟杀进程。
...
`ProcessRecord` 在构造时创建一个 `Limiter`。设置 PID、绑定 UID 以及更新进程状态时，分别调用 `setPid()`、`setUid()` 和 `onProcStateUpdated()`。Java 层只在选出的 `Limits` 与上一组不同时发送 `MESSAGE_CONFIG`，所以在使用同一档配置的状态之间切换不会重复下发。

`Handler` 保证命令在 `BackgroundThread` 上按队列顺序执行，但源码没有合并队列中连续的配置消息。不要把“相同配置不重复发送”理解为 Handler 会自动只保留最后一条。
...
本文结论只以 `android-17.0.0_r1` 为准。文件版权年份、feature flag 名称或某个开发分支中的提交时间，都不足以单独证明该机制在哪个公开 Android 版本首次交付。若要写 Android 15、16 到 17 的演进，需要分别对照对应 release tag 的 Java、JNI、flags 与设备配置。
...
2026-08-03 的 deep-review 复核重新对照了 `android-17.0.0_r1` 的 Java/JNI/XSD 三处源码：`MemoryLimiter.java` 为 1291 行，`com_android_server_am_MemoryLimiter.cpp` 为 1276 行，`memory-limiter-config.xsd` 为 54 行。复核重点是前文容易误读的四个点：JNI 实际写入 `memory.swap.max`、Native watch 只挂在 `memory.events`、cached 状态对 `memory.high` 使用 ignored 语义、联合超限后的 30 秒延迟服务于系统 profiling 而不是应用回调自救。未引入 Android 18/API 38 或 mainline 分支结论。
