...
last_deep_review_at: "2026-08-02T08:35:38+08:00"
last_deep_review_run_id: "20260802-083538-deep-review-44dab4a0"
...
Android 17 JNI 使用普通的 `android::base::WriteStringToFile()` 写入该节点，没有以 `O_NONBLOCK` 打开。因此，下调 `memory.high` 时触发的回收可能同步发生在 `memory.limiter` 的调用路径上。它不阻塞 AMS 锁内的调用者，但仍会占用 MemoryLimiter 后台处理线程。
