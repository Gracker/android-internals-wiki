...
Android 17 中，LMK 主路径可以概括成三类输入、两段决策和一条回收链。下面的图用于定位各模块职责。
...
flowchart LR
    ...
    reaper --> terminate["cgroup kill 或 pidfd SIGKILL"]
...
本文的平台源码锚点是 `android-17.0.0_r1`，内核语义锚点是 `android17-6.18-2026-06_r6`。
...
Reaper 线程会进入前台 `cpuset` 并提高自身优先级；它还给 victim group 设置专门的 reap task profile。这个行为来自 AOSP `reaper.cpp`，无需借助某个 SoC 厂商的私有补丁来解释。
...
- victim 被再次启动后的启动耗时、主线程、缺页和 IO。

如果大量 victim 都在 900～999，系统大体还在牺牲缓存命中率换可用内存；若频繁下探到 800、700 甚至 200 以下，后台工作、最近任务和用户可感知组件已经受到影响。

## 用 Perfetto、logcat 与退出历史定位

### Perfetto

下面的配置用于同时采集 userspace LMK、adj 变化和系统内存计数。旧内核 LMK 事件只在分析历史设备时添加。
...
`/proc/<pid>/oom_score_adj` 只能观察存活进程的当前值；退出历史和 logcat 用于找已死亡 victim；Perfetto 用于建立时序。三者时间要对齐。
...
16KB page size 会改变页表、内部碎片和进程内存构成，也会改变 KB 与 page count 的换算。它不直接改写 `lmkd` 的优先级原则。`ProcessList` 发送 `LMK_TARGET` 时按运行时 `PAGE_SIZE` 把 KB 换算成页，分析脚本也应读取设备页大小。
