...
这一定义很重要。看到某个产品使用 AICore、私有推理服务或厂商 NPU service 时，不能把该产品的包名、进程优先级和缓存策略写成 AOSP Android 17 的通用行为。`com.google.android.aicore` 不属于 AOSP `android-17.0.0_r1`；它在具体设备上的生命周期和内存策略要以该设备实现为准。
...
- Android 17 的 MemoryLimiter 与 lmkd 仍依据进程状态、设备配置和内存压力工作，不识别“正在执行 AI”这个业务标签。
...
|---|---|---|---|
| Java heap、匿名 native heap | 通常由单进程承担 | 主要计入该进程 | allocator 缓存使 free 后 RSS 未立即下降 |
...
这是每个进程共享的 Binder 接收缓冲区，多个正在进行的事务共同使用，并非“每个 Binder 线程 1 MiB”。请求、返回值、对象偏移以及并发事务都会占用空间。

`TransactionTooLargeException` 的 Java 文档也强调，它只是大事务失败时的启发式异常。调用方无法可靠判断是请求未送达，还是服务端已经处理请求但返回值发送失败，所以要按“可能部分完成”设计幂等性。

`IBinder.MAX_IPC_SIZE` 给出的安全建议值是 64 KiB，它不是驱动硬上限。工程上可按下面的分工选择通道：
...
结构化查询的 `CursorWindow` 初始为可写，写入 Parcel 后接收端得到只读视图。Android 17 支持为 `CursorWindow` 指定上限，内存随着写入行动态分配，因此“默认永远是 2 MiB”不能作为接口约束。
...
Android 17 的 `AppFunctionManager.executeAppFunction()` 最终由 system_server 处理。AOSP 源码中的校验包括：

- 声明的调用包名必须与 Binder calling UID 匹配；
- 跨用户调用需要相应权限，并受企业策略限制；
- 调用自身函数可以走同包规则；
- 调用其他包需要 App Functions 执行权限，并可能受设备 allowlist 和用户访问状态限制；
- 目标 service 必须要求 `android.permission.BIND_APP_FUNCTION_SERVICE`，外部应用不能绕开 system_server 直接绑定。

目标 App 通过 `AppFunctionService.onExecuteFunction()` 接收请求。该回调在主线程触发，模型推理、磁盘读取和网络访问必须切换到 worker，再通过 callback 返回结果，并处理 `CancellationSignal`。

请求中的 `GenericDocument`、`Bundle extras` 和响应对象都实现 Parcelable，仍受 Binder 事务空间约束。大图像、文档或音频应传受控 URI 或小型句柄描述；App Functions 负责“谁可以调用哪个函数”，大块数据仍走适合的内容接口。Android 17 还包含响应 URI grant 的平台支持，使用时要限制 URI、mode 和接收者。
...
Android 17 的 `MemoryLimiter.java` 包含 4 GiB visible、2 GiB notVisible 的 `sDefaultConfig`，源码注释明确说它用于测试，不应未经复核用于生产。生产系统从：
...
本节涉及的内核行为以 `android17-6.18-2026-06_r6` 为 kernel 锚点；Android 用户空间源码以 `android-17.0.0_r1` 为锚点。
...
|---|---|
| SharedMemory 映射、保护和关闭语义 | `frameworks/base/core/java/android/os/SharedMemory.java` |
| SharedMemory native 创建 | `frameworks/base/core/jni/android_os_SharedMemory.cpp` |
| Binder 接收映射大小 | `frameworks/native/libs/binder/ProcessState.cpp` |
| Binder 推荐 IPC 大小 | `frameworks/base/core/java/android/os/IBinder.java` |
| 大事务失败语义 | `frameworks/base/core/java/android/os/TransactionTooLargeException.java` |
| HardwareBuffer 格式与 usage | `frameworks/base/core/java/android/hardware/HardwareBuffer.java` |
| CursorWindow 远端只读与动态分配 | `frameworks/base/core/java/android/database/CursorWindow.java` |
| App Functions service 保护 | `frameworks/base/core/java/android/app/appfunctions/AppFunctionService.java` |
| 请求参数 Parcelable 格式 | `frameworks/base/core/java/android/app/appfunctions/ExecuteAppFunctionRequest.java` |
| 调用资格校验 | `frameworks/base/services/appfunctions/.../CallerValidator*.java` |
| MemoryLimiter 状态映射与推理服务豁免 | `frameworks/base/services/core/java/com/android/server/am/MemoryLimiter.java` |
| lmkd 受害者选择与 dma-buf 统计 | `system/memory/lmkd/lmkd.cpp` |
| isolated / isolated_compute 策略 | `system/sepolicy/private/isolated_app.te`、`isolated_compute_app.te` |
| Binder buffer 分配 | `kernel/common/drivers/android/binder_alloc.c`，`android17-6.18-2026-06_r6` |
...
- MemoryLimiter 的生产阈值来自 vendor XML，4 GiB/2 GiB 只是测试配置；默认 sandboxed inference service 还可能被豁免。
