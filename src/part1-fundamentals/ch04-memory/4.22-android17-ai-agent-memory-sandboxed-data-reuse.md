...
permission-v2 开启后，目标函数可以在 `ExecuteAppFunctionResponse` 中同时返回 URI 和对应的 `AppFunctionUriGrant`。system_server 只处理 `content://` URI，接收者固定为本次请求的 calling package；目标函数只能选择 URI 以及 read、write、prefix、persistable mode。provider 还要允许 URI grant。非 persistable grant 通常持续到设备重启；要求更短生命周期时，仍需由 provider 设计一次性 URI、过期检查或主动撤销。
...
| 响应 URI grant | [`AppFunctionUriGrant.java`](https://android.googlesource.com/platform/frameworks/base/+/android-17.0.0_r1/core/java/android/app/appfunctions/AppFunctionUriGrant.java)、[`AppFunctionManagerServiceImpl.java`](https://android.googlesource.com/platform/frameworks/base/+/android-17.0.0_r1/services/appfunctions/java/com/android/server/appfunctions/AppFunctionManagerServiceImpl.java) |
