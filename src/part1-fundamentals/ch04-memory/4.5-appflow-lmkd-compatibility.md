...
本章将 **AppFlow** 限定为厂商或产品侧可能存在的冷启动优化模块。下面讨论这种外部模块接入 Android 17 时必须遵守的 AOSP 边界。文中出现的设计建议不冒充 AOSP 类或平台 API。
...
PSI trigger fd 的生命周期由打开它的进程管理；关闭 fd 后内核删除 trigger。`memory.pressure` 是统计与事件接口，不是持久化文件。原文所说的 `/dev/memcg/memory.pressure` checkpoint、`/data/app-staging/` AppFlow 会话和 PSI 原子状态都没有 AOSP 依据。
...
userspace lmkd 启动时，`init_psi_monitors()` 失败会让初始化返回错误，daemon 随后由 init 管理其生命周期。它不会动态切换到原文虚构的 “watermark-only AppFlow 模式”。
...
原文中的 38%、73%、52% 等收益没有设备、build、样本量、命令或原始数据，全部作废。兼容性测试至少包含以下分组。
