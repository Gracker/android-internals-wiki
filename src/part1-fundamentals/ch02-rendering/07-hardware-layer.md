...
Hardware Layer 这个名称容易混淆概念。Android 图形栈里至少有三种不同对象会被称为“层”：

| 名称 | 所在范围 | 是否有独立 BufferQueue / SurfaceControl | SurfaceFlinger 能否单独看到 |
|:---|:---|:---|:---|
| View Hardware Layer / RenderNode 合成层 | 应用进程 HWUI 内部 | 否 | 否，它先被合成进应用窗口缓冲 |
| `TextureLayer` / `TextureView` 输入层 | 应用进程，HWUI 消费外部 SurfaceTexture | 输入端有独立 BufferQueue，但最终采样进宿主窗口 | 通常只能看到最终宿主应用窗口 |
| SurfaceFlinger 图层 | 系统合成层，来自 SurfaceControl | 通常携带独立的缓冲与事务状态 | 是 |

这里讨论第一种：HWUI 为某个 View/RenderNode 子树创建中间渲染结果，后续将它作为一个整体参与应用窗口绘制。它不会为这个 View 创建新的窗口，也不会给 HWC 增加一个可独立分配叠加平面的图层。
...
flowchart LR
    ...
    DL --> L["HWUI 合成层<br/>离屏 SkSurface / 渲染目标"]
    L --> W["应用窗口渲染目标"]
    W --> B["BLAST 缓冲事务"]
    B --> S["SurfaceFlinger 图层"]
    S --> D["HWC / 显示设备"]
```
...
Hardware Layer 描述单个 RenderNode 的中间合成策略：先把该节点及其子树绘制到离屏渲染目标，再把结果作为一个整体画回窗口。后续只改变平移、缩放、旋转、透明度等图层属性时，系统可以复用已经栅格化的内容。

两者的边界是：

- 窗口开启硬件加速，View 保持 `LAYER_TYPE_NONE`：现代应用的常规路径；
- 窗口开启硬件加速，View 使用 `LAYER_TYPE_HARDWARE`：显式要求中间硬件图层；
- View 使用 `LAYER_TYPE_SOFTWARE`：该 View 走软件 Bitmap 缓存，再进入宿主绘制；
- 窗口没有硬件加速：`LAYER_TYPE_HARDWARE` 会表现得与软件图层相近，无法获得 HWUI RenderThread 硬件图层。

`LAYER_TYPE_NONE` 只表示没有显式 View 图层。它不会强制每帧测量、布局或重录 DisplayList；这些工作是否发生仍由布局请求、失效操作、RenderNode 脏状态和当前帧属性决定。
...
因此，“GPU 纹理缓存”适合描述设备端常见效果，跟源码时则应使用“RenderNode 持有的图层表面/GPU 渲染目标”。它没有独立的 BufferQueue、GraphicBuffer 或 SurfaceControl，也不能直接交给 SurfaceFlinger 或 HWC。

## `setLayerType()` 在 Android 17 做了什么

### 类型切换会触发一次失效

`View.setLayerType()` 检查类型范围，把类型写入 RenderNode，清理不再需要的软件绘制缓存，更新图层 Paint，并使父缓存与当前 View 失效。下面是 Android 17 的结构摘录：
...
| `SOFTWARE` | `buildLayer()` 进入 `buildDrawingCache(true)`，生成软件 Bitmap 缓存 | 硬件路径不支持的绘制或必须使用软件语义的局部内容；需要实测 |
| `HARDWARE` | RenderNode 类型设为 RenderLayer；HWUI 创建或更新离屏图层表面 | 内容稳定、会被多帧复用，或需要明确离屏合成语义 |

软件图层的 Bitmap 在硬件加速窗口中仍需成为 GPU 可采样资源。内容频繁变化会同时增加主线程软件绘制、Bitmap 更新和图形资源准备成本，因此不适合当作通用性能优化。

`buildDrawingCache()` 所属的公开绘制缓存 API 已废弃，但 Android 17 的 `View.buildLayer()` 内部仍保留这条 SOFTWARE 分支。内部实现仍然存在，不代表应用应重新依赖这套公开 API。

## `buildLayer()`：显式预建的调用链

`View.buildLayer()` 只对 SOFTWARE/HARDWARE 生效，并要求 View 已附着到窗口、宽高非零。Hardware 分支先确保 RenderNode 有 DisplayList，再交给 `ThreadedRenderer`：
...
原生 HWUI 的 `CanvasContext::buildLayer()` 会暂停当前绘制、用 `TreeInfo::MODE_FULL` 准备目标节点、把脏图层交给渲染管线，并把节点记入 `mPrefetchedLayers`。下一次正常 `prepareTree()` 若见到该节点，会通过 `markLayerInUse()` 接管预建结果；若预建节点没有进入树，`freePrefetchedLayers()` 会记录警告并销毁该图层。
...
摘录中的字段名做了类成员前缀压缩，条件与 Android 17 原码一致。自动升层覆盖函子（functor）隔离、ImageFilter、StretchEffect，以及非零半透明度与重叠渲染的组合。尺寸超过最大纹理限制时无法走这条 RenderLayer 路径。

### `hasOverlappingRendering()` 为什么重要

透明度直接乘到每条绘制操作上，与“先把整棵子树画到离屏层，再对整体乘透明度”在重叠区域会产生不同结果。View 声明没有重叠内容时，HWUI 有机会直接调制透明度，避免离屏缓冲。
...
- `setUseCompositingLayer(true, paint)`：把原生图层类型设为 RenderLayer，并设置合成 Paint；
- `setUseCompositingLayer(false, null)`：恢复默认，让 HWUI 决定是否自动升层；
- `getUseCompositingLayer()`：查询是否显式设置了图层类型。

官方注释把 `false` 作为默认且通常推荐的值。合成 Paint 可提供额外透明度、混合模式和 `ColorFilter`；使用合成层还会带来 `clipToBounds=true` 的行为，需要同时检查视觉语义。

`View.setLayerType(HARDWARE)` 与 `RenderNode.setUseCompositingLayer(true, ...)` 最终都影响 RenderNode 的图层类型。前者还维护 View 绘制缓存、失效和 View API 状态，不能在同一 View 上随意混用两套控制方式。

## 图层内容怎样更新

### 脏状态通常触发重绘，不等于重新分配

Android 17 的 `RenderNode::prepareTreeImpl()` 先计算有效图层类型，再进入 `pushLayerUpdate()`：
...
CanvasContext::draw()
  → SkiaPipeline::renderLayers(layerUpdateQueue, ...)
      → renderLayerImpl(node, damage)
          清理或更新图层表面
          在脏区域重放 RenderNode 内容
  → 使用图层结果渲染应用窗口帧
```

内容 `invalidate()` 后，现有图层表面可以按受损区域重绘。尺寸、格式、图形上下文、可渲染状态或最大纹理限制变化时，才可能释放并重分配资源。把每次内容变化都称为“纹理重建”会高估分配次数，却仍可能低估重绘和 GPU 带宽成本。

### 属性变化与内容变化

| 变化 | DisplayList 是否可能重录 | 图层内容是否要重绘 | 可否复用中间结果 |
|:---|:---|:---|:---|
| 平移、旋转、缩放 | 通常不需要 | 通常不需要 | 可以 |
| 整体透明度、合成 Paint | 通常不需要 | 取决于是否只在合成阶段应用 | 常可复用 |
| 子 View 文本、图片、Path、绘制状态 | 需要或使节点变脏 | 需要 | 更新后才能复用 |
| View 尺寸变化 | 可能需要 | 需要 | 可能还要重分配 |
| RenderEffect/ImageFilter 参数或输入变化 | 取决于属性和内容 | 通常需要重新执行受影响效果 | 需测量 |

“属性动画”也可能在监听器中修改文本、布局或绘制状态。判断缓存命中率要看完整动画代码和跟踪数据，不能只看 Animator 的属性名。

## 成本模型

### 1. 离屏内存

Android 17 的 `SkiaGpuPipeline::createOrUpdateLayer()` 会先把图层表面宽高按 `LAYER_SIZE=64` 像素向上取整：
...
- 像素格式、色彩空间与 HDR 精度；
- 后端/驱动的渲染目标与纹理元数据；
- 模糊、阴影、MSAA 或效果处理使用的临时表面；
- 缓存、暂存与延迟释放。

64 像素是 Android 17 HWUI 图层表面的尺寸取整，不是 Linux 页面大小。CPU 进程使用 16KB 页面，也不能推出 GPU 纹理按 16KB 固定取整。GraphicBuffer/gralloc、GPU 驱动和 HWUI/Skia 分配器具有各自策略；需要使用目标设备的 memtrack、GPU 内存计数器、厂商工具或可复现实验测量。

### 2. 首次建立

首次建立要创建或取得图层表面，并把整个有效脏区域栅格化。对只使用一次的内容，这一步还增加了一轮离屏绘制和一次回写窗口的合成。

建层是否“回本”取决于：

`多帧直接重放子树的成本 - 多帧复用图层的成本 > 首次建立 + 额外内存/合成成本`

这个不等式没有跨设备固定阈值。刷新率、分辨率、GPU、Skia 后端、内容复杂度和复用帧数都会改变结果。

### 3. 内容重绘

内容变脏后，HWUI 可只重绘图层的受损区域，但复杂裁剪、效果、子树损伤扩散或整层清除会扩大工作范围。内容每帧变化时，Hardware Layer 仍可能每帧执行离屏绘制，再额外合成回窗口，成本可能高于常规路径。

### 4. 采样质量与裁剪

Hardware Layer 是已经栅格化的图像。大幅放大可能暴露采样模糊；旋转、透视和缩放的视觉结果还受采样滤镜与像素密度影响。强制合成层的边界裁剪语义也可能裁掉原先越界绘制的内容。

### 5. GPU 与内存带宽

离屏绘制会写入图层表面，最终窗口绘制又要读取它。复杂子树复用可以节省重复光栅化，但大面积图层会增加渲染目标写入、纹理采样和内存带宽。分块式 GPU 是否把部分工作留在片上、何时落到外部内存，取决于后端与驱动，AOSP View API 无法给出统一结论。

### 内核与驱动边界

内核侧统一以 `android17-6.18-2026-06_r6` 为版本锚点。View Hardware Layer 是 HWUI/Skia 内部渲染目标，不保证每个图层都对应一个可在 `drivers/dma-buf/dma-buf.c` 中单独识别的导出 dma-buf。最终应用窗口 GraphicBuffer 通常跨进程共享，内部图层纹理则可能只存在于 GPU 驱动和图形 API 的资源空间。

因此，进程 dma-buf 总量、`dumpsys SurfaceFlinger` 图层数和 View Hardware Layer 数之间没有一一对应关系。分析内部纹理分配要依赖 GPU/驱动工具；分析最终窗口缓冲才进入 gralloc、dma-buf、BufferQueue 与 SurfaceFlinger 的共享路径。

## 什么时候值得显式使用

### 候选场景

- 大而复杂的子树内容保持稳定，却要连续多帧做平移、缩放、旋转；
- 整棵子树做透明度动画，重叠内容要求整体离屏合成；
- 需要用图层 Paint 对整棵子树施加混合模式或 ColorFilter；
- 跟踪数据已显示常规路径反复栅格化同一稳定内容，预建后能把成本移出关键动画帧；
- 某些 RenderEffect/隔离语义要求中间结果，并已确认内存与 GPU 预算可接受。

### 高风险场景

- 文本、图片、列表项或自定义绘制内容每帧变化；
- 图层接近全屏或数量很多，内存和带宽压力明显；
- 子树绘制本来很便宜，复用帧数又少；
- View 尺寸在动画中持续变化；
- 内容需要清晰的大幅缩放；
- 依赖越界绘制，边界裁剪会改变结果；
...
建议用 A/B 跟踪做决定：保持设备、刷新率、页面数据、动画阶段和热状态一致，分别比较图层构建与更新、RenderThread、GPU、FrameTimeline 与内存。

## 动画期间怎样管理生命周期

### 优先考虑 `withLayer()`

`ViewPropertyAnimator.withLayer()` 会保存当前图层类型，在下一次动画准备阶段切到 HARDWARE；View 已附着到窗口时还会调用 `buildLayer()`。动画结束后恢复原类型。
...
`withLayer()` 只对下一次动画有效。调用它之后又独立修改 View 图层类型，会与动画结束时的恢复动作产生冲突。内容在动画期间持续变脏时，它也无法创造缓存收益。

### 其他 Animator 要覆盖取消路径

使用 `ObjectAnimator` 或自定义动画时，如实测证明需要图层，应在开始前设置，在结束与取消时都恢复原类型。还要处理 View 脱离窗口、动画被替换和多个动画重叠，避免一个监听器提前释放另一个动画仍在使用的图层。
...
- UI 线程：DisplayList 记录、View 失效、软件图层的绘制缓存工作；
- RenderThread：`buildLayer`、`draw layers`、`drawLayer [name]`、目标帧 `DrawFrame`；
- GPU：离屏绘制、GPU 耗时、纹理与渲染目标分配（设备支持时）；
- FrameTimeline：应用 SurfaceFrame 的预期时间、实际时间与卡顿类型；
- 内存：memtrack、GPU 内存计数器或厂商图形工具。

切片名会受构建版本、atrace 类别和 Skia 后端影响。看到 `buildLayer` 一次，只能说明发生过预建或图层更新；连续出现也可能来自预期的内容变化。必须对齐 View/RenderNode 名称、脏状态来源和动画阶段。
...
| 每帧都有 `drawLayer [name]`，同时内容失效 | 图层内容持续重绘 | 找出脏状态来源，评估拆分稳定与动态节点 |
| 图层构建与更新很短，但 GPU/带宽上升 | 离屏面积或额外采样成本 | 比较 GPU 计数器、分辨率和图层边界 |
| 去掉手动图层后视觉不变且帧时间更低 | HWUI 自动路径已足够 | 保持 `LAYER_TYPE_NONE` |

### “显示硬件层更新”只用于辅助定位

开发者选项中的“显示硬件层更新”（Show hardware layer updates）会在硬件图层更新时闪色。它适合快速确认某一区域是否持续更新，不能给出耗时、GPU 工作或内存。最终结论仍要回到 Perfetto 与设备内存证据。

## RenderEffect 与 Hardware Layer

`View.setRenderEffect()` 把效果写入 RenderNode。Android 17 的自动升层条件包含非空 ImageFilter，因此这类效果通常需要中间合成结果。

RenderEffect 用于产生模糊、颜色滤镜或其他图像效果；显式 Hardware Layer 用于控制中间结果复用与图层 Paint。二者可能在同一 RenderNode 汇合，却不保证“叠加两个 API 就一定缓存一次”。效果输入或参数变化时仍需执行相应绘制与滤镜工作。

分析 RenderEffect 时，除图层更新外还要检查效果范围、模糊半径、HDR/颜色空间、GPU 绘制轮次和受损区域。全屏模糊即使内容稳定，也可能带来很高的中间表面与采样成本。
...
| 策略 | 官方语义 | 风险 |
|:---|:---|:---|
| `Auto` | 默认；透明度小于 1 或设置 RenderEffect 等条件会使用离屏缓冲 | 由参数决定，不能只看修饰符名称 |
| `Offscreen` | 总是先渲染到离屏缓冲，再合成到目标 | 增加内存、绘制轮次和边界裁剪 |
| `ModulateAlpha` | 把透明度调制到每条绘制指令；无 RenderEffect 时可避免透明度离屏 | 重叠内容可能得到不同视觉结果 |

除 `CompositingStrategy.Offscreen` 外，1.11.4 的 `GraphicsLayerScope` 还规定：非 `SrcOver` 的 `blendMode` 和非空 `colorFilter` 都会强制离屏，语义等价于 Offscreen。`Auto` 仍可能离屏，会根据透明度、RenderEffect 和这些合成属性选择中间缓冲。
...
Offscreen 会把绘制限制在图层边界内。只有旋转或平移变化且内容不变时，复用概率较高；状态变化导致内容重新绘制时，离屏结果也要更新。Compose 优化应同时查看重组、测量与布局、绘制层、宿主 RenderThread 与 GPU，不能把所有成本归到 `graphicsLayer`。

## 与 TextureView、SurfaceFlinger Layer 的边界

`TextureView` 的 `TextureLayer` 用来消费外部 SurfaceTexture 缓冲。Android 17 的 `DeferredLayerUpdater` 按槽位包装或复用 `SkImage`，把外部内容采样进宿主应用窗口。这个对象与 View Hardware Layer 都在 HWUI 内部，但数据来源和生命周期不同：

- Hardware Layer 的内容来自 View/RenderNode 子树重放；
- TextureLayer 的内容来自外部生产者的 BufferQueue；
- 两者最终都进入宿主窗口缓冲；
- HWC 通常只能把宿主应用窗口当成一个 SurfaceFlinger 图层处理。

`TextureView` 使用“layer”不代表它获得独立 HWC 叠加平面，也不能用 View 的 `setLayerType()` 解决外部生产者队列、SurfaceTexture 获取或同步栅栏问题。
...
| Android 12 / API 31 | `View.setRenderEffect()` / RenderEffect 进入公开 API | ImageFilter/效果需要结合自动升层与离屏成本分析 |
| Android 17 / API 37 | 源码锚点：`promotedToLayer()`、`effectiveLayerType()`、损伤队列、Skia 图层渲染与 `CanvasContext::buildLayer()` | 当前条件、方法名和资源生命周期按 `android-17.0.0_r1` 解读 |
...
图层缓存的是绘制结果。测量与布局是否执行由布局请求和约束变化决定。

### “内容 invalidate 会重新分配纹理”

常见路径是在现有图层表面上按受损区域重绘。尺寸、图形上下文、格式、可渲染状态等变化才可能要求重分配。

### “Hardware Layer 一定能让属性动画更快”

简单内容、复用帧少、内容持续变化或图层面积过大时，首次建立和额外绘制轮次可能超过节省的工作。现代 HWUI 还会自动升层。
...
它先合成进应用窗口缓冲。SurfaceFlinger 通常只看到窗口 SurfaceControl 图层。
...
| 图层表面保留、销毁与受损区域 | `libs/hwui/RenderNode.cpp`：`prepareLayer()`、`pushLayerUpdate()` |
| 脏矩形合并 | `libs/hwui/LayerUpdateQueue.cpp` |
| 显式预建与预取图层 | `libs/hwui/renderthread/CanvasContext.cpp`：`buildLayer()`、`freePrefetchedLayers()` |
...
Hardware Layer 是 HWUI 内部的 RenderNode 中间渲染结果。它用一次离屏绘制换取后续多帧对子树结果的复用，也会增加内存、渲染轮次、采样、失效重绘和生命周期管理成本。
