...
这里讨论第一种：HWUI 为某个 View/RenderNode 子树创建中间渲染结果，后续将它作为一个整体参与 App Window 绘制。它不会为这个 View 创建新的窗口，也不会给 HWC 增加一个可独立分配 overlay plane 的 Layer。
...
首次建立要创建或取得 layer surface，并把整个有效 dirty 区域栅格化。对只使用一次的内容，这一步还增加了一轮离屏 pass 和一次回写窗口的合成。
...
`TextureView` 的 `TextureLayer` 用来消费外部 SurfaceTexture buffer。Android 17 的 `DeferredLayerUpdater` 按 slot 包装或复用 `SkImage`，把外部内容采样进宿主 App Window。这个对象与 View Hardware Layer 都在 HWUI 内部，但数据来源和生命周期不同：
