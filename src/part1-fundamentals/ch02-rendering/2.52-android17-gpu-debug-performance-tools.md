...
应用侧 experimental 分类在判断 late finish 时，会从 deadline delta 中扣除 `dequeueBuffer` duration。出队等待属于 buffer 背压，不应直接计入应用完成自身工作的预算。该修正没有消除出队等待的影响：它仍会增加端到端延迟，也可能与 `BufferStuffing` 同时出现。
