...
| 机制 | 核心对象 | 输出 | 最适合回答的问题 |
|---|---|---|---|
| `FrameTimeline` | `SurfaceFrame`、`DisplayFrame` | `expected_frame_timeline_slice`、`actual_frame_timeline_slice` | 哪一帧偏离了预测；异常先在应用侧还是 display 侧暴露 |
...
`traceFence()` 遇到 pending fence 时，会把记录保存在 `pendingFences`。后续对同一 layer/buffer 的 trace 调用会重新读取 signal time；超过 60 秒的旧 fence 会被丢弃。这里没有常驻轮询线程，也不保证围栏触发后立即写入数据包。
...
数值单位是纳秒。事件缺失、围栏无效或 trace 在阶段中途开始，都可能造成阶段不完整；查询结果需要与原始 slice、FrameTimeline 和 layer 状态一起核对。
...
调度侧若需核对内核实现，应以 `android17-6.18-2026-06_r6` 为基准。通用内核可证明 sched/cpufreq/cpuset 机制；具体设备的 vendor hook、Power HAL 和 thermal 策略仍需该设备源码或 trace。
