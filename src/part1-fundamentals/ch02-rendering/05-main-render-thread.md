...
讨论以 Android 17、硬件加速已开启的普通应用窗口为主线：

`Choreographer → ViewRootImpl → ThreadedRenderer → HWUI RenderThread → Surface / BLASTBufferQueue → SurfaceFlinger`

这里的 MainThread 指持有目标 `ViewRootImpl` 的 UI 线程。多数 Activity 窗口使用进程主线程，不过 `ViewRootImpl` 也可以绑定其他具有 Looper 的线程。RenderThread 是 HWUI 在应用进程中的共享渲染线程。软件 Canvas、`SurfaceView` 自建生产者、游戏引擎直接使用 EGL/Vulkan 等路径具有不同的线程与缓冲所有权，不能照搬这里的全部结论。

标准路径的线程边界可用下图定位：

```mermaid
flowchart LR
    V["VSync-app / Choreographer"] --> U["UI 线程<br/>测量 / 布局 / 记录 DisplayList"]
    ...
    D --> B["Surface / BLASTBufferQueue<br/>queueBuffer + 生产者完成栅栏"]
    D --> G["GPU 异步执行"]
    B --> F["SurfaceFlinger<br/>锁存 / 合成"]
    G -. "获取栅栏发出信号" .-> F
    F --> H["HWC / 显示"]
    P -. "满足提前解锁条件" .-> U
```

图中的虚线表示条件分支：RenderThread 完成同步后，UI 线程有时可以先返回；纹理缓存空间不足等情况下，UI 线程会继续等到本帧绘制或跳过处理结束。`queueBuffer()` 只是生产者提交点，距离 SurfaceFlinger 采纳和显示设备呈现仍有后续阶段。

## 四类执行者各自负责什么

| 执行者 | Android 17 主职责 | 不宜由该阶段单独推断的结论 |
|:---|:---|:---|
| UI 线程 | 输入与动画回调、测量、布局、把 View 绘制操作记录到 RenderNode、调用 `syncAndDrawFrame()` | `onDraw()` 返回不代表像素已经生成 |
| RenderThread | 同步 RenderNode 状态、准备渲染树和资源、驱动 HWUI/Skia 后端、获取并提交窗口缓冲 | RenderThread 的 CPU 切片结束不代表 GPU 已完成 |
| GPU | 执行图形命令，写入目标缓冲 | GPU 完成不代表 SurfaceFlinger 已锁存或显示设备已呈现 |
| SurfaceFlinger / HWC | 接收图层缓冲与事务、锁存、合成、提交显示 | 显示栅栏描述显示管线进度，不能代表光子已到达人眼 |
...
- 测量自顶向下传播 `MeasureSpec`，再自底向上返回测量结果；
- 布局确定 View 的边界；
- 是否需要重复执行取决于布局请求、尺寸变化、窗口属性、insets 等条件。

层级更深通常意味着更多遍历工作，但测量次数不具有“每多一层必然翻倍”这样的固定关系。自定义 `onMeasure()`、多轮 `requestLayout()`、权重测量和父子约束反复变化才是需要结合跟踪数据与代码确认的因素。
...
`invalidate()`、内容变化、尺寸变化、软件/硬件图层状态和 View 自身绘制实现都会影响是否重录。仅凭“调用了 `requestLayout()`”无法保证 DisplayList 一定复用；也不能断言位置变化必然重录。能映射为 RenderNode 属性的平移、透明度、缩放等更新，通常可以减少内容重录，但仍要看 View 是否同时改变了布局或绘制内容。
...
“SyncFrameState 栅栏”适合描述 UI → RenderThread 的同步等待关系，无法代表一条固定时长、固定解锁位置的 GPU 栅栏。它通过条件变量协调两个线程，与 `sync_file` 文件描述符的实现和参与者不同。
...
这里还有一个容易混淆的同名概念：`CanvasContext::waitOnFences()` 等待的是 `mFrameFences` 中的 `std::future<void>`，这些 future 对象由 `CommonPool::async()` 创建，用来保证异步帧任务在本帧结束前完成。方法名虽然含有 `Fences`，等待对象却不是 `sync_file` 图形栅栏，也不能解释成“等待 SurfaceFlinger 释放栅栏”。GraphicBuffer 的生产者/消费者栅栏由 NativeWindow、BufferQueue、BLAST 和 SurfaceFlinger 路径携带，排查时要按来源区分。

### `queueBuffer()` 表示生产者已提交

在标准应用窗口中，`swapBuffers()` 最终会使生产者提交一个缓冲，并附带描述生产者写入完成状态的栅栏。此时可以确认：

- 生产者已把槽位从 `DEQUEUED` 推向 `QUEUED`；
- 消费者能取得缓冲元数据；
- 消费者使用内容前仍需遵守输入栅栏；
- SurfaceFlinger 是否在目标周期锁存、怎样合成、何时呈现，需要继续看下游事件。

`queueBuffer()` 结束不能证明 GPU 已完成，也不能证明该帧已经显示。Android 11 之后的普通窗口通常还要经过应用进程内的 BLAST 消费者，把 `BufferItem` 组织进 `SurfaceControl.Transaction`，再进入 SurfaceFlinger 的待处理、锁存、合成和呈现阶段。

## 栅栏：先按“谁等待谁”来命名

同一个栅栏沿生产者/消费者边界传递时，名称会随观察方变化。下面的时间线用于说明两种常见方向：

```text
生产者 / RenderThread
  dequeueBuffer(槽位, 释放栅栏)
      │ 等消费者结束对旧内容的使用
      ▼
  GPU 写入新内容
      │
  queueBuffer(槽位, 输入栅栏)
      │ 输入栅栏描述生产者写入完成
      ▼
消费者 / BLAST / SurfaceFlinger
  acquireBuffer → 把输入栅栏作为获取栅栏
      │ 使用内容前遵守获取栅栏
      ▼
  锁存 / 合成 / 呈现
      │ 使用结束后产生释放信息
...
- `queueBuffer()` 携带的输入栅栏约束“消费者何时可以读取新内容”。

从消费者视角看，后一条栅栏就是获取栅栏。释放栅栏与槽位回收有关。SurfaceFlinger/HWC 的显示栅栏描述显示管线的提交完成进度。分析跟踪数据时，先写清栅栏的生产者、等待者和所保护的缓冲，再讨论耗时。
...
- 最大出队数与最大获取数配置；
- 异步/非阻塞模式；
- 槽位当前处于 `FREE`、`DEQUEUED`、`QUEUED` 还是 `ACQUIRED`；
- 释放栅栏是否已发出信号；
- BLAST 是否还有待处理释放；
- Surface 生命周期、尺寸变化与重新分配。

看到长 `dequeueBuffer` 或 `reserveNext` 时，要把槽位状态、栅栏、BLAST 事务和 SurfaceFlinger 锁存放在同一时间轴检查。只写“缓冲区全被占用”会遗漏队列上限和槽位状态。
...
这条分支可以解释某些长 `syncAndDrawFrame()`：资源压力既可能增加 RenderThread 工作，也可能把 UI 解锁推迟到绘制/跳过处理之后。确认时仍需查看同帧的缓存、上传、跳过原因和 GPU 证据。
...
- `CanvasContext::draw()` 发现没有可绘内容，但仍需让纹理上传完成并释放暂存缓冲时，也可以调用 `flushAndSubmit()`。

正常绘制还会经过具体管线的 `draw()`、`swapBuffers()` 和后端提交。Perfetto 中的切片名随 Skia 后端、系统构建和 atrace 配置变化，不能预设所有设备都有同名的刷新切片。CPU 侧刷新时长也不等同于 GPU 执行时长；应结合 GPU 队列、栅栏和 FrameTimeline 判断。

## RenderThread 动画：哪些动画能离开 UI 遍历

Android 5.0 已有 `RenderNodeAnimator` 和 `ViewPropertyAnimatorRT` 基础。透明度、平移、缩放、旋转等能直接映射到 RenderNode 属性的动画，有机会由 RenderThread 推进，减少每帧重新执行完整 View 遍历的需要。

Android 17 的 `RenderThread` 使用 `AChoreographer` VSync 回调驱动 RenderThread 帧回调。`CanvasContext::prepareTree()` 检测到仍有动画且不要求 UI 重绘时，会继续注册 RenderThread 帧回调。VSync 到达后，`CanvasContext::doFrame()` 调用 `prepareAndDraw(nullptr)`；`prepareAndDraw()` 使用 `TreeInfo::MODE_RT_ONLY` 准备树并绘制。`RenderProxy::drawRenderNode()` 也会同步调用同一个 `prepareAndDraw(node)` 入口。
...
最终选择仍由当前版本实现和动画配置决定。若回调里同时执行 `requestLayout()`、修改文本或重绘内容，UI 线程仍会参与。RenderThread 动画也没有绕过 GPU、BufferQueue 和 SurfaceFlinger 的帧截止时间。

## ADPF：性能提示不等于频率承诺

Android 17 的 `CanvasContext.cpp` 通过 `HintSessionWrapper` 更新目标工作时长并上报帧的实际工作时长。AOSP Android 14 源码中已经能看到这条 HWUI 提示会话路径。Android 16 增加的 CPU/GPU 余量 API 用于查询性能余量，两者的 API 边界不同。

ADPF 是系统调度与电源策略的提示输入。收到提示后，系统仍会综合温控、功耗、并发负载和设备策略。跟踪数据中出现提示会话更新，不能单独证明 CPU/GPU 频率已提升，也不能证明帧一定按时。需要把提示、频率/空闲计数器、线程运行位置、GPU 工作和帧结果一并核对。

## Perfetto：沿同一个 VSync ID 找证据

### 第一步：从 FrameTimeline 选择问题帧

Android 12+ 的 FrameTimeline 提供应用 `SurfaceFrame` 与系统 `DisplayFrame` 的预计/实际时间线。选中一帧后，记录 VSync ID、截止时间、卡顿类型和关联图层。高刷新率、可变刷新率和调度偏移都会改变可用预算，不应固定使用 16.67 ms 作为所有设备的阈值。

应用实际时间线的结束还会考虑 GPU 完成与缓冲提交等时间。它比单看 UI `doFrame` 更接近窗口帧结果，但光学显示边界仍需结合显示/呈现证据。
...
- `performTraversals`、测量、布局；
- `Record View#draw()` 或相关 DisplayList 记录切片；
- `syncAndDrawFrame()` / `postAndWait()` 等待区间。

切片名可能因 user/userdebug 构建、跟踪类别和厂商插桩不同。颜色只代表界面展示规则，不能充当根因证据。
...
- 管线绘制、交换、`queueBuffer`；
- RenderThread 是否处于可运行态但长时间未获得 CPU。
...
- 应用进程的 BufferQueue/BLAST 槽位状态；
- `BufferTX - <layerName>`、事务就绪与锁存；
- 生产者完成/获取/释放栅栏；
- GPU 队列、GPU 完成与厂商提供的繁忙度/计数器；
- SurfaceFlinger 合成、HWC 显示提交和 FrameTimeline DisplayFrame。

`queueBuffer()`、BLAST 获取缓冲、SurfaceFlinger 收到事务、锁存、GPU 完成和呈现是不同事件。把它们放在一条时间轴上，才能定位延迟发生在哪个所有权边界。

### 一条可移植性较高的 SQL 起点

下面的查询用于列出 UI 与 RenderThread 上常见的长切片。它只负责筛选候选项，切片名和阈值要按目标跟踪数据调整：
...
查询结果只说明某个 CPU 切片较长。GPU 执行、栅栏、FrameTimeline 和线程调度状态还要从对应表与轨道补充，不能由这张结果表直接定性。

## 六种常见跟踪组合

| 跟踪组合 | 优先检查 | 仍需排除 |
|:---|:---|:---|
| UI `doFrame` 很长，RenderThread 很晚才接到本帧 | 输入、动画、测量、布局、DisplayList 记录、UI 可运行态延迟 | RenderThread 可能同时有旧帧积压 |
| UI 长时间停在 `syncAndDrawFrame`，RT 同期执行 `syncFrameState` | `prepareTree`、资源准备、图层更新、`reserveNext`、纹理缓存 | 仅凭 UI 等待不能判定 GPU 过载 |
| UI 在等待，RT 仍处理前一帧绘制/交换 | GPU 队列、缓冲槽位、栅栏、旧帧截止时间 | 后一帧同步工作本身可能很短 |
| RT CPU 切片不长，应用实际时间线因 GPU 完成延后 | GPU 工作量、过度绘制、着色器、纹理带宽、频率与温控 | SurfaceFlinger/HWC 也可能贡献 GPU 工作 |
| `dequeueBuffer` / `reserveNext` 很长 | 空闲槽位、最大出队/获取数、释放栅栏、BLAST 待处理释放 | 缓冲数量不能固定按三块推断 |
| 应用 `queueBuffer` 按时，DisplayFrame 仍迟到 | BLAST 事务、SF 锁存、合成、HWC/呈现 | 应用侧的完成栅栏也可能晚发出信号 |

### 主线程过重：RenderThread 得到的时间窗口变窄

复杂布局、自定义绘制、同步 I/O、锁竞争或运行队列等待都可能让 UI 线程错过提交时间。排查时先区分线程处于运行、可运行、休眠还是阻塞状态，再定位 Java/Kotlin/原生调用栈。一个很长的 `performTraversals` 需要继续拆成测量、布局、记录和 `syncAndDrawFrame`，不能只按总时长优化 View 数量。

改动也要与证据对应：

- 测量/布局慢：检查多轮测量、布局请求来源和约束变化；
- DisplayList 记录慢：检查自定义绘制循环、文本、Path、阴影和无效区域；
- 可运行态延迟长：检查 CPU 竞争、线程优先级、cgroup 与调度；
- 阻塞调用长：检查锁持有者、Binder、I/O 或同步任务。

### RenderThread 同步或资源准备慢

大量 RenderNode 状态变化、图层更新、首次资源准备和纹理缓存压力可能拉长 `syncFrameState`。DisplayList 命令数量通常没有稳定的公开逐 View 计数器；可先用跟踪数据找到重录和上传发生的帧，再通过局部埋点、页面二分或可复现实验收窄 View 范围。

硬件图层适合内容稳定、需要反复做几何变换的区域。它会增加纹理内存和更新成本，内容频繁变化时可能适得其反。优化前后应比较同场景的记录、上传、RT/GPU 和内存数据。

### GPU 过载

连续高过度绘制、复杂片段着色器、大面积模糊/阴影、高分辨率纹理和频繁离屏渲染都会增加 GPU 工作。判断 GPU 过载至少需要一项直接 GPU 证据，例如 GPU 队列切片、`GPU_DURATION`（设备与版本支持时）、厂商计数器或完成栅栏，再用应用/SF FrameTimeline 确认帧结果。

“GPU 利用率接近 100%”也要结合频率与采样周期解读：低频运行时的高利用率和满频饱和含义不同；某些计数器还混合应用、SurfaceFlinger 和其他进程工作。优化时保持刷新率、分辨率、场景、温度和固件一致。

### 缓冲反压

生产者提交过快、消费者/SF 处理变慢、获取栅栏迟到、释放回调积压或队列上限变化，都可能让可出队槽位减少。观察 `dequeueBuffer` 变短只能说明等待减少，不能单独证明队列深度扩大。

Android 17 还存在缓冲积压检测与恢复逻辑。遇到 `Buffer stuffing recovery`、`swap chain stuffed`、`buffer stuffed` 或 `Negative offset` 等切片时，要结合后续排队计数、出队等待和 FrameTimeline，判断系统是在主动控制排队延迟，还是 CPU/GPU 吞吐不足。
...
来自不同进程的分屏应用拥有各自的 UI 线程和 RenderThread。它们仍共享系统 GPU、SurfaceFlinger、HWC、内存带宽和显示截止时间。两边应用都按时 `queueBuffer()`，仍可能在合成或显示阶段相互影响。

Dialog、PopupWindow、画中画、嵌入式 Surface 等场景还要确认是否创建独立窗口或生产者。减少窗口数量有时能降低调度与缓冲成本，但 UI 语义、无障碍、输入、层级和生命周期同样是设计约束，不能只按线程数量改架构。
...
当 UI 线程或 RenderThread 长时间处于可运行态，可结合 `sched_wakeup`、`sched_switch`、CPU 频率、空闲状态、优先级和 cgroup 判断调度延迟。线程在睡眠或等待锁/栅栏时，优化调度器参数通常不能解决上游依赖。

GPU 与显示栅栏的等待点还涉及 `dma_fence`/`sync_file` 框架和厂商 GPU、DRM/显示驱动。AOSP 框架的 `syncAndDrawFrame()` 条件变量、CommonPool 帧栅栏、GraphicBuffer 获取/释放栅栏属于不同层次；后两者的设备实现不能只靠 `kernel/sched` 两个文件解释。详细栅栏生命周期见 [2.16 Sync Fence 框架与帧同步机制](16-sync-fence.md)。
...
| Android 7.0 / API 24 | FrameMetrics API；`Bitmap.prepareToDraw()` 文档说明从 N 起可触发 RenderThread 异步上传 | 应用可观察更细帧耗时，也可提前准备即将绘制的 Bitmap |
...
| Android 12 / API 31 | FrameTimeline 可用于系统级帧追踪；Performance Hint 公开 API | 可用预计/实际时间线对齐应用与 DisplayFrame；提示不能当成频率保证 |
| Android 14 / API 34 | 该版本 AOSP HWUI 已有 `HintSessionWrapper` 帧工作时长上报 | 分析 RT 时可检查 ADPF 会话，同时仍需频率与帧结果证据 |
| Android 16 / API 36 | CPU/GPU 余量 API 扩展性能余量观察能力 | 余量查询与 HWUI 每帧提示会话要分别解释 |
| Android 17 / API 37 | 源码基线：`android-17.0.0_r1`；内核基线：`android17-6.18-2026-06_r6` | 方法名、条件分支、BLAST 缓冲状态和调度边界均按该版本复核 |
...
它表示 UI 正在等 `DrawFrameTask` 的解锁点。RenderThread 排队、同步、资源准备、缓冲预留和条件性晚解锁都可能贡献时长。GPU 结论需要额外证据。

### “同步结束后 UI 总会立刻返回”

Android 17 根据 `info.prepareTextures` 选择早解锁或晚解锁。纹理缓存空间不足时，UI 等到绘制/跳过处理之后。

### “`queueBuffer()` 返回就是一帧完成”

它只确认生产者完成提交调用。生产者完成栅栏、BLAST 获取缓冲、SF 锁存、合成、呈现都可能发生在后面。

### “RenderThread 的 `waitOnFences()` 就是在等 SF 释放栅栏”

`CanvasContext::waitOnFences()` 处理 CommonPool 异步帧任务。GraphicBuffer 释放栅栏来自生产者/消费者路径，两者需按对象与调用栈区分。

### “所有窗口都使用相同的缓冲数量”

Android 17 根据 NativeWindow 的最小不可出队数量和其他队列配置计算缓冲数。槽位状态、模式、上限与栅栏共同决定能否继续出队。
...
| 根 DisplayList 与 Java → 原生层入口 | `core/java/android/view/ThreadedRenderer.java`、`graphics/java/android/graphics/HardwareRenderer.java` |
...
| HWUI 绘制、异步任务栅栏、交换 | `CanvasContext.cpp`：`draw()`、`waitOnFences()` |
| 进程级 RT 与 RT VSync 回调 | `libs/hwui/renderthread/RenderThread.cpp` |
| BLAST 事务与释放回调 | `frameworks/native/libs/gui/BLASTBufferQueue.cpp` |
| 生产者槽位、入队/出队栅栏 | `frameworks/native/libs/gui/BufferQueueProducer.cpp` |
| 可运行态延迟与公平调度 | 内核 `kernel/sched/core.c`、`kernel/sched/fair.c` |

遇到厂商设备特有的 GPU、显示或栅栏行为，还要补充对应内核厂商驱动、HAL 实现和设备跟踪数据。只看 AOSP 框架无法解释所有硬件差异。

## Android 17 的主线程与 RenderThread 边界

MainThread 负责计算 View 层级并记录 RenderNode DisplayList；RenderThread 同步渲染树、准备资源、驱动 HWUI/Skia 后端并提交窗口缓冲；GPU、SurfaceFlinger 和 HWC 继续完成异步执行、锁存、合成与显示。线程拆分让相邻帧可以重叠，却没有消除它们之间的同步与反压。

分析卡顿时，可以按以下顺序收集证据：

1. 用 FrameTimeline 选定 VSync ID 和错过的截止时间；
2. 在 UI 线程拆分输入、动画、遍历、记录与 `syncAndDrawFrame()`；
3. 在 RenderThread 对齐 `syncFrameState`、资源准备、绘制、出队和入队；
4. 沿完成/获取/释放栅栏检查 BLAST 与 SurfaceFlinger；
5. 用 GPU 与调度器证据区分执行过重、缓冲反压和可运行态延迟。

这条顺序能避免把 UI 等待、RT CPU 工作、GPU 完成、缓冲提交和显示呈现压缩成一个“渲染耗时”。
