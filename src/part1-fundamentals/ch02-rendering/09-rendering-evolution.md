...
2. 应用缓冲经过哪种队列与事务交给 SurfaceFlinger，页面里是否还有独立 Surface？
3. 当时有哪些 VSync、帧截止时间和卡顿数据可以使用？
...
当前平台源码锚点是 Android 17 / API 37 的 `android-17.0.0_r1`，内核锚点是 `android17-6.18-2026-06_r6`。旧版本只用于说明演进，当前类名与行为以 Android 17 为准。

## 时间线速查

| 版本 | 已验证的里程碑 | 分析跟踪数据时的影响 |
| --- | --- | --- |
| Android 3.0 / API 11 | View 2D 硬件加速、显示列表模型、`View.setLayerType()` | 同一个 `Canvas` API 可能走软件或 HWUI 路径 |
| Android 4.0 / API 14 | 对目标 API 14 及以上应用默认启用硬件加速 | 不能只按设备版本判断某个窗口是否硬件加速 |
...
| Android 7.0 / API 24 | Vulkan NDK API、`FrameMetrics` | 原生渲染多一种底层 API；应用可按窗口取得帧阶段数据 |
| Android 8.x / API 26–27 | AOSP HWUI 可通过 `debug.hwui.renderer` 选择 SkiaGL/SkiaVulkan，未设置时仍使用旧 OpenGL 渲染器 | 看到 Skia 后端类不等于该版本默认启用 |
| Android 9 / API 28 | AOSP 将 `debug.hwui.renderer` 的默认值改为 `skiagl` | 不要把旧 `OpenGLRenderer` 类名套到 Android 9 以后的 AOSP 默认主路径 |
| Android 11 / API 30 | 主窗口路径开始使用 BLASTBufferQueue | 缓冲与 SurfaceControl 事务的关系更紧 |
| Android 12 / API 31 | FrameTimeline；`FrameMetrics` 增加 GPU 耗时与截止时间 | 可关联应用 SurfaceFrame 和 DisplayFrame，并区分 CPU/GPU/显示责任 |
| Android 13 / API 33 | `Choreographer.FrameData` / `FrameTimeline` 公共 API；AGSL `RuntimeShader` | 应用可获取候选帧时间线；Canvas 增加运行时着色器 |
| Android 15 / API 35 | ARR 在支持 Android 15 QPR1 与相应 HAL 的设备上可用；ANGLE 成为可选的 GLES-on-Vulkan 层 | VSync 间隔和渲染帧率不能再假设固定；GLES 后端需看设备选择 |
| Android 16 / API 36 | `Display.hasArrSupport()`、`getSuggestedFrameRate()`；`RuntimeColorFilter` / `RuntimeXfermode`；Vulkan 1.4 首发设备要求 | 应用能查询 ARR；AGSL 可用于滤镜与混合；设备能力仍需运行时查询 |
| Android 17 / API 37 | Jetpack WebGPU；GLES `prefer_angle` 清单请求；`Display.getFrameRateVelocityMapping()` | WebGPU 增加现代 GPU 接口但仍需独立依赖；ANGLE 仍是偏好请求；滚动帧率可结合速度映射 |
...
API 11 开始，Android 2D View 管线支持硬件加速。硬件加速窗口中，View 的绘制操作会记录到显示列表；未失效的 View 可以复用已有记录，位置、缩放、旋转或透明度等属性也可以作为 RenderNode 状态处理。

这里要区分两件事：

- `View.draw()` / `onDraw()` 的 Java 调用负责描述绘制；
- GPU 何时光栅化这些命令，取决于 HWUI 后续回放、缓冲和驱动调度。

“调用 `canvas.drawRect()` 就立即执行一条 GL 命令”不符合显示列表模型。

API 11 同时提供 `View.setLayerType()`。`LAYER_TYPE_HARDWARE` 可以把稳定内容放入硬件层，便于后续做合成属性动画；它会占用图形内存。内容失效后仍要重新绘制并更新图层，尺寸或渲染上下文变化时还可能重新分配，因此不能长期给所有 View 强制开启。
...
自定义 View 判断当前绘制路径时，应看 `Canvas.isHardwareAccelerated()`。`View.isHardwareAccelerated()` 只表示 View 附着的窗口是否硬件加速。
...
这些队列运行在 Choreographer 所属的 Looper 线程。应用主 ViewRoot 通常在主线程上，但原生引擎、SurfaceFlinger 客户端或其他 Looper 也可以拥有不同用途的 Choreographer。

### `VSYNC-app` 与 `VSYNC-sf` 是调度事件

Perfetto 里的 `VSYNC-app`、`VSYNC-sf` 或相关预测轨道描述调度节拍。它们不应被逐条解释成“应用直接收到一次硬件中断”。现代系统会根据硬件 VSync 样本、显示模式与工作时长预测应用和 SurfaceFlinger 的唤醒时间。

旧 AOSP 资料常出现 `DispSync`，Android 17 的主线使用 Scheduler 与 `VsyncPredictor` 等组件。内部预测器跨版本持续变化，跟踪分析应关注本帧的预期/实际时间线、截止时间和呈现结果，不要靠预测器类名判断问题。

### 三重缓冲解决的是流水线容量

Project Butter 将三重缓冲作为流畅性策略之一。它允许生产者、GPU 与消费者在更多情况下并行推进，减少“前一块缓冲未释放，下一帧无处可画”的概率。

三重缓冲不表示所有 Surface 永远只有三个固定槽位，也不保证卡顿后无额外延迟。现代 BufferQueue 的槽位数、出队上限、异步模式和使用者约束会共同决定可用容量。生产者持续快于显示消费时，队列会积压，FrameTimeline 可能标记缓冲积压，用户看到的输入延迟也会增加。
...
1. UI 线程执行输入、动画、测量、布局，并更新失效 View 的显示列表；
2. `ThreadedRenderer` / `HardwareRenderer` 通过原生 RenderProxy 把帧任务交给 RenderThread；
3. `DrawFrameTask::syncFrameState()` 同步 RenderNode 树、Surface 与资源状态；
4. RenderThread 准备 Skia/GPU 工作，为应用窗口缓冲执行出队与入队；
5. GPU 完成栅栏随缓冲交给下游，SurfaceFlinger 再处理事务、锁存和合成。
...
- RenderNode 或资源同步；
- 着色器/管线准备与纹理上传；
- `dequeueBuffer`、栅栏或缓冲背压；
- GPU 命令提交与 GPU 完成等待。

需要 GPU 切片、完成栅栏、`FrameMetrics.GPU_DURATION`、设备计数器或 AGI 才能进一步确认 GPU 工作量。
...
| `LAYOUT_MEASURE_DURATION` | 测量/布局 |
| `DRAW_DURATION` | 显示列表计算 |
| `SYNC_DURATION` | 显示列表与 RenderThread 同步 |
| `COMMAND_ISSUE_DURATION` | 向 GPU 发出绘制命令 |
| `SWAP_BUFFERS_DURATION` | 把帧缓冲交给显示子系统 |
| `TOTAL_DURATION` | 从预期 VSync 到帧完成的跨度 |

这些阶段可能并行，`TOTAL_DURATION` 不等于其他时长的算术和。`COMMAND_ISSUE_DURATION` 长也不等于 GPU 执行时间长；前者观察 CPU 侧发命令阶段。

API 31 又增加：

- `GPU_DURATION`：该帧在 GPU 上完成所需时间；
- `DEADLINE`：系统分配给应用生产该帧的时间预算。

API 36 增加 `FRAME_TIMELINE_VSYNC_ID`，可与合成器的帧时间线数据关联。监听器回调还会给出自上次回调以来丢弃的报告数量；需要跨线程保存时，应复制 `FrameMetrics` 对象。

### Android 7.0：Vulkan 进入 NDK

Vulkan 从 API 24 可供原生应用使用。它减少驱动隐式状态，把命令缓冲、资源同步和内存管理交给应用显式控制。代价是同步、生命周期和跨设备能力判断更复杂。

Vulkan API 可用，不表示 View/HWUI 一定使用 Vulkan。应用 Vulkan 渲染器、HWUI SkiaVulkan、ANGLE-on-Vulkan 是三条不同路径。

### Android 8.x–9：HWUI 转向 Skia GPU 后端

早期 HWUI 自己维护大量 OpenGL 渲染器逻辑。Android 8.0/8.1 的 `Properties.cpp` 已识别 `skiagl` 和 `skiavk`，但 `debug.hwui.renderer` 未设置时仍回退旧 OpenGL 渲染器；Android 9.0 的同一文件把属性默认值明确改为 `skiagl`。绘制 API 仍是 View/Canvas/RenderNode，底层由 HWUI 直接管理 GL 的路径逐步转向 Skia GPU 后端。

Android 17 的 `libs/hwui/pipeline/skia/` 仍有 `SkiaOpenGLPipeline`、`SkiaVulkanPipeline` 和公共 `SkiaGpuPipeline`。设备可以按产品配置、驱动和调试设置选择后端，不能按 Android 版本断言所有设备都走同一个后端。

Skia 项目中的 Graphite 是后端研发方向。`android-17.0.0_r1` 的 HWUI 管线目录没有 Graphite 管线或默认启用路径，因此不将它列为 Android 平台里程碑。

### Vulkan 版本要求要看首发设备条件

官方 Vulkan 实现文档给出的主要版本节点是：
...
对设备要求的严谨说法是：64 位、非低内存且以相应版本首发的设备，需要支持该版本要求的高级 Vulkan 功能集。OTA 升级后的旧硬件不会因系统版本号自动获得新 GPU 能力。应用仍应查询 `FEATURE_VULKAN_HARDWARE_VERSION`、扩展、配置集与 dEQP 等级。

`VP_ANDROID_16_minimums`、Android Baseline Profile 和兼容性定义解决的问题不同：配置集描述一组可查询能力，CDD 定义设备兼容要求，应用的最低 Vulkan 版本与扩展则由自己的渲染器决定。

## Android 11：BLAST 改变缓冲与事务的配合

Android 11 的 AOSP 已包含 `BLASTBufferQueue.cpp`，主窗口路径也开始迁移到 BLAST。BLAST 的名称来自 Buffer Layer And Surface Transactions。

它没有删除 BufferQueue。BLAST 内部仍使用生产者/消费者缓冲队列，并把缓冲更新变成 `SurfaceControl::Transaction` 的一部分，便于把缓冲、帧号、裁剪、变换和窗口几何状态按帧关系提交给 SurfaceFlinger。

现代应用窗口的简化路径是：

```text
RenderThread
  → Surface / BufferQueue 生产者
  → BLASTBufferQueue 消费者
  → Transaction::setBuffer() / apply()
  → SurfaceFlinger FrontEnd
```

这条路径说明 BLAST 位于应用生产者与 SurfaceFlinger 事务之间。`queueBuffer()` 只表示生产者提交了一块缓冲；SurfaceFlinger 还要收到事务、检查获取栅栏、选择并锁存缓冲，随后完成本轮显示合成。

Android 12 以后 BLAST 覆盖更多窗口与 Surface 场景，但旧 BufferQueue 类型和非 BLAST 队列继续存在。Perfetto 中应按图层、连接、事务和缓冲 ID 识别对象，不要只搜索某个固定切片名称。

## Android 12–14：FrameTimeline 把应用帧与显示帧关联起来

### Android 12：SurfaceFrame 与 DisplayFrame

FrameTimeline 在系统侧维护应用 `SurfaceFrame` 与 SurfaceFlinger `DisplayFrame` 的预期/实际时间：

- `SurfaceFrame` 观察某个图层的应用缓冲是否按选定时间线完成；
- `DisplayFrame` 观察 SurfaceFlinger/HWC 组织的显示帧是否按时呈现；
- VSyncId/令牌用来建立应用帧和显示帧的关系；
- 卡顿类型用于区分应用截止时间、SurfaceFlinger CPU/GPU、显示 HAL、预测误差、缓冲积压等类别。

`Actual Timeline` 迟到是诊断入口。Perfetto 文档说明应用帧结束会考虑缓冲提交和 GPU 完成；SurfaceFlinger 侧还可能因图层就绪状态、客户端合成、HWC 或显示提交迟到。

私有 `TimelineItem`、`SurfaceFrame` 和令牌保存策略会随版本调整。应用开发者应依赖 Perfetto 数据格式、公共 API 和目标版本源码，不应复制某个旧版本的私有结构体定义当作长期接口。

### Android 13：应用可以读取候选帧时间线
...
一份 `FrameData` 可以包含多个候选时间线，并标出平台偏好的时间线。原生应用有对应的 AChoreographer 帧回调数据 API，可以为 Surface 事务选择帧时间线。

这些 API 提供时间目标，不保证应用一定在截止时间前完成，也不替代缓冲、栅栏与显示提交证据。

### Android 13–16：AGSL 扩展 Canvas 效果

Android 13 的 `RuntimeShader` 让应用用 Android Graphics Shading Language 编写运行时着色器，并作为 Canvas `Shader` 使用。Android 16 增加 `RuntimeColorFilter` 和 `RuntimeXfermode`，把 AGSL 扩展到颜色过滤和源/目标混合。

AGSL 属于 Canvas/HWUI 效果接口。复杂着色器、离屏图层、模糊或多轮渲染的成本仍由实际内容与 GPU 决定，API 版本本身不提供性能保证。

## Android 15–16：ARR、ANGLE 与 Vulkan 1.4

### ARR 改变了“固定 VSync 间隔”的假设

ARR 从 Android 15 开始提供，官方文档把可用条件写为支持相应 HAL 且运行 Android 15 QPR1 及以上。它可以让显示刷新节奏按内容渲染帧率的离散 VSync 步进变化，减少不必要的高刷新率驻留和模式切换。

Android 16 / API 36 增加 `Display.hasArrSupport()` 和 `getSuggestedFrameRate()`。应用还可以通过 View、Surface、ANativeWindow 的帧率 API 表达偏好。系统会综合同一显示上的多个 View、图层、系统 UI、触摸和功耗策略，调用成功不等于立刻切到指定 Hz。

在 ARR 设备上看到 8.33 ms、16.67 ms 或更长 VSync 间隔变化时，应检查选定时间线、截止时间、请求帧率和当前显示模式，再判断是否异常。
...
### Android 16 首发设备的 Vulkan 1.4

设备以 Android 16 及更高版本首发，并满足 64 位、非低内存等条件时，需要支持 Vulkan 1.4。GPU 驱动由 SoC/IHV 提供，框架版本不能替代运行时能力查询。

## Android 17：WebGPU、ANGLE 偏好与滚动帧率映射

### WebGPU

Android 17 的发布说明把 WebGPU 列为图形新能力。公开接口来自独立发布的 Jetpack `androidx.webgpu:webgpu`，当前核验版本为 `1.0.0-alpha05`，不属于 `android.*` 框架 API；alpha 阶段的接口仍可能变化。该库提供 Kotlin/Java 绑定，以比 Vulkan 更高层的对象组织适配器、设备、队列、命令缓冲与 WGSL 着色器。

WebGPU 不会让 View/Compose、WebView 或现有 GLES 应用自动换后端。分析使用 WebGPU 的应用时，应把它按独立 GPU API 和工作提交路径处理：离屏计算只跟踪缓冲、纹理与队列；绘制到屏幕时，再沿 `GPUSurface` 的当前纹理、`present()`、目标 `ANativeWindow`、BufferQueue 和最终 SurfaceFlinger 图层追踪。

### `prefer_angle` 只表达偏好

Android 17 起，游戏可以在清单中请求优先使用 ANGLE 作为 GLES 驱动：
...
平台无法使用 ANGLE 时会回到厂商 GLES 驱动。排查问题要记录实际渲染器与驱动，不能只看清单文件。

### API 37 的帧率/速度映射

`Display.getFrameRateVelocityMapping()` 返回当前显示设备的滚动速度阈值与可行帧率组成的只读、非空映射。例如一个点可以表达“速度超过 300 dp/s 时使用 120 fps”。官方契约主要面向 RecyclerView、ScrollView、AbsListView、NestedScrollView 等惯性滚动场景。设备从内屏切到外屏，或收到 `DisplayListener.onDisplayChanged()` 后，需要针对当前窗口所在显示设备重新查询。

这些点是面向特定显示设备的策略输入，系统不会据此替应用自动完成帧率切换。调用方仍要按速度选择映射点，并通过 View、Surface 或其他帧率 API 表达请求；列表也仍需在每个选定截止时间前完成 UI、RenderThread、GPU 与缓冲提交。
...
页面有 `SurfaceView`、`TextureView`、视频、Camera、WebView、Flutter、游戏或 React Native 时，要先确认生产者、消费者与最终图层。`SurfaceView` 常有独立的子 Surface；`TextureView` 会先被宿主 HWUI 采样进应用窗口；框架名无法替代 Surface 拓扑。
...
- `Build.VERSION.SDK_INT`、构建指纹和厂商镜像；
- 应用目标 SDK、图形 API 与渲染器；
- 显示模式、刷新率、ARR 支持；
- 页面里的窗口、SurfaceView、TextureView 和其他独立 Surface；
- Perfetto/Android Studio/AGI 版本与采集配置。

轨道名和 atrace 切片名属于实现细节，厂商也会增加或改名。找不到旧教程中的名字，不代表对应机制不存在。

### 2. Android 5.0+ 分开 UI 与 RenderThread

UI 线程慢时看输入、动画、遍历、测量/布局和显示列表记录。RenderThread 慢时继续分辨 CPU、资源、缓冲、栅栏、GPU 提交和完成。不要用“哪个切片最长”直接给出 GPU/CPU 结论。

### 3. Android 11+ 同时看 BLAST、BufferQueue 和事务

需要回答四个问题：

1. 生产者何时将哪个帧号入队；
2. BLAST 何时把缓冲放入哪次事务；
3. SurfaceFlinger 何时认为事务就绪并锁存缓冲；
4. 对应 DisplayFrame 何时完成显示提交。

`BufferTX - <layerName>` 只表示 SurfaceFlinger 服务端的待处理缓冲事务发生变化，不表示屏幕已经显示。

### 4. Android 12+ 用 FrameTimeline 锁定帧

选中目标卡顿 `SurfaceFrame` 后，再跟到对应 `DisplayFrame`。应用截止时间错过、SF 截止时间错过和显示 HAL 问题需要不同证据。多 Surface 页面不能只看宿主应用窗口的一条时间线。

### 5. Android 15+ 分开渲染帧率与显示刷新率

应用可能以 30 fps 更新，显示以 60/90/120 Hz 或 ARR 步进工作；多个图层也可能按不同节奏提交。帧是否准时应按选定时间线和截止时间判断，不能固定拿 16.67 ms 作为所有设备、所有帧的预算。

## API、平台实现与设备能力是三层约束

| 层次 | 例子 | 正确检查方式 |
| --- | --- | --- |
| 公共 API | `FrameMetrics`、`FrameTimeline`、WebGPU、`hasArrSupport()` | 看 API 级别、功能标志与官方契约 |
| AOSP 平台实现 | RenderThread、BLAST、SurfaceFlinger FrontEnd、Skia 管线 | 看目标标签的源码和系统属性 |
| 设备能力 | GPU/Vulkan 扩展、ANGLE、HWC 平面、ARR HAL、计数器 | 看运行时查询、dumpsys、驱动与目标设备跟踪数据 |

“Android 17 支持某 API”只覆盖第一层；“AOSP 有某管线”只覆盖第二层。具体手机是否启用、性能如何，仍由第三层决定。

## 内核与厂商边界

内核源码锚点是 `android17-6.18-2026-06_r6`。其中 `drivers/dma-buf/dma-buf.c` 管理共享缓冲对象，`dma-fence.c` 定义异步完成依赖，`sync_file.c` 把栅栏暴露为可跨进程传递和等待的文件描述符；通用内核还提供线程调度、内存回收和频率框架等基础机制。

HWUI 后端、Vulkan/GLES 驱动、GPU 任务调度、图形内存分配、DPU/HWC 平面与 ARR HAL 含有大量厂商实现。一个框架版本里程碑不保证厂商驱动同步采用相同策略。内核栅栏只能说明异步依赖是否完成；要解释迟到原因，还需找到栅栏所有者、提交者和对应硬件工作。

## 常见误区

### “BLAST 已经替代 BufferQueue”

BLAST 使用并管理 BufferQueue，把缓冲更新纳入 Surface 事务。现代系统里两者同时存在。

### “RenderThread 的 DrawFrame 长就是 GPU 慢”

`DrawFrame` 跨度包含 RenderThread CPU、资源、缓冲、栅栏和 GPU 相关阶段。需要 GPU 完成事件或直接 GPU 证据。

### “Android 16/17 设备都用 SkiaVulkan 或 Graphite”

Android 17 AOSP 提供 SkiaOpenGL 与 SkiaVulkan 管线，产品选择与设备配置有关；当前标签没有 HWUI Graphite 管线。

### “Vulkan 1.4 要求适用于所有升级设备”

首发设备条件、64 位与低内存条件决定兼容要求。旧设备 OTA 后仍以硬件和驱动上报为准。

### “ARR 让 VSync 随意变化”

ARR 按设备支持、内容投票和离散步进工作。间隔变化可以是正常策略，也可能是模式切换或调度问题，需要结合显示模式与 FrameTimeline。

### “FrameMetrics 可以解释 SurfaceFlinger”

FrameMetrics 是应用窗口的渲染里程碑。SurfaceFlinger、HWC 和多图层显示问题需要 Perfetto、图层状态、合成类型与显示提交/栅栏证据。
...
- [`FrameMetrics.java`](https://android.googlesource.com/platform/frameworks/base/+/android-17.0.0_r1/core/java/android/view/FrameMetrics.java)：当前指标、GPU 耗时、截止时间与 VSyncId。
...
- [`DrawFrameTask.cpp`](https://android.googlesource.com/platform/frameworks/base/+/android-17.0.0_r1/libs/hwui/renderthread/DrawFrameTask.cpp)、[`CanvasContext.cpp`](https://android.googlesource.com/platform/frameworks/base/+/android-17.0.0_r1/libs/hwui/renderthread/CanvasContext.cpp)：UI/RenderThread 同步、绘制和缓冲提交。
- [`SkiaOpenGLPipeline.cpp`](https://android.googlesource.com/platform/frameworks/base/+/android-17.0.0_r1/libs/hwui/pipeline/skia/SkiaOpenGLPipeline.cpp)、[`SkiaVulkanPipeline.cpp`](https://android.googlesource.com/platform/frameworks/base/+/android-17.0.0_r1/libs/hwui/pipeline/skia/SkiaVulkanPipeline.cpp)：当前 HWUI GPU 后端。
- [`BLASTBufferQueue.cpp`](https://android.googlesource.com/platform/frameworks/native/+/android-17.0.0_r1/libs/gui/BLASTBufferQueue.cpp)：缓冲与事务。
- [SurfaceFlinger `FrameTimeline.h`](https://android.googlesource.com/platform/frameworks/native/+/android-17.0.0_r1/services/surfaceflinger/Scheduler/FrameTimeline.h)、[`FrameTimeline.cpp`](https://android.googlesource.com/platform/frameworks/native/+/android-17.0.0_r1/services/surfaceflinger/Scheduler/FrameTimeline.cpp)：SurfaceFrame、DisplayFrame 与卡顿分类。
- [SurfaceFlinger FrontEnd](https://android.googlesource.com/platform/frameworks/native/+/android-17.0.0_r1/services/surfaceflinger/FrontEnd/)、[`HWComposer.cpp`](https://android.googlesource.com/platform/frameworks/native/+/android-17.0.0_r1/services/surfaceflinger/DisplayHardware/HWComposer.cpp)：RequestedLayerState、LayerSnapshot、验证与显示提交。

### 内核 / `android17-6.18-2026-06_r6`

- [`dma-buf.c`](https://android.googlesource.com/kernel/common/+/android17-6.18-2026-06_r6/drivers/dma-buf/dma-buf.c)：共享缓冲对象与附着/映射边界。
- [`dma-fence.c`](https://android.googlesource.com/kernel/common/+/android17-6.18-2026-06_r6/drivers/dma-buf/dma-fence.c)、[`sync_file.c`](https://android.googlesource.com/kernel/common/+/android17-6.18-2026-06_r6/drivers/dma-buf/sync_file.c)：异步完成依赖与 sync_file 文件描述符。
