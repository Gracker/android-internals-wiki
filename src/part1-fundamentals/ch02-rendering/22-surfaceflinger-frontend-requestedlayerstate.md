...
SurfaceFlinger 收到 `SurfaceControl.Transaction` 后，不能立刻把每个字段原样交给 HWC。它还要解决四个问题：
...
Android 17 的 SurfaceFlinger FrontEnd 就位于这段边界上。它消费 transaction，维护 layer 的服务端请求状态和生命周期，构建可遍历的 layer 图，再生成 CompositionEngine 使用的 `LayerSnapshot`。

本章固定到：

- 平台源码：Android 17 / API 37 / `android-17.0.0_r1`
- kernel 源码：`android17-6.18-2026-06_r6`

本章没有需要下探 kernel 的实现。acquire fence、release fence 和 dma-fence 的内核语义见 §2.16；这里仅讨论 SurfaceFlinger 如何把 fence 作为 transaction readiness 和 buffer 使用条件。

## 1. 先建立正确的边界

FrontEnd 处理的是“客户端请求怎样变成本轮可消费状态”，不是完整的显示流水线。
...
这张图是职责图，不是逐行调用栈。Android 17 的 `SurfaceFlinger::updateLayerSnapshots()` 中，主要顺序是：
...
前者是请求状态更新，后两者还涉及 buffer、fence、CompositionEngine、HWC 和显示设备。

## 2. FrontEnd 没有让旧 `Layer` 对象立刻消失

有些资料把 FrontEnd 描述成“以 `RequestedLayerState` 完全替换 `Layer`”。这不符合 Android 17 源码。
...
- 也不要假设看见 FrontEnd 后，所有 legacy 路径都已删除。

## 3. `RequestedLayerState` 保存什么

`RequestedLayerState` 的源码注释很直接：它保存某个 layer 的客户端请求状态，不保存其他 layer 的状态。Android 17 中它继承 `layer_state_t`：
...
`kMustComposite` 是其中一组“变化后需要推动 composition”的集合，并不等同于全部 flags。比如 `FrameRate` 还会触发 attached choreographer 的刷新率更新；是否需要合成由调用点分别判断。

### 3.2 为什么 layer 关系保存为 id

`RequestedLayerState` 使用 `parentId`、`relativeParentId`、`layerIdToMirror`、`touchCropId` 等 id 表示跨 layer 关系，不继续持有客户端 handle。这样不会因为状态对象保存了一份 handle，而无意延长 handle 生命周期。
...
客户端的 `setPosition()` 只提供局部位置请求。`LayerSnapshotBuilder` 还要叠加：
...
所以 Winscope 中看到的最终 bounds 与 transaction 参数不同，并不自动说明 transaction 丢失。先确认父层级和 traversal path。

## 4. `LayerLifecycleManager` 管理创建、更新和销毁

`LayerLifecycleManager` 拥有 `RequestedLayerState` 集合，并维护 id 到状态以及反向引用的映射。它不是线程安全类；Android 17 的成员由 SurfaceFlinger 主线程上下文保护。只有 transaction 的入口收集使用 `LocklessQueue`，不能把整个 FrontEnd 称为“无锁”。
...
`applyTransactions()` 按 transaction 中的 `ResolvedComposerState` 找到目标 layer，然后调用 `RequestedLayerState::merge()`。第一次发生变化的 layer 会进入 `mChangedLayers`；每个 layer 的 flags 再汇总到 `mGlobalChanges`。
...
普通 parent-child 关系可以画成树，但 relative Z 与 mirror 会让同一个状态节点从多条路径被访问。Android 17 用图表示这组关系，不为每条镜像路径复制一份 `RequestedLayerState`。
...
这比“一个 layer id 永远只有一份最终状态”更准确。`LayerSnapshotBuilder` 因而同时维护：
...
需要跨进程排序时，Android 17 还有显式 transaction barrier。不要把“提交时间先后”当成跨 applyToken 的生效顺序。
...
任一 filter 返回 `NotReady` 或 `NotReadyBarrier`，当前 applyToken 队列就停在队首。
...
跨 applyToken 的 barrier 可能在一次扫描的后半段才被 signal。`flushTransactions()` 因此反复扫描 pending queues，直到等待 barrier 的 transaction 数量不再变化，以便在同一帧继续解开能够满足的依赖链。

这些秒数是 `android-17.0.0_r1` 的内部实现值，不是 App 可以依赖的稳定 API 契约。

## 7. 从请求状态生成 `LayerSnapshot`

`LayerSnapshot` 继承 `compositionengine::LayerFECompositionState`，保存 CompositionEngine 和 RenderEngine 需要的计算结果，包括：
...
这条交接仍然没有决定每个 display layer 最终走 DEVICE 还是 CLIENT composition。CompositionEngine 与 HWC 要按目标 Output 的能力、几何、效果和资源约束继续协商。

## 8. 如何从 trace 判断问题在哪一段

先把“请求排队”“状态计算”“buffer latch”“display present”分开观察。
...
`TransactionQueue` 在 `queueTransaction()` 时增加，在 ready transaction 被 flush 后按数量减少。它持续升高只说明消费跟不上入队，下一步还要找：
...
如果第二组的 snapshot 更新显著增长，而 HWC/present 时长接近，证据更支持 hierarchy/snapshot 压力；如果两组 FrontEnd slice 接近但 fence 或 HWC 变长，瓶颈在下游。没有同机、同配置对照数据时，不给 FrontEnd 节省了多少毫秒这类结论。
...
| 版本 | 本章可以确认的边界 |
| --- | --- |
| Android 13 / API 33 | 公开 `android-13.0.0_r1` tag 中没有本章这组 `FrontEnd` 文件。旧资料通常围绕 `Layer` 内部状态与 latch 路径组织。 |
...
| Android 17 / API 37 | 本章锚点。以上函数名、`PostProcess` change、WAIT/SIGNAL barrier、fast path 和 legacy `Layer` 共存关系均按 `android-17.0.0_r1` 解释。 |

这些类位于 SurfaceFlinger 内部，不是稳定 SDK API。调试其他 Android 版本或厂商分支时，应回到对应 tag/commit，不能只按 API level 猜测内部实现。
...
- 对照素材：`Writer/rendering_pipelines/S01_rendering_types_overview.md`、`S03_surfaceview_type.md`、`S05_mixed_rendering_type.md`、`S06_multi_window_type.md`
