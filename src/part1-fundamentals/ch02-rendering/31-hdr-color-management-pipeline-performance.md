...
本章以 `android-17.0.0_r1` 为平台源码锚点。内核只负责 dma-buf 的内存共享与 dma-fence 的同步，不决定 dataspace、tone mapping 曲线或 HWC 的 plane 分配；涉及内核边界时，以 `android17-6.18-2026-06_r6` 为准。Buffer 与 fence 的细节分别见 2.15、2.16。
...
因此，“某个 Layer 是 HDR”与“显示当前运行在 HDR 输出模式”是两个不同事实。屏幕能力、可见 Layer 集合、强制 SDR 设置、镜像目标和 HWC 模式都会影响结果。
...
判断某台设备的结果，只看该帧 HWC validate 后的 composition type。
...
Android 17 源码里没有名为 `LinearTube` 的组件。实际类型是 `shaders::LinearEffect`。它把需要色彩处理的 Layer 组织成下面五步：
...
RenderEngine 对这些分支有互斥处理：已经应用 LUT、AGTM 或 local tone map 后，会跳过标准 `libtonemap` 的重复映射。描述 Android 17 时，应使用这些源码名称，不使用“Universal Tone Mapping”之类无法对应到类型、接口或 flag 的称呼。
...
`Display.getHdrSdrRatio()` 从 API 34 开始报告当前比值；环境光、热状态、面板限制和系统策略都可能让它变化。API 36 增加 `getHighestHdrSdrRatio()`，用于查询设备当前能报告的最高可能比值。
...
应用若要随实际 headroom 调整绘制，应查询 `Display.getHdrSdrRatio()`，并监听显示变化，而不是把某个 nits 数值写死。
...
HWC 通过 `DimmingStage` 告诉框架在何处 dim：
...
- 加载带 ICC profile 的 PNG、JPEG、WebP；
- 用 `Bitmap.getColorSpace()` 检查解码结果；
- 通过 EGL 扩展或 `VK_EXT_swapchain_colorspace` 输出 P3/scRGB。

请求不等于获得。应用还要检查 `Window.isWideColorGamut()`、`Display.isWideColorGamut()` 和当前配置。`Window.setColorMode()` 也明确说明：Window 色彩模式不影响独立的 SurfaceView 或 SurfaceControl。
...
带 profile 的 Bitmap 被绘制到不同目标色彩空间时，Skia/HWUI 会执行色彩转换。转换可能包括 EOTF/OETF、3×3 matrix、色域映射和精度转换，不能一律简化为一组 P3↔sRGB matrix。
...
应用应先确认图片有 gain map，再按可见范围决定是否请求 HDR，而不是只按文件扩展名切换。
...
Android 13 起，对于声明支持 HDR 播放的设备，HLG10 是最低要求之一；HDR10 用于专业内容播放。厂商可以增加 HDR10+ 或 Dolby Vision。Android 17 新增公开的 `HDR_TYPE_HLG_PLUS`，使用前仍要查询当前 display mode，不能按 API 37 直接假设设备支持。
...
这是显示系统的输出策略，不是普通应用给单个 Bitmap 选 tone mapping shader 的接口。外接电视、机顶盒和 HDMI 场景尤其要区分“内容是 HDR”与“链路被系统强制转换成某种 HDR 输出”。

---

## 九、Compose 中的色彩空间

Compose `Color` 从 1.0 起就能携带 `ColorSpace`，构造函数的 `colorSpace` 参数默认是 sRGB。`androidx.compose.ui.graphics.colorspace.ColorSpaces` 提供 Display P3、BT.2020、BT.2100 HLG 等空间，并能通过 connector 转换。原先“Compose Color 不携带 ColorSpace、1.6 才支持”的说法不成立。
...
颜色动画没有一个跨版本、跨 API 通用的额外开销比例。应确认插值在哪个空间执行、是否每帧分配 connector、参与动画的像素覆盖范围，再用基准测试判断是否值得缓存转换结果。
...
色彩场景变化后若 GPU 时间增加，先判断是不是从 `DEVICE` 改成 `CLIENT`，再分析 tone mapping shader。否则容易把 Layer 数量、模糊或几何限制引起的回退错算成 HDR 算法成本。
...
| 内容 | 文件 hash、HDR 格式、MaxCLL/MaxFALL、APL、帧率 |
...
发灰常见于 PQ/HLG 被按 SDR transfer 解释，或 tone mapping 使用了错误的内容峰值。先证明 metadata 链路，再调曲线。
...
| Android 17 / API 37 | `HDR_TYPE_HLG_PLUS`；本章源码锚点包含 LUT、AGTM、`LinearEffect`、`libtonemap` 与 local tone map 路径 |
...
沿这四个问题取证，才能区分格式带宽、GPU tone mapping、硬件 plane 竞争与面板亮度功耗。任何脱离设备能力和逐帧 composition result 的固定毫秒或固定百分比，都不适合作为 Android 17 的平台知识。
