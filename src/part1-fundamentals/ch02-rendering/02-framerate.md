...
打开 Perfetto 后，应用轨道里可能有一帧标红，显示轨道的刷新率又恰好从 120 Hz 切到 60 Hz。仅凭这两个现象，不能断定刷新率切换导致了卡顿。红色帧可能来自应用迟交、GPU 迟完成或 BufferQueue 积压；刷新率变化也可能只是内容投票或系统策略的正常结果。

分析帧率问题时，先把三个量分开：

- **应用帧率**：应用向某个 Surface 产出新帧的速率；
- **显示刷新率**：显示设备更新画面的速率；
- **呈现节拍**：连续显示帧在时间轴上的间隔是否均匀。

平台源码以 Android 17 / API 37 / `android-17.0.0_r1` 为基线，以下说明这些量如何进入 Choreographer、LayerHistory、RefreshRateSelector、FrameTimeline 和 HWC。涉及通用 DRM VBlank 边界时，内核基线为 `android17-6.18-2026-06_r6`。
...
这些数值描述相邻刷新机会的距离，不等于主线程、RenderThread 和 GPU 必须串行塞进同一个时间片。Android 图形管线允许不同阶段并行处理不同帧。目标帧仍需满足自身的 FrameTimeline 截止时间（deadline），但不能把各线程切片简单相加后与 8.33 ms 比较。
...
| 口径 | 起点与终点 | 能证明什么 |
| --- | --- | --- |
| `Choreographer#doFrame` 持续时间 | 主线程进入到退出该回调 | 主线程本轮帧回调的占用时间 |
| RenderThread `DrawFrame` 持续时间 | RenderThread 处理该任务的 CPU 区间 | 同步、准备、提交及可能的等待 |
| 应用 SurfaceFrame 实际持续时间 | 应用帧起点到 `max(GPU completion, buffer post)` | 应用侧是否按预期时间线完成 |
| DisplayFrame 实际持续时间 | SurfaceFlinger/显示帧的实际时间线 | 系统合成与显示端是否按计划完成 |

`doFrame` 不包含完整 GPU 和显示阶段；RenderThread CPU 持续时间也不能代替 GPU 持续时间。若文章、指标平台或测试报告只写“帧时间”，需要先追问它使用了哪一组时间戳。
...
45 FPS 内容若直接映射到 120 Hz，也需要在 2 个和 3 个刷新周期之间安排节拍。设备若同时支持 90 Hz，90 Hz 往往更适合 45 FPS；最终选择仍受其他 Layer、显示策略、无缝切换限制和设备能力影响。
...
是否属于卡顿，要结合该帧的预期时间线、实际时间线、显示类型（present type）和业务目标判断。
...
Android 11 为多刷新率（Multiple Refresh Rate，MRR）增加了专门的平台和 Composer HAL 2.4 支持。设备可以暴露多个显示配置，例如 1080p@60 Hz 和 1080p@120 Hz。

`CONFIG_GROUP` 用于标识哪些配置适合相互切换。同组通常表示除刷新率外的关键显示属性兼容，平台可以要求无缝切换（seamless switch）。是否能在某个时刻无缝切换仍由 HWC 返回结果决定，不能只凭“分辨率相同”下结论。

MRR 的特征是：选择结果可能要求从一个显示模式切换到另一个模式。

### 3.2 ARR：单个显示配置内按离散 VSync 步进更新

Android 15 引入 Adaptive Refresh Rate（ARR）。在支持 ARR 的配置中：

- 显示 VSync/TE 节拍与内容实际刷新节拍可以解耦；
- 面板在同一显示模式内按离散 VSync 步进选择呈现时机；
- 内容刷新率可以取 TE 速率允许的离散除数；
- 减少了仅为改变刷新率而切换显示模式的需求。

Composer3 的 `DisplayConfiguration.aidl` 与 `VrrConfig.aidl` 描述 `vsyncPeriod`、`minFrameIntervalNs` 等能力。特定显示配置的 `vrrConfig` 为空时，它按非 ARR 配置处理。应用不能仅凭设备采用 LTPO 面板就推断 ARR 已启用。
...
3. 跟踪数据或 dumpsys 显示当前配置和渲染帧率确有变化；
4. HWC/厂商实现按该配置提供相应能力。
...
Android 框架看到的是 HWC 报告的显示配置、模式组、VSync 周期与 ARR 配置。具体面板驱动、TE 和厂商功耗策略需要在目标设备上验证。

## 4. SurfaceFlinger 如何选择渲染帧率
...
显式 `setFrameRate()` 请求、内容提交节拍的启发式判断、View 类别、Game Mode 覆盖等信息都会影响投票。不可见或不活跃 Layer 不应与前台主内容等权处理。
...
Android 17 代码中，触摸与空闲信号都有提前返回的分支，但触发条件受显式投票、类别和策略影响。不能写成“任何触摸都会无条件拉满，停止后固定若干秒降频”。

### 4.4 24 FPS 视频与 60 FPS UI 的例子

假设屏幕上同时有：

- 一个获得焦点的 24 FPS 视频 Layer，使用固定源（fixed-source）语义；
- 一个持续更新的 60 FPS UI Layer；
- 候选渲染帧率为 60、90、120 Hz。

120 Hz 同时是 24 和 60 的整数倍，通常具有较好的节拍。但以下条件都可能改变结果：

- 120 Hz 不在当前策略范围；
- 固定源倍数阈值限制低帧率 Layer 对高档位的贡献；
- 某 Layer 只允许无缝切换；
- UI 没有持续更新或面积权重很低；
- 设备处于省电、热限制或其他策略状态；
- 当前显示配置只支持另一组渲染帧率。

源码能说明算法如何评分；具体设备在某一帧选了什么，需要跟踪数据或 dumpsys 证据。
...
`FIXED_SOURCE` 适合视频等固有帧率内容。`CHANGE_FRAME_RATE_ALWAYS` 允许非无缝切换，长视频可以在用户接受一次切换中断的前提下使用。短视频、普通 UI 和游戏通常不应照搬这组参数。

游戏或自建渲染器若以 60 FPS 为目标，通常使用默认兼容性（default compatibility），并自行控制帧节拍：
...
该调用只提供投票。游戏仍要控制逻辑节拍、GPU 在途帧数量和呈现时间。
...
类别由平台和显示配置解释，HIGH 不等于所有设备固定 120 Hz。对于普通 View 动画，类别往往比硬编码某个赫兹值更能适配不同设备。
...
Android 13 / API 33 的 `Choreographer.FrameData` 可以提供多个候选时间线。公开的 `SurfaceControl.Transaction.setFrameTimeline(long vsyncId)` 在 Android 15 / API 35 加入 SDK：
...
选择某个 vsyncId 只是告诉 SurfaceFlinger 期望的呈现目标；获取栅栏、事务顺序和截止时间仍需满足。普通 View/HWUI 窗口由框架维护时间线，业务代码通常不需要自行设置。

### 5.6 `preferredDisplayModeId` 的使用边界

`WindowManager.LayoutParams.preferredDisplayModeId` 指定包含分辨率、刷新率等属性的显示模式。它比帧率提示更容易触发模式变化，适合明确知道目标模式且能处理切换影响的场景，例如部分电视播放。

只需要表达内容帧率时，优先使用 Surface/View 的帧率 API，让系统结合其他 Layer 和策略选择。

## 6. Choreographer 与帧节拍控制
...
同一轮多次 `invalidate()` 通常汇入一个待处理帧。没有更新需求时，应用也不必在每个显示 VSync 执行一次完整 `doFrame()`。

因此，用相邻 `doFrame` 的起点间隔统计“显示 FPS”并不可靠：

- 静态窗口可能没有对应回调；
- 回调起点会受主线程调度影响；
- SurfaceView、视频或游戏主体可能由另一个生产者出帧；
- 回调完成后仍有 RenderThread、GPU、BufferQueue 和显示阶段。

`doFrame` 适合观察主线程帧调度，呈现结果应看 FrameTimeline 或显示侧时间戳。

### 6.2 FrameData 提供多个候选时间线
...
`getPreferredFrameTimeline()` 是平台建议值。自建 SurfaceControl 事务可以选另一个可满足的时间线，再通过 API 35 的 `setFrameTimeline()` 提交。选择更晚时间线会改变目标呈现时机，但不能减少渲染工作本身。

### 6.3 帧节拍控制的是呈现节拍和队列深度

帧节拍控制（frame pacing）的目标包括：
...
“每次收到 VSync 就立即渲染”只覆盖起帧节奏。若生产者总是尽快提交，BufferQueue 可能积压，输入延迟仍会升高。

### 6.4 Swappy 适合游戏与自建渲染循环

AGDK 帧节拍库（Frame Pacing Library，Swappy）支持 OpenGL ES 和 Vulkan 游戏。它结合：
...
Swappy 可以主动等待以限制队列深度。跟踪数据中看到交换或栅栏等待时，要先判断它是合理节拍控制、GPU 依赖还是被动背压，不能一律当成性能浪费。

### 6.5 缓冲区堆积会维持吞吐量却增加延迟

生产者连续提交速度高于显示消费速度时，多个缓冲区可能在队列中等待。表现可能是：
...
Android 17 的 Choreographer/HWUI 还包含缓冲区堆积恢复机制。看到恢复跟踪时，应同时检查队列深度、主动延迟和后续积压是否回落。

### 6.6 Android 17 的生产者节流

API 37 增加 `Surface.setProducerThrottlingEnabled(boolean)`，用于控制 Vulkan/EGL 生产者的 CPU 节流。默认机制会在消费者仍处理前一缓冲区时给生产者施加背压。

这个 API 不负责选择刷新率，也不能代替帧节拍控制。官方建议 Vulkan 生产者在具备正确显式同步时关闭默认节流，避免把 `vkPresentKHR()` 的 CPU 停顿当成生产者/消费者同步；队列耗尽时的自然出队背压仍会发生。异步模式下节流始终启用，此开关没有效果。
...
对应用 SurfaceFrame，实际结束时间覆盖应用提交和 GPU 完成边界；对 SurfaceFlinger DisplayFrame，实际时间线覆盖系统合成与显示呈现。应用侧实际时间线按时，只能证明生产者侧达标，不能证明 DisplayFrame 一定按时。
...
### 7.4 卡顿不等于“持续时间大于一个刷新周期”

帧截止时间由当前调度和时间线决定。以下情况都可能产生不同分类：
...
固定拿 16.67 ms 或 8.33 ms 与任意切片比较，会在高刷、ARR、管线重叠和调度相位场景中误判。

### 7.5 错过目标帧与丢帧要描述具体对象

“错过目标帧（missed frame）”可以表示错过目标截止时间，也可以泛指显示端没有更新新内容。“丢帧（dropped frame）”在 FrameTimeline 有具体分类：SurfaceFlinger 可能选择更新的帧，应用侧也可能没有及时把 UI 状态推给 RenderThread。

不要写“所有错过目标帧都属于卡顿”这类集合关系。可操作的表述是：
...
| 字段 | 语义 |
| --- | --- |
| `TOTAL_DURATION` | 帧从开始到渲染完成并发送给显示子系统的总时长 |
| `DEADLINE` | 系统分配给应用生产该帧的时间，API 31+ |
| `GPU_DURATION` | 该应用帧的 GPU 完成时间，API 31+ |
| `LAYOUT_MEASURE_DURATION` | 失效 View 层级的测量/布局时间 |
| `DRAW_DURATION` | 绘制回调阶段 |
...
API 31 及以上可用 `TOTAL_DURATION < DEADLINE` 判断应用是否满足其生产截止时间。`TOTAL_DURATION` 不是屏幕呈现时间，各阶段还可能并行，因此它不一定等于所有持续时间字段之和。

监听回调可能因处理线程繁忙而丢失中间通知，`dropCountSinceLastInvocation` 表达的是监听器通知丢失数量，不是屏幕掉帧数量。

### 8.2 JankStats

JankStats 为不同 API 级别封装帧时序并附加 UI 状态。默认启发式倍数（heuristic multiplier）为 2，但这是库的报告阈值，不是系统 FrameTimeline 的卡顿定义。
...
查询结果先按 `layer_name` 区分应用窗口、SurfaceView、视频或其他 Surface。再用帧标识和流程找到对应预期帧与 SurfaceFlinger DisplayFrame。表结构可能随 Perfetto 版本演进，执行前应查看当前 Trace Processor 数据表结构。
...
若主体是 SurfaceView、视频或游戏 Surface，应跟踪它自己的 Layer，不能只看宿主应用窗口。
...
| 120 Hz 下每个线程都必须在 8.33 ms 内结束 | 以目标帧截止时间和跨线程依赖为准，保留管线重叠 |
| 60 FPS 在 120 Hz 一定浪费一半 GPU | 应用只需生产 60 帧；显示重复不要求 GPU 重画同一帧 |
| 高刷新率一定更流畅 | 检查应用是否按时、呈现节拍、输入延迟和队列深度 |
| LTPO 等于 Android ARR | 查询 `hasArrSupport()`、HWC 配置和跟踪数据 |
| 1–120 Hz LTPO 可以取任意连续值 | 查看设备报告的离散能力和限制条件 |
| `setFrameRate(120)` 会把屏幕锁到 120 Hz | 它是 Layer 投票，最终受策略、其他 Layer 和设备能力影响 |
| `Display.getRefreshRate()` 不等于请求值，所以发生覆盖 | 可能是正常选模、策略、其他 Layer 或 ARR；需要系统侧证据 |
| 游戏模式 PERFORMANCE 会锁高频 | 游戏模式表达用户选择，游戏和 OEM 干预共同决定策略 |
| FrameMetrics `TOTAL_DURATION` 是触摸到屏幕延迟 | 它到发送给显示子系统为止，不包含完整输入与面板边界 |
| `DrawFrame` 很长就说明 GPU 慢 | 区分 CPU 工作、出队/栅栏等待、GPU 完成时间 |
| 平均 FPS 达标就没有卡顿 | 同时看分位数、长帧簇、显示间隔和高延迟状态 |

## 11. 游戏模式与 120 Hz 功耗

### 11.1 游戏模式是协作接口

游戏模式 API 在部分 Android 12 设备提供，Android 13 及以上设备支持更完整。`PERFORMANCE` 与 `BATTERY` 让游戏针对低延迟/帧率或续航调整自身策略。
...
游戏应在 `onResume()` 查询当前模式，并根据自身能力选择分辨率、画质、目标 FPS 和节拍控制。测试时还要记录 OEM 干预是否生效。
...
一个游戏若只能在 120 FPS 与 80 FPS 之间持续波动，稳定 90 FPS 或 60 FPS 可能提供更均匀的呈现节拍、更低的队列压力和更好的持续性能。判断目标值时同时看：
...
Swappy 或引擎节拍控制应根据这些数据选择交换间隔，而不是在每帧临时追随瞬时耗时。

## 12. 内核与厂商显示边界

框架的 RefreshRateSelector 负责投票与策略，Composer HAL 把所选配置或显示时机交给设备实现。再往下可能涉及厂商显示驱动、面板 TE、DRM/KMS VBlank、时钟和电源管理。
...
- 面板的最低刷新率、TE 与无缝切换能力必须查设备配置和厂商跟踪。
...
| Android 13 / API 33 | `Choreographer.FrameData` / `FrameTimeline` 公开多个候选时间线 |
...
| Android 17 / API 37 | `Display.getFrameRateVelocityMapping()`；`Surface` 生产者节流控制；RefreshRateSelector 现行九类投票与 ARR/MRR 分支按 `android-17.0.0_r1` 解读 |

当前源码能证明 Android 17 的实现状态，不能自动证明某段逻辑是 Android 17 首次加入。涉及“从某版本开始”的结论，应同时检查 API 级别、旧标签或官方版本说明。
...
1. FPS 是吞吐量，显示间隔描述视觉节拍，输入到显示的延迟描述交互延迟；
2. `doFrame`、RenderThread、SurfaceFrame 和 DisplayFrame 各有不同的帧时间口径；
3. MRR 在显示模式之间切换，ARR 在支持的配置内按离散 VSync 步进改变刷新节拍，LTPO 只是可能提供这些能力的面板技术；
4. Android 17 通过 LayerHistory 汇总 Layer 投票，再由 RefreshRateSelector 在策略允许的候选中评分；
5. Surface、View、Window 和 SurfaceControl API 都是提示或目标表达，不能代替应用自身的帧节拍控制；
6. 卡顿结论应来自目标 SurfaceFrame、DisplayFrame、帧标识、截止时间、栅栏和显示证据；
7. 平均 FPS、单条 `doFrame`、单次刷新率切换都不足以独立解释卡顿。

排查时先锁定目标 Surface 和 FrameTimeline，再判断应用、队列、SurfaceFlinger 与显示端谁先错过时序。刷新率变化需要放回 Layer 投票、显示策略与选模结果中解释。
