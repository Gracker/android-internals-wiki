...
本章以 Android 17 / API 37 / `android-17.0.0_r1` 为平台源码基线，说明这些量如何进入 Choreographer、LayerHistory、RefreshRateSelector、FrameTimeline 和 HWC。涉及通用 DRM VBlank 边界时，kernel 基线为 `android17-6.18-2026-06_r6`。
...
| Present FPS | 显示端实际呈现的新帧 | 用户看到的新内容更新速度如何 |
| Display refresh rate | 显示扫描或更新频率 | 显示设备当前以什么节拍工作 |
...
24 FPS 内容放在 60 Hz 显示上，常见做法是 3:2 pulldown：有的内容帧保持 3 个刷新周期，有的保持 2 个。内容播放速度可以正确，但帧保持时长交替，平移镜头中容易看到 judder。
...
是否属于 jank，要结合该帧的 expected timeline、actual timeline、present type 和业务目标判断。
...
Android 11 为 Multiple Refresh Rate（MRR）增加了专门的平台和 Composer HAL 2.4 支持。设备可以暴露多个 display config，例如 1080p@60 Hz 和 1080p@120 Hz。

`CONFIG_GROUP` 用于标识哪些配置适合相互切换。同组通常表示除刷新率外的关键显示属性兼容，平台可以要求 seamless 切换。是否能在某个时刻无缝切换仍由 HWC 返回结果决定，不能只凭“分辨率相同”下结论。

MRR 的核心特征是：选择结果可能要求从一个 display mode 切换到另一个 mode。
...
Variable Refresh Rate（VRR）描述显示硬件可以改变刷新节拍的能力。AOSP ARR 代码中也会使用 `vrr` 命名，但硬件市场中的 VRR、Composer3 `VrrConfig` 和 Android 的 ARR policy 不应视为完全相同的概念。
...
## 4. SurfaceFlinger 怎样选择 render rate
...
这条路径说明“应用请求 120 FPS”和“显示最终运行在 120 Hz”之间还有多个决策层。
...
- 一个 focused 的 24 FPS 视频 Layer，使用 fixed-source 语义；
...
游戏或自建渲染器若以 60 FPS 为目标，通常使用 default compatibility，并自行 pacing：
...
这段代码用于说明 API 的关联关系。选择某个 vsyncId 只是告诉 SurfaceFlinger期望的 presentation target；acquire fence、事务顺序和 deadline 仍需满足。普通 View/HWUI 窗口由框架维护 timeline，业务代码通常不需要自行设置。
...
AGDK Frame Pacing Library（Swappy）支持 OpenGL ES 和 Vulkan 游戏。它结合：
...
Expected timeline 表示调度器给该帧的预期时间窗口，Actual timeline 表示该帧的实际执行/呈现结果。

对应用 SurfaceFrame，actual end 覆盖应用提交和 GPU 完成边界；对 SurfaceFlinger DisplayFrame，actual timeline覆盖系统合成与显示呈现。应用 actual 按时只能证明 producer 侧达标，不能证明 DisplayFrame 一定按时。
...
| 蓝色 | dropped frame |
| 浅绿色 | high-latency state，节拍可能稳定但整体呈现偏晚 |

颜色是 UI 辅助。最终结论应读取 `jank_type`、`present_type`、`on_time_finish`、Layer 名称和对应 flow。
...
“missed frame”可以表示错过目标 deadline，也可以泛指显示端没有更新新内容。“dropped frame”在 FrameTimeline 有具体分类：SurfaceFlinger 可能选择更新的帧，应用侧也可能没有及时把 UI 状态推给 RenderThread。

因此，不要写“所有 missed 都属于 janky”这类集合关系。更可操作的表述是：
...
API 31 及以上可用 `TOTAL_DURATION < DEADLINE` 判断应用是否满足其生产 deadline。`TOTAL_DURATION` 不是屏幕 present 时间，各阶段还可能并行，所以它不一定等于所有 duration 字段之和。

监听回调可能因处理线程繁忙而丢失中间通知，`dropCountSinceLastInvocation` 表达的是 listener 通知丢失数量，不是屏幕掉帧数量。

### 8.2 JankStats

JankStats 为不同 API 级别封装 frame timing 并附加 UI 状态。默认 heuristic multiplier 为 2，但这是库的报告阈值，不是系统 FrameTimeline 的 jank 定义。
...
先用下面的查询列出目标进程的 actual SurfaceFrame，而不是用 `doFrame` 数量代替帧数：
...
当前源码能证明 Android 17 的实现状态，不能自动证明某段逻辑是 Android 17 首次加入。涉及“从某版本开始”的结论，应同时检查 API level、旧 tag 或官方版本文档。
...
帧率问题的核心不在“FPS 越高越好”，而在于 producer、SurfaceFlinger 和显示设备能否以一致的目标节拍协作：
...
排查时先锁定目标 Surface 和 FrameTimeline，再判断应用、队列、SurfaceFlinger 与显示端谁先错过时序。刷新率变化只有放回 Layer vote、display policy 与选模结果中，才具有可解释性。
