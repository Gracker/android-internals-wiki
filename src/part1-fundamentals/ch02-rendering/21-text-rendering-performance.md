...
sources:
- type: aosp
  url: https://android.googlesource.com/platform/frameworks/base/+/refs/tags/android-17.0.0_r1/core/java/android/widget/TextView.java
- type: aosp
  url: https://android.googlesource.com/platform/frameworks/base/+/refs/tags/android-17.0.0_r1/core/java/android/view/View.java
- type: aosp
  url: https://android.googlesource.com/platform/frameworks/base/+/refs/tags/android-17.0.0_r1/core/java/android/view/ViewRootImpl.java
- type: aosp
  url: https://android.googlesource.com/platform/frameworks/base/+/refs/tags/android-17.0.0_r1/core/java/android/text/BoringLayout.java
- type: aosp
  url: https://android.googlesource.com/platform/frameworks/base/+/refs/tags/android-17.0.0_r1/core/java/android/text/StaticLayout.java
- type: aosp
  url: https://android.googlesource.com/platform/frameworks/base/+/refs/tags/android-17.0.0_r1/core/java/android/text/DynamicLayout.java
- type: aosp
  url: https://android.googlesource.com/platform/frameworks/base/+/refs/tags/android-17.0.0_r1/core/java/android/text/PrecomputedText.java
- type: aosp
  url: https://android.googlesource.com/platform/frameworks/base/+/refs/tags/android-17.0.0_r1/core/java/android/text/MeasuredParagraph.java
- type: aosp
  url: https://android.googlesource.com/platform/frameworks/base/+/refs/tags/android-17.0.0_r1/graphics/java/android/graphics/text/MeasuredText.java
- type: aosp
  url: https://android.googlesource.com/platform/frameworks/base/+/refs/tags/android-17.0.0_r1/libs/hwui/SkiaCanvas.cpp
- type: aosp
  url: https://android.googlesource.com/platform/frameworks/minikin/+/refs/tags/android-17.0.0_r1/include/minikin/LayoutCache.h
- type: aosp
  url: https://android.googlesource.com/platform/frameworks/minikin/+/refs/tags/android-17.0.0_r1/libs/minikin/LayoutCore.cpp
- type: aosp
  url: https://android.googlesource.com/platform/external/harfbuzz_ng/+/refs/tags/android-17.0.0_r1/meson.build
- type: aosp
  url: https://android.googlesource.com/platform/external/skia/+/refs/tags/android-17.0.0_r1/src/text/gpu/StrikeCache.cpp
- type: official
  url: https://developer.android.com/reference/android/text/PrecomputedText
- type: official
  url: https://developer.android.com/reference/android/text/StaticLayout.Builder
- type: official
  url: https://developer.android.com/develop/ui/views/text-and-emoji/emoji2
- type: official
  url: https://perfetto.dev/docs/learning-more/android
- type: material
...
文字相关卡顿不能只归到 `TextView.onMeasure()`。一段文字从业务数据变成屏幕像素，至少会经过内容处理、段落测量、换行、View 绘制命令录制、Skia 字形资源准备和窗口 buffer 提交。它们分布在不同线程，也受不同缓存约束。

排查时先回答两个问题：

1. 本帧为什么需要重新排版或重录文字？
2. 最早超时的是主线程、RenderThread/GPU，还是后面的窗口显示链路？

这两个问题能避免两种常见误判：列表正在滚动，不代表每个可见 `TextView` 每帧都重新 measure；RenderThread 上出现文字相关开销，也不能反推主线程一定重新做了 shaping。

## 先按线程和产物拆开

Android 17 上，普通 View 页面仍属于标准 HWUI App Window 路径。结合 `rendering_pipelines` 的 S01/S02，可以把文字工作放进下面五层：
...
`TextView.makeNewLayout()` 最终调用 `makeSingleLayout()`。Android 17 的选择顺序不是“单行用 Boring、多行用 Static、EditText 用 Dynamic”这么简单。
...
因此，`DynamicLayout` 不只服务于 `EditText`。可选择的普通 `TextView` 或需要监听 Span 变化的文本也可能使用它。它通过 `reflow()` 更新受编辑影响的区域，并维护供硬件加速绘制使用的 block 信息；“增量”只表示不必无条件重建全部文本，不保证每次编辑成本都很小。
...
Android 17 的 Minikin `LayoutPiece` 先调用 `FontCollection.itemize()` 把输入分成字体 run，再对 script run 调用 `hb_shape()`。当前 `android-17.0.0_r1` 的 `external/harfbuzz_ng` 对应 HarfBuzz 11.4.1。
...
自动连字符只有在 break strategy 不是 `BREAK_STRATEGY_SIMPLE`，且 hyphenation frequency 不是 `NONE` 时才进入对应测量路径。Android 17 的 `TextView` 注释还保留了版本边界：

- Android 10 之前，主题默认可能是 `HYPHENATION_FREQUENCY_NORMAL`；
- Android 10 起，`TextView` 和 `EditText` 的主题默认改为 `NONE`；
- 自建 `PrecomputedText.Params.Builder` 的默认值与 `TextView` 未必相同，优先从目标 View 取得 `getTextMetricsParams()`。

优化前先读取实际 `breakStrategy` 和 `hyphenationFrequency`。在默认已经为 `NONE` 的设备上重复设置，并不会减少排版工作。
...
可用宽度不在这个 shaping cache key 中。因此：
...
`TextLine` 还有一个容量为 3 的静态对象池，用于减少临时对象分配。它缓存的是可复用对象，不是排版结果。Skia 的 `StrikeCache` 则管理 GPU 文字 strike/glyph 资源；它和 Minikin shaping cache 属于不同阶段，不能合并成一个“文字缓存命中率”。
...
AndroidX 默认 initializer 会把字体加载推迟到首个 Activity resume 之后，避免与首屏直接争用；手动配置时也要把下载、初始化和首屏绘制分别计时。

## 优化时优先减少无效重建

### `PrecomputedText` 预计算的是段落测量，不包含最终宽度
...
这段代码只演示预计算结果的所有权。实际项目还要取消无用任务、限制队列长度，并按 API 版本选择 platform 或 AndroidX 实现。`TextView.setText(PrecomputedText)` 遇到不兼容的测量参数会抛出 `IllegalArgumentException`；只出现可重算的方向差异时，framework 也可能重新计算。

AndroidX 的 `AppCompatTextView.setTextFuture()` 会在 `onMeasure()` 里调用 `future.get()`。如果后台任务尚未完成，主线程仍会阻塞等待。它适合提前 prefetch，不是“调用后主线程一定没有文字成本”的保证。
...
API 35 增加了 bounds-for-width 与 start-overhang 相关 API。Android 17 的 `TextView` 对 target SDK 35 及以上默认启用 `useBoundsForWidth`；`shiftDrawingOffsetForStartOverhang` 默认仍为 false，并且只有前者启用时才生效。自建 `StaticLayout.Builder` 的默认值要按 Builder 文档单独确认。
...
先用 FrameTimeline 锁定 jank frame，再按下面顺序判断：
...
窗口级 `measure` 无法定位到某个控件时，可以在应用可控的边界补少量 trace。下面的标记用于区分业务富文本构造和 `setText()`，不要在每个字符或每个 Span 上打点。
...
文字缓存会让一次 warm run 掩盖冷启动成本。可复用的测试对照表是：
...
| Android 17 / API 37 | 本文源码锚点；Minikin 当前 layout/variation cache、HarfBuzz 11.4.1、HWUI/Skia text blob 路径 | 当前方法名和缓存 key 按 `android-17.0.0_r1` 解读；是否为 Android 17 新增需另查历史 tag |
...
`onMeasure()` 在主线程；RenderThread 处理 DisplayList、Skia/GPU 和窗口 buffer。两边可能因同一次文本变化同时增加工作，但必须分别取证。
...
- Writer `rendering_pipelines`：`S01_rendering_types_overview.md`、`S02_aosp_standard_type.md`
