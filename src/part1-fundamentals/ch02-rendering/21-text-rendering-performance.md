...
文字相关卡顿不能只归到 `TextView.onMeasure()`。一段文字从业务数据变成屏幕像素，至少会经过内容处理、段落测量、换行、View 绘制命令录制、Skia 字形资源准备和窗口缓冲区提交。这些工作分布在不同线程，也受不同缓存约束。
...
Minikin 的 `WordBreaker` 使用 ICU 断行迭代器提供候选断点，`LineBreaker` 再按当前宽度和断行策略选择实际行断点。CJK、泰文、URL、表情序列等文本使用的规则不同；CJK 并非统一依赖字典查找。

只有断行策略不是 `BREAK_STRATEGY_SIMPLE`，且连字符频率不是 `NONE` 时，自动连字符才进入对应的测量路径。Android 17 的 `TextView` 注释还保留了版本边界：
...
评估富文本时，应统计会改变测量指标的 Span 边界、替换项数量和段落级 Span，不能把所有 Span 的数量直接换算成排版耗时。

### 系统 emoji 与 EmojiCompat 是两条路径

系统表情通过系统或厂商字体参与字体回退、文字整形和 Skia 绘制。EmojiCompat/emoji2 则先把需要兼容的序列处理成带 `EmojiSpan` 的 `Spanned`；当前 AndroidX 实现中，`TypefaceEmojiSpan` 最终切换到表情字体并调用 `Canvas.drawText()`，并不会在每次绘制前解码位图。
...
AndroidX 默认初始化器会把字体加载推迟到首个 Activity 恢复之后，避免直接与首屏争用资源；手动配置时也要分别测量下载、初始化和首屏绘制。
...
这段代码只演示预计算结果的所有权。实际项目还要取消无用任务、限制队列长度，并按 API 版本选择平台或 AndroidX 实现。`TextView.setText(PrecomputedText)` 遇到不兼容的测量参数会抛出 `IllegalArgumentException`；只有可重新计算的方向差异时，框架也可能重新计算。
...
1. 用载荷或内容差异比较，避免对未变化字段重复调用 `setText()`；
2. 固定列表项的文字宽度约束，避免动画期间反复改变可用宽度；
...
`includeFontPadding` 决定首尾行使用字体顶部/底部还是上升部/下降部。关闭后布局可能更紧凑，但不能据此宣称测量更快；对阿拉伯文、卡纳达文或带高低延伸的字体，还要验证是否发生裁切。
...
这些选项用于修正字形步进宽度与字形边界不一致造成的裁切和对齐。它们会影响宽度、断行或绘制偏移，开启前应做视觉回归和基准测试，不应给出脱离字体与文本的固定成本结论。

### 可变字体动画要检查轴值是否稳定

Android 17 的 Minikin `Font.cpp` 会缓存调整后的 HarfBuzz 字体和 Typeface；同时，变体设置也是 `LayoutCacheKey` 的一部分。重复使用相同的轴值组合可能复用对象和文字整形结果，连续使用大量不同轴值则会产生不同的缓存键。

字重动画若每帧修改变体设置，可能同时带来排版、DisplayList 重录和字形资源变化。应使用实际动画范围做冷、热两组测量，再决定是否降低更新频率、量化轴值，或改用不改变文字测量指标的视觉方案。
...
文字缓存可能让一次热运行掩盖冷启动成本。可复用的测试对照如下：
...
只有改变测量指标、替换内容或段落布局的 Span 会直接改变对应排版阶段。颜色和点击 Span 的主要成本在绘制或交互，不应与 `MetricAffectingSpan` 混算。
...
`onMeasure()` 在主线程执行；RenderThread 处理 DisplayList、Skia/GPU 和窗口缓冲区。两边可能因同一次文本变化同时增加工作，但必须分别取证。
