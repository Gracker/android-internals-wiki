...
本章以 `android-17.0.0_r1` 为平台源码锚点。内核侧以 `android17-6.18-2026-06_r6` 为锚点：dma-buf 管共享内存对象与 fd 生命周期，dma-fence/sync_file 管访问时序；内核不知道 BufferQueue 的 slot 编号，也不负责 slot 选择。
...
若 stride 无意义或计算可能溢出，代码会回退到输入 width。dump 自己也明确写着：
...
在 `BQ_EXTENDEDALLOCATE` flag 生效的构建中，slot 保存的 additional-options generation id 与队列当前 generation 不一致，也会要求重分配。
...
分配时暂时放开核心锁，避免一次慢 allocator 调用把所有队列状态操作都压在同一把锁后面。代码仍使用 `mIsAllocating` 对同一队列的相关分配进行协调。

### 4.4 `requestBuffer()` 是 slot 映射握手
...
这也解释了一个常见误判：`mFreeBuffers` 中有 slot，不代表下一帧一定立即开始写；有 buffer 可复用与访问安全是两项条件。
...
这说明 slot 编号本身没有跨 disconnect 的永久身份。重连后，即使数字相同，双方也要重新建立映射。
...
- 只拥有 import 后的 handle，析构时走 Mapper `freeBuffer()`；
- 只包装外部 handle，不取得所有权。
...
选项用于“不改变总体 usage、但会影响分配方式”的扩展信息，AIDL 文档给出的例子是 surface compression level。实现必须拒绝无法识别的选项。flag 在源码存在不代表任意量产设备都启用，也不能把 protected content 之类已有明确 usage 语义的能力随意归入该数组。
...
3. 是否进入 `GraphicBufferAllocator::allocate` / Gralloc Binder 调用；
...
内核数据不会告诉你“这是 BufferQueue slot 3”。要用 handle、pid、时间戳、Layer/queue 名和尺寸把用户空间事件与内核对象关联。更完整的 dma-buf 与 fence 说明见 2.15、2.16。
...
每阶段同时记录目标 BufferQueue dump、进程 meminfo、gralloc trace 和对象生命周期。若 slot/buffer 数已回落而 mtrack 不降，继续查 GPU import 或厂商层；若目标队列仍存在，先找仍持有 Surface/Consumer 的用户空间对象。
...
优化目标应是减少无必要的规格变化和生命周期抖动。不要缓存已失去所有权的 raw handle，也不要绕过 fence 强行复用。

---

## 十一、版本与源码边界

本章的行为断言以 Android 17 / API 37 / `android-17.0.0_r1` 为准。Android 8 到 Android 16 可沿用 Producer/Consumer、slot cache 与 fence 的基本模型，但具体接口、flag、BLAST 组织方式、allocator/mapper 版本和 trace 名可能不同。
...
本章不再给每个机制标注未经 git history 验证的“首次引入版本”。需要版本考古时，以对应 tag 的文件与提交历史为证据。
