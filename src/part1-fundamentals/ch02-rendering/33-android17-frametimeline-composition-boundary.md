...
FrameTimeline 回答的是一帧有没有按调度器预测的时间显示，以及延误更接近应用、SurfaceFlinger、GPU composition 还是 Display HAL。它不是一张覆盖所有 Producer 的万能流水线图，也不能单独证明某段 CPU 或 GPU 工作就是根因。

本文以 Android 17 / API 37 的 `android-17.0.0_r1` 为平台锚点，以 `android17-6.18-2026-06_r6` 为内核同步锚点。分析时采用下面这条公共显示主线：
...
这条主线把“应用提交”“SurfaceFlinger 采纳”“合成”“显示反馈”和“buffer 复用”分成了不同边界。`queueBuffer()` 返回只代表提交动作完成，不能当作上屏时间。

## 先确认 FrameTimeline 覆盖了哪条出图路径

FrameTimeline 对标准 App Window 最有解释力。页面包含独立 Surface、外部流或多个 Producer 时，应先画出 layer 与 BufferQueue 拓扑，再决定 FrameTimeline 能证明哪一段。
...
- FrameTimeline 没有 App slice，不等于应用没有提交 buffer；
...
`FrameTimelineInfo.aidl` 中还包含 `startTimeNanos`、`vsyncResyncedJitterNanos` 与 `dequeueBufferDurationNanos`。这些字段说明 token 不只是一个 UI 标签，它还携带分类所需的应用侧时序信息。
...
SurfaceFrame 的 Actual slice 结束于上述 ready 边界，不延伸到最终上屏；`present_type` 与 `jank_type` 则会在 DisplayFrame 获得 present 反馈后回填分类结果。不能从 slice 右边界直接读取屏幕更新时间。
...
- SF CPU end 由 `setSfPresent()` 调用时的时间保存；
...
FrameTimeline 的 jank 定义围绕 predicted present 与 actual present 是否匹配。单独用 `Actual Display Time - Expected Display Time > 0` 作为所有场景的“用户可见卡顿”判据，会把高延迟平滑状态、预测误差、模式切换和无效预测混在一起。
...
| `QUEUE` | 与上项同一条件，时间戳使用 transaction 的 `postTime` | 该 buffer update 进入 SF transaction 的时间锚点 |
| `ACQUIRE_FENCE` | `latchBufferStatsAndHandles()` 追踪 buffer acquire fence | fence span 的终点是 signal time；表示 Consumer 可安全读取 |
| `LATCH` | SF 采纳该 buffer | queue 与 latch 之间可包含 transaction readiness、fence 与调度等待 |
...
`POST`、`HWC_COMPOSITION_QUEUED`、`RELEASE_FENCE` 等枚举在这组 Android 17 SurfaceFlinger 生产调用点中没有活跃发射点。分析脚本不应要求每帧出现完整枚举序列，也不要因为缺少 `RELEASE_FENCE` 事件就断言 buffer 已经或尚未可复用。

`android.surfaceflinger.frame` 数据源承载这些 `GraphicsFrameEvent`；`android.surfaceflinger.frametimeline` 承载 SurfaceFrame/DisplayFrame。两者用途不同，需要按 layer、buffer id、frame number 与时间邻近关系联合观察。
...
因此，SurfaceFrame 的 `gpu_composition = true` 表示对应 output layer 在这一轮需要 client composition。`FALLBACK_COMPOSITION` 是 proto 的历史事件名，不能扩写成“原本一定可以由 HWC 合成，后来因异常退回 GPU”。
...
把这组标志称为 CLIENT/DEVICE 合成边界，比“GPU/CPU 合成边界”更准确。CPU 调度与控制工作贯穿两条路径，差别在于 SurfaceFlinger 是否用 RenderEngine 生成 client target。

## Jank 分类与颜色

### Android 17 的主要分类

`frame_timeline_event.proto` 把 `jank_type` 定义为 bitmask，一帧可以同时具有多个原因。工程分析常用的分类如下：
...
| `SurfaceFlinger CPU Deadline Missed` | SF 的 ready deadline 迟到，且没有 client composition GPU fence 作为 GPU 分支依据 | SF 主线程运行与调度、HWC 阻塞调用 |
...
| `Prediction Error` | predicted present 与显示反馈的偏差不符合当前预测 | VSync 预测、刷新率/模式切换；孤立样本不宜放大 |
...
proto 同时携带 legacy 与 experimental 的 jank/present 值，并明确标注 experimental 字段不用于 jank 分析。报告应以 Perfetto 公开的 `jank_type`、`present_type`、`prediction_type` 为主，实验字段只用于平台开发调试。
...
颜色适合定位候选帧，根因仍应由字段、token flow、线程 slice、buffer 与 fence 共同证明。
...
Android 用户态通过 `Fence`/`FenceTime` 与 sync file 传递这些同步对象；指定内核锚点中的 `dma_fence` 提供 signal、wait 与 callback 基础。FrameTimeline 读取的是用户态和 HAL 传回的时间，不是直接把某个内核函数耗时当作 jank 根因。

排查 buffer starvation 时，应观察 `dequeueBuffer` 等待、可用 slot、pending buffer、release callback/fence 与前序帧的 present。把“当前 Actual end 到下一帧 Expected start”的差当作 release fence 时间，会混淆两个不同对象。
...
Perfetto UI 的 Android preset 通常会启用 FrameTimeline。需要可复现的命令行配置时，下面的配置同时收集两类 SurfaceFlinger 数据和 App/SF 常用 atrace：
...
把 `com.example.app` 替换成目标包名。`android.surfaceflinger.frametimeline` 提供 Expected/Actual 与 jank 分类，`android.surfaceflinger.frame` 提供 buffer/fence 事件，`gfx`/`view`/`sched` 用于解释应用与线程调度。
...
结果中的 `dur` 是应用 ready 区间，`present_type` 是后来根据显示反馈完成的分类。相同 `app_token` 出现多行时，先看 process 与 layer，不要立即去重。

### 把 App 与 SurfaceFlinger 按 DisplayFrame 关联
...
一个 SF token 关联多条 App 行是正常现象：一次显示更新可以合成多个 layer。SF 行缺失时，应检查 token 是否无效、预测是否过期、trace 是否从帧中途开始，以及目标是否属于 FrameTimeline 覆盖有限的独立 Surface。
...
SurfaceFrame `gpu_composition = false` 只表示该 layer 没被 SF 放入 client composition，不能排除 HWUI 生产 buffer 时的 GPU 延迟。

### SurfaceView 或游戏：画面节奏不稳

不要假定 Native 引擎一定把 `Choreographer.FrameData` 正确传到了目标 Surface。先验证 token 是否存在，再沿独立 layer 检查：
...
这条序列用来定位“逻辑 tick 晚、GPU 完成晚、buffer 到达晚、SF 没采纳、显示后段晚”中的哪一段。buffer 早于目标时间 ready 通常只是留出排队余量，不能单凭“早”判为 pacing 错误。

### 大量 `gpu_composition = true`

这只说明 SF 的 CLIENT composition 路径频繁出现。要证明它造成 deadline miss，还需要同时看到：
...
当 App `on_time_finish = true`、SF CPU/GPU ready 也在 deadline 内，而 DisplayFrame 仍 late 并标记 `Display HAL`，排查重点应移到 HWC/DRM/厂商显示栈、刷新率或显示模式切换。present fence 能给出显示侧时间锚点，但不能继续细分 panel scanout 与光学响应。
...
5. **检查 SF 是否采纳新内容**
   对照 QUEUE、`BufferTX`、LATCH 与 pending buffer。queue 发生但本轮没有 latch，说明问题仍在显示前段。
...
| 平台 | 本章采用的可验证边界 |
|---|---|
| Android 12 / API 31 | FrameTimeline 开始提供 App/SF Expected 与 Actual timeline，成为本文的最低版本 |
...
| Android 16 / API 36 | 本章的 token、Expected/Actual 与 CLIENT/DEVICE 判读方法继续适用 |
| Android 17 / API 37 | 源码锚点为 `Scheduler/FrameTimeline.{h,cpp}`、当前 BLAST、CompositionEngine 与 HWC `presentOrValidate`/validate 路径 |

表中“源码位于某目录”只描述 Android 17 当前结构，不反推该文件在 Android 17 才出现。涉及更早版本时，应切换到对应 tag 核查函数位置和字段，不要用当前目录结构编造版本演进。
