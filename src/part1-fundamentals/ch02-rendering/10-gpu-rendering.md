...
>
> [说明: ASTC 与 ETC2 的带宽对比数据基于 ARM Mali-G715 / Adreno 740 级别 GPU 在 Android 14-15 上的厂商优化指南。不同 GPU 代际、驱动版本和纹理内容（平滑渐变 vs 高频细节）都会影响实际压缩效果。建议在目标设备上用 `adb shell cmd gpu vkjson` 确认 ASTC block size 支持，再做针对性对比。]
