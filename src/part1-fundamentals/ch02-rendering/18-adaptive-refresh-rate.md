...
`setRequestedFrameRate()` 也接受 30、60、120 等正数。数值表示内容偏好，不要求它正好等于设备的物理刷新率。多个数值投票互为整数倍时，框架通常取较高值；不互为整数倍时，超过 60 Hz 的请求按 `High` 倾向处理，其余按 `Normal` 倾向处理。官方文档明确说明这套合并策略可能调整，业务代码不应依赖精确的内部优先级。
...
Android 17（API 37）的 `Surface.setProducerThrottlingEnabled()` 调整 EGL/Vulkan Producer 在 queue 阶段承受的 CPU backpressure（下游忙时对上游形成的反压），属于队列节拍控制，不是刷新率投票。该 API 的细节见 2.17；排查 ARR 时应分别检查 `setFrameRate()` 的投票和 producer 限速。
