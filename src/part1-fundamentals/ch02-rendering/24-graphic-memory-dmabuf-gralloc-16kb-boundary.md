...
“16KB 设备上的图形 buffer 也按 16KB 对齐”只说对了一小段。Android 图形内存从逻辑尺寸走到 GPU、显示控制器或 Camera ISP，要经过多种相互独立的粒度：
...
其中前两项决定 native 代码能否可靠装载，第三和第四项影响通用内存管理，后几项决定图形 buffer 怎样被设备访问。它们可能在某个数值上恰好都是 16KB，却没有同一个控制来源。

本文以 Android 17 / API 37 / `android-17.0.0_r1` 为用户空间锚点，kernel 以 `android17-6.18-2026-06_r6` 为锚点。厂商 Gralloc、GPU、Composer HAL、Camera HAL 与 display driver 不在 AOSP 通用实现内，涉及具体布局和收益时必须补充设备证据。
...
Android 17 的 framework 仍保留 Gralloc 2/3/4/5 wrapper，以适配多代 vendor HAL。当前 Stable AIDL `IAllocator` 包含：
...
旧 `allocate(byte[] descriptor, count)` 仍在接口中。注释规定：Allocator V2 与 `AIMAPPER_VERSION_5` 配合时，由 `allocate2()` 取代；设备仍使用 graphics mapper 4 时，旧入口还要实现。因此，“Android 17 只走 `allocate2()`”也不准确。
...
这也说明 `legacy_align` 不能作为 Android 17 DMA-BUF 分配的 16KB 对齐证据。
...
## 三、kernel 16KB page 怎样作用于 DMA-BUF Heap
...
这段代码的用途是确保所有 heap allocation 从页边界开始并在页边界结束。16KB kernel 上，这里的 kernel page size 是 16KB；请求长度会向上取整到 16KB 的整数倍。长度为 0 会返回 `-EINVAL`。
...
Android 17 kernel IOMMU core 从 `domain->pgsize_bitmap` 选择硬件支持且满足地址、长度对齐的映射页大小。一个 16KB CPU kernel 不会把所有 IOMMU domain 强制成单一 16KB 映射粒度。
...
16KB kernel 可以为部分 4KB-aligned 应用启用 page-size backcompat。Android 17 还允许把兼容模式设为 `fatal`，让不兼容二进制立即终止，便于测试：
...
| 版本 | 与本章相关的变化 | 阅读边界 |
|---|---|---|
| Android 12 | GKI 2.0 以 DMA-BUF Heaps 取代 ION 分配框架 | 迁移文档中的 ION fallback 属于当时兼容路径 |
...
排查时分三组取证：页大小/二进制、图形对象、内核共享对象。不要把三组数据混成一个 “graphics memory” 数字。
...
Writer `rendering_pipelines` 的 Native Graphics、Camera、Video 章节给出了三个典型 owner：
...
精确 tag 中的 ION 实现已移除，打开失败直接返回错误。官方迁移文档描述的是历史兼容机制。
...
同一个 dma-buf 可以被多个进程映射。先按共享对象去重，再区分 backing storage、页表、IOMMU mapping 和每进程统计。
