...
应用不要在同一个 `ANativeWindow` 上同时让 Swappy 和业务代码持续调用 `Surface.setFrameRate()` 与 `ANativeWindow_setFrameRate()`。它们更新的是同一 Surface 的当前 vote，后续写入会改变先前设置，容易造成 mode 选择与 pacing 目标来回变化。系统如何综合多个 layer 的 vote 见 §2.18。
