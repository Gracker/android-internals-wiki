...
Android 17 能完整确认的注册路径属于标准 HWUI App Window：
...
`ViewRootImpl::updateRendererSurfaceControlAndBbq()` 把 `mChoreographer::onWaitForBufferRelease` 注册给 `ThreadedRenderer`。callback 经 JNI、`RenderProxy`、`CanvasContext` 和 `SkiaPipeline` 传到对应 `BLASTBufferQueue`。
...
Writer `rendering_pipelines` 的 Native Graphics 与 Game 章节强调了同一个诊断原则：先确认画面由谁 acquire、render、present，再判断它是否受主线程 Choreographer 驱动。不能因 trace 里出现 `eglSwapBuffers()` 或 `dequeueBuffer()` 就假设主 Choreographer 一定收到 stuffing 信号。
...
### 2.2 BLAST 用 release channel 等待
...
这段伪代码用于说明阈值，不表示它会读取 BufferQueue depth。`isStuffed` 是 `AtomicBoolean`，因为等待发生在 RenderThread/图形路径，后续恢复判断在 Choreographer 所在 Looper 线程。
...
| 字段 | Android 17 含义 |
|---|---|
| `isStuffed` | 是否收到新的长等待信号 |
| `isRecovering` | 当前是否处于恢复期 |
| `numberWaitsForNextVsync` | 恢复期间主动/被动多等了多少次 VSync |
...
`+1` 表示自然等待下一次 VSync 的预期间隔。若自上次未偏移 callback 以来的空闲间隔比预期等待更长，状态机认为动画已经 idle，结束 async trace 并清空状态。
...
flag 关闭时，这个 100ms 值不会限制 `DELAY_FRAME`。因此，看到 `MAX_BUFFER_STUFFING_DELAY_NS` 常量不足以证明某台 Android 17 设备最多只延迟 100ms。
...
`FrameData.update()` 把这些候选复制进 Choreographer 的 `FrameData`。线程已经迟到时，另一个 overload 会在已有候选中寻找 deadline 尚未过去的项，必要时向 `DisplayEventReceiver` 查询最新数据。
...
两个行为由独立 flag 控制。精确产品配置和 trace 才能证明分支。

### “七个 FrameTimeline 代表七套恢复策略”
...
1. [`ViewRootImpl.java`](https://android.googlesource.com/platform/frameworks/base/+/android-17.0.0_r1/core/java/android/view/ViewRootImpl.java)：确认标准窗口怎样注册 callback；
2. [`HardwareRenderer.java`](https://android.googlesource.com/platform/frameworks/base/+/android-17.0.0_r1/graphics/java/android/graphics/HardwareRenderer.java) 与 HWUI RenderProxy/CanvasContext/SkiaPipeline：确认 callback 怎样到达 BLAST；
