...
| --- | --- | --- | --- |
| 普通 View / Compose UI | 应用进程中的 HWUI | 宿主应用窗口的 Surface | 主线程生成 UI 状态和绘制记录，RenderThread 驱动 Skia 绘制 |
...
| 视频解码 | Codec / 厂商组件 | Surface 对应的 BufferQueue | 生产节拍、色彩格式和保护内容约束与普通 UI 不同 |
...
“标准应用窗口路径”指普通 View 或 Compose 内容经过宿主应用窗口的 HWUI Surface 输出。它不包括 `SurfaceView` 的独立内容生产路径，也不能概括所有 Flutter、WebView、Camera 或视频场景。
...
BLASTBufferQueue 将缓冲区放入 SurfaceControl Transaction
    ↓
SurfaceFlinger 接收事务、生成前端快照、锁存可用缓冲区
    ↓
HWC 验证：设备合成或客户端合成
    ↓
必要时由 RenderEngine 生成客户端目标
    ↓
HWC 显示提交 → 显示栅栏 → 释放栅栏
```
...
三者都有条件判断。Perfetto 出现 `performTraversals` 切片时，不能直接断言该帧完整测量、布局和绘制了整棵 View 树。
...
布局阶段通过 `layout(l, t, r, b)` 和 `onLayout()` 确定子 View 的边界。子 View 的位置和尺寸由父容器决定，但内容是否被裁剪属于绘制阶段行为，还会受 `clipChildren`、`clipToPadding`、轮廓、显式裁剪和变换影响。
...
- 是否发生窗口重新布局、Insets 或配置变化；
- 遍历相对 VSync 的开始时间和结束时间。
...
生产者无法无限制地 `dequeueBuffer()`。最大已出队（dequeued）数、最大已获取（acquired）数、异步模式、消费者是否会阻塞以及当前可用槽位共同决定它是否需要等待。
...
### 6.1 标准应用窗口中的 BLASTBufferQueue

在现代 Android 的普通应用窗口路径中，应用进程内的 `BLASTBufferQueue` 承担应用侧 BufferQueue 消费者角色，并把取得的缓冲区包装进 `SurfaceComposerClient::Transaction`。它通过 `setBuffer()` 把栅栏、帧号和释放回调等信息提交给 SurfaceFlinger。
...
它表达 SurfaceFlinger 侧待处理的缓冲区事务数量，可用于观察事务积压。它不是普通应用 BufferQueue 的槽位状态计数，也不直接等于已经排队等待扫描输出的显示帧数。
...
| FrameTimeline 预期/实际（expected/actual） | 帧预计呈现与实际呈现结果 |
| HWC 验证/显示提交（validate/present） | 显示合成与提交阶段 |
...
在应用到 SurfaceFlinger 的队列中，应用是生产者，BLAST/系统侧消费逻辑接收带有获取栅栏的缓冲区。到了 HWC 和显示设备边界，SurfaceFlinger 又要处理客户端目标（client target）、各 Layer 和显示提交相关的栅栏。

### 7.1 显示栅栏的边界

显示栅栏是 Android 显示栈的呈现完成时序锚点，但它不等同于面板像素的光学响应完成。面板扫描方式、像素响应时间、显示后处理和外部显示设备都可能位于 Android 软件可观测边界之外。

对于“输入到用户看到变化的总延迟”，应用跟踪数据、FrameTimeline 和显示栅栏只能覆盖其中一部分，还需结合触摸采样、显示扫描和面板特性。
...
- Layer 与事务状态；
- 当前调度和锁存策略；
- 当前版本与场景是否允许有限处理未发信号的栅栏。

Android 13 以后存在受约束的未发信号缓冲区锁存（unsignaled latch）优化，但它有严格条件，不能扩写成 SurfaceFlinger 会忽略所有获取栅栏。

### 8.2 SurfaceFlinger VSync 与应用 VSync

应用和 SurfaceFlinger 都受显示节拍驱动，但调度器可以给它们设置不同的相位和截止时间。调度目标是让应用先生产，SurfaceFlinger 再在合适的时间消费并提交显示。

高刷新率、可变刷新率和多显示设备让“固定 16.67 ms、应用与 SurfaceFlinger 同时唤醒”的模型失效。性能判断应读取当前显示模式以及 FrameTimeline 给出的预期/实际时序。

## 9. HWC 与 RenderEngine：每帧决定如何合成

### 9.1 HWC 验证

Hardware Composer HAL 连接 SurfaceFlinger 与设备显示合成能力。SurfaceFlinger 为 Layer 提供候选合成类型，HWC 验证后可以接受或要求调整。
...
| 客户端合成（Client composition） | SurfaceFlinger 的 RenderEngine | 先把相应 Layer 合成为客户端目标，再交给 HWC |
...
应用 GPU 切片很短，不代表 SurfaceFlinger 的客户端合成也很短；反过来也一样。Perfetto 中需要按进程和上下文区分 GPU 工作来源。
...
普通窗口的应用 BufferQueue 保存应用生成的窗口缓冲区。SurfaceFlinger 若执行客户端合成，还会生成客户端目标。显示控制器和面板扫描又可能有各自的内部缓冲与流水结构。
...
把它们统一称作“前台、后台、显示中三个缓冲区”会混淆应用 BufferQueue、HWC 客户端目标与扫描输出（scanout）。排查时应明确当前缓冲区的所有者、队列、槽位、帧号和栅栏。
...
窗口级是否启用硬件加速由清单配置、窗口标志和系统条件决定。View 级图层类型主要影响该 View 的缓存/绘制策略，两者不能混为一谈。

### 11.3 软件渲染没有统一的胜负结论

软件 Canvas 由 CPU 执行光栅化，某些小型、低频或特定操作可能成本可控；复杂动画、大面积更新和高分辨率内容通常更容易受 CPU 与内存带宽限制。硬件渲染也可能受驱动、GPU 队列和资源管理影响。

选择渲染方式应基于目标设备、内容和跟踪数据，而不是用“GPU 核心更多”或固定倍数推导速度与功耗。
...
- 实际时间线（actual timeline）和卡顿分类；
- 对应的应用帧与 SurfaceFlinger 帧；
- 当前刷新率、VSync 周期和截止时间。

帧率只适合描述一段时间的吞吐量。定位单帧应以时间线和截止时间为主。
...
- GPU 队列、GPU 完成时间与相关栅栏；
- 资源上传、着色器/管线编译和缓存抖动。

RenderThread CPU 切片长度不等于 GPU 持续时间。二者可能重叠，也可能通过栅栏形成依赖。
...
- 事务应用与 Layer 快照；
- 目标缓冲区是否按期锁存；
- SurfaceFlinger 是否错过自己的截止时间；
- HWC 验证返回的合成类型；
- 是否发生 RenderEngine 客户端合成；
- 显示栅栏和释放栅栏的时间；
- 多显示器、刷新率切换或显示模式变化。
...
| 观察 | 不能直接推出 | 还需要的证据 |
| --- | --- | --- |
| `performTraversals` 出现 | 完整测量/布局/绘制都已执行 | 内部切片、布局标志、脏区域 |
| `syncAndDrawFrame` 返回 | GPU 或显示已完成 | GPU 栅栏、`queueBuffer`、FrameTimeline |
| `queueBuffer` 成功 | 画面已经显示 | BLAST/SF 锁存、HWC 显示提交 |
| GPU 轨道超过一个周期 | 该工作必然导致当前帧掉帧 | GPU 依赖、帧 ID、截止时间 |
| `BufferTX` 增加 | 应用 BufferQueue 一定塞满 | 对应 Layer 事务与 BufferQueue 槽位状态 |
| HWC 使用 CLIENT | 一定是 Layer 太多 | 每层合成原因与设备能力 |
| 显示栅栏发出信号 | 面板像素完成响应 | 面板扫描和硬件测量数据 |
...
4. 若 UI 侧按时提交，检查 RenderThread 是否在同步、出队、绘制或交换阶段等待。
5. 若应用已按时 `queueBuffer`，按 Surface 和帧号追踪 BLAST 事务。
6. 检查 SurfaceFlinger 是否及时锁存，以及获取栅栏是否满足。
7. 检查 HWC 合成类型、RenderEngine 工作和显示提交时序。
...
SurfaceControl 事务和渲染相关公开 API 继续扩展，系统合成与应用内容提交的关系更容易通过跟踪数据观察。
...
FrameTimeline 提供预期/实际显示时间与卡顿分类，为跨应用和 SurfaceFlinger 的单帧分析提供稳定入口。

### Android 13 到 Android 16

SurfaceFlinger Scheduler、Layer 前端、FrameTimeline、刷新率和合成策略持续演进。部分版本支持有条件的未发信号缓冲区锁存，但同步规则仍需按源码条件判断。
...
- 性能结论以 FrameTimeline、栅栏、BufferQueue 和 HWC 的逐帧证据为准。
...
5. SurfaceFlinger 更新 Layer、锁存缓冲区并组织合成；
6. HWC、RenderEngine 和显示设备完成显示提交；
7. 获取、释放、显示栅栏维护各阶段的安全依赖。

定位问题时，先确认输出拓扑，再按帧 ID、Surface、BufferQueue 槽位、栅栏和 FrameTimeline 逐段核对。这样才能区分应用生产晚、GPU 执行晚、队列背压、SurfaceFlinger 合成晚和显示呈现晚，避免用单个切片为整帧下结论。
