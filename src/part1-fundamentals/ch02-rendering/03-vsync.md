...
第二点更适合解释 Android 17。App 收到的 VSync 回调不是“屏幕此刻开始扫描”的原样广播，SurfaceFlinger 也不会等硬件脉冲到来后才开始合成。系统先根据硬件样本和 present fence 建立显示时序模型，再从目标呈现时刻向前扣除各阶段的工作预算。
...
垂直消隐区（Vertical Blanking Interval，VBlank）来自栅格扫描时序，表示上一帧有效扫描结束到下一帧有效扫描开始之间的区间。传统显示系统在这个边界更新 scanout 配置，能够避免在有效扫描中途换图。
...
| TE | Panel 接口 | Panel 用来提示安全更新窗口或扫描节奏的 Tearing Effect 信号 |
...
### 1.3 现代 Android 不是全局 Front/Back Buffer 互换
...
- HWC 决定哪些 layer 走 device composition，哪些交给 GPU 生成 client target；
- 显示控制器最终 scanout 的对象可能是硬件 plane，也可能包含 GPU 合成结果。

所以，VSync 的作用不是让系统交换一对全局缓冲。它为 buffer 生产、latch、合成和 present 提供时间约束。某个 App 的 buffer 未就绪时，SurfaceFlinger 可能继续使用旧内容；其他 layer 仍可正常更新。

---

## 二、“HW-VSync → SF-VSync → App-VSync”该怎么理解
...
### 3.4 present fence 是反馈，不是光学完成时间
...
不能据此说 phase offset 已经消失。`VsyncConfig` 同时保留：
...
- App、SF、DisplayHAL 的 deadline miss 类型；
...
4. dispatch 到时后，`EventThread::onVsync()` 生成带 FrameTimeline 数据的 VSync event；
5. 事件经 connection 的 `BitTube` 发送到客户端；
6. native `DisplayEventReceiver` 从 fd 读取事件；
7. Java `DisplayEventReceiver` 回调到 `FrameDisplayEventReceiver.onVsync()`；
...
- `VSYNC-app` counter 跳变，不证明目标进程已经开始画；
...
`Choreographer.FrameData` 把这些信息交给回调。应用和系统可以围绕首选 timeline 工作，也能在错过首选目标时识别后续合法目标。高刷、不同渲染 cadence 和早启动配置下，这比只传一个裸时间戳更有表达力。

---

## 六、SurfaceFlinger 的 VSync 路径

SurfaceFlinger 不通过 App 的 EventThread 驱动主循环。`Scheduler::initVsync()` 把 SF 的 `MessageQueue` 注册到同一个 pacesetter display 的 `VSyncDispatch`，注册名为 `"sf"`。
...
Android 图形文章常把卡顿解释为“双缓冲切三缓冲”。这可以说明“增加一块可周转缓冲有时能减少 producer 阻塞”，但不能作为现代 BufferQueue 的固定配置。
...
- BLAST 的 pending release 和当前刷新率策略。

所以，Android 没有对所有 Surface 保证“默认正好三块缓冲”。

### 8.2 队列加深会改变什么

更多可周转缓冲可能让 producer 更少阻塞，但也可能让旧内容排在队列中，增加从输入到呈现的等待。是否丢弃旧 buffer、SF 本轮是否 latch 新内容、producer 是否被 backpressure 限制，还取决于队列模式和 transaction 状态。
...
因此，不能用“平均半个 VSync”概括触摸等待，也不能只根据 phase offset 推导端到端延迟。可靠做法是用同一帧的输入事件、FrameTimeline token、buffer 和 present 证据测量。
...
Perfetto 的 FrameTimeline 从 Android 12 开始提供 Expected 和 Actual 轨迹。Expected 描述系统给这一帧安排的预算，Actual 描述帧的执行与呈现结果。二者按 token 对齐，比数 `VSYNC-app` counter 更能说明问题。
...
建议把 dumpsys 当成“当前配置快照”，Perfetto 当成“运行过程证据”：
...
| kernel/display driver | 显示控制器中断、vblank/TE、commit 与 fence 的底层实现 | vendor display driver；DRM/KMS 设备可看 `drm_vblank.c` |
| panel | 扫描、TE、自刷新、像素响应 | panel/controller 规格与 vendor 实现 |

本章 kernel 锚点固定为 `android17-6.18-2026-06_r6`。通用内核的 `drivers/gpu/drm/drm_vblank.c` 和 `include/drm/drm_vblank.h` 说明 DRM vblank 计数、事件与时间戳框架；Android 设备是否走该路径，要看 SoC 显示驱动和 HWC 实现。通用 AOSP framework 无法证明某款设备使用哪根 panel 信号或哪种中断接线。
...
| 大纲要求 | 核验结果 | 证据等级 |
|----------|----------|----------|
| VSync 与画面撕裂 | 已区分扫描撕裂、VBlank、TE、HWC callback 和调度 VSync | L2：AOSP 图形文档、Composer HAL、kernel DRM vblank |
| HW/SF/App 三级信号 | 已保留历史模型，并按 Android 17 预测与分发架构修正 | L1：`VsyncSchedule`、`VSyncPredictor`、`VSyncDispatchTimerQueue` |
| Phase Offset | 已说明 offset、工作预算和 `Late/Early/EarlyGpu` 的关系 | L1：`VsyncConfiguration`、`VsyncModulator`、`VsyncConfig` |
| SF 与 Choreographer 传递路径 | 已分别核对 SF MessageQueue 和 App EventThread/BitTube 路径 | L1：`MessageQueue.cpp`、`EventThread.cpp`、`DisplayEventReceiver`、`Choreographer.java` |
| Perfetto 中的 VSync | 已明确 counter、`doFrame` 与 FrameTimeline 的证据边界 | L2：Perfetto FrameTimeline 文档与 Android 17 trace 埋点 |
| 可变刷新率 | 已区分 ARR 的 VSync/TE rate、最小帧间隔和实际呈现 cadence | L1/L2：Composer3 v3 AIDL、AOSP ARR 文档 |
| Phase 与输入延迟 | 已拆分输入、App、SF、present 和 panel 的逐段时延 | L2：Android 17 调度源码与逐帧 FrameTimeline 方法 |
| `[自动发现]` Android 17 预测器参数 | 已核对默认 20 个历史样本、最少 6 个样本、20% 异常点阈值，以及 ARR 单样本分支 | L1：`VsyncSchedule::createTracker()`、`VSyncPredictor.cpp` |
...
| Android 17 | `VsyncSchedule`、预测/反馈/分发模型延续并加强 ARR 时序支持 | 本章源码基线为 `android-17.0.0_r1` |
...
只要每个问题都绑定到具体时间戳、线程、buffer 或 fence，VSync 就不再是一条抽象竖线，而是一套可以逐帧验证的调度机制。
