...
1. 提供或校准显示设备的时间节奏。
2. 围绕预计呈现时刻，为应用和 SurfaceFlinger 安排合适的唤醒时间。

第二点更适合解释 Android 17。应用收到的 VSync 回调并非“屏幕此刻开始扫描”的原样广播，SurfaceFlinger 也不会等硬件脉冲到来后才开始合成。系统先根据硬件样本和显示栅栏（present fence）建立显示时序模型，再从目标呈现时刻向前扣除各阶段的工作预算。

因此，分析一帧时要分清四个时刻：

| 时刻 | 回答的问题 | 常见证据 |
|------|------------|----------|
| 显示时序样本 | 显示设备当前按什么节奏运行 | HWC VSync 回调、TE、DRM 垂直消隐、显示栅栏 |
| 应用唤醒 | 应用应在何时开始生产目标帧 | `VSYNC-app`、`Choreographer#doFrame` |
| SF 唤醒 | SF 应在何时开始处理目标显示帧 | `VSYNC-sf`、SF 主线程、FrameTimeline |
| 显示提交 | 本轮合成何时越过 Android 显示栈的呈现边界 | 显示栅栏、DisplayHAL、实际时间线 |
...
| TE | 面板接口 | 面板用来提示安全更新窗口或扫描节奏的撕裂效应（Tearing Effect）信号 |
| HWC VSync 回调 | Composer HAL | HWC 向 SurfaceFlinger 报告的时间戳事件 |
| 应用/SF VSync | Android 调度 | Scheduler 依据预测模型安排的客户端或 SF 唤醒 |

某台设备可以从面板 TE、显示控制器中断或其他厂商路径获得底层时间基准，再由 HWC 上报给框架。不能把所有实现概括成“屏幕引脚产生 GPIO 中断”。DRM 垂直消隐也只适用于采用 DRM/KMS 的显示驱动；它不是 Android 对所有设备强制规定的唯一路径。

### 1.3 现代 Android 没有全局前后缓冲互换

双缓冲是理解撕裂的入门模型：一块缓冲供显示读取，另一块供生产者写入，到安全边界后交换角色。现代 Android 的对象更多：

- 每个可独立提交内容的 Surface 通常有自己的 BufferQueue 或等价队列；
- 应用、视频解码器、相机、游戏引擎等生产各自的缓冲；
- SurfaceFlinger 从多个图层选择可用内容并组织合成；
- HWC 决定哪些图层由设备合成，哪些交给 GPU 生成客户端目标缓冲；
- 显示控制器最终扫描输出的对象可能是硬件平面，也可能包含 GPU 合成结果。

VSync 不负责交换一对全局缓冲，而是为缓冲生产、锁存、合成和显示提交提供时间约束。某个应用的缓冲未就绪时，SurfaceFlinger 可能继续使用旧内容，其他图层仍可正常更新。
...
- `HW_VSYNC`：硬件显示节奏；
- `VSYNC` 或应用 VSync：应用开始处理输入、动画和绘制；
- `SF_VSYNC`：SurfaceFlinger 开始锁存与合成。

这张图有助于理解“应用和 SF 要在呈现前预留时间”，但箭头 `HW-VSync → SF-VSync → App-VSync` 容易造成两个误解：

1. 应用/SF 每次都由刚刚到达的硬件中断逐级转发；
2. 三条信号永久保持同一周期和固定相位。

Android 17 的实现不满足这两个前提。Scheduler 维护预测模型，应用和 SF 分别注册定时回调；ARR 还允许 TE/VSync 频率与内容呈现频率不同。
...
```mermaid
flowchart LR
    A["显示时序来源<br/>HWC VSync / 显示栅栏"] --> B["VSyncReactor<br/>接收反馈、判断是否继续采样"]
    B --> C["VSyncPredictor<br/>维护周期与相位预测"]
    C --> D["VSyncDispatchTimerQueue<br/>计算目标 VSync 和唤醒时刻"]
    D --> E["SurfaceFlinger MessageQueue<br/>SF 帧回调"]
...
- `VSyncController`：当前实现是 `VSyncReactor`，负责接收硬件样本和显示栅栏；
...
持续接收硬件回调会增加唤醒和功耗。模型稳定后，`VSyncReactor` 可以告诉 `VsyncSchedule` 当前不再需要更多硬件样本，系统随后关闭 HWC VSync；发生显示模式切换、模型漂移或需要重新校准时，再开启采样。

Android 17 的硬件 VSync 状态包括 `Enabled`、`Disabled` 和 `Disallowed`。显示栅栏是否参与模型、内核空闲定时器、外部显示和设备配置都会影响采样策略。看到硬件 VSync 持续开启，只能说明当前实现仍在请求样本；还要结合控制器状态和设备配置，不能直接判定为驱动错误。

### 3.4 显示栅栏是反馈，不代表光学完成

HWC 的显示提交操作会按显示、按帧返回显示栅栏。栅栏发出信号后，会为 Android 显示栈提供本轮提交的时间锚点，SurfaceFlinger 可用它校准模型和更新 FrameTimeline。

显示栅栏不表示面板所有像素已经完成响应，也不表示人眼此刻已经看到稳定图像。面板扫描、传输、像素响应和显示后处理仍可能发生在这个边界之后。

---

## 四、从目标呈现时刻反推应用与 SF 的唤醒
...
第一行选择一个足够晚、仍能容纳全部预算的预测 VSync；第二行向前扣除工作时间和就绪时间。实际源码还会处理已经设定的回调、避免跳过既定目标、合并相近唤醒以及定时器误差。
...
`workDuration` 是该消费者完成自身工作的预算，`readyDuration` 是目标呈现前还要留给后继阶段的预算。对应用 EventThread，`readyDuration` 通常承接 SF 工作预算；对 SF MessageQueue，`readyDuration` 为 0，`workDuration` 是 SF 自己的预算。
...
这段结构说明配置层仍能用偏移量表达相对相位，也能用工作时长表达截止时间预算。`VsyncConfiguration` 支持 `PhaseOffsets` 和 `WorkDuration` 两套构造方式，最终提供统一的 `VsyncConfigSet`。

阅读 Android 17 运行时调度时，优先使用 `workDuration`、`readyDuration`、预计呈现时间和实际呈现时间解释；排查设备配置时，再回到偏移量与时长的换算关系。固定写成“应用永远提前 N ms、SF 永远提前 M ms”会漏掉刷新率、状态切换和厂商配置的影响。
...
| `early` | 提前处理事务、刷新率切换等 | 给事务和合成预留更多时间 |
| `earlyGpu` | 检测到 GPU 合成后的若干帧 | 给 GPU 合成路径增加预算 |

`VsyncModulator` 根据提前唤醒请求、事务状态、刷新率切换和近期是否使用 GPU 合成，选择下一组配置。它不是一张恒定的毫秒表。设备覆盖配置、刷新率和工作负载改变后，具体时长都可能变化。

### 4.4 调整相位的代价

把应用或 SF 唤醒向目标呈现靠近，可以减少等待时间，但也会缩短可用预算。向前移动则增加容错，同时可能增加输入到显示的排队时间和功耗。

调优时至少同时观察：

- 应用、SF、DisplayHAL 错过截止时间的类型；
...
仅凭平均帧耗时调整偏移量，很容易让高分位慢帧变多。

---

## 五、应用 VSync 如何进入 Choreographer
...
这段代码说明两件事：同一待处理帧的多次 `invalidate()` 不会一一换成多次 VSync 申请；从其他线程发起调度时，会先把异步消息放到 Choreographer 所在线程的队首，再由该线程申请 VSync。
...
2. 某个 `DisplayEventReceiver` 连接调用 `requestNextVsync()`；
3. `EventThread` 把该连接的请求标记为 `Single`；
4. 分发时间到达后，`EventThread::onVsync()` 生成带 FrameTimeline 数据的 VSync 事件；
5. 事件经连接的 `BitTube` 发送到客户端；
6. 原生层 `DisplayEventReceiver` 从文件描述符读取事件；
...
这个路径中，Binder 用于创建连接和发出请求；每帧事件通过 BitTube 通道传递。把每次应用 VSync 都描述成一次 Binder 回调并不准确。

### 5.3 VSync 到达不等于 doFrame 立即开始

`FrameDisplayEventReceiver.onVsync()` 会记录待处理的 VSync 并投递异步消息。源码注释明确允许时间戳更早的消息先执行。主线程若正忙于长任务、锁等待或同步 Binder 调用，`doFrame()` 仍可能晚于预计唤醒点。

因此：

- `VSYNC-app` 计数器跳变，不证明目标进程已经开始绘制；
- 一次系统侧 VSync 事件不必然对应一个成功提交的应用缓冲；
- 判断应用起帧，应看目标进程的 `Choreographer#doFrame` 及其 FrameTimeline 令牌；
- 判断迟到原因，还要看线程从唤醒到运行的调度延迟。

### 5.4 FrameData 提供多个候选时间线

`EventThread` 会为 VSync 事件生成 `VsyncEventData`，其中包含候选帧时间线。每项带有：

- `vsyncId`；
- `deadlineTimestamp`；
- `expectedPresentationTime`。

`Choreographer.FrameData` 把这些信息交给回调。应用和系统可以围绕首选时间线工作，也能在错过首选目标时识别后续合法目标。在高刷新率、不同渲染节奏和提前启动配置下，这比只传一个裸时间戳更有表达力。
...
当 SF 有一帧需要处理时，`MessageQueue::scheduleFrame()` 使用当前 SF `workDuration` 调度回调。到时后，SF 处理事务、更新图层快照、选择缓冲、制定合成策略，随后和 HWC 完成验证与显示提交。

应用与 SF 共享同一物理显示的预测基础，但它们有不同的注册项、预算和回调路径：

| 对象 | 调度入口 | 工作内容 |
|------|----------|----------|
| 应用 | `EventThread("app")` → DisplayEventReceiver → Choreographer | 输入、动画、遍历、RenderThread/GPU、提交缓冲 |
| SF | `MessageQueue` 的 `"sf"` 注册项 | 事务、锁存、合成、HWC 显示提交 |
...
Android 15 引入自适应刷新率（Adaptive Refresh Rate，ARR）。ARR 所需的 `vrrConfig`、`getDisplayConfigurations()` 与 `notifyExpectedPresent()` 基础契约从 Composer3 AIDL 第 3 版开始出现。Android 17 标签同时保留第 3、4、5 版的冻结快照；此处以第 3 版标出这组契约的起点，不表示 Android 17 设备只能实现第 3 版。支持 ARR 的显示模式在 `DisplayConfiguration` 中提供 `vrrConfig`：

- `vsyncPeriod` 表示显示 VSync/TE 节奏；
- `VrrConfig.minFrameIntervalNs` 约束最快呈现间隔；
- `DisplayCommand.frameIntervalNs` 提示后续内容节奏；
- `notifyExpectedPresent` 可提前告知下一次预计呈现及后续间隔。

在非 ARR 显示模式中，`vsyncPeriod` 通常对应当前显示刷新周期。在 ARR 显示模式中，两者可以解耦。

### 7.2 一个具体例子

假设面板的 TE/VSync 为 240 Hz，周期约 4.17 ms；`minFrameIntervalNs` 对应 120 Hz，最快每 8.33 ms 呈现一帧。系统还可以按内容节奏在后续离散 VSync 步长呈现，例如约 16.67 ms 一帧。
...
`VSyncPredictor` 根据显示模式的峰值刷新周期与 VSync 频率，计算每个显示帧跨越多少个 VSync 节拍；`minFramePeriod()` 返回最小显示帧间隔。这里的倍数描述显示模式的节奏约束，不表示预测器要“收集多帧才输出一帧”。
...
Android 图形文章常把卡顿解释为“双缓冲切三缓冲”。这个模型可以说明“增加一块可周转缓冲有时能减少生产者阻塞”，但不能作为现代 BufferQueue 的固定配置。
...
更多可周转缓冲可能减少生产者阻塞，但也可能让旧内容排在队列中，增加从输入到呈现的等待。是否丢弃旧缓冲、SF 本轮是否锁存新内容、生产者是否受到背压限制，还取决于队列模式和事务状态。
...
VSync 只提供时间机会。缓冲是否赶上目标，要结合提交时刻、获取栅栏、锁存和显示提交逐帧判断。
...
不能用“平均半个 VSync”概括触摸等待，也不能只根据相位偏移推导端到端延迟。可靠做法是用同一帧的输入事件、FrameTimeline 令牌、缓冲和显示提交证据测量。
...
不同 Android 版本、跟踪配置和设备实现显示的轨迹名称可能不同。常见对象包括：

- SurfaceFlinger 中的 `VSYNC-app`、`VSYNC-sf` 或 `VSYNC-predicted` 计数器；
- 应用进程主线程的 `Choreographer#doFrame`；
- RenderThread 的 `DrawFrame`；
- 应用与 SurfaceFlinger 的 FrameTimeline Expected/Actual；
- SurfaceFlinger 主线程、RenderEngine/GPU、HWC/DisplayHAL；
- `BufferTX - <layerName>`、获取/释放/显示栅栏；
- `sched_wakeup`、`sched_switch` 和线程状态。

不要要求跟踪结果必须出现一组固定轨迹名。轨迹缺失时，先检查跟踪配置和设备是否暴露对应数据源。

### 10.2 逐帧分析顺序

对一帧 UI 更新，按下面的顺序核对：

1. 应用的 Expected Timeline 给出的唤醒、截止时间和预计呈现是什么？
2. `Choreographer#doFrame` 何时开始，开始前有多少可运行态等待？
3. UI 线程、RenderThread 和 GPU 何时结束，缓冲何时提交？
4. SF 是否在目标显示帧接收并锁存该缓冲？
5. SF CPU、SF GPU 或 DisplayHAL 哪一段错过截止时间？
6. Actual Timeline 的显示提交与 Expected Timeline 偏差是多少？
7. 同期是否发生刷新率切换、ARR 节奏变化或提前配置切换？

Perfetto 的 FrameTimeline 从 Android 12 开始提供 Expected 和 Actual 轨迹。Expected 描述系统给这一帧安排的预算，Actual 描述帧的执行与呈现结果。二者按令牌对齐，比统计 `VSYNC-app` 计数器更能说明问题。

### 10.3 常见现象如何解释

| 现象 | 先检查 | 不应直接得出的结论 |
|------|--------|--------------------|
| `VSYNC-app` 在跳，应用没有 `doFrame` | 应用是否请求帧、目标进程是否有回调、主线程状态 | VSync 丢失 |
| `doFrame` 晚于预计起点 | 可运行态等待、长消息、锁、Binder、GC | UI 逻辑一定很重 |
| 应用按时提交，Actual 仍晚 | 锁存、SF CPU/GPU、HWC、DisplayHAL | 应用渲染慢 |
| SF 持续使用提前配置 | 事务、刷新率切换、GPU 合成 | 相位参数配置错误 |
| HW VSync 一直开启 | Reactor 是否需要样本、显示栅栏、显示模式状态 | HWC 驱动有故障 |
| VSync 频率高于呈现帧率 | ARR 的 TE 频率、最小帧间隔、渲染节奏 | 系统重复显示了每个新帧 |

### 10.4 `dumpsys SurfaceFlinger` 的定位价值

`dumpsys SurfaceFlinger` 在具体分支和设备上会打印当前 VSync 控制器、分发器、预测器、硬件 VSync 状态、显示模式与调度配置。字段名会随版本变化，排查时应以目标设备输出为准。

可以把 dumpsys 当成“当前配置快照”，把 Perfetto 当成“运行过程证据”：

- dumpsys 能说明当时选择了哪种显示模式、预算和控制状态；
- Perfetto 能说明每一帧何时被唤醒、执行、提交和呈现；
- 厂商跟踪数据或 DRM/HWC 跟踪点才能继续观察显示后段。
...
| 层级 | 负责内容 | 常见源码入口 |
|------|----------|--------------|
| 应用框架 | 请求帧、分发回调、组织 `doFrame()` | `Choreographer.java`、`DisplayEventReceiver.java` |
| SurfaceFlinger Scheduler | 预测、反馈控制、分发应用/SF 唤醒 | `VsyncSchedule`、`VSyncPredictor`、`VSyncReactor`、`VSyncDispatchTimerQueue` |
| Composer HAL/HWC | 上报 VSync、接收预计呈现提示、返回显示栅栏 | Composer3 AIDL、厂商 Composer 实现 |
| 内核/显示驱动 | 显示控制器中断、垂直消隐/TE、提交与栅栏的底层实现 | 厂商显示驱动；DRM/KMS 设备可看 `drm_vblank.c` |
| 面板 | 扫描、TE、自刷新、像素响应 | 面板/控制器规格与厂商实现 |
...
| 版本 | 变化 | 阅读提示 |
|------|------|----------|
| Android 4.1 | Project Butter 把应用、合成和显示节奏纳入统一帧调度 | 适合理解应用/SF VSync 的起点 |
| Android 4.x～10 | DispSync 软件 PLL 与固定相位偏移是主流说明模型 | 旧源码和官方 VSync 文档仍大量使用这些术语 |
| Android 11～12 | Scheduler、VSyncPredictor、FrameTimeline 等逐步成为分析重点 | 开始从固定相位转向目标呈现与逐帧截止时间 |
| Android 13～14 | 刷新率策略、FrameTimeline 和调度模型继续演进 | 不能把旧属性名直接套到新分支 |
| Android 15 | Composer3 v3 引入 ARR 支持，TE/VSync 频率可与呈现帧率解耦 | 需要同时看 `vsyncPeriod` 与 `minFrameIntervalNs` |
| Android 16 | API 36 增加 ARR 能力查询与建议帧率接口 | 应用更容易识别和利用 ARR |
...
- 应用是否申请了下一帧？
- EventThread 是否为该连接安排了单次 VSync？
- 应用主线程是否按预计时间运行？
- 应用的缓冲是否在目标截止时间前提交并可读？
- SF 是否在目标显示帧锁存了这块缓冲？
- 当前使用 `late`、`early` 还是 `earlyGpu` 配置？
- VSync 预测是否正在重新采样或切换显示模式？
- 当前是固定刷新模式、MRR 还是 ARR 模式？
- 跟踪数据中的 VSync 频率、渲染帧率、实际呈现帧率是否被混为一谈？
- 显示栅栏晚，是 SF/HWC 提交晚，还是显示后段反馈晚？
- 设备使用 DRM 垂直消隐、面板 TE 还是厂商私有时间源？

把每个问题都绑定到具体时间戳、线程、缓冲或栅栏，便能把 VSync 还原为一套可逐帧验证的调度机制。

---

## Android 17 的 VSync 调度边界

1. VBlank、TE、HWC VSync 回调、应用/SF VSync 分属不同层，不能当成同一个 GPIO 信号。
2. 现代 Android 使用各 Surface 的缓冲队列、图层合成和 HWC 显示提交，不存在一对全局前后缓冲统一交换的实现。
3. “HW-VSync、SF-VSync、App-VSync”适合解释历史相位关系；Android 17 应按“显示反馈 → 预测目标 → 预算反推唤醒”理解。
4. `VsyncSchedule` 持有预测器、反馈控制器和分发器；应用与 SF 使用不同注册项和工作预算。
5. 相位偏移仍存在于配置层，运行时诊断应同时看 `workDuration`、`readyDuration`、预计呈现和实际呈现。
6. Choreographer 按需请求下一次 VSync，系统侧计数器不等于应用已经执行 `doFrame()`。
7. ARR 允许 TE/VSync 频率与呈现帧率解耦，不能再用单一刷新周期解释所有帧。
8. BufferQueue 深度是动态约束结果，没有通用的双缓冲/三缓冲固定延迟公式。
9. Perfetto 分析要用 FrameTimeline 令牌对齐应用、SF 与显示提交，再结合调度、缓冲和栅栏证据定位超时。
