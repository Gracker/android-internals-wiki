...
Android 17 的官方构建文档要求 64 位 x86 Linux 主机、glibc 2.17 或以上、至少 400 GB 可用磁盘和至少 64 GB 内存。400 GB 由约 250 GB checkout 与约 150 GB 产物组成；做多 target、多工作树或内核构建时还要预留更多空间。

官方给出的构建时间样例是：72 核、64 GB 内存的 Google 主机完成全量构建约 40 分钟，6 核、64 GB 主机约 6 小时。这组数字只用于说明硬件差距，不能当作任意 Android 17 target 的 SLA。I/O、内存带宽、远程文件系统、目标产品和当次依赖图都会改变耗时。

主机软件边界如下：

- Android 11 及以上要求 Ubuntu 18.04 或更新版本，官方也允许满足 glibc 条件的其他 64 位 Linux 发行版。
- AOSP 从 2021 年 6 月 22 日起不再支持 macOS 平台构建。
- 最新 release checkout 已携带 OpenJDK、Make 和 Python 3 预编译版本，不要额外改动 JDK 版本覆盖它们。
- Cuttlefish 还要求 KVM。云主机和虚拟机需要额外确认嵌套虚拟化以及 `/dev/kvm` 权限。
...
安装完成后运行 `repo version`，确认 launcher 可以工作。官方当前要求 Repo 2.4 或以上；团队环境还应记录实际版本，避免不同主机使用差异过大的 launcher。
...
`--partial-clone` 减少初始对象下载，`-c` 只同步当前 manifest 分支。`-j8` 来自官方示例，主机可以按网络、文件系统和服务器响应调整并发；“CPU 核数的数倍”没有通用保证。
...
`pinned-manifest.xml` 应与实验记录一起归档。只记录 `android-17.0.0_r1` 仍可能漏掉本地 cherry-pick、未提交修改或后来单独切换的项目，所以还要保存 `repo status` 和补丁集标识。

## Soong、Kati 与 Ninja 的职责

AOSP 平台使用 Soong 构建系统。Soong 读取 `Android.bp`，Kati 兼容仍存在的 Make 描述，二者生成由 Ninja 执行的构建图。日常命令 `m` 是构建环境提供的入口，开发者通常不直接调用 Ninja。

三层的故障表现不同：

- `Android.bp` 属性、模块名或可见性错误发生在 Soong 解析阶段。
- 遗留 `Android.mk` 的转换问题会出现在 Kati 阶段。
- 编译器、链接器和命令执行失败由 Ninja 报出具体 action。
...
这个 goal 会解析并验证构建结构，不生成模块产物。它通过后仍要编译受影响模块，因为类型检查、链接和生成代码错误只会在对应 action 执行时出现。

平台构建文档把 `Android.bp` 描述为语法接近 Bazel BUILD 文件。Android 17 平台的常规入口仍是 Soong；Android Common Kernel 则由 Bazel/Kleaf 构建。看到 Bazel 文件不能据此把平台和内核构建命令混用。
...
- `release_config` 选择 feature launch flag 的发布配置，例如 `aosp_current`。
...
`envsetup.sh` 每个新 shell 都要加载一次。`m` 未指定 `-j` 时会自行选择并发度，先采用默认值；只有经过主机测量后才调整 `-jN`。
...
模块名来自 `Android.bp` 的 `name`，文件路径不等于模块名。一个 Java 文件也可能进入 boot jar、APEX 或多个变体。修改前应查看同目录 `Android.bp` 及依赖，确认所属模块和安装分区。

常见源码位置包括：

- `frameworks/base/core/java/android/view/`：Choreographer、ViewRootImpl 等 Framework 类。
- `frameworks/base/services/core/java/com/android/server/`：system_server 服务实现。
- `frameworks/native/services/surfaceflinger/`：SurfaceFlinger。
- `frameworks/native/libs/renderengine/`：RenderEngine。
- `packages/modules/`：Mainline 模块；产物可能位于 APEX 或 APK。

增量构建耗时与改动触发的依赖图有关。Java API 变化、`Android.bp` 变化、生成代码、APEX 和 bootclasspath 都可能扩大构建范围，不应承诺固定秒数。

## 用 Cuttlefish 验证 Framework

Cuttlefish 是 AOSP 当前的规范虚拟设备，适合验证纯 AOSP Framework 行为、CTS、系统服务和自定义测试。它与真机的主要差异集中在 HAL 以及依赖具体硬件的部分。GPU 合成、热控制、SoC 调度、相机和功耗结论仍需真机。
...
两个命令分别检查设备节点是否存在以及当前用户是否有权限。x86_64 主机也可以检查 CPU 的 `vmx` 或 `svm` 标志；ARM64 主机以 `/dev/kvm` 为直接依据。

ARM64 Linux 主机可以运行匹配架构的 Cuttlefish 预编译镜像。这项能力没有改变平台源码构建文档的 x86_64 主机要求；在 ARM64 主机上应把“运行预编译虚拟设备”和“本地编译完整 AOSP”分开记录。

使用 Android CI 产物时，`cvd-host_package.tar.gz` 和设备 image 必须来自同一个 build。主机包、镜像和分支混搭会引入协议或配置不兼容，不能靠“能启动”证明组合受支持。

解压 host package 后，在该目录启动 Cuttlefish，再用包内 ADB 检查设备：
...
Cuttlefish host package 的具体版本若要求独立运行目录，以同 build 的官方启动说明为准，不要在用户 shell 中永久改写 `HOME`。Cuttlefish 会向 ADB 注册设备，通常不需要手工连接 `0.0.0.0:6520`。`0.0.0.0` 是监听地址语义，也不适合作为客户端目标。多实例环境的 TCP 端口会随实例号变化。

默认启动会开启 WebRTC 界面，浏览器访问 `https://localhost:8443`。跨主机访问还要配置防火墙、TLS 和 WebRTC 使用的 TCP/UDP 端口，不能把本地开发端口直接暴露到不可信网络。
...
`stop` / `start` 会重启 zygote 和依赖它的 Java 进程，设备界面会暂时消失。它不会模拟完整开机，也不会重新执行所有 bootloader、init 和 early-boot 路径。研究系统启动时延必须执行完整重启。

Native 服务可以单独重启，但要先确认 init 配置和依赖。SurfaceFlinger 崩溃或退出后通常由 init 拉起；这类操作会中断显示，并可能使当前测试状态失效。自动化脚本应等待服务重新注册，再开始采样。
...
`atest` 会按测试配置构建、安装并执行。模块名需要从目标目录的 `Android.bp` 或 `TEST_MAPPING` 确认；一个大型聚合测试并不能替代改动附近的单元测试。

## 调试信息：优先低扰动证据

### 日志与 Trace

在高频路径增加 Log 会改变锁竞争、调度和 I/O。Choreographer、Binder、SurfaceFlinger 等路径尤其敏感。临时日志适合确认控制流，测量时应移除或改成受控 trace 点。

如果使用动态日志级别，属性必须已经由目标代码读取并受属性策略允许。`setprop log.tag.Choreographer VERBOSE` 只会影响遵循 log tag 属性的调用，不能自动开启所有 Choreographer 诊断。日志时间戳还需要与 Perfetto 使用的时钟域对齐后再做关联。

### System properties

系统属性受命名、类型、稳定性、分区边界、`property_contexts` 和 SELinux 规则控制。shell 不能任意创建可写的 `persist.debug.*` 属性。`persist` 只应在确有跨重启需求且系统属性是合适载体时使用。

Android 17 平台新增属性时，流程包括：

1. 明确 owner、读写进程与跨分区稳定性。
2. 在 sepolicy 定义属性类型和访问规则。
3. 在 `property_contexts` 绑定名称、上下文与数据类型。
4. 优先用 `.sysprop` 和 `sysprop_library` 生成 Java、C++、Rust 类型安全 API。
...
`dumpsys` 通过 Binder 调用服务的 dump 接口，执行本身也有开销。输出通常是某一时刻的状态或有限历史，不能替代连续 trace。分析卡顿时可用 Perfetto 定位时间段，再用 dumpsys 补充进程、内存、Layer 或服务配置。
...
关闭 verity 会降低设备完整性保护，只能用于隔离的开发设备。动态分区和只读文件系统通常通过 OverlayFS 获得可写上层；重刷、清理 overlay 或切换镜像会使修改消失。

单文件 push 的风险高于分区同步：类路径中可能还有 dexpreopt 产物、架构变体、映射表或关联 APEX。安全做法是确认 Soong 安装路径，使用对应分区的 `adb sync`，再执行满足该组件激活条件的重启。
...
2. driver binaries 与设备、平台 build 匹配。
3. bootloader 和 radio 满足该系统镜像要求。
4. 设备允许 OEM unlocking，且已备份全部数据。

官方说明：AOSP 可以直接运行在 Cuttlefish 上，物理硬件则需要设备专用闭源库。driver binaries 是自解压脚本，接受许可证后把文件和 make 配置安装到 `vendor/`。页面出现某个 Pixel 型号，只能证明 Google 提供过相应二进制；仍要检查当前 tag 的产品配置。
...
`fastboot flashing unlock` 和 `fastboot flashall -w` 都会涉及数据清除；前者由设备端再次确认。刷写前应记录 factory image，并准备官方恢复路径。平台调试设备不应保存个人数据或生产凭据。
...
该命令只说明 Kleaf 的标准目标。复现 r6 时还要确认 common 仓库精确 tag、manifest revision、Kleaf 配置和输出摘要。设备 vendor modules、DTBO、vendor_boot、签名和 KMI 兼容性不包含在单个 GKI Image 里。

Cuttlefish 可以通过 `cvd create` 指定 kernel 与 initramfs 产物，适合验证 Android Common Kernel 与平台的组合。物理设备还需要厂商模块和设备启动链；把 GKI Image 单独刷入任意 Pixel 并不构成完整方案。
...
| 构建配置 | product、release config、variant、关键 aconfig/build flags |
...
先确认差异是否位于 HAL、kernel、GPU、thermal、Power HAL 或 vendor service。纯 Framework 状态机更适合在 Cuttlefish 复现；硬件相关性能必须回到目标设备。
