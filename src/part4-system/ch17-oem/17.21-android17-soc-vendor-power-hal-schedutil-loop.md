...
本章以 `android-17.0.0_r1` 和 `android17-6.18-2026-06_r6` 为锚点，重点回答三个问题：
...
部分切换由 `SCX_OPS_SWITCH_PARTIAL` 控制：只有 `SCHED_EXT` policy 的任务交给 SCX，`SCHED_NORMAL`、`SCHED_BATCH` 和 `SCHED_IDLE` 仍由 fair class 调度。因此旧文“SCX 启用后 CFS 只作备用”的说法遗漏了部分切换。
...
在 `android17-6.18-2026-06_r6/include/trace/hooks/sched.h` 中，名字含 `android_vh_scx_` 的 hook 声明共有 11 个，而旧文记录的 13 个与当前 tag 不符。它们可以按用途分组：
...
`vendor_available: true` 表示接口库可供 vendor 使用，不代表厂商必须上报固定名称的 CPU、GPU、Modem 或 sleep state。实体名称、数量和支持范围都要从目标机枚举。旧文按 Qualcomm、MediaTek、Samsung 列出的固定表缺少 AOSP 契约依据。
