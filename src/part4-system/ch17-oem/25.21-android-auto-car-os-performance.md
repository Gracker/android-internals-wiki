...
sources:
  - "Android for Cars App Library official docs"
  - "Android for Cars media/navigation/testing official docs"
  - "AAOS Automotive overview / VHAL / CarWatchdog / Power Management official docs"
  - "packages/services/Car car-lib and service sources @ android-17.0.0_r1"
  - "hardware/interfaces/automotive/vehicle/aidl IVehicle.aidl @ android-17.0.0_r1"
...
Android Auto 手机投射与 AAOS 车机原生的责任边界，host 渲染与客户端模型，四类版本轴

### 🔹 车载UI性能优化
模板 UI 由 host 绘制，客户端负责快速提供稳定模型；地图 Surface 与停驻 Activity 才进入应用渲染管线

### 🔹 车载网络性能
投射连接与业务网络分离，网络切换与断线恢复，可恢复数据管线设计

### 🔹 多设备协同性能
Android Auto 与车载系统的协同性能，双端状态同步，数据传输性能优化

### 🔹 车载应用性能边界
CarWatchdog 磁盘写入监控与资源超限，车辆电源状态机，CarService/VHAL 权限边界
...
### 🔸 车载环境适配
车内高温与阳光直射的热负载，热压力下降级策略，充电与持续导航的叠加影响

### 🔸 智能驾驶辅助集成
VHAL 与车辆属性订阅，ADAS 安全控制与信息娱乐应用的故障域隔离，陈旧数据防护
...
本章的平台锚点是 Android 17 / API 37 / `android-17.0.0_r1`。AAOS 系统侧源码以该 tag 为准，内核侧以 `android17-6.18-2026-06_r6` 为准。Android Auto 场景中，这个内核锚点只能描述运行客户端的 Android 端点；车载 host 的操作系统和内核由车厂产品决定，普通应用无法检查或控制。
