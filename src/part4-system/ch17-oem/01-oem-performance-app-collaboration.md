...
分析掉帧时，应围绕 deadline（帧完成时限）回看 runnable delay、运行时长、Binder 依赖、GPU fence、合成类型和频率变化。runnable delay 是线程已经可运行、却还没获得 CPU 的等待时间；fence 是 CPU、GPU 和显示模块之间的同步信号。渲染各阶段的职责可结合[渲染管线总览](../../part2-performance/ch18-rendering-pipelines/01-android-view-pipeline-analysis.md)阅读。
