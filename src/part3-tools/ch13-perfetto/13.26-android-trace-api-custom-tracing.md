...
Trace 埋点不会自行启动采集，也不会保存历史记录。设备上必须存在一场选择了目标应用的 Perfetto/系统跟踪会话，事件才会进入 trace。本文以 Android 17 / API 37 / `android-17.0.0_r1` 为平台锚点；涉及 ftrace 的内核实现统一参考 `android17-6.18-2026-06_r6`。
...
`atrace_apps` 控制哪些应用的 `TRACE_TAG_APP` 事件进入会话。系统服务类别则通过 `atrace_categories` 选择，例如 `am`、`wm`、`view` 或 `aidl`。两种配置承担不同职责。
...
DecodedFrame decodeFrame(const EncodedFrame& input) {
    ATrace_beginSection("VideoDecoder.decodeFrame");
    DecodedFrame frame = decoder.decode(input);
    ATrace_endSection();
    return frame;
}
```

启用异常的 C++ 工程应使用 RAII scope guard，保证异常路径也调用 `ATrace_endSection()`。名称同样不能包含换行或 `|`，高频调用前可用 `ATrace_isEnabled()` 避免昂贵标签构造。
