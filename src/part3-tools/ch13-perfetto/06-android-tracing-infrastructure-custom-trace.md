...
consolidated_from:
- src/part3-tools/ch13-perfetto/08-tracing-infrastructure.md
- src/part3-tools/ch13-perfetto/20-android-trace-api-custom-tracing.md
---
...
kprobe 与 uprobe 依赖函数符号、编译器内联（把函数体直接展开到调用点）、优化和二进制版本。平台升级后，探针位置与参数解释必须重新验证。BPF 程序若只在 map 中计数，与每次命中都向用户空间输出事件的成本差异很大。因此，静态 tracepoint、eBPF 聚合和 Perfetto 全量时间线应按问题组合，不能用一个固定开销表决定取舍。§14.16 会继续讨论 Android 动态探针的权限、版本边界和验证方法。
