...
前面几节我们分别介绍了 Perfetto 的基本概念、Trace 抓取、界面操作和命令行工具。掌握了这些基础之后，真正考验能力的是：面对一个具体的性能问题，你知道该看什么、怎么找、找到之后怎么判断。
...
冷启动是 Android 性能优化中最复杂的分析场景之一，原因在于它横跨多个进程、涉及大量 Binder 调用、受到 Zygote fork 和类加载等底层机制的影响。你在 Trace 中看到的不是"一个线程做了某件事"，而是"三个进程的十几个线程在几百毫秒内密集协作"。没有清晰的分析框架，很容易在 Trace 的信息洪流中迷失方向。
...
另一个常用技巧是 Perfetto 的 **Critical Path** 功能。选中主线程的某个长耗时 Slice，在底部信息区点击 "Critical path"，Perfetto 会自动高亮与这个 Slice 有依赖关系的所有 Task，帮我们快速追溯"到底是谁拖了后腿"。这个功能在启动分析中尤其好用，因为冷启动的链路很长，手动追踪唤醒关系非常耗时。
...
Perfetto 的 Flow 箭头是跨进程分析的核心工具。当你在 Trace 中选中一个 Binder Transaction Slice 时，Perfetto 会用箭头把 Client 端的 `transact` 和 Server 端的执行过程连接起来。沿着箭头，我们可以从一个进程跳到另一个进程，追踪一次跨进程调用的完整路径。

对于更复杂的场景（如 App 启动），一次操作可能涉及 App → `system_server` → Zygote → 新 App 进程 → `surfaceflinger` 的完整链路。虽然 Flow 箭头不会自动画出所有环节，但结合手动查找和 Critical Path 功能，我们可以逐步还原整条链路。
