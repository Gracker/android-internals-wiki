...
- `linux.perf` 由 tracing service 按需拉起 `traced_perf`。数据源被列出、daemon 已连接、采样结果可用是三个不同检查层级；产品内核、LSM/SELinux、build 类型和目标进程的 profileable 状态都可能限制采样。
...
### 1.8 Client composition 与 `FALLBACK_COMPOSITION`
...
AOSP 用户态源码能证明 `linux.perf` producer 和配置路径存在，不能证明每台 Android 17 量产设备都允许同样的采样：

- App 用户栈采样要求目标 App 为 profileable/debuggable，或设备使用 debuggable build；
...
| 外部限制 | kernel、LSM、SELinux、profileable、符号 | SF 启动时序、trace-start filter、厂商实现 | 发射点覆盖、pending fence、厂商扩展 |
...
Android 17 平台的 `external/perfetto` 基于 v54 版本线并带平台补丁。开发机使用更新的 Trace Processor 打开 trace 时，stdlib 与公开视图可能增加字段；查询前仍要用 `PRAGMA table_info(...)` 或官方 schema 确认列名。主机工具版本变化不会改变设备端数据源是否发出了 packet。
...
下面的配置同时采集帧预测/实际时间线和 BufferQueue phase，适合滑动、动画和首帧调查：
...
数据源无需额外 config message。复现动作要在 session 启动后发生；10 秒只用于示例，长时间或高 layer churn 场景应根据实际写入量调整 buffer。

### 6.4 `linux.perf` 最小调用栈配置

下面的配置以 100Hz `SW_CPU_CLOCK` 采样一个目标进程，并补充调度与进程元数据：
...
留空 `target_cpu` 会覆盖所有在线 CPU。若要开启 `kernel_frames: true`，先在目标 build 上验证内核栈权限与符号可用性。调高频率或改成高频 tracepoint 前要逐级压测，避免 ring-buffer overrun 和 unwinder overload。
