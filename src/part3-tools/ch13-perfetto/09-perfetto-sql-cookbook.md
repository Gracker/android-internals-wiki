...
PerfettoSQL 继承 SQLite 语法，并增加 `INCLUDE PERFETTO MODULE`、`CREATE PERFETTO TABLE`、`SPAN_JOIN` 等追踪分析能力。标准库模块会创建已经整理好的表、视图和函数。以 Binder 为例，`android.binder` 已经通过流关系配对客户端事务与服务端回复，也会区分同步调用和 `oneway` 异步调用。直接用 `slice.name GLOB '*binder*'` 无法获得同等语义。
...
- `pid`、`tid` 是操作系统可见编号，进程或线程退出后可能复用。
- `upid`、`utid` 是 Trace Processor 在一份 Trace 内分配的唯一标识，关联表时优先使用它们。
...
结果可能有多行。同一个包名在采集期间重启时，会出现多个 `upid`；主线程也可能因信息不完整而同时命中 `is_main_thread` 与 `tid = pid` 的兼容条件。正式查询应选定与问题时段相交的 `upid`，不能只按进程名对整份 Trace 聚合。
...
时间窗口采用半开区间 `[start_ts, end_ts)` 更容易处理相邻事件。两个区间相交的条件是 `a.ts < b.end_ts AND a.end_ts > b.ts`，交集长度为 `MIN(a.end_ts, b.end_ts) - MAX(a.ts, b.ts)`。先按 `upid`、`utid` 与窗口过滤，再做区间关联，可以大幅减少大 Trace 的计算量。
...
输出中的每一行都已经裁到目标窗口。把这段查询物化为 `CREATE PERFETTO TABLE` 后再复用，适合几十秒以上且包含全量调度事件的 Trace；一次性检查则保留 CTE 即可。

### 指标必须带上分母和边界

“主线程 CPU 占用 80%”缺少窗口就无法复查。“Binder 超过 1 ms 就慢”也忽略了接口语义、设备负载与调用是否同步。SQL 可以稳定地产生 P50、P90、P99，但分位数依旧需要同场景、同设备配置、同采集配置的对照组。
...
### `Choreographer#doFrame` 用于寻找 UI 线程重活

Android 版本和追踪点实现会让 `doFrame` 名称带上帧号等后缀，因此查询使用 `GLOB 'Choreographer#doFrame*'`。这张表适合做 UI 线程耗时排序与分布统计，不能单独给出系统级卡顿结论。

计算目标进程 `doFrame` 的墙上时间分位数，可以判断 UI 线程回调分布是否发生回归，并找出需要回到时间线检查的长尾。
...
FrameTimeline 从 Android 12 / API 31 开始提供期望与实际时间线。`actual_frame_timeline_slice` 包含 `jank_type`、`jank_severity_type`、`on_time_finish` 和 `present_type` 等平台判定；`android.frames.timeline` 会把 `Choreographer#doFrame`、`DrawFrame` 与时间线事件整理为 `android_frames`。

Android 17 的 `android_frames` 仍有明确边界：同一帧可能关联多个 `layer`，表会聚合它们；当前实现只选第一个 `DrawFrame`，源码保留了对应待改进项；缺失或重复时间线事件时，`actual_frame_timeline_count` 与 `draw_frame_count` 会暴露异常。自动化统计应保留这些完整性列。
...
`deadline_overrun_ms > 0` 表示实际结束时间晚于期望结束时间。负值表示仍有余量。`jank_type` 由平台记录，可继续区分应用、SurfaceFlinger、调度等类别。表为空时应检查采集配置、Android 版本与目标进程是否产生了 FrameTimeline 数据。
...
`one_core_occupancy_pct` 的分母是查询中的 5 秒窗口。高占用只说明线程经常运行；函数热点仍需结合采样栈、调用栈或 `slice`。低占用伴随长延迟时，应检查 `Runnable` 排队、睡眠、I/O 与同步等待。
...
较长的 `R` / `R+` 区间说明线程已经具备运行条件，却在运行队列中等待。它可能来自 CPU 争用、优先级、CPU 亲和性、`cgroup` 或频率策略，不能只凭时长归因。较长的 `S` 也要与 Binder、`futex`、monitor 锁竞争和应用 `slice` 对时。
...
`interface`、`method_name` 与 `aidl_name` 依赖 AIDL/HIDL 追踪 `slice`。字段为空时，事务配对仍可能有效，只是 Trace 缺乏接口名。`client_monotonic_dur` 和 `server_monotonic_dur` 会剔除全机休眠时间；分析用户感知延迟时仍要保留墙上时间对照。

按接口、方法和同步类型统计目标进程发起的 Binder 事务，可以找出高频接口和长尾接口，同时保留样本量。
...
`thread_state_type = 'binder_txn'` 表示客户端区间，`binder_reply` 表示服务端回复区间。客户端处于 `S` 往往符合等待同步回复的预期；服务端出现较长 `R` / `R+`，才为“服务端线程排队等 CPU”提供直接证据。仍需回到该事务的 `slice` 子树确认具体工作。

所谓“Binder 线程池利用率”也要给出容量与窗口。只累加 Binder 线程 CPU 时间，得到的是运行时间，不是线程池占用率。线程池可能睡眠等待工作，也可能在处理非 Binder `slice`；若要分析饱和，应同时观察服务端并发事务数、可用 Binder 线程数、事务排队与各线程状态。
...
手工用 `bindApplication` 到第一个 `doFrame` 推导冷启动容易漏掉 `system_server` 发起阶段、`RenderThread`、可见帧以及热启动分支。`android.startup.startups` 会按 Trace 中的 SDK 与事件格式选择对应解析路径，输出 `android_startups` 和 `android_startup_processes`。`android.startup.time_to_display` 继续关联首帧与 `reportFullyDrawn`，提供 TTID 与 TTFD。

列出目标包的启动区间与显示时间，可以保留平台识别的 `cold`、`warm`、`hot` 分类，并区分启动区间、初始显示与完全显示。
...
TTFD 依赖应用调用 `reportFullyDrawn()` 以及对应追踪事件；空值不能当作零毫秒。进程在启动期间死亡并重建时，一个 `startup_id` 可能关联多个进程，查询会保留每个 `upid`。

### 标准库分解启动主线程时间

`android.startup.startup_breakdowns` 会把启动主线程 `slice` 与 `thread_state` 裁成互斥区间，并给每段生成 `reason`。该表属于带判断的标准库结果，适合找候选原因，仍要用原始时间线和源码确认。
...
输入分发超时常见默认值为 5 秒，Broadcast、Service、JobService、前台服务和 `system_server` 看门狗使用不同规则，OEM 与前后台状态还可能改写超时。Android 17 的 `android.anrs` 模块会从 `system_server` 的 ErrorId、Subject 与计时事件中解析 `anr_type`、`anr_dur_ms` 和 `default_anr_dur_ms`；源码也明确标注默认值仅对应 AOSP/Pixel 的常见配置。
...
该表依赖对应平台事件被写入 Trace。表为空时，可以另查 `android_logs` 或外部 `bugreport`，但日志与 Trace 需要有可校准的时间基准。`default_anr_dur_ms` 适合解释平台默认路径，不应覆盖设备现场配置。
...
ART 会把 monitor 锁竞争记录成带方法信息的 `slice`。Android 17 的 `android.monitor_contention` 模块解析阻塞线程、持锁线程、方法、源码位置、锁名和 Binder 回复关系。`lock_name` 已是标准表字段，但它来自周围 `slice` 的解析，缺失时应回到 `blocking_method` 与 `blocked_method`。
...
`waiter_count` 是该解析模型中的从零开始计数，不能直接当作线程池总等待人数。`binder_reply_id` 有值时，持锁工作可能发生在 Binder 服务线程；应继续检查对应事务和持锁线程状态。
...
`SPAN_JOIN` 接受两张至少含 `ts`、`dur` 的表或视图，并输出它们的时间交集。`PARTITIONED cpu` 表示只在相同 CPU 内求交。输入在每个分区内不能互相重叠；普通业务 `slice` 经常嵌套，直接交给 `SPAN_JOIN` 会破坏这个前提。
...
2. 从 FrameTimeline 判定、启动实例或 ANR 事件选定时间窗，避免先看整份 Trace 的全局 Top N。
...
6. 用 Android 17 平台源码和内核锚点确认追踪点语义；缺少证据时保留为候选原因。
