...
本节以 Android 17 / API 37 / `android-17.0.0_r1` 为平台锚点，讨论 Perfetto v53 引入的一组 profile 能力。它们属于 Perfetto 导入器、Trace Processor 和 Web UI 的演进，并不是从某个 Android API 级别开始提供的应用 API。分析端可以使用比设备内置版本更新的 Perfetto UI 或 `trace_processor_shell`，所以还要把“采集端版本”和“打开 trace 的工具版本”分开记录。

§13.12 已经介绍 CPU profile、火焰图和符号化的完整操作路径。本节集中回答几个容易混淆的问题：
...
| Simpleperf protobuf 导入 | Trace Processor + UI | 读取 `report-sample --protobuf` 结果及样本时间戳 | v57.2 仍不导入 protobuf 内的 context-switch 记录 |
...
Simpleperf protobuf 保留单个样本的时间戳。Perfetto 将它们写入公开表 `cpu_profile_stack_sample`，调用栈则经 `stack_profile_callsite`、`stack_profile_frame` 和 `stack_profile_mapping` 关联。到 Perfetto v57.2 为止，文件里的 context-switch 记录仍没有转换成 `sched_slice`。因此，Simpleperf 样本间的空白不能直接解释成线程被抢占、休眠或阻塞。

`linux.perf` 数据源把 perf event 样本写进原生 Perfetto trace。在 Android 15 及以上版本上，它适合“某段主线程为何变慢”“CPU 热点是否和 Binder 等待重叠”这类需要统一时钟的问题。`user` 构建通常只允许 profileable 或 debuggable 应用被采样；`userdebug` / `eng` 构建的限制较少。采样频率会影响设备负载，官方文档建议非 native 场景按每个 CPU 低于 200 Hz 控制，本节示例使用 100 Hz。
...
若目标是解释一次 Android 启动中的 120 ms 空洞，只打开 pprof 不够。该问题需要带时间戳的 CPU 样本和 system trace。可以改用 `linux.perf` 采集，或分别采集 profile 与 trace 后，在确认时钟可对齐的前提下合并。缺少时钟映射时，只能做统计对照，不能声称两类事件发生在同一时刻。

## 4. Simpleperf protobuf 导入流程

下面的命令用于采集一个可调试或可 profile 的应用，并输出 Perfetto 能直接读取的 Simpleperf protobuf。`app_profiler.py` 和 `simpleperf` 均应取自与 Android 17 源码或设备 ABI 匹配的 Simpleperf 工具集：
...
固定源码 `android-17.0.0_r1` 中，`cmd_report_sample.cpp` 为 protobuf 写入 `SIMPLEPERF` magic 和格式版本 1，并实现了上述参数。这个格式版本是 Simpleperf 文件协议版本，和 Android API 37、Perfetto v53 都不是同一套版本号。

导入后，可以先用下面的 SQL 检查目标进程各线程是否有足够样本。该查询只评估样本覆盖范围，不计算函数热点：
...
`sample_count` 很低时，火焰图的宽度容易受采样随机性影响；`first_sample_s` 与 `last_sample_s` 可以暴露采集窗口过短、目标进程启动过晚等问题。函数聚合优先使用 Perfetto 的 profile/flamegraph 视图或公开的标准库模块，不要依赖以 `__intrinsic_` 开头的内部表，它们不承诺查询兼容性。
...
v53 在 `contrib/rust-sdk` 中加入第一个社区维护的 Rust SDK，并以 `perfetto-sdk` 发布到 crates.io。初始实现由 Rivos 工程师贡献。到 v57.2 源码时，crate 版本已经演进到 1.0.2，支持 Track Event、自定义 DataSource 和 TracingSession 等接口。

| 维度 | v53 初始版本 | v57.2 源码状态 |
|---|---|---|
| 目录 | `contrib/rust-sdk` | `contrib/rust-sdk` |
| `perfetto` crate 版本 | 0.1.2 | 1.0.2 |
| 维护属性 | 社区维护 | 仍位于 `contrib`，兼容性需按 crate 文档核对 |
| 主要用途 | Rust 程序写入 Perfetto trace | Track Event、DataSource、TracingSession |
...
Android 17 固定源码中存在 `use_lockfree_taskrunner` 的 Perfetto flag，但 flag 的存在只说明源码保留了选择能力。产品构建值、运行平台和对应分支实现共同决定实际路径。排查 tracing service 自身开销时，应读取当前构建的 flag 值和二进制源码版本，再结合 trace 证据判断。

## 10. Android 17 与 Perfetto UI 的版本关系

Android 平台 tag、Perfetto 上游 release 和浏览器中的 UI 可以处于不同版本。本文采用下面的版本口径：

| 版本点 | 能力与用途 | 与 Android 17 的关系 |
|---|---|---|
| Perfetto v53.0 | 本节六项能力首次成组发布 | Android 17 固定源码已包含这些上游能力 |
...
| Perfetto v57.2 | 本节复核导入器和 Trace Processor 行为时使用的较新分析端 | 可以作为宿主机 UI/CLI；不能写成 Android 17 内置版本 |
