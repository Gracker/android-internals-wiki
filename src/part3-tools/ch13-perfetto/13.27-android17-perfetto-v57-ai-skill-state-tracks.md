...
> 本章同时出现 Android 平台版本、Perfetto 上游版本和 AI skill 版本。它们各自升级，不能用“Android 17 支持 Perfetto v57”概括。本文的平台基线固定为 Android 17 / API 37 / `android-17.0.0_r1`，上游工具基线固定为 Perfetto v57.1。
...
| 对象 | 本章采用的版本 | 在哪里运行 | 能否直接提供 v57 状态轨道 |
|---|---|---|---|
...
v57.1 的 `TrackEvent.Type` 增加 `TYPE_STATE = 5`。事件的 `track_uuid` 必须指向带 `StateDescriptor` 的轨道。`StateDescriptor` 在 v57.1 中是空消息，只承担类型标记；状态字符串通过事件名称路径传递。空值或未设置状态会结束当前状态，使轨道进入 idle。
