...
它不适合被想象成“接了以后平台就有了”。  
很多团队用 Matrix 的难点，最后并不在 SDK 接入，而在 schema、采样、回查和治理流程。

### KOOM：只有内存问题真正成为主矛盾时，它的价值才会特别明显
...
那 KOOM 的优先级会非常高。  
但如果团队当前的主要问题是首页慢、列表卡、响应延迟，它通常不应该作为第一站。
...
DoKit 覆盖的能力很杂，也确实很实用：FPS、启动耗时、网络、页面检查、调试辅助、沙盒浏览。  
...
没有任何线上 APM 是“零成本”的。真正成熟的方案，一定要同时看：
...
没有哪套方案会在所有维度都最优。  
选型真正要做的，是把“换来了什么”以及“付出了什么”同时看清楚。
