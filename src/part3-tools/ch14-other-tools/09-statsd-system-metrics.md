...
| JobScheduler | `ScheduledJobStateChanged`、`DeferredJobStatsReported`、`DeviceWideJobConstraintChanged` | Job 状态变化、约束变化、是否存在延迟执行迹象 | `dumpsys jobscheduler`、WorkManager 日志，详见 5.3 节 |
...
| UprobeStats / eBPF | `UprobeStatsInternalError`、`UprobeStatsInvocation`、`UprobeStatsBpfAttached`、`UprobeStatsBpfMapPolled` | 用户态探针 uprobe 是否挂载、eBPF（内核中的受限程序）是否运行、eBPF map（内核与用户态共享的数据结构）是否被读取，或是否出现内部错误 | eBPF 程序、Perfetto trace、UprobeStats 模块日志，详见 14.16 节 |
...
2. 用 Perfetto 还原事件附近的时序：查主线程、`RenderThread`（渲染线程）、Binder、调度、memory counter（内存数值轨道）、FrameTimeline（逐帧生命周期轨道）。Perfetto 采集基础设施详见 13.6 节，SQL 分析详见 13.7 节。
