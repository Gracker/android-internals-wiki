...
本章的工具源码引用固定到 AGI `v3.3.3`（commit `5f97b4fd99a9459320b782203ce2de5351a1e661`），使用流程再与 2026 年更新的 Android Developers AGI 文档交叉检查。AGI 二进制版本和 Android OS 版本互相独立；复现实验时应同时记录二者。

当前官方 quickstart 要求受支持设备运行 Android 11 或更高版本。历史 AGI release 对 Android 10 的兼容记录不能代替当前工具验证；在 Android 17 上也要经过 AGI 的 device validation，支持状态由 OS、GPU 与 driver 共同决定。
...
layer 设置以 package 为单位。某些游戏会在包内启动独立的渲染进程，AGI 用私有系统属性 `debug.agi.procname` 指定要捕获的进程名。`Spy::Spy()` 会读取当前进程名并与该属性比较；属性为空时接受任意进程，名称不匹配时使用空输出路径。
...
3. GAPIS 经 adb forward 连接该 socket，发送 capture header。header 包括 API bitmask、起始 frame、捕获 frame 数、是否延迟开始、是否关闭 buffering、是否记录时间戳等选项。
...
- capture header：格式版本、设备、ABI 等信息；
...
清理后重启目标 App，再用 `settings get global ...` 和 `getprop debug.agi.procname` 确认没有残留。global settings 会跨重启保存，遗留的 layer 配置可能继续影响同包进程。
...
- [AGI 开发文档：Life of a gfxtrace](https://github.com/google/agi/blob/v3.3.3/DEVDOC.md)
- [Android 抓帧启动与 `debug.agi.procname`](https://github.com/google/agi/blob/v3.3.3/gapii/client/adb.go)
- [Android Vulkan layer settings 写入与清理](https://github.com/google/agi/blob/v3.3.3/core/os/android/layers.go)
- [GraphicsSpy layer manifest](https://github.com/google/agi/blob/v3.3.3/gapii/vulkan/vk_graphics_spy/cc/GraphicsSpyLayer.json)
- [GraphicsSpy wrapper 加载 `libgapii.so`](https://github.com/google/agi/blob/v3.3.3/gapii/vulkan/vk_graphics_spy/cc/layer.cpp)
- [gapii 进程选择、socket 与 capture 初始化](https://github.com/google/agi/blob/v3.3.3/gapii/cc/spy.cpp)
- [gapii 主机连接与 capture protocol](https://github.com/google/agi/blob/v3.3.3/gapii/client/capture.go)
- [`.gfxtrace` 解析与导出](https://github.com/google/agi/blob/v3.3.3/gapis/capture/graphics.go)
- [ProtoPack v2 文件格式](https://github.com/google/agi/blob/v3.3.3/core/data/pack/README.md)
- [GAPIR replay VM](https://github.com/google/agi/blob/v3.3.3/gapir/README.md)
- [gapid APK manifest](https://github.com/google/agi/blob/v3.3.3/gapidapk/android/apk/AndroidManifest.xml.in)
- [`replay2` 公开源码目录](https://github.com/google/agi/tree/v3.3.3/replay2)
