...
截至 2026 年 7 月 30 日，GitHub Releases 的最新稳定版仍是 AGI `v3.3.3`，对应 commit `5f97b4fd99a9459320b782203ce2de5351a1e661`，发布于 2025 年 1 月 20 日。本章用这个 tag 核对工具内部实现，再用更新至 2026 年 5 月 19 日的 Android Developers 文档核对当前支持流程。AGI 二进制版本和 Android OS 版本互相独立；复现实验时应同时记录二者。
...
`Writer/rendering_pipelines` 的 Native Graphics 与 Game 路径还给出两个使用限制：
...
显示路径和验收边界还对照了 `Writer/rendering_pipelines/S08_native_graphics_type.md` 与 `S13_game_type.md`。这两份原稿用于确认 acquire、submit、producer fence、BufferQueue、SurfaceFlinger/HWC 与 present 的先后关系；AGI 组件实现仍以 `v3.3.3` 源码为准。
