...
last_verified_against: "AOSP android-17.0.0_r1 (bionic linker, ART, system/sepolicy)；Android Developers 16 KB/JVMTI/ProfilingManager 文档；ByteHook v1.1.2；ShadowHook v2.0.1；xHook 391d0c4；Matrix 3b8293b；KOOM df3b8c3"
confidence: medium
sources:
- type: reference
...
平台锚点是 Android 17 / API 37 / `android-17.0.0_r1`，讨论范围限于自有应用进程中的性能观测。Android 内核锚点为 `android17-6.18-2026-06_r6`；ELF（Executable and Linkable Format，可执行与链接文件格式）重定位、ART（Android Runtime）和应用域 SELinux 规则位于平台源码侧。需要更深入的三类 native 方案对比时，可继续阅读 [20.17 Native Hook 技术选型与实现](../../part5-app/ch20-stability/17-native-hook-technology-selection-implementation.md)。
...
- API 35 及以上可用 [`ProfilingManager`](11-profiling-manager.md) 请求 system trace、native heap profile、stack sampling 或 Java heap dump；
...
**延伸阅读**：[14.11 ProfilingManager](11-profiling-manager.md) · [20.12 FD 资源监控与治理](../../part5-app/ch20-stability/12-fd-resource-monitoring.md) · [20.17 Native Hook 技术选型与实现](../../part5-app/ch20-stability/17-native-hook-technology-selection-implementation.md)
