...
选型时至少记录目标符号、拦截位置、ABI、装载时机、链式调用规则、失败降级、4 KB / 16 KB page size、BTI/PAC、CFI（控制流完整性）/unwind 和目标 ROM。具体实现、回调安全与验证矩阵统一放在 [Hook 基础设施与性能工具实现原理](26-hook-infrastructure.md)，本节不再重复维护两套原理说明。
