...
Simpleperf 是 Android 平台的原生 CPU 性能分析工具。它借助 Linux `perf_events` 子系统采集计数器、程序计数器和调用栈，再把结果写入 `perf.data`。它适合回答“CPU 时间花在哪些函数”“某段代码消耗了多少条指令”“热点由哪条调用路径进入”等问题。
...
内核决定事件能否打开、何时产生样本以及样本进入哪个环形缓冲区。Simpleperf 负责配置事件、持续读取记录、处理调用栈并保存分析所需的元数据。报告工具再把地址映射到库、函数和源码行。
...
1. `profileable` 发布应用：Android 10 起可允许 shell 使用设备预装的分析工具采样，性能形态比调试构建更接近发布版本。
2. `debuggable` 应用：具备应用上下文采样权限，但调试构建会改变 Java、JNI 和编译优化行为。Android 12 及更高版本上，A17 的 `app_profiler.py` 默认拒绝调试构建；接受这项偏差后，需显式传入 `--unrepresentative_profile_debug_app`。
3. root/AOSP 调试设备：可分析普通发布应用、原生系统进程或全系统目标；最终能力仍由内核和安全策略决定。
...
`android:shell="true"` 允许 shell 通过 Simpleperf、Perfetto 等预装工具读取有限的性能分析数据。它不开放内存数据，也不允许调试器任意检查应用状态，具体限制见 [`<profileable>` 官方说明](https://developer.android.com/guide/topics/manifest/profileable-element)。

`adb shell am set-debug-app` 不会修改 APK 的 `debuggable` 或 `profileable` 属性，不能作为发布包的采样授权。`persist.simpleperf.profile_app_uid` 服务于“应用内控制 Simpleperf”的专用 API；Android 13 起，`api_profiler.py prepare --days` 可为这项授权设置有效天数。常规外部采样交给 `app_profiler.py` 处理。

## 14.2.3 工具位置与环境检查

NDK 发行包把设备端程序、主机端程序和 Python 脚本放在顶层 `simpleperf/` 目录，不在 LLVM 工具链的 `bin/` 目录：
...
Android 16/17 上应使用同一版本 NDK 附带的脚本发起应用采样。脚本会选择 `simpleperf_app_runner` 和设备内置程序。手工 `adb push` 一个二进制再从应用上下文执行，可能可以显示帮助信息，却无法获取内核样本。

## 14.2.4 一条可靠的应用采样路径

下面以 `profileable` 发布应用为例，记录十秒用户态 CPU 时间和 DWARF 调用栈。命令从环境变量读取真实包名和未剥离原生库目录，缺少输入时会立即退出。
...
这组记录参数也是 A17 `app_profiler.py` 的默认值；显式写出它们便于复现实验。脚本在当前目录生成 `perf.data`，再用 `UNSTRIPPED_LIB_DIR` 准备 `binary_cache/`。`report_html.py` 读取两者并生成 `report.html`。采样窗口内必须操作目标功能，否则报告只反映空闲或启动状态。
...
脚本会先结束目标进程、布置采样，再启动 Activity。它不会替实验清除应用数据或系统文件缓存。比较冷启动结果时，应固定应用数据、ART 编译状态、存储状态和业务输入。
...
并发采集的硬件事件超过可用 PMU 计数器时，内核可能复用 PMU。复用后的计数依赖时间缩放，误差会随负载和调度变化。工程分析可采用两步：
...
`-g` 默认选择 DWARF 调用栈。内核为样本保存寄存器和用户栈数据，Simpleperf 使用 Android 的 `libunwindstack` 展开。它对 Java/原生混合栈更稳健，也依赖 ELF 中的 `.eh_frame`、`.debug_frame`、`.ARM.exidx` 或 `.gnu_debugdata`。A17 的默认单样本栈数据上限是 65528 字节；深栈仍可能在到达线程入口前结束。
...
生成缓存后重新运行报告。若 `[unknown]` 仍集中在应用库，应检查 build ID、ABI、拆分 APK 和被采样进程实际加载的路径。

## 14.2.8 多进程、线程与离开 CPU 时间

### 目标选择

下面每条命令都是独立的目标选择方式，执行时只选与问题相符的一条。PID、线程 ID、进程名正则和包名都从真实环境变量读取。
...
普通 CPU 采样只在线程运行时产生样本。`--trace-offcpu` 额外记录 `sched:sched_switch` 样本和上下文切换记录，用相邻调度时间戳估算线程离开 CPU 后停留在哪条调用路径。
...
`--trace-offcpu` 要求整次记录只选择 `cpu-clock` 或 `task-clock` 中的一个事件。`on-off-cpu` 报告模式会把两类权重分开显示；离开 CPU 的权重来自调度时间戳，丢失样本或切换记录都会降低精度。若问题涉及可运行队列等待、线程唤醒者、Binder 对端或 CPU 频率，Perfetto System Trace 能提供更完整的时间上下文。

## 14.2.9 丢样、截断栈与缓冲区

采样数据要经过内核 `perf` 环形缓冲区和 Simpleperf 用户态记录缓冲区。内核缓冲区溢出会增加 `kernelspace` 丢失记录；对栈数据超过 1 KiB 的 DWARF 样本，A17 会在用户态缓冲区紧张时先缩短栈，再丢弃样本以保留 mmap、fork 等后续分析需要的记录。
...
生成的文件可在 Android Studio CPU Profiler 或 Perfetto UI 中打开。转换入口是 `report-sample`；`--show-callchain` 决定输出中是否包含调用链。

Perfetto Trace Processor 会把这种 protobuf 中的样本导入 `cpu_profile_stack_sample` 和栈分析表。该导入器当前不导入 Simpleperf 的上下文切换记录，也不按事件类型提供完整时间线语义；它适合查看样本火焰图，不能替代一次包含调度数据的系统跟踪。

Perfetto 的 `linux.perf` 是另一套采集入口。Android 17 的 Perfetto 具备该数据源，它也调用 `perf_event_open`，并在同一次 Perfetto 会话中直接生成调用栈样本，因此能与 ftrace 数据共享时间轴。已经生成的 Simpleperf `perf.data` 不会自动并入新会话。

下面的 A17 配置以真实系统进程 `surfaceflinger` 为目标，按 100 Hz 采集用户态调用栈，并在同一会话加入调度和进程信息。系统进程采样通常需要 userdebug/eng 构建或等价权限。
...
| 模块 | Android 17 中的职责 |
| --- | --- |
| [`event_selection_set.cpp`](https://android.googlesource.com/platform/system/extras/+/android-17.0.0_r1/simpleperf/event_selection_set.cpp) | 组织事件、CPU、线程和 perf 事件文件描述符，启动记录读取线程 |
...
分析 big.LITTLE 或更多 CPU 簇时，应保存以下实验条件：
...
不要关闭温控服务、修改 `sysctl`、解除厂商 PMU 限制或强制锁频来“修复”采样。此类修改会改变调度和功耗环境，也可能损害设备。需要解释频率和调度时，可另采 Perfetto 的 `power/cpu_frequency`、调度与温控数据；目标设备是否开放对应跟踪点仍由产品配置决定。
