...
本章聚焦 Android 17（`android-17.0.0_r1`）中 `frameworks/native/services/gpuservice/` 的 GPU 内存观测路径。GpuService 不分配 GPU 内存，也不维护用户态显存池；它读取 GPU 驱动上报的累计值，并通过 dumpsys、Perfetto 和 statsd 暴露给诊断工具。buffer 的分配、复用与回收仍由 gralloc、图形实现和厂商驱动负责。
...
| 统计归因 | `GpuStats` | GL / Vulkan / ANGLE 驱动加载与功能使用上报 | statsd pull atom、`dumpsys gpuservice --gpustats` | 驱动加载、失败、耗时和图形功能使用情况怎样？ |

`GpuMem` 与 `GpuMemTracer` 读取同一份 BPF map，Perfetto 的 ftrace 更新也来自驱动的同一个 tracepoint ABI。`GpuStats` 不统计 GPU 内存字节，它保存驱动加载和功能使用事实。排查 GPU 内存增长时，GpuMem 与 Perfetto 负责占用趋势，GpuStats 用来补充驱动选择和失败模式。
...
`dump()` 和 `traverseGpuMemTotals()` 通过 `getFirstKey()` / `getNextKey()` 逐项读取 map。遍历期间驱动仍可更新 map，因此输出是 best-effort 快照，不具备跨 entry 的原子一致性。GpuMem 能回答驱动最近上报的进程总量，无法指出哪一个 DMA-BUF 或 BufferQueue slot 占用最大。
...
`dumpsys gpuservice --gpustats --global` 与 `--app` 可以限定输出；追加 `--clear` 会清除所选统计。`--clear` 会改变后续 dumpsys 和 statsd pull 的结果，采集证据前不要使用。
...
| 数据源 | 统计口径 | 能回答的问题 |
|---|---|---|
| `dumpsys gpuservice --gpumem` | 驱动上报的 `(gpu_id, pid)` 当前总量 | 哪个进程的 GPU 可寻址内存增长？ |
...
```bash
adb shell dumpsys gpuservice --gpumem
...
第一条定位进程总量，第二条核对 Framework 内存分类，`dmabuf_dump` 给出 inode 与 exporter 线索，SurfaceFlinger 输出用于关联 layer。Android 17 的 `SurfaceFlinger::doDump()` 命令表没有 `--bufferstats`，不要依赖该参数。默认 SurfaceFlinger dump 也不是完整的 DMA-BUF 引用跟踪器，它只能提供当前合成与 layer 上下文。
...
§10.8 讨论应用与图形内存症状，§14.8 介绍 GPU 调试工具，§14.25 展开 eBPF 观测范围。本节只处理 GpuService、GPU memory tracepoint、GpuStats 和 DMA-BUF 对账之间的接口关系。
