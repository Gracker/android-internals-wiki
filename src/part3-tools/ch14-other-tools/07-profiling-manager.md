...
- 🔹 **BufferFillPolicy 公开枚举**：只有 `DISCARD` 和 `RING_BUFFER`，没有 `FLUSH_FULL`
...
`FLUSH_FULL` 不在公开 API 里，代码和正文都不该再出现。写“保住窗口开头”时用 `DISCARD`，写“保住结束点附近现场”时用 `RING_BUFFER`，语义就能和真实枚举对上。
