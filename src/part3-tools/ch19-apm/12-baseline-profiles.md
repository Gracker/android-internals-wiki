...
官方文档给出的经验值是：不少应用在优化后观察到约 30% 的代码执行速度提升。这个数字是汇总经验，不是项目验收标准。应用是否受益、受益多少，仍要在自己的 release 产物和目标设备上成对测量。

Baseline Profile 不采集线上性能数据，也不会定位慢点。它与 APM 的关系是：

1. APM 或 Android vitals 发现某个版本的首开、启动或关键交互退化。
2. Perfetto、Macrobenchmark 等工具确认其中包含解释执行、JIT、类加载或 DEX 访问成本。
...
这条链路要分成“构建期”和“设备端”两段理解。下面的图只表达数据交接关系，不表示这些组件在所有安装渠道上彼此直接调用。
...
构建期生成的是 Human Readable Format（HRF）规则。AGP 汇总应用和依赖库规则，R8 按 release 产物中的类名、方法名和优化结果重写规则，再生成供设备使用的二进制 profile。Startup Profile 也在这一阶段影响 DEX 布局。

设备端路径受 Android 版本和安装来源影响。Google Play、PackageManager、ART Service 与 `androidx.profileinstaller` 都可能参与 profile 的投递或编译调度，不能把它简化成 `ProfileInstaller` 直接调用 `profman` 和 `dex2oat`。

Android 17（API 37，`android-17.0.0_r1`）源码提供了几个可核对的边界：

- `artd.cc` 的 `copyAndRewriteEmbeddedProfile()` 提取嵌入式 profile，并通过 `profman --copy-and-update-profile-key` 按当前 DEX 校验和重写 profile key。profile 与 APK 不匹配时会返回 `BAD_PROFILE`。
- `profman.cc` 负责读取、合并、检查与转换 profile；其 HRF 解析代码明确识别 `H`、`S`、`P` 三种方法 flag。
- `dex2oat.cc` 在收到 profile 输入时把默认 compiler filter 设为 `speed-profile`；如果 profile 缺失、为空或不能用于 PGO，则把 `speed-profile` 降为 `verify`。
...
|---|---|---|
| Android 5.0-6.0（API 21-23） | 平台采用 full AOT；ProfileInstaller 不安装 Baseline Profile。 | 不把这个设备段写成 Baseline Profile 覆盖范围，也不能用 `CompilationMode.Partial` 做同类对照。 |
...
| Android 9-17，其他商店或 sideload | 商店未必支持安装期 profile。ProfileInstaller 可把 profile 放入 current profile，等待后台 dexopt。 | 包内文件、current profile、reference profile 和编译状态要分开检查。 |
| Android Studio 或 Gradle 安装 non-debuggable build | AGP 8.4 起会在本地安装流程处理 Baseline Profile。 | AGP 版本、安装 task 和目标 variant 都要写进实验记录。 |
...
- Google Play Internal App Sharing 不支持 Baseline Profile；Internal Testing track 支持。
- 非 Play 渠道可能要等到设备空闲和充电时运行后台 dexopt，常见现象是隔夜后才出现 `speed-profile`。
- AGP 8.4 之前的 Studio、Gradle 或其他非 Play 安装不会自动完成这段本地编译流程。
- Firebase Test Lab 设备，包括 Gradle-managed Test Lab device，不支持生成 Baseline Profile。
- 省电策略和 OEM 行为可能延迟后台 dexopt；本地复现时需要记录这些条件。
...
`H`、`S`、`P` 可以组合。`Lcom/example/app/MainActivity;` 中的 `L` 不属于 flag，它是 DEX 类型描述符中“引用类型”的前缀。参数列表中的 `Landroid/os/Bundle;` 也是同一语法；`I`、`J`、`Z`、`V` 分别表示 int、long、boolean 和 void 等类型。
...
生成 profile 的 variant 则需要关闭混淆和优化，使采集结果能对应原始签名。Baseline Profile Gradle Plugin 会创建并配置 `nonMinifiedRelease`、`benchmarkRelease` 等 variant；R8 在构建 release 时把未混淆规则重写到优化后的程序。不要把旧版 `mapping.txt` 对应的 HRF 文件直接塞进新版 release 包。

## 当前工具链与 Gradle 配置

截至 Android 17 基线，官方最低建议为 AGP 8.0.0、Macrobenchmark 1.4.1 和 ProfileInstaller 1.4.1。AGP 8.2+ 的 Android Studio 模板能一次建立 producer module、consumer 配置、生成测试和 benchmark；新项目宜从模板开始。
...
producer 是只承载 instrumentation test 的 `com.android.test` module。下面的 GMD 使用 Android 17 AOSP 镜像生成规则：
...
API 33 及以上可以在非 root 设备生成 profile；较低版本需要 rooted 环境，官方自动生成流程的下限是 rooted API 28。生成规则可以使用模拟器或 GMD，因为这一阶段不输出性能结论；收益测量应使用物理设备。
...
- `automaticGenerationDuringBuild = true` 会在 release assembly 时运行设备测试，增加构建时间；更适合受控的 CI job，不宜在开发者每次 assemble 时无条件启用。
...
`strictStability = true` 会拒绝部分不稳定的 profile 采集结果；它不能替代业务断言。示例中的 ready resource id 让脚本确认异步内容已完成，而不是只等 Activity 出现。测试数据应来自固定 fixture，不能依赖实时网络、轮播内容或随机账号状态。

场景选择按线上频率和延迟敏感度排序：

- launcher 冷启动到 TTID 与 TTFD。
- 通知、deep link、widget 等常见启动入口。
- 首页首次展示、短距离滚动和主导航切换。
- 搜索、详情、播放、编辑、支付等高频关键操作。
- 大型 SDK 或核心 library 的首个热点调用。
...
profile 生成 variant 应保持 non-debuggable、profileable、未混淆且未优化；release variant 应开启 R8。Baseline Profile Gradle Plugin 1.2.4 及以上会固定 producer 的关键属性，包括 `isDebuggable=false`、`isMinifyEnabled=false` 和 `isProfileable=true`，避免采集 variant 被误配。

AGP 对 profile 的关键演进如下：

- AGP 7.4：应用可消费依赖库 profile，并提供自己的 `src/main/baseline-prof.txt`。
- AGP 8.0：提供 Baseline Profile Gradle Plugin，并为 app module 支持 profile source set。
- AGP 8.2：R8/D8 重写规则，并加入 Startup Profile 支持。
- AGP 8.3：library module 支持 variant-aware source directory，profile 也能包含 desugared class。
...
库作者需要 sample app、library 和 producer 三个 module。sample app 驱动库的公开能力，producer 记录路径，library consumer 过滤掉 sample app 和其他依赖的规则。下面是 library module 的核心配置：
...
APK 至少应包含 `assets/dexopt/baseline.prof`；某些目标格式还会带 `baseline.profm` 元数据。AAB 的 profile 位于 `BUNDLE-METADATA/com.android.tools.build.profiles/`。这里查不到文件时，应回到 variant、consumer plugin、producer dependency 和生成输出排查；查到文件仍不能说明设备已编译。

同时记录二进制 `baseline.prof` 的大小，必须低于 1.5 MB。不要用文本 `baseline-prof.txt` 的大小套用这个限制。

### 第二层：区分 ProfileInstaller 回调与 ProfileVerifier 状态

ProfileInstaller 1.4.1 默认通过 AndroidX Startup 注册 `ProfileInstallerInitializer`。它在首帧后等待约 5 秒并加入少量随机延迟，再创建后台线程写 profile，避免把磁盘操作压进首屏路径。
...
- `COMPILED_WITH_PROFILE_NON_MATCHING`：编译后 reference profile 比之前记录的 current profile 小；这是文件大小启发式判断，不会逐方法验证命中。
...
预期状态是 `status=speed-profile`。`reason=install-dm` 表示安装期 profile 触发，`reason=bg-dexopt` 表示后台 dexopt，`reason=cmdline` 表示由命令触发。`status=verify` 可能只是 profile 仍在排队，也可能是 profile 缺失或不可用；要结合产物检查和 ProfileVerifier 判断。
...
两组必须使用同一个 APK、物理设备、系统 fingerprint、数据 fixture、启动入口与 iteration。关注 TTID、TTFD 和逐轮结果；若目标是列表或动画，再加 `FrameTimingMetric`。还可用 `ArtMetric` 观察 JIT compilation、类加载与校验工作是否减少，但 trace 证据不能替代用户可感知指标。
...
| 主线程 I/O、网络和锁等待 | 不能 | Perfetto、StrictMode、异步化与依赖调整 |
| 不必要的 SDK eager initialization | 只能降低部分代码执行成本 | App Startup 依赖图、lazy init、按需加载 |
| CPU 调度和唤醒延迟 | 不直接处理 | Perfetto scheduling 轨道与 kernel 证据 |
...
- API、设备型号、系统 fingerprint 和应用 version code。
...
- 启动、通知、deep link、首页、核心 tab、搜索、详情和关键交易路径是否仍能走通。
- ready 条件是否代表可交互状态，而非只看到 Activity 或占位 UI。
- 生成数据是否固定，网络、账号、实验开关和区域配置是否可复现。
- 启动外 CUJ 是否错误设置了 `includeInStartupProfile = true`。
