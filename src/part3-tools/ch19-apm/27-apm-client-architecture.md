...
千万级 DAU 的 APM 端侧架构，目标不是多采几个指标，而是在宿主 App 最差状态下仍能低成本记录事实。主线程卡住、进程被杀、网络不可用、低端机存储慢、用户只复现一次，这些条件同时出现时，APM 才暴露真实质量。本文把端侧 APM 看成一个小型数据系统：入口要轻，缓冲要可丢，存储要能恢复，上传要受控，远程指令要有边界。

## 1. APM 端侧引擎的职责边界

端侧 APM 不是看板前置模块。它负责在设备上完成四件事：采集、暂存、压缩编码、受控上传。服务端再负责聚合、检索、归因和告警。端侧一旦把分析逻辑做重，会把性能工具变成性能问题。
...
这段代码不是完整 SDK，只表达入口约束：禁止在记录函数里序列化、压缩、加密、写文件、发网络请求。业务线程的失败分支也不能打印大量日志，否则队列满会变成日志风暴。
...
但 `mmap` 不是“崩溃不丢日志”的保证。进程崩溃时，页缓存何时刷盘、文件头是否已提交、加密压缩块是否完整，都要靠文件格式兜底。可靠的 APM 存储要至少处理四个细节：
...
APM 自保的触发条件要写成可执行规则，而不是口头约定。只要命中红线，SDK 立即降低采集强度，并把原因写入自监控事件。
