...
**同场景**——同一个 App 的不同页面，性能表现天差地别。对比启动速度时，我们测试的都是从 Launcher 点击图标到首屏完全展示的完整流程；对比滑动流畅性时，选取的都是类似的 Feed 流或列表页面。场景定义要足够具体，不能只说"首页"，而要明确到"首页首次加载完成后的列表滑动"，因为首页冷启动和热启动的性能数据完全不可比。

**同网络**——对于有网络依赖的场景（大多数现代 App 都有），网络条件是必须控制的变量。理想做法是在同一 Wi-Fi 环境下测试，或者用 Charles / mitmproxy 搭建代理，用 Network Link Conditioner 模拟固定的网络带宽和延迟。当两个 App 在不同的网络条件下测试时，启动时间会差出几百毫秒，这个差异和 App 本身的性能完全无关。
...
部分 App 使用多进程架构，冷启动时主进程和子进程的启动顺序会影响 TotalTime。这种情况下，`am start -W` 的计时通常不包含子进程的完整初始化。如果需要精确分析多进程启动，建议配合 Perfetto Trace 做更深入的时间线分析。
...
**编译模式差异**：ART 的编译模式会影响启动速度。当某个 App 刚安装未经过后台优化（`dex2oat`）时，启动会比已优化过的慢很多。在正式测试前，可以对所有待测包执行同一组 profile 引导步骤，再强制按 profile 编译：
...
启动速度可以用一个数字概括，但流畅性不行。两个 App 的 FPS 可能都是 58，但一个掉帧均匀分布（每次掉 1-2 帧），另一个集中掉帧（偶尔卡顿 100ms+），用户体验天差地别。所以流畅性对比必须看**帧耗时分布**，不能只看平均 FPS。
...
**DEX 体积**：反映代码量。当竞品的 DEX 体积明显小于我们时，原因包括：使用了更激进的语言（Kotlin 编译后的字节码通常比 Java 体积稍大）、更好的代码混淆（R8 的 tree-shaking 更彻底）、或者功能模块拆分更合理。

**Native 库体积**：这是很多 App 包体积差异的最大来源。支持多少个 ABI（armeabi-v7a、arm64-v8a、x86、x86_64）直接决定了 Native 库的体积倍数。当竞品只提供 arm64-v8a 的 AAB（通过 Google Play 的 ABI 分发），而我们同时打包了多个 ABI 时，Native 部分的体积会差出几倍。
...
同一个 App 在不同版本之间的性能差异很大。当拿我们的最新版本去跟竞品的旧版本比时，赢了也不光彩。反过来，当竞品刚发了一个大版本做了大量优化，我们拿旧数据去比就会低估差距。竞品对比应该尽量使用各方最新发布版本。

编译状态也是一个隐蔽的变量。ART 的 JIT 和 Profile 引导编译会在 App 使用几天后逐渐优化启动路径。刚安装的 App 和已使用一周的 App，启动速度会差 20% 以上。当我们在 A/B 测试前重装了竞品但没有重装自己的 App 时，数据就会有偏差。
...
在不同品牌手机上测试同一个 App，性能数据差别很大。高通骁龙和联发科天玑的 CPU 调度策略不同，GPU 能力不同，内存带宽不同。当我们在骁龙 8 Gen 3 上测了竞品 A，在天玑 9300 上测了竞品 B，然后得出"A 比 B 流畅"的结论时，我们比较的可能是两个 SoC 而不是两个 App。
...
不同 Android 版本的 dumpsys gfxinfo 输出格式有差异，需要做版本适配确认。

## 扩展：竞品功耗对比方法

功耗对比是竞品分析中难度最高的维度，功耗测量对实验条件的要求极其严格，且需要专业的硬件设备。
...
软件方案的精度有限，只能看到"相对耗电"而非"绝对耗电"。当两个 App 的功耗差异在 5% 以内时，软件方案很难可靠地分辨。
