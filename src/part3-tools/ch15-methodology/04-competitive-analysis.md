...
本节以 Android 17 / API 37 / `android-17.0.0_r1` 为平台源码锚点。正文只在解释 Android 平台计时和帧数据时使用源码结论。竞品的业务实现不可见时，原因只能写成假设，并列出还缺少什么证据。本节没有依赖 Linux 内核实现的结论；若后续通过调度、频率或功耗事件解释差异，内核证据应固定到 `android17-6.18-2026-06_r6`。
