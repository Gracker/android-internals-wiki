...
### 🔹 Android Bench 测量什么
- 真实 Android 仓库中的 issue resolution 能力
- 单次 agent 运行生成 patch 后的测试通过率
- Android/Jetpack/Gradle 相关任务覆盖

### 🔹 数据集怎样构建
- 100 个任务来自 38,989 个 PR 的候选池与人工补充
- 仓库、PR、测试、issue、时间范围和人工复核条件
- 语言、UI 技术、项目类型与 patch 大小分布

### 🔹 执行与验证怎样分离
- inference agent 读取问题和代码，生成 patch
- verifier 在固定环境应用 patch、构建并运行 acceptance tests
- Harbor、mini-swe-agent v2、native tool calling 的版本边界

### 🔹 结果怎样统计
- 每任务二元通过结果与整体 pass@1
- 多次运行、置信区间、稳定性和基础设施错误
- cost、token、latency 的口径与偏差

### 🔹 排行榜怎样解读
- 模型、agent、prompt、工具、预算和 endpoint 共同影响结果
- 公开数据污染、测试覆盖、任务规模与统计功效限制
- 公共排行不能直接代替团队私有任务评测

## 扩展

### 🔸 方法版本演进
- 2026 年 3 月首版 mini-swe-agent v1 与归档技术报告
- 当前 Harbor + mini-swe-agent v2 方法
- 数据集、agent 和排行应分别固定版本

### 🔸 团队内部评测
- 建立私有 shadow set
- 冻结环境、模型、agent、预算和重复次数
- 同时审计正确性、成本、时延、稳定性与失败类型

<!-- outline-end -->

## 先纠正章节错位

旧稿把 Android Bench 写成 Android 17 GPU 调试工具链，并列出了 VDA、Frame Pacing Score、统一 GPU Power HAL 等“新增架构”。这些内容与 Android Bench 无关，给出的来源也不能支持相关结论。

Android Bench 是面向 Android 软件工程任务的 AI coding benchmark。它测量模型在 agent 环境中阅读仓库、修改代码并通过验证测试的能力，不测 GPU 性能、应用运行时性能或 Android 17 系统服务。GPU 分析应转到本书渲染与 AGI/Perfetto 章节。
...
本章讨论评测框架，没有平台或 kernel 机制结论。书中涉及 Android API 行为时，最高版本仍固定为 Android 17 / API 37 / `android-17.0.0_r1`；Android Bench 自身必须按 dataset 和 harness 版本固定，不能用 AOSP tag 替代。本文不涉及内核机制，因此没有套用 `android17-6.18-2026-06_r6` 来装饰来源。
