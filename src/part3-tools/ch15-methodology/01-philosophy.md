...
正因如此，这本书前半部分花了大量篇幅讲机制原理。只有理解了 VSync 是怎么工作的(第 2.3 节),你才能在 Perfetto 中正确解读 VSYNC-app 和 VSYNC-sf 的信号间距;只有理解了 Binder 的通信模型(第 1.4 节),你才能判断一个 binder call 的耗时是正常的还是异常的。工具给你数据,知识给你判断力,方法论给你决策路径--三者缺一不可。
...
一个典型例子：团队花了大量精力优化渲染管线，把平均帧时间从 12ms 降到 9ms，用户反馈「还是觉得卡」。进一步调查发现，问题不在渲染慢，而在 Input 事件分发延迟——从触摸屏幕到 App 收到事件就花了 40ms，加上渲染的 9ms，总延迟接近 50ms，用户当然觉得「不跟手」。
...
忽视度量还有另一种表现形式:只度量了平均值,忽略了长尾。如果 95% 的帧都在 8ms 内完成,但剩下 5% 的帧耗时超过 50ms,用户感知到的依然是「卡」。这种问题只有看 P99(第 99 百分位)帧时间才能发现。平均帧时间 8ms 和 P99 帧时间 50ms 同时存在,所以我们强调要看分布而不是只看均值。
