...
翻开这本书的读者，大多数是带着具体问题来的：App 启动慢怎么办？滑动卡顿怎么查？ANR 怎么分析？这些问题当然重要，后面都有专门的章节解答。但如果我们直接跳到「怎么做」，就容易陷入一个困境——每次遇到问题都像第一次遇到，换一个问题就不知道从哪下手。

这本书从第 1 章到第 14 章，讲了 Android 系统的底层机制，讲了各种性能问题的原理和工具的使用方法。这些是「知识」。而第 15 章要讨论的，是这些知识背后的「思维框架」：面对一个性能问题，我们怎么想、怎么判断优先级、怎么选择工具、怎么验证结果。这不是理论空谈，而是一个有经验的工程师在面对问题时脑子里的决策路径。
...
理解了这三点，我们再来讨论性能优化的「道」「术」「器」，就能站在一个更扎实的基础上了。
...
Lord Kelvin 有句名言：「如果你不能度量它，你就不能改进它。」在性能优化领域，这句话可以升级为：**如果你不能度量它，你甚至不知道它是不是一个问题。**
...
无论哪种瓶颈，优化的核心原则都是一样的：**减少关键路径上的工作量**。不是「让代码跑得更快」，而是「让关键路径需要做的工作变少」。有时候，最快的方法不是优化一个方法的执行效率，而是想办法让这个方法根本不需要被调用。
