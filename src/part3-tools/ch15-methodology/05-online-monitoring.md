...
应用侧 API 能看到窗口或 View 渲染的一部分信息。需要判断帧是否按期呈现、卡在 App、RenderThread（承担部分渲染工作的线程）、GPU 还是合成阶段时，应回到 FrameTimeline（把 App 生产帧与系统呈现帧关联起来的 Trace 数据）与渲染流水线分析，参见 §2.4、§7.1 和 §8.1。
