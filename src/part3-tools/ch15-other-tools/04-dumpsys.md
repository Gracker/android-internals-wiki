...
`dumpsys` 提供的是各系统服务在某一时刻或一段累计周期内的内部状态，适合快速缩小范围，不等同于完整时间线。调用每个子命令前要确认统计窗口、重置行为和字段口径，再与 trace、日志或复现步骤交叉验证。
...
完整的对象图、native 分配栈和 dma-buf 归因流程见 §15.3 [内存分析工具](03-memory-hprof-heapdump-tools.md)。
