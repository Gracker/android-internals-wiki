...
如果你做过页面、列表、启动、图片、网络或数据库等常见开发工作，书中的问题大多能对应到日常场景。阅读时可以先看现象、观测方法和短结论，再沿着章节给出的类名、方法名和源码路径补系统知识。

## 2. 中级应用与性能优化工程师

如果工作经常涉及卡顿、启动、ANR、内存或功耗，本书可以作为排障手册。章节会交代机制、trace 观察点、源码入口、指标含义和判断边界，方便把单次经验整理成可重复的分析步骤。

## 3. Android Framework / 系统工程师

如果工作更靠近 Framework、SystemUI、ROM、芯片平台、调度或图形栈，可以直接从前半部分进入源码路径。渲染、输入、内存、调度和功耗章节默认以 Android 17 平台与 6.18 Android Common Kernel 基线核对，同时保留必要的历史版本差异。

## 4. 测试、质量和基础架构同学

如果不一定每天写业务代码，但需要理解性能问题怎样产生、怎样度量和怎样复现，可以优先阅读后半部分的分析方法、工具和线上监控章节。
