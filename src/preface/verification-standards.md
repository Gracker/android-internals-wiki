...
本章是全书的验证口径，不替代各技术章节自己的证据链。它只定义“怎样标注、怎样降级、怎样关闭待核对项”，不为某个 framework、kernel 或设备行为作额外机制结论。
...
当未来线索与 Android 17 基线冲突时，正文应优先保留 Android 17 结论，并把未来变化放入独立段落或脚注。只有当章节重新验证到新的正式基线并同步更新 frontmatter 后，才能把新行为提升为正文默认结论。
...
置信度描述证据强度，不代表文章是否写完。章节的 `status`、`pipeline_stage` 等 frontmatter 字段属于编辑流水线；正文中的“已验证”“待核对”“版本边界”属于技术结论。两者不能互相代替。
...
frontmatter 的 `sources` 是入口清单，不等于正文全部证据。每个包含关键机制结论的章节，正文仍应在相关段落附近给出可定位的 tag、路径、符号、文档页或实验记录。只在 YAML 里放一个仓库根路径，不能证明具体结论。
