...
- 启动慢：从第 8 章切入，再配合工具章节核对。
- ANR：结合第 9 章、Perfetto 章节和分析方法章节。
- 快速排查线索：查阅**附录**中的 Checklist、命令速查表和 Perfetto 模板。
...
已有明确问题时，可以直接从对应章节切入，再回头补前置知识。阅读源码结论时同时确认章节的适用版本：平台代码默认看 AOSP `android-17.0.0_r1`，内核代码默认看 Android Common Kernel `android17-6.18-2026-06_r6`；旧版本只用于说明演进和兼容边界。
