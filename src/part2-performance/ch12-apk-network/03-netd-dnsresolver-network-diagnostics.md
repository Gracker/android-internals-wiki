...
12.1 介绍请求阶段、连接和传输协议，12.2 讨论 TLS，1.62 说明平台网络状态与系统如何选择默认网络。下面沿 Android 17 的源码路径说明系统 resolver，并给出应用侧可执行的诊断顺序。
...
Private DNS 保护系统 resolver 到上游 DNS provider 之间的 DNS 流量。它不替代 HTTPS，也不自动隐藏 TLS ClientHello 中的服务器名称；ECH 的适用条件见 12.2。
...
复用已有连接时，没有 DNS 与 connect 事件属于正常现象。若 DNS 很快而 connect 超时，应检查路由、地址族与目标 IP；若 TLS 慢，转到 12.2；若首字节慢，应检查服务端、CDN 与回源链路。
