...
基准版本为 Android 17 / API 37、AOSP `android-17.0.0_r1`、OkHttp 5.3.0 和 Play services Cronet 18.0.1。`netd`（Android 网络管理守护进程）与 DNS Resolver（解析器）的内部细节见 12.3，系统选网与 `NetworkAgent` 见 1.62。
...
一个进程通常只创建一个 `CronetEngine`。多个 engine 不能并发使用同一个 storage directory（存储目录）。若应用打包 native Cronet provider，还要按 [25.18 Native SO 体积优化](../../part5-app/ch25-power-size/18-native-so-size-optimization.md) 验证 ABI（应用二进制接口）、符号和 16 KB page size（内存页大小）兼容性；页大小变化对初始化耗时没有通用收益比例。
