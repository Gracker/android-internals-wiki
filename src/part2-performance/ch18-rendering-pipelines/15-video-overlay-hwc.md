...
设备声明 `Capability::SKIP_VALIDATE`（android-8.0.0_r1 引入 `HWC2_CAPABILITY_SKIP_VALIDATE`，composer3 AIDL 中该能力为 `@deprecated - enabled by default`）后，如果 layer 栈、buffer 属性和显示配置没有变化，SurfaceFlinger 可以跳过本帧 `validateDisplay()`，直接走 `presentDisplay()`。这只省掉"向 HWC 再确认一次"的开销，不代表 HWC 绕过 SurfaceFlinger；一旦 composition type、damage、color mode 或 fence 条件变化，下一帧仍要重新 validate。在 Android 16 中，`HWComposer::getDeviceCompositionChanges()` 在有 client composition 时必须 validate，并受 present timing 保护；通过 `presentOrValidate()` 试探，失败后回落 validate。
