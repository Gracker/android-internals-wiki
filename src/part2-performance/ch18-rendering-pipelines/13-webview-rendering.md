...
| `SurfaceControl` 子 Surface 候选 | Android 10+ 平台具备 `SurfaceControl` / DrawFn 基础 | 记录 `versionName` 对应的 Chromium milestone，并结合 trace / layer dump 看运行时是否命中 | 能否走独立子 Surface 不能只按 Android major version 判断 |
