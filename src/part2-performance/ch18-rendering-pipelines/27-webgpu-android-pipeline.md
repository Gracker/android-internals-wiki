...
截至 2026-07-25，最新公开版本是 `androidx.webgpu:webgpu:1.0.0-alpha05`，最低系统版本为 Android 7.0 / API 24。它仍处于 alpha 阶段，适合评估、原型和能够承担 API 变更成本的产品；选型时不能只看 Android 版本，还要检查 adapter、feature、limit、surface capability 和目标设备上的实测结果。
...
| Android 内核 | `android17-6.18-2026-06_r6` | dma-buf、sync fence、调度和具体设备的 GPU/display driver |

alpha05 的 AAR 在 `assets/dawn_build_metadata.json` 中记录上述 Dawn SHA。AAR 还包含四个 ABI 的 `libwebgpu_c_bundled.so`：解压后 arm64-v8a 约 5.8 MiB、armeabi-v7a 约 3.7 MiB、x86 约 6.5 MiB、x86_64 约 6.3 MiB。安装体积要以应用自己的 ABI 配置、压缩方式和 App Bundle 拆分结果为准，不能把四个解压尺寸直接当成单台设备的安装增量。
...
这意味着 WebGPU 绘制结果不会进入宿主 HWUI 的 RenderNode/display list，也不会由宿主 Compose 画布自动裁剪、变换或应用任意 effect。需要复杂 View/Compose 变换时，可以评估 `AndroidEmbeddedExternalSurface` 的 TextureView 路径，但要重新测量中间纹理、合成和延迟成本。
...
alpha05 helper 的 `close()` 有一条值得留意的源码边界：`device.close()` 被 TODO 注释掉，理由是待修复 issue；helper 当前关闭 surface、instance、adapter，并停止事件 poller。应用不能由此推导“helper 会替所有子资源完成释放”，资源管理与版本升级测试仍要覆盖 native heap、GPU memory 和重复进入/退出页面的场景。

## 性能边界：只给可验证的判断

目前没有一组官方 Android benchmark 能证明 Jetpack WebGPU 固定达到 Vulkan 的某个百分比，也没有证据支持“Dawn 加载固定需要 50—100 ms”。设备、ABI、Dawn commit、backend、shader、pipeline cache、draw/dispatch 数量、分辨率和热状态都会改变结果。
...
渲染 workload 要看 vertex/fragment 时间、overdraw、attachment load/store、texture bandwidth 和 present 等待；计算 workload 要看 occupancy、workgroup size、访存合并、cache、带宽和 CPU readback。遇到性能差异时，应对比同设备上的 backend、命令规模和 GPU counter，不能只按“WebGPU vs Vulkan”给出原因。
...
- 前后台状态和 Surface 重建次数；
- CPU/GPU 频率、温度与功耗限制；
- pipeline warm-up 与磁盘/driver cache；
...
| 端侧 ML 推理 | 自定义 GPU kernel 有发挥空间 | 优先比较 LiteRT/NNAPI/厂商 delegate 的算子覆盖、量化和运维成本 |
...
设备提供 GPU render-stage producer 时，可继续查看 hardware queue、submission、stage 和 GPU duration；提供 GPU counter 时，再观察 busy、频率、带宽与 cache。然后把 WebGPU layer 的 BufferQueue、SurfaceFlinger latch、composition 和 display present 对齐。
...
| 页面反复进入后内存增长 | handle close、helper TODO、Surface 重建、native/GPU memory | Kotlin GC 单一原因 |
...
- [alpha05 AAR](https://dl.google.com/dl/android/maven2/androidx/webgpu/webgpu/1.0.0-alpha05/webgpu-1.0.0-alpha05.aar) 与 [对应 Dawn commit](https://dawn.googlesource.com/dawn/+/9d41fdf36977cca92361c6ae2769129bbaaafd9b/)：核对 `libwebgpu_c_bundled.so`、`dawn_build_metadata.json` 和固定原生实现。
...
- Android 17 的 [`native_window_jni.h`](https://android.googlesource.com/platform/frameworks/native/+/android-17.0.0_r1/include/android/native_window_jni.h)、[`Surface.cpp`](https://android.googlesource.com/platform/frameworks/native/+/android-17.0.0_r1/libs/gui/Surface.cpp)、[`BufferQueueProducer.cpp`](https://android.googlesource.com/platform/frameworks/native/+/android-17.0.0_r1/libs/gui/BufferQueueProducer.cpp) 与 [`SurfaceFlinger.cpp`](https://android.googlesource.com/platform/frameworks/native/+/android-17.0.0_r1/services/surfaceflinger/SurfaceFlinger.cpp)：核对 Java Surface、ANativeWindow、BufferQueue 与显示合成边界。
