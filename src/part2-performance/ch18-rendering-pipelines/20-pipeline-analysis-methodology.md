...
BLAST 是当前 Android 17 标准窗口与许多独立 Surface 路径的重要实现，但不要仅凭系统版本推断现场对象。Android 11 的标准窗口迁移与后续 SurfaceView 路径的演进并非同一个版本结论。诊断 Android 17 时，直接确认应用进程里的 `BLASTBufferQueue`、SurfaceControl transaction 和对应 Layer；复盘旧设备时再回到该版本源码。
...
下面的查询用于从 trace 中缩小候选时间窗。Slice 名称需要根据设备上的实际轨迹调整，查询结果也必须回到 UI 中核对调用上下文。
...
-- 2. 找目标进程内超过 5 ms 的 Buffer 提交/获取候选点
SELECT
  p.name AS process_name,
...
Android 12+ 可以从 FrameTimeline 切入，但 `actual_frame_timeline_slice` 同时包含 App SurfaceFrame 与 SurfaceFlinger DisplayFrame，也可能包含多个 Layer。统计时应按 `upid`、`layer_name`、surface/display token 过滤和去重。FrameTimeline 对标准窗口最有帮助；`SurfaceView`、Camera、Video 或引擎型独立 Surface 不一定提供同等完整的 App 侧时间线，此时要回到 Producer、BufferQueue、Layer、fence 和 present 证据。Android 9—11 没有当前 FrameTimeline 主表，应使用对应版本可用的 `doFrame`、VSYNC、BufferQueue 与 SurfaceFlinger 轨迹复原时间窗。
...
假设现场是：视频详情页静止播放时功耗正常，浮层动画出现后 GPU 和内存带宽同时上升。`SurfaceView` 让视频具备独立 Layer 条件，但 HWC 是否选择 `DEVICE` 由设备能力和本帧 Layer 属性共同决定。
...
5. 只有当属性变化、composition type 和 GPU/功耗变化稳定对应时，才能把该属性写成这台设备上的回退触发条件。
...
假设现场是：页面滚动时宿主窗口 miss deadline，主线程的 RecyclerView 工作不重，RenderThread 的 WebView functor 相关区间却持续变长。

复盘步骤如下：

1. 从 Layer 树确认网页内容并入宿主窗口，还是另有 child Surface；不能只因页面里有 WebView 就预设 functor 路径；
2. 若内容并入宿主窗口，按 surface token 对齐宿主 `doFrame`、`DrawFrame`、functor、App GPU 和 queue；
3. 同窗查看 Chromium renderer/compositor/Viz 的实际线程名和调度状态，线程名随 provider 版本可能变化；
...
如果现场存在独立子 Surface，宿主窗口 FrameTimeline 不能覆盖它的全部显示路径；还要单独追踪该 Layer 的 Buffer transaction、fence、SF/HWC 与 present。第 18.13 节给出 WebView 各分支的源码边界。
...
- AOSP Android 17, [`DrawFrameTask.cpp`](https://android.googlesource.com/platform/frameworks/base/+/android-17.0.0_r1/libs/hwui/renderthread/DrawFrameTask.cpp) 与 [`CanvasContext.cpp`](https://android.googlesource.com/platform/frameworks/base/+/android-17.0.0_r1/libs/hwui/renderthread/CanvasContext.cpp)
- AOSP Android 17, [`BLASTBufferQueue.cpp`](https://android.googlesource.com/platform/frameworks/native/+/android-17.0.0_r1/libs/gui/BLASTBufferQueue.cpp) 与 [`BufferQueueProducer.cpp`](https://android.googlesource.com/platform/frameworks/native/+/android-17.0.0_r1/libs/gui/BufferQueueProducer.cpp)
- AOSP Android 17, [`SurfaceFlinger.cpp`](https://android.googlesource.com/platform/frameworks/native/+/android-17.0.0_r1/services/surfaceflinger/SurfaceFlinger.cpp) 与 [`Layer.cpp`](https://android.googlesource.com/platform/frameworks/native/+/android-17.0.0_r1/services/surfaceflinger/Layer.cpp)
- AOSP Android 17, [`HWComposer.cpp`](https://android.googlesource.com/platform/frameworks/native/+/android-17.0.0_r1/services/surfaceflinger/DisplayHardware/HWComposer.cpp) 与 [`HWC2.cpp`](https://android.googlesource.com/platform/frameworks/native/+/android-17.0.0_r1/services/surfaceflinger/DisplayHardware/HWC2.cpp)
- AOSP Android 17, [`HardwareBufferRenderer.java`](https://android.googlesource.com/platform/frameworks/base/+/android-17.0.0_r1/graphics/java/android/graphics/HardwareBufferRenderer.java)
