...
案例分析最容易犯的错误，是在时间线上找到一个可疑点，便把它写成根因。本节给每个结论标注证据强度：

| 等级 | 判定条件 | 文中措辞 |
|---|---|---|
| 已确认 | 日志、trace、源码语义和复现或 A/B 验证能够组成因果链 | 根因已确认 |
| 强推断 | 多项证据指向同一方向，仍缺少一段直接证据 | 高概率相关因素 |
| 未闭合 | 只找到相关日志、异常负载或静态堆栈，时间或对象无法对齐 | 保留线索，继续取证 |

本节包含五个线上样本和一个匿名化的锁顺序示例。样本中的包名、时间和关键数值来自原始材料；锁顺序示例用于完整演示死锁图的还原过程，不冒充线上原始事件。

| 案例 | ANR 表象 | 结论强度 | 能学到什么 |
|---|---|---|---|
| 1 | Launcher 没有焦点窗口 | 强推断 | PSI、iowait 和主线程 native 栈怎样配合阅读 |
| 2 | Gesture Monitor 连接超时 | 未闭合 | 怎样识别时间错位和错误归因 |
| 3 | `QueuedWork.waitToFinish()` | 已确认到等待点 | `apply()`、组件收尾和 SP 初次加载的边界 |
...
设备为 MTK 平台、Android 14，用户看到 Launcher 偶发无响应。下面的 event log 确认了 ANR 类型和被记账的进程：
...
这条记录表示 InputDispatcher 等待焦点窗口超时。它没有说明 Launcher 主线程正在执行慢代码，也没有给出焦点为何消失。
...
Java 栈顶显示 `nativePollOnce`，native 栈却还位于 VSync 回调返回路径，并在 ART 条件变量上等待。因此，不能把这份栈简化成“主线程空闲”。单凭 `WaitHoldingLocks` 也无法确认 GC；还需要 GC cause、暂停日志或 Perfetto 中对应的 ART slice。
...
三个 load 值依次对应 1、5、15 分钟，所以 1 分钟 load 是 `56.48`。PSI 的 `some` 表示统计窗口内至少有任务因该资源停顿，`full` 表示所有非 idle 任务同时停顿；memory PSI 不能直接翻译成“都在等待内存回收”。
...
`kswapd0` 活跃、major fault、memory PSI、I/O PSI 和 `43% iowait` 同时出现，足以确认设备正处于严重的内存与存储压力中。load 还会计入不可中断睡眠任务，不能只用“load 除以 CPU 核数”判断 CPU 是否饱和。
...
- Perfetto 的 `sched`、`ftrace/print`、block I/O、reclaim 与 ART 数据源；
- Launcher 主线程从 VSync 回调进入到返回的完整 slice；
- ANR 前后进程的 `D` 状态、major fault 和设备存储延迟。

### 修复方向

系统团队应先定位压力来源：匿名页增长、文件页抖动、频繁 major fault、写回拥塞、低速存储或某个进程的突发 I/O。应用团队应清理主线程上的文件访问、大对象分配和高频数据库写入，并在压测中复现相同 PSI 区间。调整 I/O 优先级或 WAL checkpoint 参数必须基于设备、数据库页大小和写入模型测量，本案例不支持给出固定阈值。

这个案例留下一个实用提醒：`nativePollOnce` 只是一帧采样的 Java 入口，native 栈、调度历史和系统压力才决定它代表空闲、回调执行，还是等待资源。
...
括号内字符串来自 InputChannel 名称。AOSP 创建一对输入通道时会给两个端点附加 `(server)` 和 `(client)`，所以 `(server)` 不能证明事件消费者运行在 system_server，也不能单独锁定责任进程。

### 对齐时间再解释日志

原材料里还有一条 system_server 主线程慢消息：
...
这条消息发生在 `15:00:45` 附近，ANR 发生在 `15:01:37.293`。两者相差约 52 秒，已经离开此次 5 秒输入超时的直接观察窗口。
...
ANR 报告覆盖 `15:01:27.683` 至 `15:01:37.233` 的 CPU 窗口，system_server 达到 `215%`，全局 CPU 约 `64%`，同时存在 memory、CPU 和 I/O PSI。system_server 的高占用值得追查；多核占用超过 `100%` 本身也不能推出主线程被阻塞。
...
1. 从 `dumpsys input` 找到该 InputChannel 对应的 connection、窗口句柄、PID 和 monitor 类型。
2. 抓取 ANR 5 秒窗口内该 PID 的 Java/native trace，以及所有 system_server 线程 slice。
3. 检查 WaitQueue 中最老事件的序号、派发时间、超时时间和完成回调。
4. 将高 CPU 拆到线程，确认消耗来自 system_server 主线程、Binder 线程、GC 线程或其他 native 线程。

### 修复方向

修复对象取决于缺失证据。若连接消费者线程被长任务占用，应移动该任务或缩短临界区；若输入 monitor 已失去有效消费者，应修复注册和销毁时序；若 system_server 因调度或 I/O 延迟无法运行，需要继续定位压力源。当前材料不足以要求修改 `Notifier`。
...
以 `android-17.0.0_r1` 为锚点，组件收尾与 `QueuedWork` 的关系如下：

| 组件路径 | Android 17 行为 |
|---|---|
| `ActivityThread.handlePauseActivity()` | 仅 pre-Honeycomb 兼容路径调用 `waitToFinish()` |
...
| manifest Receiver 的 `PendingResult.finish()` | 有待完成工作时，把 `sendFinished()` 作为 finisher 排到 `QueuedWork` 后面 |

`SharedPreferencesImpl.apply()` 会先更新内存，再创建落盘任务和 finisher。Android 17 的 `QueuedWork.waitToFinish()` 会在当前调用线程执行尚未处理的 work，随后等待并运行 finishers。于是，本来排给单线程 executor 的写入可以在组件收尾点转移成主线程上的同步成本。

诊断时还要区分两个栈：

- `QueuedWork.waitToFinish()` 指向全进程共享的待完成工作，常见来源是 `apply()`，也可能有其他框架 work；
- `SharedPreferencesImpl.awaitLoadedLocked()` 表示该 SP 实例仍在读取和解析文件，属于初次加载或加载未完成。

看到前者后，应继续寻找 `SharedPreferencesImpl$EditorImpl`、`enqueueDiskWrite()`、`writeToFile()`、`fsync()` 或 block I/O 证据。看到后者后，应检查文件大小、XML 解析、备份文件恢复和调用时机。两种等待不能合并成同一个根因。

### 根因结论

**结论强度：已确认到等待点。** 该样本的主线程已经进入 `QueuedWork.waitToFinish()` 并停在那里。若 trace 或 I/O 数据还能证明等待队列中的任务来自 `SharedPreferencesImpl.apply()`，便可把根因收敛到 SP 落盘；只有当前两帧时，仍要排除其他 `QueuedWork` 使用者。

`apply()` 保证内存更新立即可见，不保证调用后完全脱离同步等待。把 `apply()` 换成 `commit()`会更早地同步阻塞调用线程，不能解决生命周期阶段的卡顿。

### 修复方案

- 合并同一时段的多次编辑，避免每改一个 key 就单独 `apply()`；
- 控制单个 XML 的键数量和序列化体积，清理不再使用的大 value；
...
修复验收不能只看平均写入耗时。应在应用退后台、Service 完成、广播密集和低速存储压力下，确认主线程等待的 P95/P99 与 ANR 数量同时下降。
...
这里应沿 InputChannel 找消费者 PID，而不能只看被记账的前台应用。
...
输入事件发出 13 毫秒后，持有该输入连接的进程被冻结；约 5 秒后，未完成事件触发超时。实验还提供了 A/B 结果：关闭 freezer 后不再复现，重新启用后恢复复现。时间关系、对象 PID 和开关实验彼此吻合。

### 根因结论

**结论强度：已确认。** 该设备的软件分支在 Gesture Monitor 仍有未完成输入事件时冻结了 `com.android.systemui:screenshot`，消费者无法运行，InputDispatcher 因等待完成回执超时。

这说明该 Android 14/OEM 集成存在 freezer 与输入 monitor 生命周期协同缺陷，不能外推成所有 Android 版本都具备同一问题。Cached Apps Freezer 的启用条件、豁免和厂商修改会改变行为。

在 Android 17 源码里，event log tag 仍明确记录 `am_freeze` 和 `am_unfreeze`，字段是 PID 与进程名。复核新平台时，还应同时记录 freeze/unfreeze reason、目标 cgroup 状态、输入连接身份和 event ID，防止 PID 复用或旧事件干扰判断。

### 修复方案

系统修复需要围绕对象生命周期展开：

- 持有活跃输入 monitor、未完成 WaitQueue 事件或关键 SystemUI 会话的进程，不应进入可冻结状态；
- monitor 销毁时先停止派发并清理连接，再允许进程降级与冻结；
- 若产品设计允许按输入活动解冻，应验证解冻到线程恢复运行的时延能覆盖输入超时预算；
- 加入回归用例：事件派发后立刻触发进程状态迁移，检查 WaitQueue 能否完成或被有序取消。

应用侧无法稳定规避系统冻结输入消费者的问题。诊断脚本应把 `am_freeze`、`am_unfreeze`、InputDispatcher reason 和 PID 自动拼成时间线。
...
该 reason 由 InputDispatcher 的 focused application 超时路径生成，表示系统已有 focused application，等待期间却没有可接收输入的 focused window。
...
在 `android-17.0.0_r1` 中，`ProcessList` 启动进程后安排 `PROC_START_TIMEOUT_MSG`。应用通过 Binder attach 到 `ActivityManagerService`，系统进入 `attachApplication()` 并准备发送 `bindApplication` 时，会移除这条 timeout 消息。超时处理会以 `ApplicationExitInfo.REASON_INITIALIZATION_FAILURE` 和 `start timeout` 杀进程。
...
Dialer attach 失败应检查 zygote fork 返回、进程是否进入 `D` 状态、调度延迟、native runtime 启动、seccomp/SELinux 拒绝、崩溃信号和 Binder attach 事务。应用 Java 初始化优化不属于这条证据链的起点。

### 修复方案

进程侧修复取决于 attach 前的阻塞点。系统侧应在目标进程 start timeout 或启动事务取消时，撤销对应的 focused application/transition 状态，并让上一可见窗口重新参与焦点计算。回归测试需要同时断言：

- 新进程未 attach 时会按预算终止；
- 启动 Activity 与转场记录被正确清理；
- 原窗口或错误界面重新取得输入焦点；
- InputDispatcher 中不存在悬空 focused application。

排查 “Application does not have a focused window” 时，Launcher 的静态 trace常常信息很少。`input_focus`、Activity 启动、进程 attach 和窗口可见性时间线更有辨识度。
...
Service 超时预算由 AMS 的 Service 执行状态决定。常见的 20 秒前台预算取决于 `isExecServicesFg()` 等系统状态，不能只凭“这是 foreground service”或某个回调名判定。ANR reason 也没有说明超时发生在 `onCreate()`、`onStartCommand()`、`onBind()` 还是销毁路径，调用栈才负责区分。
...
主线程的等待边是 `main → DatabaseHelper`，锁持有关系是 `DataManager → main`。
...
后台线程的等待边是 `SyncWorker-2 → DataManager`，锁持有关系是 `DatabaseHelper → SyncWorker-2`。四条边组成有向环，静态采样已经足以确认死锁。

### 代码路径

主线程路径在持有 `DataManager` monitor 时跨类调用：
...
这段改动消除了“持有 Database 锁时获取 DataManager 锁”的边。仍需确认 `snapshot` 的所有权和线程安全，避免把死锁换成数据竞争。

项目还应维护可执行的锁规则：

- 为跨模块锁定义固定层级，低层级锁不能在持有期间调用高层级模块；
- 禁止在锁内执行 Binder、文件 I/O、回调未知代码或等待 Future；
- trace 中记录锁地址、持有线程和等待线程，自动检测有向环；
- 用 Error Prone、SpotBugs 或自定义静态检查标记锁内跨类调用；
- `tryLock(timeout)` 只适合业务允许取消或降级的路径，不能当作通用死锁修补。

### Binder 场景的同类问题

跨进程死锁通常同时包含 Java 锁、同步 Binder 调用和线程池资源。典型等待环是：
...
即便还有空闲 Binder 线程，这个锁环也不会自行解除。另一种形态是所有处理线程都被嵌套事务或外部等待占用，新回调无法获得执行线程。

Android 17 的 `ProcessState.cpp` 定义 `DEFAULT_MAX_BINDER_THREADS = 15`，并把它作为 driver 请求的默认最大线程数。调用线程加入线程池、已启动线程和 Binder 实现细节都会影响进程中观察到的线程总数，不能把 `15` 当成固定池大小，也不能靠“再留一个线程”证明系统不会死锁。诊断时应画出事务方向、调用同步性、线程状态、锁持有关系和 executor/线程池容量。
...
- atrace counter `iq` 表示 inbound queue，`oq:<channel>` 表示每连接 outbound queue，`wq:<channel>` 表示每连接 wait queue。

`isConnectionResponsive()` 会遍历 waitQueue，检查事件的 `timeoutTime` 是否已经过去。生成连接 ANR reason 时，`onAnrLocked()` 会读取队首条目用于诊断；源码注释也提醒，超时预算发生变化后，队首不一定就是触发当前超时判断的条目。

所以，队列长度只描述积压数量：

- 单个条目可能是一项长时间未完成的事件，也可能刚进入等待；
- 多个条目可能来自消费者吞吐不足、事件类型差异、MOVE 合并或超时预算变化；
- `length=1` 不能证明单个主线程长任务，较长队列也不能直接证明系统负载过高。

排查时要读取每个条目的 event type、sequence、delivery time、timeout time 和 connection 身份，再与主线程/Binder 线程 trace、`wq/oq` counter、调度 slice 和进程冻结事件对齐。公开 AOSP 中没有名为 `android.input.input_event_waiting_duration` 的固定 Perfetto track，采集配置不应依赖这个名称。

## 一套可复用的案例复盘顺序

1. 从 `am_anr` 和 reason 确认 detector、被记账进程与输入连接名称。
...
7. 把结论写成证据链；链条缺口保留为待取证项，不用猜测填满。

§9.3 介绍通用采集流程，本节强调的是复盘纪律：每一条日志都要同时回答“什么时候”“属于谁”“源码里代表什么”。
...
这个输入流不是完整 `/data/anr/traces.txt` 的稳定镜像。Android 17 的 ANR 保存路径面向目标进程，内容通常包括头信息和目标进程的首段 trace；其他进程、CPU/PSI、event log 和完整系统上下文不能假定都在流内。具备系统 DUMP 权限的诊断工具与普通应用的可见范围也不同。
...
这段代码只展示查询和限量读取入口。生产实现还要去重 exit record、校验时间戳、限制网络与磁盘开销，并接受 trace 缺失。

### 主线程历史

ANR 瞬时 trace 可能采到 `nativePollOnce`、锁等待或已经返回后的状态。应用可用公开的 `Looper.setMessageLogging(Printer)` 记录主线程消息边界，但它会增加热路径开销，输出也不保证包含同步屏障、native 回调和每个耗时来源。`Looper.Observer` 属于隐藏接口，不应通过绕过 Hidden API 限制部署到普通应用。

历史窗口应按内存预算和目标 ANR 类型配置，用环形缓冲区保存。固定“过去 10 秒”只是一种工程选择；Input、前台 Service、后台 Service 和广播的超时预算不同。采样数据至少带上消息目标、开始/结束时间、线程 CPU 时间、wall time、队列延迟和采集版本。

### 聚合键

只按 Handler 类名与 message code 聚合容易把不同原因混在一起。更稳妥的归一化维度包括：

- ANR detector 与 reason 模板；
- 首个应用栈帧、等待锁类型或 Binder 接口；
- 主线程前一条超长消息；
- 应用版本、Android 版本、设备/SoC 和进程状态；
- CPU/PSI 压力桶、采样相对 ANR 的时间偏移；
- focused-window、process-start、freeze 等特征。

聚合结果应保留若干原始样本供人工复核。某个簇的数量很大，不代表每个样本都共享同一根因；证据完整度应作为簇内字段参与排序。
