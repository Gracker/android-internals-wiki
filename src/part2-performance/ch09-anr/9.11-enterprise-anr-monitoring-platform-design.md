...
sources:
  ...
  - "Android Developers: Android vitals ANR"
  - "AOSP android-17.0.0_r1 frameworks/base ActivityManager/ApplicationExitInfo/Looper/Debug"
  - "AOSP android-17.0.0_r1 ProfilingManager"
---

# 9.11 企业级 ANR 监控平台架构设计

> 校勘提示：下面的 outline 区块是 Hermes 保护的历史提纲，保留用于任务追踪。区块中的 LSTM、Method Trace 和“自动识别根因”等条目只是早期设想，技术取舍以保护区后的 Android 17 校勘正文为准。
...
本文的平台源码固定在 Android 17 / API 37 / `android-17.0.0_r1`。Android 8—17 都能使用低成本主线程哨兵和应用内 breadcrumbs；系统确认、退出历史和触发式 profiling 必须按 API 与权限分层。
...
通用端侧实现可参考 [19.27 千万级 DAU 的 APM 端侧架构](../../part3-tools/ch19-apm/27-apm-client-architecture.md)；本节只强调 ANR 的证据优先级。
...
[已验证: AOSP android-17.0.0_r1, Android SDK API 26-37]
