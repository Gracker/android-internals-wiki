...
```bash
# 开启 malloc debug（需要 root 或 debuggable 应用）
adb shell setprop libc.debug.malloc.program com.example.app
adb shell setprop libc.debug.malloc.options "backtrace enable_on_start"
```

开启后，malloc debug 会跟踪每一次 Native 内存分配的调用栈，并能在检查到内存错误（如 use-after-free、buffer overflow）时输出详细信息。相比于 heapprofd，malloc debug 提供更完整的分配记录（非采样），但性能开销也大得多。

[已验证: 官方文档, source.android.com/docs/core/debug/native-crash]
...
ASan 的工作原理是在每次内存分配前后插入"红区"（poison bytes），当程序访问这些红区时，ASan 就能检测到越界访问。在 Android 上，从 Android 8.0 开始就支持在非 root 设备上使用 ASan 调试应用。
...
- **Java Heap 使用率**：通过 `Runtime.getRuntime().totalMemory()` 和 `freeMemory()` 计算
- **PSS**：通过 `Debug.getMemoryInfo()` 获取（`getTotalPss()`）
- **是否接近 `largeHeap` 上限**：PSS 占 `largeHeap` 的比例
