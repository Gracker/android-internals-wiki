...
知道"掉帧了"只是第一步——这一帧为什么没画完？

答案分散在渲染管线的各个环节：可能是 App 侧的代码写法（滑动回调里做了太多计算），可能是系统侧的调度问题（CPU 把时间片给了别的进程），也可能是硬件合成的限制。没有一套系统化的原因分类体系，对着 Perfetto Trace 里密密麻麻的时间线，很容易陷入"到处看看、碰运气"的模式。

本节把"一帧为什么没画完"的所有可能原因，按渲染管线的阶段整理成一个清晰的原因体系。读完这一节，再在 Perfetto 中看到一帧超时，就能快速判断问题出在哪个环节、是 App 的问题还是系统的问题、下一步该往哪个方向深挖。

先回顾一帧渲染在 Perfetto 中的完整路径——后面的原因分类就是按这条路径的阶段来组织的：
