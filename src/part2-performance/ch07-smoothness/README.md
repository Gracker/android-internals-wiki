...
- 卡顿与功耗、温控同时变化时，结合 25.1 与 25.9 的功耗归因记录、thermal（温控）和调度证据判断。
- 卡顿集中在页面切换、窗口形态变化、图片、WebView 或显示合成时，分别查阅 22.11、22.12、22.9、22.16/13.9 与 `7.5`。
- 无障碍、ContentCapture（内容捕获）或 Autofill（自动填充）改变页面开销时，查阅 `7.6`；用于自动寻找目标方法调用路径的 GAPS 工具见 15.13。
