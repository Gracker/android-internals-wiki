...
在 Android 性能优化的日常工作中，我们习惯用"掉帧"作为衡量流畅性的核心指标——FrameTimeline 标红了就是 jank，没标红就是流畅。但实际开发中有一个更隐蔽的问题：FrameTimeline 显示所有帧都在 VSync 预算内完成，没有任何一帧超时，用户却反馈"感觉有点卡"。

这不是用户矫情，而是我们衡量流畅性的维度不完整。传统的 jank 检测关注的是"这一帧有没有在规定时间内完成"，但用户的视觉系统关注的是"相邻两帧之间的画面变化是否均匀"。这两者不是同一件事。

本章要讨论的就是这个被传统工具忽略的维度：**步幅波动（step-size jitter）**——即使不掉帧，帧与帧之间位移量的不均匀也会导致视觉上的不连贯。我们来看这个问题是怎么产生的、如何在 Trace 中量化它、以及有哪些优化方向。
...
最常见的感知流畅性问题是手势导航时的窗口动画。在多任务界面（Recent Apps）上划回桌面时，窗口缩小的动画前期变化速度过快、后期突然变慢——这不是掉帧，而是动画插值曲线的加速度分布不合理，导致画面位移量在动画开头和结尾差异太大。
...
可以看到，ms 取整导致帧间时间在 8ms 和 9ms 之间交替跳动。对于匀速滚动的列表来说，这意味着每帧的滚动像素数也在两个值之间交替——用户看到的就是列表在"微颤"。
...
### 成因三：从 Choreographer 到动画引擎的精度损失链路
...
链路的前两段保持了纳秒精度。问题出在第三段到第四段的转换：Android 的动画框架（包括 OverScroller、ValueAnimator、ObjectAnimator）内部使用 `AnimationUtils.currentAnimationTimeMillis()` 作为时间源，在 ns 到 ms 的转换过程中丢失了精度。
...
这两者常常被混淆，但它们是完全独立的维度。我们可以用一个 2x2 矩阵来理解：
...
在 Perfetto 中，步幅波动的 Trace 特征不是"红色帧"，而是：

- **帧间隔的规律性跳动**：在 Main Thread track 中，doFrame slice 之间的间距不是等距的，而是呈现有规律的交替（如 8-9-8-9ms）
