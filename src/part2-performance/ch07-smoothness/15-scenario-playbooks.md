...
last_task9_review_log: "logs/deep-review/2026-05-05-17-deep-review.md"
 | 2026-05-05 Task2B 20:34：P0 atrace bitmask已修正(gfx=1L<<1, view=1L<<3, 组合0xA不是0x30)；P0 SQL jank_type已修正为Buffer Stuffing(带空格)；P1 Android 8-11 fallback已明确排除FrameTimeline字段
---
...
- **gfx/view trace tag**：在 Perfetto 配置中通过 `atrace_categories: "gfx"` 和 `atrace_categories: "view"` 启用，观察 `Choreographer#doFrame` slice 的耗时和 RenderThread 的 `DrawFrame` 区间。如果使用旧版 `setprop` 方式，`gfx`（`TRACE_TAG_GRAPHICS = 1L << 1 = 0x2`）和 `view`（`TRACE_TAG_VIEW = 1L << 3 = 0x8`）的组合值为 `0xA`，不是 `0x200`（`0x200` 对应的是 `TRACE_TAG_VIDEO`）。推荐直接使用 Perfetto/atrace category 方式：`atrace_categories: "gfx"` 和 `atrace_categories: "view"`，同时补上 `sched`、`input`、`wm`
...
`BufferStuffing` 作为 jank type 标注是 Android 12+ `FrameTimeline` 的能力。Android 8-11 没有 `FrameTimeline`（因此也没有 `Actual Present` / `Expected Present` 字段和 `Buffer Stuffing` jank type），但可以通过以下方式间接识别管线积压：
...
| BufferStuffing | `SELECT * FROM actual_frame_timeline_slice WHERE jank_type LIKE '%Buffer Stuffing%'` | Android 15+ Perfetto UI 中显示为浅绿色轨道。注意 Perfetto FrameTimeline 的 jank_type 枚举值是 `Buffer Stuffing`（带空格），不是 `BufferStuffing` |
