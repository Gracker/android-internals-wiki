...
| 删除普通应用调用 `EconomyManager` 的示例 | 已完成 |
| 删除“每 UID BatteryUsageStats 决定 ARC”的表述 | 已完成 |
| 将 `JobDebugInfo` 改为 pending-reason API | 已完成 |
| 将 `dumpsys tare` 改为 `dumpsys jobscheduler` | 已完成 |
| 把 `PENDING_JOB_REASON_QUOTA` 解释为 JobScheduler quota | 已完成 |
| 保留 Android 13—14 的 ARC 设计时标注隐藏、默认关闭和历史版本 | 已完成 |
| Android 17 的数字以官方近似值和设备实测为准 | 已完成 |
| WorkManager 排障包含 JobScheduler 资源限制 | 已完成 |
