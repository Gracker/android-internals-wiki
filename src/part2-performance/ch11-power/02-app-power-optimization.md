...
客户端对象显示 held（已持有），不等于这把锁在当前电源策略下仍有效；App tag、PowerManagerService 的 suspend blocker（系统休眠阻止项）、SystemSuspend 和内核 `wakeup_source`（唤醒源）也无法一一对应。源码调用链、安全示例、Doze/LPS（Low Power Standby，低功耗待机）边界、Android Vitals 口径与逐层排障方法统一见 [11.5 WakeLock 机制与功耗分析](05-wakelock.md)。
