...
App 主线程收到 Input 事件后，执行以下工作：

1. **事件处理**：View.onTouchEvent() → OnClickListener → 业务逻辑更新
2. **UI 状态变更**：调用 View.invalidate() 或 View.requestLayout() 标记需要重新绘制
3. **申请 VSync**：Choreographer 向 SurfaceFlinger 注册下一个 VSync-app 信号
...
关键在于：这 100ms 的临界点可以通过视觉技巧来"骗过"。如果我们不能在 100ms 内完成真正的处理，但能在 50ms 内给用户一个视觉反馈（即使这个反馈不包含最终结果），用户的感知仍然是"系统立即响应了我"。

华为在《交互流畅体验设计》文档中也强调了这一点：**感知流畅性不等同于系统性能。优秀的系统性能是保证用户感知流畅的必要条件，但好的系统性能不一定带来好的感知流畅性。** [来源: Cubox/华为-交互流畅体验设计-2025-02-18.md]

### 骨架屏（Skeleton Screen）

骨架屏是最常用的感知优化手段。原理很简单：在真实内容加载完成之前，先显示一个与最终布局结构一致的灰色占位界面。

骨架屏的优势：
- **消除布局跳变**：用户看到的界面结构从一开始就是稳定的，不会出现内容突然"跳"出来的情况
- **制造进度感**：即使用户知道内容还没加载完，灰色块的闪烁或渐变动画也在暗示"正在处理"
- **降低焦虑感**：心理学研究表明，明确的等待状态比不确定的等待更容易被接受
...
**占位图（Placeholder）**——图片加载前显示模糊缩略图或默认图标。Glide、Coil 等图片加载库都内置了 placeholder 支持。

**过渡动画（Transition Animation）**——在状态切换时播放动画。动画的心理学作用是"桥接"——它把用户的注意力从前一个状态引导到后一个状态，让等待变得不那么明显。但要注意，动画时间不能太长（一般不超过 300ms），否则就从"优化"变成了"浪费"。

**即时反馈（Immediate Feedback）**——最简单也最有效的方式。按钮按下时立即变色、列表项滑动时立即跟随手指移动，这些都是"零延迟"的反馈。如果后续的处理需要时间，在反馈之后再异步加载内容。
...
**Step 1：找到用户的操作时间点**。在 Input Track 中定位触摸事件的 dispatch 时间。
