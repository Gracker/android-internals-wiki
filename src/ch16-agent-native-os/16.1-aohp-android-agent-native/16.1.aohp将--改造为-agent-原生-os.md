...
chroot 只改变路径解析根目录，本身不是完整安全边界。评价“可安全执行 agent 代码”时还要核对 UID/GID、SELinux domain、mount namespace、capability、seccomp、cgroup、网络出口、设备节点和凭据挂载。当前原型适合受控 Cuttlefish/userdebug 实验，不能直接承担不可信代码的生产隔离。
