...
def check_file_indexed(file_path):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            # If search fails, assume not indexed to be safe
            return False
...
def score_file_simple(file_path):
    ...
    android_keywords = ['android', 'linux', 'framework', 'perfetto', 'surfaceflinger', 'choreographer',
                       ...
                       'recyclerview', 'view', 'handler', 'messagequeue', 'gc', 'compose', 'jobscheduler',
                       'workmanager', 'selinux', 'pipeline', 'deliqueue', 'blas', 'hwui', 'sync']
...
def append_to_daily_funnel(file_path, score_result):
    ...
    try:
        ...
        if any(keyword in title for keyword in ['渲染', 'surfaceflinger', 'vsync', 'choreographer', 'view', 'pipeline', 'blast', 'sync']):
            chapter = 'ch02'
        elif any(keyword in title for keyword in ['内存', 'lmk', 'zram', 'gc', 'compose']):
            chapter = 'ch04'
        elif any(keyword in title for keyword in ['cpu', 'eas', '调度', 'handler', 'messagequeue']):
            chapter = 'ch05'
        elif any(keyword in title for keyword in ['启动', '响应', 'jobscheduler', 'workmanager']):
            chapter = 'ch08'
        elif any(keyword in title for keyword in ['anr', 'watchdog']):
            chapter = 'ch09'
        elif any(keyword in title for keyword in ['功耗', 'battery', 'doze']):
            chapter = 'ch11'
        elif any(keyword in title for keyword in ['perfetto', 'trace', 'profiler', '性能']):
            chapter = 'ch13'
        else:
            chapter = 'ch16'
...
def add_to_index(file_path, score_result):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return append_result.get('added', 0) > 0
        else:
...
def main():
    # Select a subset of promising files from the incremental scan
    selected_files = [
        # DeepResearch Android files (these we already processed successfully)
        ...
        # Additional high-potential files
    ...
    print(f"Processing {len(selected_files)} selected incremental files...")
    ...
    print(f"⏭️  跳过（已索引）：{results['skipped']} 个")
    print(f"📊 评分过低：{results['low_score']} 个")
    
    print(f"\n📊 索引状态：")
    try:
        result = subprocess.run(['python3', 'scripts/source_index_helper.py', 'stats'], 
                              cwd='/Users/gracker/Library/Mobile Documents/iCloud~md~obsidian/Documents/Obsidian/Android-Internal-Wiki',
                              capture_output=True, text=True, timeout=10)
        if result.returncode == 0:
            stats = json.loads(result.stdout)
            print(f"  - 总索引数：{stats['total_indexed']} 个")
            print(f"  - 高质量：{stats['high_quality']} 个")
            print(f"  - 中等质量：{stats['medium_quality']} 个")
    except Exception as e:
        print(f"  - 无法获取统计：{e}")
