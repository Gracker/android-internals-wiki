...
BASE=Path('/Users/gracker/.hermes/scripts/aiw-review-finalize.py')
STATE=Path('/Users/gracker/.hermes/state/aiw-review-finalize-dispatcher')
OB=Path('/Users/gracker/Library/Mobile Documents/iCloud~md~obsidian/Documents/Obsidian')
REPORT_DIR=OB/'OpenClaw定时任务/AIW自动化流水线'
LANES=[
  ('draft-review', lambda h: h%2==1),
  ('deep-tech-review', lambda h: h%2==0),
  ('task2b-verifier', lambda h: h in {3,7,11,15,19,23}),
  ('reader-final', lambda h: h in {1,7,13,19}),
]
...
def main():
    ...
    payload={'schema_version':1,'profile':'aiw-review-finalize-dispatcher','date':day,'hour':hour,'status':status,'due_lanes':due,'results':results,'generated_at':now.isoformat()}
    ...
    report=REPORT_DIR/f'{day}-aiw-review-finalize-dispatcher.md'
    text=f"# AIW Review Finalize Dispatcher · {day} {hour:02d}:00\n\n- status: {status}\n- due_lanes: {', '.join(due)}\n- evidence: `{ev}`\n\n"
    for r in results:
        text += f"## {r['mode']}\n\n- returncode: {r['returncode']}\n\n```text\n{r['stdout'] or r['stderr']}\n```\n\n"
    report.write_text(text,encoding='utf-8')
    changed = '未修改正文，仅运行 review/finalize 候选复查 lane'
    impact = '; '.join([f"{r['mode']} rc={r['returncode']}" for r in results])
    print(f"# AIW Review Finalize Dispatcher · {day}\n- status: {status}\n- 修改内容: {changed}\n- 影响范围: due_lanes={', '.join(due)}; {impact}\n- Obsidian: {report}\n- Evidence: {ev}\n## 详情\n" + '\n'.join([f"### {r['mode']}\n```text\n{(r['stdout'] or r['stderr'])[:1200]}\n```" for r in results]))
