...
- **四维评分**：对每个文件进行四维评分（相关性+技术深度+时效性+可验证性，各1-5分），总分 ≥16 才纳入索引
...
3. 四维评分：
   ...
   - **时效性**（1-5）：<6月=5，6-12月=4，1-2年=3，>2年=1-2
   - **可验证性**（1-5）：含 AOSP/官方文档引用=高分，无法验证=低分
4. 如果总分 ≥ 16：映射到 1-3 个章节，纳入索引
5. 如果总分 < 16：记录到 skipped-files.json，附跳过原因和分数
